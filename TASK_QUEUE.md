...
C-028 was CLEARED 2026-07-11 and the arc is fully closed: PRs #41-#63 are
ALL merged (the 2026-07-13 restart-close session merged #61-#63 at
delta-audited heads and deployed the shard capsule live under the Lakebed
cap). Every Window-A software gate and P0-003 are satisfied. **A
whole-project comprehensive audit is the declared next gate before further
feature work (Ed, 2026-07-13); method proposal pending Ed's approval.**
Normal ranking applies afterward, subject to the machine-state lanes below.
...
| 11e | SITE-02 | P4 Polish | NEW 2026-07-13 (SITE-01 delta-audit deferrals D1/D2, dispositions on PR #63) [AGENT] | SITE-01 follow-ups: (D1) pack_capsule executable discovery must not silently enter advisory estimator mode off this machine (prefer explicit env/OS-path discovery + loud mode print); (D2) node decode test should execute the EMITTED TS (tsx/ts-node when available) instead of a rewritten JS mirror | Both close with regressions; measured mode provably selected on a clean clone with lakebed available |
...
| 16d | TOOL-01 | P3 Tooling | NEW (C-028; personal tooling, non-repo; +2 defects 2026-07-12) [LEAD] | codex-run-v3 known defects: resume-after-NEEDS_SCOPE no-op; preventive permission profiles (phase 2); NEEDS_RULING runner recognition; effort default passes through ~/.codex config (ran a whole arc at unintended ultra — should default xhigh when --effort omitted); upstream stream-death can exit OK with thin/empty out-file (should be FAILED, not OK+warn) | Fixes recorded in ~/.claude adapter ops lessons; next tooling round |
...
| CODEX-BRIDGE-3 | P0 Safety | 2026-07-13 (gate-complete; MERGE PENDING — move fully complete when Ed merges #65) | bridge-protocol/v1.1 for maximum co-work (Ed-directed): discussion lane, session-open/close wrappers, tolerant envelope, per-call reverse effort, peer channels + proposal diffs | PR #65 gate-complete AWAITING ED'S MERGE (agent self-merge declined by harness); 3 lenses + 3 delta re-audits, findings 13→6→2→1; suite 1387 OK; CI green at `8b96bd4`; D-065; report `2026-07-13-bridge-v11.md` |
