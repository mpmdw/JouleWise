...
| 6 | P1-006 | P1 Phase Gate | open | Confirm NVIDIA/Orin telemetry access paths | SSH/runtime/telemetry command evidence recorded or marked pending with blocker |
| 7 | P1-007 | P1 Phase Gate | open | Perform Phase 2 readiness review | `docs/phase_1/phase_1_exit_checklist.md` states mock-first Phase 2 can begin |
| 8 | P2-001 | P2 Next Slice | queued | Implement mock run-bundle vertical slice | One command creates a complete mock run bundle and tests pass |
| 9 | P2-002 | P2 Next Slice | queued | Implement reducer for deterministic synthetic traces | Reducer test computes gross and idle-subtracted energy from known samples |
| 10 | P2-003 | P2 Next Slice | queued | Begin Mac MLX + powermetrics vertical slice | Only after mock bundle/reducer path exists |

## Completed Queue Items

| ID | Priority | Completed | Task | Evidence |
|---|---|---|---|---|
| Q-000 | P0 Safety | 2026-06-09 | Resolve the local `Energy_Benchmark_Architecture.docx` deletion decision | User confirmed the Word doc was unrelated to the actual project and the deletion does not matter; removal can be committed |
