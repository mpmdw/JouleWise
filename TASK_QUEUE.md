...
| 16a2 | SITE-01 | P2 Next Slice | PR #63 OPEN 2026-07-12 (gzip shards, measured artifact 865,973 B / 17.4% under cap; lead-gated); REMAINING: delta re-audit pre-merge + lead live deploy post-merge [AGENT] | Trim/paginate the site capsule so deploys fit the cap; redeploy | Live capsule current again; freshness drift banner clear; report `docs/run_reports/2026-07-12-agent-lane-triple.md` |
| 16b | P2-049 | P3 Hardening Candidates | PR #61 OPEN 2026-07-12, FULLY GATED (lens CLEAN, lead replay 1261 OK, CI green); needs only the merge click (harness denied lead self-merge) [ED or AGENT] | analysis_manifest.py ROOT default (line 27) resolves to site-packages when installed — replace with explicit-root-or-fail-closed | Installed-package manifest validation refuses with clear message instead of wrong-dir lookup |
