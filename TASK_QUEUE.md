...
|---:|---|---|---|---|---|
| 1 | P1-002 | P1 Phase Gate | waiting-user (the ONLY blocker for 2H: privileged sample + D-004 sudoers; MLX-install half completed 2026-07-06) | Complete Mac-local Phase 1 telemetry evidence | Privileged `powermetrics` sample + field list, D-004 sudoers rule verified (exit-checklist instrumentation section). Gates 2H. |
| 2 | P1-001 | P1 Phase Gate | waiting-user | Capture supervisor approval and scope notes | Dated notes in `docs/phase_1/phase_1_exit_checklist.md`; `RUN_STATE.md` updated if scope changes. Also unblocks full D-016 closure (P2-004). |
| 3 | P0-003 | P0 Safety | waiting-user | Replace interim backup destination (R-016) with external/cloud location | `~/JouleWise-backup` is same-disk (errant-delete protection only); user names the real destination; one fresh restore test; R-016 updated to fully mitigated |
...
| 8 | P2-003 | P2 Next Slice | partial (2G complete 2026-07-06; 2H gated on P1-002; 2I gated on 2H) | Mac MLX + powermetrics vertical slice (slices 2G-2I) | Real bundle + 3-rep variance in a run report. Code-level spec: `docs/phase_2/hardware_slice_implementation_guide.md`. |
...
| 10 | P2-008 | P2 Next Slice | open (found during the 2G live smoke) | Harden mock telemetry × SystemClock composition | MockTelemetryAdapter under SystemClock stamps first/last samples outside the D-026 marker window (first-sample-at-`start_sampling`, last-at-`stop_sampling`); short real-runtime windows at low power_hz reduce to 0 interior samples. Make the composition robust explicitly (interior stamping or reducer-side documented tolerance) WITHOUT breaking the FakeClock closed-form tests; bring-up config works around it at 20 Hz meanwhile. |
...
| 13 | P3-000 | P3 Research Expansion | queued (Stage 3.0.1 needs only 2G + the chosen small model — now unblocked; 3.0.0 kv-size helper is ungated desk work) | KV persistence feasibility spikes (Phase 3 Stage 3.0) | Verdicts in `docs/phase_3/kv_feasibility.md`; must complete before any borrow-window scheduling |
