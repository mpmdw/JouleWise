...
| 0h | P2-040 | P2 Next Slice | NEW (C-027; pre-Window-A) [AGENT] | Reducer/gate correctness batch: zero-length measured window must FAIL strict/claim paths (ARC-3); metric-specific evidence gates (gross request must not require idle-drift evidence, STA-5); joint-edge interpolation bound (STA-6); D-058 runtime-observed token denominator wins over config (STA-7, update the locking test); zero-MAD fallback review flag (STA-8); atomic experiment-manifest writes (ARC-5); unknown-config-key warning + delete-or-implement `warmup_seconds` (ARC-8); local cleanup-failure surfaced into run quality (ARC-6) | Each item lands with a test that FAILS against current behavior (mutation-style per STA-11); C-027 disposition table is the authority: `docs/reviews/2026-07-09-c027-whole-project-review.md` |
...
| 0n | P2-042 | P2 Next Slice | NEW (C-027) [AGENT] | Frozen analysis manifest from matrix generation: cell_id/block_id/condition_id per entry, sentinel linkage, enumerated contrasts with `contrast_id` (STA-10) | Manifest emitted deterministically beside the order manifest; P2-037 consumes it |
...
| meta | P0-003 | P0 Safety (user-deferred 2026-07-06 — interim same-disk backup accepted for now; C-026 meeting ESCALATION: becomes REQUIRED before Window-A data accumulates — campaign bundles on one disk are unrecoverable evidence) | waiting-user, PRE-WINDOW-A | Replace interim backup destination (R-016) with external/cloud location | User names the destination; one fresh restore test; R-016 → fully mitigated |
