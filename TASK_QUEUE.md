...
|---:|---|---|---|---|---|
| 0 | RESUME-CP5 | P0 Safety | PAUSED at stable checkpoint 2026-07-08 (Ed: "start again" resumes THIS row first) | Finish the pre-campaign review session from CP-5: gate 3 worktree fix-round diffs (envgate live-gated vs real affine bundle) -> 3 PRs; merge PRs #22+new after fresh final-head passes + CI; post-merge integration review; adjudicate the methodology synthesis; land accepted pre-campaign changes (hash-domain-aware validity check expected); deliver Ed's four-part answer + Window-A go/no-go | Full sequence + artifact paths: `docs/stream_logs/2026-07-08-precampaign-review.md` CP-5; Fable directs, Codex executes |
| 1 | P2-015 | P2 Next Slice | NEW (C-011, amended C-014) [QUIET-MAC] | Detection-floor calibration campaign — expanded Window A floors for gross request, idle-sub request, phase window, and item/level window, plus comparative MDE from same-condition repeats/ABBA | Versioned strict-valid calibration bundles; `docs/phase_2/detection_floor.md` as per-consumer table; per-metric/window-class `floor_abs_j` + `floor_cmp_j`; claim gate = `max(floor_abs, floor_cmp)`; calibration manifest hash cited by later reports; pre-Window-A lead-run tasks-sampler overhead smoke required before enabling any extra powermetrics samplers and validating that the 2s env-capture settle absorbs the probe burst (C-015/R2); NEW post-suite-build shakedown gate (C-019): before any Window-A data, ONE tiny production-shaped run through the campaign-runner path (not bare `run`) → strict validation → reduce → backup, on the merged suite-substrate code |
...
| 4 | P2-010 | P2 Next Slice | SUBSTRATE + LADDER CORE BUILT AND MERGED 2026-07-08 (PRs #17/#18/#20; D-044..D-047; run report `2026-07-08-suite-build.md`); REMAINING: envelope-gate analysis script [AGENT], then the smoke CAMPAIGN (B=5) on a quiet-window tail [QUIET-MAC] | P2-010b remainder: envelope-gate analysis script (E1-E4 gate + E5 advisory per D-047.3/4, sentinel-tag exclusion, distinct-item denominators, D-036 computed verdict) + smoke campaign execution | Script emits `envelope_validated`/`envelope_failed([codes])` from strict-valid smoke bundles; acceptance in AP-5; ledger pointer `docs/stream_logs/2026-07-08-affine-ladder.md` AFF-CHECKPOINT |
| 5 | P2-019 | P2 Next Slice | NEW (C-014) [QUIET-MAC] | `q4_l3_shape_grid_v1` campaign — Window B, AP-1, two models, n sized from Window A | 4x3 prompt/decode grid `{128,512,2048,4096}` x `{64,256,512}`; holdouts `(512,256)` and `(4096,512)`; categorical-additive fit first; top-up near-floor cells before L3 wording |
| 6 | P2-020 | P2 Next Slice | GENERATOR BUILT AND MERGED 2026-07-08 (PR #19; five ids-native BOS-less conditions per D-046); campaign execution Window B [QUIET-MAC] with real-tokenizer manifests (see P2-012 row) | Content-sensitivity sentinel campaign — Window B, AP-6 | Five equal-shape ids-native conditions, n sized from Window A; request-energy deltas and MDE verdicts; AP-6 non-generalization caveat applies (D-046) |
| 7 | P2-012 | P2 Next Slice | phase-1 ENGINE BUILT AND MERGED 2026-07-08 (PR #19: six category generators, exact-shape fill, B7 identity); REMAINING: lead-side real-tokenizer (`.venv` Qwen2.5-1.5B) jw_mixed + sentinel manifest generation and commit (GEN-CHECKPOINT in `docs/stream_logs/2026-07-08-jw-mixed-generators.md`) [AGENT with lead .venv step]; natural-EOS pilot + full panels later phases | Generate + commit the real-tokenizer manifests + sidecars; identification-core campaign after Window A | Manifests substrate-valid, sidecars complete (B7 rows, ground truths), per AP-4; no category claims outside matched strata |
| 8 | P2-022 | P2 Next Slice | NEW (C-015; post-2M + post-P2-010a) [AGENT] | Marker-shim energy-layer feasibility spike — verdict-shaped export path only | `external_markers_supported` / `partial(<limitation>)` / `external_markers_unsupported`; 3+ marked items, external result artifact hashed, strict bundle valid; energy-layer-only pin: no accuracy interpretation, no leaderboard join, no pass@k-energy ratio, no general adapter framework; AP row required before any L2 claim; contract in `docs/contracts/adapter_contracts.md` |
...
| 11 | P1-008 | P1 Phase Gate | waiting-user; ELEVATED to ED-EXTERNAL rank 1 (C-019 reassessment: every phase target is TBD, R-012 is the biggest active management risk for an undergrad timeline; late dates force uncontrolled descoping) [ED-EXTERNAL] | Map phases to academic calendar AND capture the evaluator's acceptance bar (minimum figures, demo expectation, reproducibility threshold, whether Mac-only + split-deferral is acceptable) | Colloquium/report dates + borrow window in `docs/milestones.md`; phase targets derived; acceptance-bar notes beside the P1-001 scope notes |
| 11a | P2-026 | P2 Next Slice | NEW (C-020 merit debate) [AGENT] | Fix the D-033 strict-validation legacy bypass: a bundle lacking summary_provenance skips all workload-provenance strict checks — new-era bundles must not be able to validate as legacy (gate on reducer_version/schema era; tamper test) | Strict validation rejects a provenance-stripped new-era bundle; regression test; contract wording updated |
| 11c | P2-027 | P2 Next Slice | NEW (C-020; D5 ruling #1) [AGENT prep; ED-EXTERNAL for the outsider] | Bundle-pack publication prep: select 2-3 strict-valid corpus bundles, package with re-reduction instructions (one command), so ONE external party can run strict re-reduction — converts auditability from design property (L0) to demonstrated property | Published pack + a documented external re-reduction; until then the review doc's auditability claim stays L0-scoped |
...
| 11b | P2-025 | P2 Next Slice | NEW (2026-07-08 suite-build oversight, B8 deferred half); ADJACENCY (C-019 reassessment): execute WITH the P2-012 real-tokenizer manifest work, BEFORE any text-path suite campaign — it is the fail-closed validity guard for those campaigns [AGENT] | Campaign-runner expected-vs-realized prompt-hash check for text-path suite items (ids-native items are already validator-checked) | Runner compares generator sidecar `expected token_ids_sha256` against realized per-item hashes post-window; mismatch fails closed; lands with the first real-tokenizer campaign tooling |
