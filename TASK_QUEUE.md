...
| 4 | P2-010 | P2 Next Slice | BUILD UNBLOCKED (D-042 owner directive; quarantine rules + minimal-substrate cap apply; research input `docs/phase_2/suite_implementation_research.md` — adjudicate its amendments first) [AGENT] | P2-010a minimal generic suite substrate + P2-010b smoke ladder (`affine_mod_ladder_v1` envelope validation); full scored campaign deferred | Substrate: suite/item/block/level markers, `BundleReader.item_windows()`, category/source/output-policy fields, per-item stop/token/response hashes, order/cache metadata, manifest validation, and per-item status model per the bank C-015 section (spec home); future `run_bundle_layout.md` contract fields for suite/item/block/level markers and per-item outputs are named in `docs/research_question_bank.md` and land with implementation; smoke ladder acceptance in AP-5 |
...
| 6 | P2-020 | P2 Next Slice | NEW (C-014); generator BUILD unblocked D-042 [AGENT]; campaign execution Window B [QUIET-MAC] | Content-sensitivity sentinel — Window B, AP-6; generator design in `docs/phase_2/suite_implementation_research.md` | Five equal-shape content conditions (repeated-seed, random-token, natural prose, code-like, multilingual), n sized from Window A; request-energy deltas and MDE verdicts reported |
| 7 | P2-012 | P2 Next Slice | phase-1 BUILD UNBLOCKED (D-042; after P2-010a substrate; research input `docs/phase_2/suite_implementation_research.md`) [AGENT] | Implement `jw_mixed_v1` workload expansion in phases — identification core first, natural-EOS pilot second, full panels only after above-floor structure | Per `docs/research_question_bank.md` + AP-4: common-shape 512/256 `fixed_budget_exact` stratum across 6 categories, synthetic + realistic; natural-EOS pilot >=4 items/category on reasoning/JSON/chat/multilingual; additive fields and per-item stop/token/response hashes; no category claims outside matched strata |
