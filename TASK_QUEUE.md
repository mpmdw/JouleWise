...
| A11 | DOC-008 | P2 Next Slice | ACTIVE | Machine-readable state kernel generating the RUN_STATE restart block and live queue view; PROJECT_STATUS compaction with history archive; retire the planning reflection protocol as standalone intake; two-writer rule and credential-boundary push procedure into docs/orchestration.md. | Schema-version-2 kernel carries the fixed derived-view authority notice; generated blocks are byte-reproducible; one next-action surface remains. Evidence: Top-level authority notice is required and fixed; python3 scripts/gen_state.py --check exits 0; python3 -m unittest tests.test_gen_state passes; One next-action surface remains. Authority: [DOC-008 spec (D-063 Stage 1)](docs/specs/c027/doc-008_state_kernel.md). Acceptance: [DOC-008 acceptance](docs/process/state_kernel.json). Note: Final C-028 live-state refresh is in progress; live RUN_STATE/TASK_QUEUE marker conversion remains lead-owned. |
