...
|---:|---|---|---|---|---|
| 1 | P1-002 | P1 Phase Gate | ONE item left: user installs the D-004 sudoers line (`sudo visudo -f /etc/sudoers.d/joulewise-powermetrics` → `edr ALL=(root) NOPASSWD: /usr/bin/powermetrics`), verify `sudo -n` | Complete Mac-local Phase 1 telemetry evidence | Sample captured + field list recorded 2026-07-06; fixture committed. Unblocks the 2H live smoke AND 2I (same command). |
| 2 | P2-003 | P2 Next Slice | 2G complete; 2H implementation complete (2026-07-06, review/counterreview loop); remaining: 2H real-machine smoke + 2I (both = one `mac_mlx_local.json` run once sudoers lands) | Mac MLX + powermetrics vertical slice (2H live smoke, then 2I) | Real idle baseline + measured window + 3-rep variance in a run report. |
