...
| 0a | P2-029 | P2 Next Slice | NEW (C-023 work order item 4; C-024 wave 2) [AGENT] | Reducer/aggregator uncertainty propagation + claim gates per the merged spec | Implements `docs/phase_2/detection_floor.md` §3: idle-variance + drift-bound terms, `not estimable` single bundles, phase/item claim-gate thresholds honoring identifiability flags; tests; 5.5 pipeline + lead gate |
| 0b | P2-030 | P2 Next Slice | NEW (C-023 M2, elevated by D1; pre-campaign blocker for suite claims) [AGENT] | Ordering executability: per-repetition manifests or a real runtime order policy so C-015 rotation promises are executable | Sequencing spec + implementation agree; claims tooling downgrades to `manifest_order` wording when rotation absent; must land before any suite campaign's category/position inference |
| 0c | P2-031 | P2 Next Slice | NEW (C-023 M3 + L4 stack-identity gap; C-024 wave 2) [AGENT] | Token-normalization contract + stack-identity table | Contract: request energy primary, J/token tokenizer-scoped, cross-tokenizer comparisons need J/char / J/byte / semantic-pair companions; stack-identity fields required in every reader-facing figure; rides `docs/contracts/` |
| 0d | P2-032 | P2 Next Slice | NEW (C-023 work order item 8; behind registry/linter cut-line for broad packs) [AGENT] | Pre-hardware campaign packs for the fully-preparable list | Per `docs/research_question_registry.md` pre_hardware_preparable=fully rows (Q1-Q3 split suite, Q6 rail-vs-wall, C5-2.3 KV economics, C5-2.7/2.8, C5-3.1/3.5 replication runbook, C5-I.1..I.5 contracts): analysis plan + manifest/config templates + expected-artifact spec + figure skeletons per pack |
| 1 | P2-015 | P2 Next Slice | NEW (C-011, amended C-014; CP-6: + Window-B-start revalidation cell) [QUIET-MAC] | Detection-floor calibration campaign — expanded Window A floors for gross request, idle-sub request, phase window, and item/level window, plus comparative MDE from same-condition repeats/ABBA | Versioned strict-valid calibration bundles; `docs/phase_2/detection_floor.md` as per-consumer table; per-metric/window-class `floor_abs_j` + `floor_cmp_j`; claim gate = `max(floor_abs, floor_cmp)`; calibration manifest hash cited by later reports; pre-Window-A lead-run tasks-sampler overhead smoke required before enabling any extra powermetrics samplers and validating that the 2s env-capture settle absorbs the probe burst (C-015/R2); post-suite-build shakedown gate (C-019): before any Window-A data, ONE tiny production-shaped run through the campaign-runner path (not bare `run`) → strict validation → reduce → backup, on the merged suite-substrate code; CP-6: include a Window-B-start floor-revalidation cell |
