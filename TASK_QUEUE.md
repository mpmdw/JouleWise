...
| 13 | P2-017 | P2 Next Slice | NEW (2026-07-08 final-gate 5.5 review of the critique-record session) [AGENT] | Make `scripts/build_site.py` provenance stamps honest: pages are stamped `git log -1` at build time, so a dirty-tree regen labels uncommitted content with the previous commit's hash. Fix: stamp the source file's own last-commit hash and append a `+ uncommitted` marker when the source is dirty at build time | Regen on a dirty tree marks affected pages as such; regen on a clean tree stamps the commit that actually contains the rendered source; interim convention until fixed: commit sources first, regen, commit site second (adopted this session) |
