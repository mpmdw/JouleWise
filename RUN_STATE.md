...
- CONTINUE — A248 `STAGE-A-EVIDENCE-EXECUTOR-01`: Build an unattended executor for Stage A, the initial evidence campaign that will derive the night gate's quiet-admission cutoff: the maximum busy-core level at which an unattended measurement may start. Busy cores are busy processor seconds per elapsed second; 1.0 means one fully busy core. No ruling has activated this threshold. The cold gate, an independent adjudication, ruled a separate evidence payload (the program run overnight) under the unchanged version-2 diagnostic plan. Implement a ruled-registration table in the night gate, keyed by a digest (a cryptographic content hash) and bound to the chain-source digest, the hash of the tracked execution-script template. Add a typed evidence probe receipt, a structured record of verification without measurement, dispatched by one `NIGHT_PAYLOAD_KIND` environment-variable export; amend `docs/process/NIGHT_HANDBACK.md`, the night handoff contract, and the runbook's verify-only probe row. Add the authoring tool `scripts/gen_evidence_night.py` and freeze the idle-only pilot protocol before collection: twelve 600 s envelopes (scheduled measurement intervals), each with a 480 s interior used for analysis; delta (δ), the target energy difference, is 1 joule (J). Size block two, the follow-up measurement block, from the upper confidence bound on the paired spread, the variability of paired energy differences. Pre-register the branch that concludes "no cutoff qualifies".
