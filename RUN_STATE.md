...
Source of truth for work selection: [state kernel](docs/process/state_kernel.json) (updated 2026-09-17). Latest report: [T38d — 2026-09-09: rehearsal harvest, stub-chain finding and post-completion cleanup](docs/process_traces/2026-09-02-hands-free-week/21i-rehearsal-20260909-harvest-record.md).
...
- CONTINUE — A231 `NIGHT-GATE-QUIET-ADMISSION-01`: The unattended night gate, the pre-measurement check that allows or refuses an unattended run (`joulewise/night_gate.py`, `evaluate_night` at line 947 on `a90ab4e8`) decides once, at the planned start instant t0, whether the machine is quiet enough to measure, and one of its predicates is the 1-minute load average (the operating system's recent average of runnable or waiting work) against a fixed maximum of 2.0 (`LOAD_MAX`, line 57). Four consecutive nights produced zero data (2026-09-13 an agent in the census, the scan for active AI-agent processes; 09-15 load; 09-16 an 11 h file-consent hang, cured by PR #350; 09-17 load 3.66 with an EMPTY census, caused by a burst from Spotlight (macOS search indexing) and `mediaanalysisd` (macOS media analysis)). Three hours after the 09-17 refusal the load was 1.3–1.5 (the gate would have PASSED) while `fseventsd`, the file-system event logger, alone burned 85–100 % of one core, which is on the order of 5 W, i.e. ~300 J per 60 s slot against a ~5 J floor tolerance: the load average refuses clean machines and admits contaminated ones. Adopted design (gpt-6-astra xhigh consult `docs/process_traces/2026-09-17-interactive-45a0774c/01-gate-redesign-consult.md`, magistrate brief `docs/process_traces/2026-09-17-interactive-5c919872/01-brief-seat-quiet-admission.md`): a plan schema v4 carrying a sealed `quiet_admission` block (policy `cpu_interval_v1`, bind budget, sample interval, consecutive quiet samples, busy-core cutoff (busy-core equivalents measure busy CPU seconds per elapsed second, so 1.0 is one fully busy core), post-bind runway (the time reserved for acquisition after admission)); the driver polls inside a bind window, the bounded pre-reservation wait for sustained quiet, that starts at t0 and ends at min(t0 + bind, E − runway) where E = t0 + window is unchanged; GO (permission to start) on the first run of consecutive quiet intervals; the quiet predicate is busy CPU time per interval (per-process cumulative CPU deltas keyed by (pid, start time) and host idle, normalised to cores, observer (the sampler and its children) included, no daemon exemptions), the load average becomes diagnostic; census and every non-CPU hard predicate stay terminal (an immediate refusal, not a wait for improvement) on every sample; receipt v3 records every sample (`quiet_samples.jsonl`) and the top consumers on every GO and refusal; v2 plans keep byte-identical semantics; the desk retry policy (`joulewise/arm_retry.py`, NIGHT_HANDBACK R1) gains a separately evidenced zero-capture new-plan successor route. The numeric thresholds are provisional plan data pending cold-gate packet 70, the evidence submitted for independent adjudication (`docs/process_traces/2026-09-17-interactive-5c919872/70-coldgate-packet-quiet-admission/`), which rules the nine atomic propositions of the consult's section 4.
