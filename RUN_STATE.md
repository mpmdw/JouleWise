...
Source of truth for work selection: [state kernel](docs/process/state_kernel.json) (updated 2026-09-18). Latest report: [T38d — 2026-09-09: rehearsal harvest, stub-chain finding and post-completion cleanup](docs/process_traces/2026-09-02-hands-free-week/21i-rehearsal-20260909-harvest-record.md).
...
- CONTINUE — A238 `TEST-LARGE-FRAME-ARGV-PORTABILITY-01`: The real-exec fault bench (a test fixture that launches actual child processes and injects failures), `tests/night_gate_fixtures/bind_supervision.py`, drives bind-supervision regressions: tests of how the night driver supervises worker processes (short-lived child processes performing individual checks) inside the bind window, the bounded wait for sustained quiet starting at t0 (the planned measurement start instant). The night gate is the check that decides whether an unattended measurement may start. For `large_frame`, the parent builds a 200,000-character string and passes the whole worker specification as ONE argv string (one command-line argument; measured 200,884 bytes including its terminating NUL). Linux's `execve(2)` MAX_ARG_STRLEN caps one string at 131,072 bytes on the hosted runner's 4-KiB pages and refuses the launch with E2BIG (argument list too long); macOS has no such per-string cap. Thus `test_large_frame_is_incremental_and_still_bounded` passes on this Mac but failed twice identically on hosted `ubuntu-latest` job `test (3.13, 5)` on main `b55909e3` with `'REFUSED' != 'GO'`: the driver records the launch error as `night_probe_error`. Production is unaffected by this fixture defect: sample workers receive only small identifiers; the uncapped production request is the separate BIND-REQUEST-PAYLOAD-CAP-01 lane. Fix only the test: expand the large string inside the worker, record the largest argv string per launched task in the bench, and add a regression requiring every launched string to stay under 131,072 bytes, which fails against the old fixture. In flight this activation on branch `fix/2026-09-18-large-frame-argv-portability`; seat brief `docs/process_traces/2026-09-18-activation-f0b608b7/02-brief-seat-argv-fix.md`.
