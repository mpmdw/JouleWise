...
**Now (2026-09-17):** the unattended-night hang of 2026-09-16 is cured and merged to main on 2026-09-17. Every custody-evidence read on the night path is now time-bounded and refuses with a named reason instead of waiting on a person; that refusal reaches the morning email; a night can only be armed after a probe run from a launchd job proves the same reads complete in budget; and the capture writer now re-reads the evidence corpus twice per slot instead of four times, so the arm-time probe's single measured pass certifies the writer fits. **Next:** settle the wall-clock abort grace (NIGHT-STALL-WALLCLOCK-ABORT-01) through the cold gate, then re-plan the equivalence night with the launchd probe as its dry run and record its first real custody timing. Nothing is armed.
