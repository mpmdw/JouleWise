...
**Now (2026-09-23, late evening):** the third-version idle pilot ran this morning and kept eleven of twelve power captures; one was excluded because a background macOS process used too much processor time. Today’s merged work added the third-version rule that admits or refuses an idle night, released the agent-free machine hold early after a start refusal with no capture, and detected a looping Wi-Fi logging helper. It also added a frozen importer for the MATH competition-problem benchmark and an exact-answer scorer, enforced the limit of one replacement night after a refusal with no capture, corrected the notice sent before an unattended night, and repaired a continuous-integration test. A separate pull request for a table of measurement-night kinds passed its final independent review and awaits the final-head replay and merge; it preserves the existing idle night’s behaviour. Review of the draft code that schedules scored problem blocks, sums their measured energy and estimates energy per correct answer found defects, so the work has been split into a newly specified packer, a reducer that checks sealed capture records, and an estimator governed by an explicit decision table. Next: finish that pull-request gate; write and independently check the revised packer against the cold review’s rules; add the scored night; and draft the revised analysis plan for Ed to decide. Ed’s choices about publishing problem text, adopting the analysis plan, model decoding, and sample counts are still pending. Nothing is armed.
