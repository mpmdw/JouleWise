...
**Now (2026-09-23, evening):** on 2026-09-23 the third-version idle pilot night ran at 07:00 (twelve captures, eleven kept; one excluded because a macOS asset daemon kept a core busy), and main gained the third-version admission rule, an early release of the agent-free hold after a start refusal that captured nothing, and detection of the Wi-Fi log helper's respawn loop. In the latest session four more changes landed, each through independent reviews and a cold final check by a separate model instance. (1) The **MATH benchmark importer**: 4,040 frozen, hashed competition-math problems across the benchmark authors' five difficulty levels, and an exact-fraction answer scorer. This is the first piece of the headline experiment, which asks how much energy a model spends per *correct* answer as problems get harder and models get bigger (Qwen3 1.7B vs 8B on this Mac). (2) A **replacement-night limit**: after a night refused at its start that captured nothing, the rules allow exactly one replacement night. That limit is now enforced in code, where before it rested on procedure. (3) The **arm notice** Ed receives before each unattended night now states the current rules truthfully. (4) A CI test fix. Nothing is armed. Four decisions on the headline experiment's design are with Ed by email (whether to publish problem text, the analysis-rule change, how the models decode in 'thinking' mode, and problems per level). Next: review the experiment's energy-per-correct-answer code (on a draft branch), build the new kind of measurement night that runs the models, and a bench check of model decoding.
