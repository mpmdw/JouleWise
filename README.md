...
**Now (2026-09-12):** the first epoch-equivalence measurement night is armed for 02:56 PDT on 2026-09-13 from a frozen clone; this afternoon the merged methods paper had its advisor-readiness pass: three independent reviews (a fact-and-metrology check of every number against its registry row, a plain-language first-use check, and a blind first-contact read by a fresh reviewer acting as the advisor) plus a verification of the two load-bearing related-work papers against their PDFs of record, then two delegated fix rounds and one bench round under a cold-gate ruling; the paper now says on page one that no sensitivity ratio is computed on measured data and that the raw captures are retained but unreleased, its introduction no longer describes a comparison campaign it did not run, Section 2 reads as a physical procedure, and Section 5 states the noise-scale floor and the one-directional edge lag plainly (PR #331). **Next:** merge the paper pass under the twelve-row gate, then harvest the night on 2026-09-13 and apply the pre-registered equivalence rule.
