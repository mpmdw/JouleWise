...
**Now (2026-09-22):** the unattended-night entry point now refuses to arm a new measurement night unless every earlier night still sitting in the custody directory has a terminal record (a completed delivery, a result file, a chain exit, or a refusal) and its scheduling span is over by the same rule the watchdog uses; a root that is still active, or that cannot be classified, stops the arm. The fix went through two cold-gate rulings after review found the explanatory text repeatedly drifting from the code; every rule sentence now has an executed probe behind it. The next real night is armed automatically once the machine is quiet (no interactive agent sessions open).
