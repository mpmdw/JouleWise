...
**Now (2026-09-16 evening):** the machine is being moved, so everything is parked: no measurement night is scheduled, the background scheduler (the "magistrate watchdog") is unloaded, and all local work is archived to a verified bundle and to iCloud. Today's equivalence night started on time at 09:45 but its reservation step stalled for eleven hours and it produced no data; the reason is not yet known and is the first thing to investigate once the machine is back in a quiet spot.
