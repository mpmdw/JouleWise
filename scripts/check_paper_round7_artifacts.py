...
The final output line is always ``R7F COMPARED n / MISMATCHES m``.
"""
...
def render_row(row: DXRow, spec: RegistrySpec, artifacts: dict[str, Any]) -> str:
    ...
    if rule.startswith("source_sha256_"):
        code = rule.removeprefix("source_sha256_").upper()
        if code not in spec.sources:
            raise ValueError(f"unknown source digest renderer {rule}")
        return spec.sources[code].sha256
...
def check_gates(artifacts: dict[str, Any]) -> list[Comparison]:
    gates = (
        ("XD", "calibration_gate.b_fiducial_s_matches_exactly"),
        ("XD", "calibration_gate.projection_evaluated_cell_count_matches_exactly"),
        ("AQ", "worked_capture_gate.matches_exactly"),
    )
    comparisons: list[Comparison] = []
    for source, path in gates:
        try:
...
def check_figure(
    ...
    for name, shapes, x_key, y_key, value_key in series:
        ...
        for index in range(59):
            ...
            expected_value = float(pulses[index][value_key])
            match = (
...
def check_skeleton_literals(text: str, spec: RegistrySpec) -> list[Comparison]:
    ...
    for match in FILL_RE.finditer(text):
        ...
        if row_id in IDENTITY_ROWS:
            ...
            continue
        suffix = text[match.end() : text.find("\n", match.end()) if "\n" in text[match.end() :] else len(text)]
...
def replay_half(
    ...
    with tempfile.TemporaryDirectory(prefix="joulewise-r7f-", dir=tmp_parent) as directory:
        ...
        xs = _run_producer(
            [
                sys.executable,
                str(repository_root / spec.sources["XS"].path),
                "--repository-root",
                str(repository_root),
                "--corpus-root",
                str(corpus_root),
        ...
        if xs.returncode != 0:
            output = (xs.stdout + xs.stderr).lower()
            if xs.returncode == 3 or "unavailable" in output or "no such file" in output:
        ...
        if anchor.returncode != 0:
            output = (anchor.stdout + anchor.stderr).lower()
            if "unavailable" in output or "no such file" in output:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="root containing both retained diagnostic corpus trees",
    )
    ...
    if any(not comparison.match for comparison in comparisons) or spec is None:
        ...
        print(f"R7F COMPARED {len(comparisons)} / MISMATCHES {mismatches}")
        return 2

    if not args.literals_only:
        try:
            comparisons.extend(replay_half(repository_root, args.corpus_root.resolve(), spec))
