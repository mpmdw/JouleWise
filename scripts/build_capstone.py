...
from the committed rpt001-v2 analysis artifacts, resolves
...
FIGURE_REL = "figures/rpt001-v2/F1_legacy_l1_instrument_results.svg"
...
def generate_results_page() -> str:
    ...
    waiver_restatements = {
        "token-normalized companion metrics": (
            "legacy bundles predate complete runtime stop-reason and output-policy "
            "provenance; the companion is omitted from the v2 figure, table, and report"
        ),
    }
    waiver_lines = "\n".join(
        f"- **{w['scope']}** — {waiver_restatements.get(w['scope'], w['reason'])}"
        for w in claims["quality_waivers"]
    )
