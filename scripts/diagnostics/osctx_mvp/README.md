...
Each live stage writes an fsynced, sealed `ledger.jsonl` decision for every accepted or discarded block attempt, including the U1 warm-up. Analysis reads accepted cells only from this ledger and refuses incomplete histories, orphaned cells, missing cells, or changed run custody. A live start records network-time state and each launched job in `owned.jsonl` before acting and releases ownership only after restoration or cleanup proof. If a run is interrupted with unreleased ownership, use `runner.py --recover DIR` for the stage or session directory before any new start. Recovery retries cleanup and restores network time; a failed recovery leaves the entry pending for another attempt.
