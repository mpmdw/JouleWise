...
Each live stage starts its fsynced, sealed `ledger.jsonl` with `stage_opened`, binding the command sequence, exact config file bytes, harness Python sources, and production source config. Every accepted or discarded block decision stores a SHA-256 and size manifest of every regular file under each recorded cell directory, including bundles. The runner writes no cell files after that decision. Analysis verifies every manifest before building rows and refuses added, missing, or changed evidence. `analyze.py power --stage0-summary` recomputes stage0U directly from the ledger and requires the supplied summary to agree. A live start records network-time state and each launched job in `owned.jsonl` before acting and releases ownership only after restoration or cleanup proof. Process proof checks the cell wrapper, production child, and cell-scoped powermetrics capture path after launchd bootout or SH group cleanup. If a run is interrupted with unreleased ownership, use `runner.py --recover DIR` for the stage or session directory before any new start. Recovery restores network time first, then tries every parseable job ownership entry. A damaged journal is reported and retained for inspection; incomplete recovery leaves ownership pending.
