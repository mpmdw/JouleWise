...
def canonical_json_bytes(value: Any) -> bytes:
    ...
    return (json.dumps(thaw(value), indent=2, sort_keys=True) + "\n").encode("utf-8")
...
def verify_entry(panel_path: Path, entry: Mapping[str, Any]) -> dict[str, Any]:
    ...
    tokenizer_config = mirror / "tokenizer_config.json"
    expected_template_sha = str(entry["chat_template_sha256"])
    if mirror.is_dir():
        if not tokenizer_config.is_file():
            reasons.add("tokenizer_config_missing")
            checks["chat_template"] = _check(
                "refused",
            ...
            try:
                tokenizer_config_value = json.loads(
                    tokenizer_config.read_text(encoding="utf-8")
                )
                if not isinstance(tokenizer_config_value, dict):
                    raise ValueError("tokenizer_config.json must contain an object")
            except (OSError, UnicodeDecodeError, json.JSONDecodeError, ValueError) as exc:
                reasons.add("tokenizer_config_json_invalid")
                checks["chat_template"] = _check(
                    "refused", path=str(tokenizer_config), detail=str(exc)
                )
            else:
                chat_template = tokenizer_config_value.get("chat_template")
                if not isinstance(chat_template, str):
                    reasons.add("chat_template_missing")
                    checks["chat_template"] = _check(
                        "refused",
                        path=str(tokenizer_config),
                        expected_sha256=expected_template_sha,
                    ...
                    observed_template_sha = sha256_bytes(chat_template.encode("utf-8"))
                    template_status = (
                        "passed"
                        if observed_template_sha == expected_template_sha
                        else "refused"
                    )
                    if template_status == "refused":
                        reasons.add("chat_template_sha256_mismatch")
                    checks["chat_template"] = _check(
                        template_status,
                        path=str(tokenizer_config),
                        expected_sha256=expected_template_sha,
                        observed_sha256=observed_template_sha,
                    )
