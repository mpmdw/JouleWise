...
CUTS = (
    ...
    (continuation, "envelope_holds_over_all_valid", "max(values) - min(values) <= bracket", "max(values) - min(values) <= max(values) - min(values)",
     "prepare_refuses_unresolved_valid_row_widening_range_without_writing"),
    (continuation, "authenticate_epoch_continuation", "set(ledger_snapshot.refusal_reasons) <= SNAPSHOT_STATE_REFUSALS",
     "not ledger_snapshot.refusal_reasons", "open_capture_session_still_crosschecks_terminal_continuation"),
    (continuation, "authenticate_epoch_continuation", "set(ledger_snapshot.refusal_reasons) <= SNAPSHOT_STATE_REFUSALS",
     "True", "integrity_and_unknown_snapshot_refusals_remain_invalid"),
    (continuation, "authenticate_epoch_continuation", "ledger_snapshot.is_governed_open_bracket_extension",
     "True", "integrity_and_unknown_snapshot_refusals_remain_invalid"),
    (continuation, "authenticate_epoch_continuation", 'session.state in TERMINAL_STATES and session.state == evidence["session_state"]',
     "True", "continuations_own_open_session_still_refuses"),
    (continuation, "load_epoch_continuations", "isinstance(exc, OSError)", "False",
    ...
    (issuer, "derive_record", "not envelope_holds_over_all_valid(all_valid, rule)",
     ...
     "one_quantum_above_level_fails_without_writing"),
    (issuer, "derive_record", 'lexeme is None, f"slots.{name}.b_fiducial_s_required_for_valid_row"',
     'False, f"slots.{name}.b_fiducial_s_required_for_valid_row"', "prepare_valid_null_bound_names_slot"),
    (continuation, "authenticate_epoch_continuation", 'slot["b_fiducial_s"] is not None,',
     'True,', "unresolved_valid_row_still_requires_a_decimal_bound"),
)
