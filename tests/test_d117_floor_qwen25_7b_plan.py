...
EXPECTED_SHA256 = {
    ...
    "generate_configs.py": (
        "a3f652ff72cfcad15c2b0bf0193d4e1336c526ef330dd76c05a74076f19d7da5"
    ),
...
# (files in the committed pack) - (the generator's own `expected_pack_files()`)
...
def minted_successor_pack_rels() -> list[Path]:
    """Return the successor family packs this repository has already minted."""

    current_rel = PACK.relative_to(REPO_ROOT)
...
def unminted_successor_family_suffix() -> str:
    ...
    return f"_v{len(minted_successor_pack_rels()) + 2}"
...
class D117Qwen25SevenBPlanTests(unittest.TestCase):
    ...
    def test_successor_generation_threads_plan_identity_and_lineage(self) -> None:
        ...
        successor_id = PACK.name.removesuffix(
            GENERATOR_MODULE.CURRENT_FAMILY_SUFFIX
        ) + successor_suffix
        successor_rel = PACK.relative_to(REPO_ROOT).with_name(successor_id)
        successor_spec_rel = Path(
            f"configs/floor_mint/d117_qwen25_7b{successor_suffix}"
            "_extraction_spec.json"
        )
        next_spec_rel = Path(
            f"configs/floor_mint/d117_qwen25_7b{next_suffix}_extraction_spec.json"
        )
        with tempfile.TemporaryDirectory(prefix=f"d117-beta{successor_suffix}-") as temp:
            output_root = Path(temp)
            ...
            for minted_rel in minted_successor_pack_rels():
                with self.subTest(minted_successor=minted_rel.name):
                    ...
                    if (
                        REPO_ROOT / minted_rel / "arm_readiness.freeze.receipts"
                    ).is_dir():
                        self.assertEqual(
                            {
                                relative.parts[0]
