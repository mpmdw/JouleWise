...
def minted_successor_pack_rels() -> list[Path]:
    ...
    current_rel = GENERATOR_MODULE.PACK_REL
...
class D117GammaPlanTest(unittest.TestCase):
    ...
    def test_successor_generation_threads_plan_identity_and_lineage(self) -> None:
        ...
        successor_id = GENERATOR_MODULE.PACK_REL.name.removesuffix(
            GENERATOR_MODULE.CURRENT_FAMILY_SUFFIX
        ) + successor_suffix
        successor_rel = GENERATOR_MODULE.PACK_REL.with_name(successor_id)
        with tempfile.TemporaryDirectory(
            prefix=f"d117-gamma{successor_suffix}-"
        ) as temp:
            output_root = Path(temp)
            ...
            self.assertEqual(
                tree["plan"]["plan_id"],
                GENERATOR_MODULE.PLAN_ID.removesuffix("v1") + successor_token,
            )
            self.assertEqual(
                tree["window_identity"]["evidence_root_id"],
                GENERATOR_MODULE.EVIDENCE_ROOT_ID.removesuffix("v1")
                + successor_token,
            )
            reservation = next(
                stage
            ...
            commit_fixture(output_root, f"track emitted gamma {successor_token}")
            preserve_command = [
                sys.executable,
                str(pack_root / "generate_configs.py"),
                "--output-root",
