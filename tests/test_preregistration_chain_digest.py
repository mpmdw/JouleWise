...
class PreregistrationChainDigestTests(unittest.TestCase):
    ...
    def test_issuer_epoch_pins_agree_across_revisions_one_and_four(self):
        text = PREREGISTRATION.read_text(encoding="utf-8")
        ...
        self.assertEqual(len(powermetrics), 2)
        self.assertEqual(len(set(os_builds)), 1)
        self.assertEqual(len(set(powermetrics)), 1)
        self.assertEqual(
            issuer.preregistration_epoch_pins(text),
            (os_builds[0], powermetrics[0]),
        )
