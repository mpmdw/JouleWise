...
def _constant_command_value(node: ast.AST) -> str | None:
    """Fold literal string addition; the resolver walks argv containers."""
...
class _CommandLiterals:
    ...
    def __call__(self, node: ast.AST, seen: frozenset[str] = frozenset()) -> tuple[str, ...]:
        ...
        if isinstance(node, ast.Constant) and isinstance(node.value, str):
            result = (node.value,)
        elif (folded := _constant_command_value(node)) is not None:
            result = (folded,)
