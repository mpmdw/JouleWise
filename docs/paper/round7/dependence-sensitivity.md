...
For replay, `raw_two_sided_p_replay` prints \(p=I_x(\nu/2,1/2)\) with \(x=\nu/(\nu+t^2)\), so a library that evaluates this function can reproduce each printed p-value. Here \(I_x\) is the **regularized incomplete beta function**: the fraction of a Beta(\(\nu/2,1/2\)) distribution's probability lying below \(x\). The implementation calls `two_sided_student_t_p_value` (in `joulewise/analysis_engine/distributions.py`, line 166; Lentz continued-fraction support, an iterative numerical fraction evaluation, lines 49–115) for p and `student_t_quantile` (same file, line 131) for the critical value; the latter supplies 2.262 and 2.776, rounded to three decimal places at `scripts/dependence_sensitivity.py:194`.
...
A **fill registry** is the table that says which authenticated result supplies each paper location. A **placement** is one such paper location. A **source artifact** is the calculator's saved JSON result. A **STOP_FILL rule** instructs the paper writer to omit a placement when its stated evidence is absent or invalid. **KEY_FROZEN** means the placement's field names and derivation are fixed before data. **VALUE_UNISSUED** means no campaign value has been produced for that fixed key. An **insertion anchor** is the quoted frozen-draft sentence at the line before which the placement belongs. These are proposed new placements; no registry or paper-rendering file changes here. The sensitivity tables use `joulewise.dependence_sensitivity.v1`; each disagreement row also uses its registered DS-31 or PG-07 direction-gate supplier for the bracketed word.
