...
> **STATUS (2026-08-29, ruled): HELD — awaiting the `_v5` rewrite round; NOT usable for mechanical substitution.**
> The magistrate ruling (`docs/process_traces/2026-08-27-t26/paper-round7-prep/04-MAGISTRATE-RULING.md`) ADOPTED the built-terms lexicon + first-use lint (`scripts/paper_terms_lint.py`; 74 findings currently open against this sheet) and DEFERRED the lexicon-constrained rewrite until the `_v5` registry pins the token namespace: D-164 (Qwen3-1.7B/8B) supersedes every `1p5B`/`7B` token and model name below, D-165 (dominance ratio R ≥ 2 with common-mode R_cm disclosure) supersedes the item-34-predicate outcome definitions, and D-166 changes the workload prose. Outcome D was ruled a combinable prefix, item 60's duplicate scope sentence was resolved, and U01–U06 (+ the two census sentences) were approved into the rewrite's scope.
