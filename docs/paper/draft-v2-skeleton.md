...
> This paper examines a physical ambiguity in measuring the energy of two consecutive parts of an on-device large-language-model request: a power sample—one reading that averages power from its recorded start time to its recorded end time—can straddle the time between the parts, so uncertainty about that dividing time can move energy from one part to the other without changing the request total. JouleWise measured the allowed movement with commanded graphics-processor pulses—fixed-duration GPU work whose start and stop commands were time-stamped—inside one uninterrupted collection, meaning one continuous power recording that was never stopped. The pulse-derived limit is the largest timing displacement those pulse records allowed; applying it to sustained model work assumes matching edge behavior, meaning that the power record locates the starts and stops of pulses and model work with the same timing error. A configuration cell is a group with the same work, model, machine, software, and power definition, and a component is one separate within-model repeated-measurement calculation or between-model four-run calculation. At the recorded edge—the dividing time stored by the runtime—the analysis found the largest spurious difference, meaning the largest false energy difference that the fixed calculation could manufacture, and compared it with the corresponding limit after every permitted edge movement. An independent-edge ratio divides the moved-edge limit by the recorded-edge limit while each run's edge moves separately; a comparative shared-error ratio repeats that division while timing error common to a four-run comparison—four runs ordered first model, second model, second model, first model—moves together. Every required ratio was authenticated and evaluable: authentication matched its inputs to the named source bytes, and evaluation divided by a nonzero recorded-edge limit. Every ratio was at least 2, so the moved-edge limit was at least twice the recorded-edge limit for every component. The fixed Qwen3-8B and Qwen3-1.7B pair then exercised a decision rule: it stated a direction only when the measured difference exceeded the largest spurious difference and its uncertainty range—the lowest through highest difference allowed after known measurement errors—remained on the direction fixed before collection; otherwise it stated why no direction was warranted. Separately, the retained short-input diagnostic records—earlier, non-claim measurements of brief requests—contained [FILL:DG-067] of [FILL:DG-068] measured parts with too few overlapping power samples, meaning fewer than three sampler records crossed the part, to separate the two request parts, while [FILL:DG-069] had at least three and could be separated. The result is limited to the tested Apple computer and software and to its internal processor-power fields, the CPU, GPU, and neural-engine powers reported by macOS rather than power at the wall outlet. The campaign itself did not test whether the pulse-derived limit applies to inference; independently, the post-campaign inserted-gap check—an experiment that placed about 500 ms of no work between the two inference parts and compared its time-stamped edges with the power record—[FILL:TR-01].
<!-- OUTCOME-BRANCH:A:END -->

<!-- OUTCOME-BRANCH:B:START -->
**B — an authenticated, evaluable ratio is below 2:**

> This paper examines a physical ambiguity in measuring the energy of two consecutive parts of an on-device large-language-model request: a power sample—one reading that averages power from its recorded start time to its recorded end time—can straddle the time between the parts, so uncertainty about that dividing time can move energy from one part to the other without changing the request total. JouleWise measured the allowed movement with commanded graphics-processor pulses—fixed-duration GPU work whose start and stop commands were time-stamped—inside one uninterrupted collection, meaning one continuous power recording that was never stopped. The pulse-derived limit is the largest timing displacement those pulse records allowed; applying it to sustained model work assumes matching edge behavior, meaning that the power record locates the starts and stops of pulses and model work with the same timing error. A configuration cell is a group with the same work, model, machine, software, and power definition, and a component is one separate within-model repeated-measurement calculation or between-model four-run calculation. At the recorded edge—the dividing time stored by the runtime—the analysis found the largest spurious difference, meaning the largest false energy difference that the fixed calculation could manufacture, and compared it with the corresponding limit after every permitted edge movement. An independent-edge ratio divides the moved-edge limit by the recorded-edge limit while each run's edge moves separately; a comparative shared-error ratio repeats that division while timing error common to a four-run comparison—four runs ordered first model, second model, second model, first model—moves together. Every required ratio was authenticated and evaluable, but at least one independent-edge or required shared-error ratio was below 2; authentication matched each ratio's inputs to the named source bytes, and evaluation divided by a nonzero recorded-edge limit. The components with a below-two ratio were [FILL:OB-01]. The evidence therefore did not reproduce the proposed twofold boundary contribution—the claim that allowed boundary movement at least doubled the recorded-edge limit—for every component. The fixed Qwen3-8B and Qwen3-1.7B pair still exercised a decision rule: it stated a direction only when the measured difference exceeded the largest spurious difference and its uncertainty range—the lowest through highest difference allowed after known measurement errors—remained on the direction fixed before collection; otherwise it stated why no direction was warranted. Separately, the retained short-input diagnostic records—earlier, non-claim measurements of brief requests—contained [FILL:DG-067] of [FILL:DG-068] measured parts with too few overlapping power samples, meaning fewer than three sampler records crossed the part, to separate the two request parts, while [FILL:DG-069] had at least three and could be separated. The result is limited to the tested Apple computer and software and to its internal processor-power fields, the CPU, GPU, and neural-engine powers reported by macOS rather than power at the wall outlet. The campaign itself did not test whether the pulse-derived limit applies to inference; independently, the post-campaign inserted-gap check—an experiment that placed about 500 ms of no work between the two inference parts and compared its time-stamped edges with the power record—[FILL:TR-01].
...
> This paper examines a physical ambiguity in measuring the energy of two consecutive parts of an on-device large-language-model request: a power sample—one reading that averages power from its recorded start time to its recorded end time—can straddle the time between the parts, so uncertainty about that dividing time can move energy from one part to the other without changing the request total. JouleWise measured the allowed movement with commanded graphics-processor pulses—fixed-duration GPU work whose start and stop commands were time-stamped—inside one uninterrupted collection, meaning one continuous power recording that was never stopped. The pulse-derived limit is the largest timing displacement those pulse records allowed; applying it to sustained model work assumes matching edge behavior, meaning that the power record locates the starts and stops of pulses and model work with the same timing error. A configuration cell is a group with the same work, model, machine, software, and power definition, and a component is one separate within-model repeated-measurement calculation or between-model four-run calculation. At the recorded edge—the dividing time stored by the runtime—the analysis found the largest spurious difference, meaning the largest false energy difference that the fixed calculation could manufacture, and compared it with the corresponding limit after every permitted edge movement. An independent-edge ratio divides the moved-edge limit by the recorded-edge limit while each run's edge moves separately; a comparative shared-error ratio repeats that division while timing error common to a four-run comparison—four runs ordered first model, second model, second model, first model—moves together. The decision rule treated each required ratio as usable only after authentication and evaluation: authentication would match its inputs to the named source bytes, and evaluation would divide by a nonzero recorded-edge limit. A below-two result after those checks would be the opposite of the proposed twofold boundary result. A required comparison value was absent, could not be verified against its source, or divided by a repeated-measurement limit of zero, so the fixed rule authorized neither the proposed twofold boundary result nor its opposite and preserved the reason that the calculation stopped. The Qwen3-8B and Qwen3-1.7B records therefore supplied no direction between the models and no conclusion about the size of the boundary contribution. Separately, the retained short-input diagnostic records—earlier, non-claim measurements of brief requests—contained [FILL:DG-067] of [FILL:DG-068] measured parts with too few overlapping power samples, meaning fewer than three sampler records crossed the part, to separate the two request parts, while [FILL:DG-069] had at least three and could be separated. The stopped comparison is limited to the tested Apple computer and software and to its internal processor-power fields, the CPU, GPU, and neural-engine powers reported by macOS rather than power at the wall outlet. The campaign itself did not test whether the pulse-derived limit applies to inference; independently, the post-campaign inserted-gap check—an experiment that placed about 500 ms of no work between the two inference parts and compared its time-stamped edges with the power record—[FILL:TR-01].
...
>
> Because every independent-edge ratio and every required shared-error ratio was at least 2, measurement practice must change before collection: characterize the named workload boundary—identify the exact transition whose energy assignment can change and measure how far its recorded time may move—construct a separate bound for each configuration cell, and size the comparison against that bound—choose before collection a workload difference whose expected energy separation is larger than the bound. Additional repeats can narrow the point-only component, but they cannot remove the larger contribution from allowed boundary movement; the fixed Qwen3-8B-versus-Qwen3-1.7B pair demonstrates that decision behavior, not a model-size scaling law. Earlier non-claim measurements produced prompt-processing, token-generation, and short-prompt ratios of [FILL:DG-099], [FILL:DG-100], and [FILL:DG-101], respectively; these historical examples neither supplied nor selected the campaign result. Independently of that ratio outcome, the inserted-gap check described below [FILL:TR-01].
<!-- OUTCOME-BRANCH:A:END -->
...
>
> A required ratio was absent, could not be verified against its source, or had a zero denominator, so the evidence supports neither the all-pass statement nor the claim that an evaluated component fell below 2. The protocol therefore reports only that incomplete evidence was stopped with a recorded reason; it supports neither a directional model result nor a boundary-movement quotient from those records. Earlier non-claim measurements produced prompt-processing, token-generation, and short-prompt ratios of [FILL:DG-099], [FILL:DG-100], and [FILL:DG-101], respectively; these historical examples do not replace the unavailable campaign result, and the fixed Qwen3 pair supplies no scaling law. Independently of that ratio outcome, the inserted-gap check described below [FILL:TR-01].
<!-- OUTCOME-BRANCH:REFUSAL:END -->
...
> JouleWise makes a false phase-energy difference measurable before using that limit to judge a model comparison: it corrects clock placement inside each collection, measures allowed edge movement with deliberately started graphics-processor work, and combines the resulting boundary contribution with repeat-to-repeat variation for each configuration cell. Every independent-edge ratio was at least 2, and every comparative ratio remained at least 2 when timing error shared within a four-run block moved together, so allowed boundary movement at least doubled every component's point-only bound. The fixed Qwen3-8B-versus-Qwen3-1.7B comparison demonstrates the resulting decision behavior rather than a model-size scaling law; the retained short-input diagnostic separately found that [FILL:DG-067] of [FILL:DG-068] measured phases failed the minimum overlap rule and [FILL:DG-069] passed it. The result applies to one Apple computer, one software and internal-counter configuration, and the tested workloads. Independently of the ratio outcome, the post-campaign inserted-gap check [FILL:TR-01].
<!-- OUTCOME-BRANCH:A:END -->

<!-- OUTCOME-BRANCH:B:START -->
**B — an authenticated, evaluable ratio is below 2:**

> JouleWise makes a false phase-energy difference measurable before using that limit to judge a model comparison: it corrects clock placement inside each collection, measures allowed edge movement with deliberately started graphics-processor work, and combines the resulting boundary contribution with repeat-to-repeat variation for each configuration cell. Every required ratio was authenticated and evaluable, but at least one independent-edge ratio or required shared-error ratio was below 2, so the paper does not claim that boundary placement doubled every point-only bound. The components with a below-two ratio were [FILL:OB-01]. The fixed Qwen3-8B-versus-Qwen3-1.7B comparison demonstrates the resulting decision behavior rather than a model-size scaling law; the retained short-input diagnostic separately found that [FILL:DG-067] of [FILL:DG-068] measured phases failed the minimum overlap rule and [FILL:DG-069] passed it. The result applies to one Apple computer, one software and internal-counter configuration, and the tested workloads. Independently of the ratio outcome, the post-campaign inserted-gap check [FILL:TR-01].
<!-- OUTCOME-BRANCH:B:END -->

<!-- OUTCOME-BRANCH:REFUSAL:START -->
**Refusal — a required ratio is missing, unauthenticated, or has a zero denominator:**

> JouleWise fixes how a false phase-energy difference would be measured before using that limit to judge a model comparison: it corrects clock placement inside each collection, measures allowed edge movement with deliberately started graphics-processor work, and combines the resulting boundary contribution with repeat-to-repeat variation for each usable configuration cell. A required ratio was absent, could not be verified against its source, or had a zero denominator, so the procedure preserved the reason it stopped and reported neither a boundary-doubling result nor its evaluated opposite. The Qwen3-8B-versus-Qwen3-1.7B records therefore support no model direction or scaling law; the retained short-input diagnostic separately found that [FILL:DG-067] of [FILL:DG-068] measured phases failed the minimum overlap rule and [FILL:DG-069] passed it. The stopped result is confined to one Apple computer, one software and internal-counter configuration, and the tested workloads. Independently of the ratio outcome, the post-campaign inserted-gap check [FILL:TR-01].
<!-- OUTCOME-BRANCH:REFUSAL:END -->
...
|---|---|---|---|
| commanded graphics-processor pulses / uninterrupted collection | Abstract | glossed-at-first-use | Fixed-duration GPU work with time-stamped start and stop commands, recorded inside one continuous power recording that was never stopped. |
| pulse-derived limit / edge behavior | Abstract | glossed-at-first-use | Largest timing displacement allowed by the pulse records; edge behavior asks whether the power record locates pulse and model-work starts and stops with the same timing error. |
| component | Abstract | glossed-at-first-use | One separate within-model repeated-measurement calculation or between-model four-run calculation. |
| largest spurious difference / permitted edge movement | Abstract | glossed-at-first-use | Largest false energy difference the fixed calculation could manufacture, first at the stored dividing time and then after every allowed movement of that time. |
| independent-edge ratio / four-run comparison / timing error common to a four-run comparison | Abstract | glossed-at-first-use | Moved-edge limit divided by recorded-edge limit with separate movement per run; the shared version moves one timing error across four runs ordered first model, second model, second model, first model. |
| moved-edge limit / quotient / repeated-measurement limit / opposite | Abstract | glossed-at-first-use | The moved-edge and recorded-edge limits are the ratio operands; the quotient is their division, and the opposite of the proposed twofold result is the authenticated below-two result. |
| decision rule / uncertainty range / direction fixed before collection | Abstract | glossed-at-first-use | A direction prints only when the measured difference clears the false-difference limit and the lowest-to-highest range after known measurement errors stays on the direction chosen before collection. |
| short-input diagnostic records / overlapping power samples / internal processor-power fields | Abstract | glossed-at-first-use | Earlier non-claim measurements of brief requests; sampler records crossing the measured part; and the CPU, GPU, and neural-engine powers reported by macOS rather than wall-outlet power. |
| twofold boundary contribution / required ratio | Abstract | glossed-at-first-use | A required ratio compares the moved-edge and recorded-edge limits; the twofold claim means the former is at least twice the latter. |
...
| missing / malformed / physical ambiguity / power sample | Abstract | glossed-at-first-use | The Abstract defines a power sample as one start-to-end average and shows the ambiguity caused when it straddles the dividing time; missing and malformed retain their plain-English meanings. |
...
| cell | Abstract | glossed-at-first-use | A configuration cell groups runs sharing work, model, machine, software, and power definition. |
...
| authenticated / unauthenticated / authentication / evaluable / evaluation | Abstract | glossed-at-first-use | Authentication matches ratio inputs to named source bytes; evaluation divides by a nonzero recorded-edge limit. |
...
| shared-error ratio | Abstract | glossed-at-first-use | The moved-edge to recorded-edge division when timing error common to the four-run comparison moves together. |
...
| A — every required ratio passes / B — an authenticated, evaluable ratio is below 2 / Refusal — a required ratio is missing, unauthenticated, or has a zero denominator / outcome A / outcome B / recorded edge | Abstract | glossed-at-first-use | The control labels state all-pass, evaluable-below-two, and cannot-evaluate dispositions and are removed by selection; each selected paragraph defines the recorded edge as the dividing time stored by the runtime. |
| MLX | Outcome sentence forms | glossed-at-first-use | Apple's on-device inference framework. |
| inserted-gap check | Abstract | glossed-at-first-use | An approximately 500-ms no-work gap inside inference whose independently time-stamped edges are compared with the power record. |
...
a failure. Terms inventoried: 233; FAILS: 0.
