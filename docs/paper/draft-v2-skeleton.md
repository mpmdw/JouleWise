...
Running Average Power Limit (RAPL) is a processor-exposed energy counter. Khan et al.'s *RAPL in Action* and Jay et al. own the gain axis: how accurately a software counter reports the magnitude of energy use [1] [2]. For phase-resolved `powermetrics` inference on Apple Silicon, JouleWise opens the complementary time axis: where in time a counter places the energy it reports. Khan et al. align lag, model the relationship between RAPL and wall power, account for temporal correlation, and inspect update granularity, sampler overhead, jitter, overflow, and timestamps [1]. Jay et al. show through controlled regression against wall power that disagreement changes with load, and they decline component claims that their reference meter cannot test [2]. Those studies establish how to validate counter gain; an external wall meter still cannot determine how a software trace should divide a correct total between prompt processing and token generation.

Hähnel et al. are the closest ancestor to this boundary problem. RAPL's update interval limits how short a code path can receive a defensible energy attribution, and they respond by aligning the start and end of the measured path to the counter's own update boundaries — spinning on the register until it advances before entering the code path, and again on leaving it — then enumerating the errors that remain when entry and exit fall inside a single update interval [3]. That is edge placement as an explicit technique, on a different interface and at a different scale. Burtscher et al. show that a GPU built-in power sensor's lag and smoothing can misplace energy in time [4]. Dauner et al. provide the strongest corroboration. Across RAPL and the NVIDIA Management Library (NVML) software power counter, they show that counter-update behavior and requested sampling frequency can materially change an energy reading; on one evaluated GPU, very frequent polling severely underestimated integrated power, with agreement recovering only at a much longer interval [5]. JouleWise's distinct contribution is to calibrate runtime phase edges in the same measurement window, propagate their permitted positions through the energy integral, and make the resulting cell-specific resolution bound a claim gate (Sections 2, 3, and 5).

### LLM energy measurement

*The Illusion of Power Capping in LLM Decode* is the closest methodological rival. It is phase-aware, repeats configurations, and independently checks sufficiently long sampled-power integrals against a hardware energy counter [6]. JouleWise lacks that independent cross-check. Its narrower advance is different: the power-capping study reports counter agreement, repetition, and timing regimes as separate diagnostics, whereas JouleWise carries uncertain phase-edge placement into the bound that decides whether a phase contrast may be reported.

TokenPowerBench reports prefill and decode energy and groups measurements by context length [7]. Its disclosed method does not specify the boundary events, alignment rule, repetition and variance protocol, idle baseline, or external validation needed to reconstruct a phase-attribution error budget. Ruf and Detyniecki isolate prefill by generating one token and infer decode by subtraction, from one run per context length without error bars [8]. Broader efforts such as ML.ENERGY, Intelligence per Watt, and Apple-focused inference characterizations map energy across useful deployed configurations [9] [10], and Benazir and Lin characterize inference throughput on Apple silicon without energy measurement [11]. They answer system-selection questions; JouleWise instead asks whether one named software-counter boundary can support a phase claim at all.

### Benchmark and metrology lineage

JouleSort established that an energy-efficiency benchmark needs a fixed workload, a comparison metric, and explicit rules for executing the workload and measuring energy [12]. Its boundary is specific: wall power includes conversion losses and every participating component, including idle components; any net change in stored battery energy must be shown no greater than zero with 95% confidence or included in the total. JouleSort also identified synchronization between meter readings and the actual run, alongside the meter's ±1.5% specification, as a reason not to use a fixed-energy-budget metric. JouleWise inherits that boundary discipline rather than replacing it: JouleSort names the synchronization problem at whole-run scale; JouleWise measures its consequence at phase scale.

SPECpower fixes a graduated-load server workload and accepted-analyzer reporting discipline [13]. MLPerf Power extends public energy benchmarking across machine-learning systems, while its associated SPEC methodology requires load-specific analyzer uncertainty, fixed ranges, minimum measurement intervals, invalid-sample accounting, clock synchronization, and controlled battery behavior [14] [15]. JouleWise translates their run-level refusal discipline to a consumer software counter: missing timing evidence invalidates the phase claim rather than disappearing into an average.

Rigorous performance metrology supplies the experimental lineage. Georges, Buytaert, and Eeckhout make repetition, warmup, independence, and uncertainty part of performance evaluation [16]. Mytkowicz et al. show that apparently harmless experimental choices can create systematic measurement bias [17]. JouleWise operationalizes those warnings through paired order, bracketed calibration, fixed-before-collection rules, and explicit refusals, while evidence from one host and configuration cannot establish generality.

Paired minimum-detectable-effect methods use paired variation to estimate the smallest effect a planned study has adequate power to detect. They can allow observed variability to raise, but not lower, a threshold fixed by pre-registration—before results are seen [18]. That work concerns quantization-accuracy benchmarking, not energy. JouleWise borrows its prospective discipline, but treats worst-case phase-edge placement as bounded systematic uncertainty and does not combine it statistically as though it were independent random noise; doing so would take credit for cancellation that the instrument has not demonstrated [19].

Split and disaggregated inference remain a demanding application rather than this capstone's contribution. Prior work reports whole-run or GPU-only energy for disaggregation and phase-aware placement [20] [21] [22], while SplitZip makes no energy claim [23]. A future JouleWise study would need named boundaries at both endpoints, cross-device clock alignment, and a resolution bound established before collection.
...
1. K. N. Khan, M. Hirki, T. Niemi, J. K. Nurminen, and Z. Ou. “RAPL in Action: Experiences in Using RAPL for Power Measurements.” *ACM Transactions on Modeling and Performance Evaluation of Computing Systems* 3(2), 2018, Article 9. https://doi.org/10.1145/3177754.
2. M. Jay, V. Ostapenco, L. Lefèvre, D. Trystram, A.-C. Orgerie, and B. Fichel. “An Experimental Comparison of Software-Based Power Meters: Focus on CPU and GPU.” *23rd IEEE/ACM International Symposium on Cluster, Cloud and Internet Computing (CCGrid)*, 2023, pp. 106–118. https://doi.org/10.1109/CCGrid57682.2023.00020; https://hal.science/hal-04030223.
3. M. Hähnel, B. Döbel, M. Völp, and H. Härtig. “Measuring Energy Consumption for Short Code Paths Using RAPL.” *ACM SIGMETRICS Performance Evaluation Review* 40(3), 2012, pp. 13–17. https://doi.org/10.1145/2425248.2425252.
4. M. Burtscher, I. Zecena, and Z. Zong. “Measuring GPU Power with the K20 Built-in Sensor.” *Proceedings of the 7th Workshop on General Purpose Processing Using GPUs (GPGPU-7)*, 2014, pp. 28–36. https://doi.org/10.1145/2576779.2576783.
5. M. Dauner, M. Steinberg, A. Brunnert, B. Schicker, and B. Zönnchen. “Evaluating the Influence of Measurement Frequency on Energy Readings Using Intel RAPL and NVIDIA NVML.” *HotCarbon Workshop on Sustainable Computer Systems (HotCarbon '26)*, Seattle, WA, July 2026. https://hotcarbon.org/assets/2026/paper-46.pdf. To appear in *ACM SIGENERGY Energy Informatics Review*.
6. B. Ma, A. Afzal, J. Eitzinger, and G. Wellein. “The Illusion of Power Capping in LLM Decode: A Phase-Aware Energy Characterisation Across Attention Architectures.” arXiv preprint, 2026. https://arxiv.org/abs/2605.11999.
7. C. Niu, W. Zhang, J. Li, Y. Zhao, T. Wang, X. Wang, and Y. Chen. “TokenPowerBench: Benchmarking the Power Consumption of LLM Inference.” *Proceedings of the Fortieth AAAI Conference on Artificial Intelligence* 40(38), 2026, pp. 32582–32590. https://arxiv.org/abs/2512.03024.
8. B. Ruf and M. Detyniecki. “The Cost of Context: Profiling the Energy Footprint of Input Tokens in Large Language Models.” *HotCarbon Workshop on Sustainable Computer Systems (HotCarbon '26)*, Seattle, WA, July 2026. https://hotcarbon.org/assets/2026/paper-17.pdf. To appear in *ACM SIGENERGY Energy Informatics Review*.
9. J.-W. Chung et al. “The ML.ENERGY Benchmark: Toward Automated Inference Energy Measurement and Optimization.” *Advances in Neural Information Processing Systems, Datasets and Benchmarks Track*, Spotlight, 2025. https://arxiv.org/abs/2505.06371.
10. J. Saad-Falcon, A. Narayan, et al. “Intelligence per Watt: Measuring Intelligence Efficiency of Local AI.” arXiv preprint, 2025. https://arxiv.org/abs/2511.07885.
11. A. Benazir and F. X. Lin. “Benchmarking and Characterization of Large Language Model Inference on Apple Silicon.” *Proceedings of the ACM on Measurement and Analysis of Computing Systems* 9(3), December 2025, pp. 1–26. https://doi.org/10.1145/3771563.
12. S. Rivoire, M. A. Shah, P. Ranganathan, and C. Kozyrakis. “JouleSort: A Balanced Energy-Efficiency Benchmark.” *Proceedings of the 2007 ACM SIGMOD International Conference on Management of Data*, 2007, pp. 365–376. https://doi.org/10.1145/1247480.1247522.
13. K.-D. Lange. “Identifying Shades of Green: The SPECpower Benchmarks.” *IEEE Computer* 42(3), 2009, pp. 95–97. https://doi.org/10.1109/MC.2009.84.
14. A. Tschand et al. “MLPerf Power: Benchmarking the Energy Efficiency of Machine Learning Systems from μWatts to MWatts for Sustainable AI.” *IEEE International Symposium on High-Performance Computer Architecture (HPCA)*, 2025, pp. 1201–1216. https://doi.org/10.1109/HPCA61900.2025.00092; https://arxiv.org/abs/2410.12032.
15. Standard Performance Evaluation Corporation. *Power and Performance Benchmark Methodology*, V2.3. SPECpower Committee, n.d. https://www.spec.org/power/docs/SPEC-Power_and_Performance_Methodology.pdf.
16. A. Georges, D. Buytaert, and L. Eeckhout. “Statistically Rigorous Java Performance Evaluation.” *Proceedings of the 22nd Annual ACM SIGPLAN Conference on Object-Oriented Programming Systems, Languages and Applications (OOPSLA '07)*, 2007, pp. 57–76. https://doi.org/10.1145/1297027.1297033.
17. T. Mytkowicz, A. Diwan, M. Hauswirth, and P. F. Sweeney. “Producing Wrong Data Without Doing Anything Obviously Wrong!” *Proceedings of the 14th International Conference on Architectural Support for Programming Languages and Operating Systems (ASPLOS XIV)*, 2009, pp. 265–276. https://doi.org/10.1145/1508244.1508275.
18. Z. Zhuang, Y. Li, and Z. Fan. “Pre-Registering the Detectable Effect: A Paired-MDE Budget for 4-bit Quantization Benchmarks, with a Pilot Audit.” arXiv preprint, 2026. https://arxiv.org/abs/2605.28873.
19. M. Milanese and A. Vicino. “Optimal Estimation Theory for Dynamic Systems with Set Membership Uncertainty: An Overview.” *Automatica* 27(6), 1991, pp. 997–1009. https://doi.org/10.1016/0005-1098(91)90134-N.
20. J. Li, Y. Zhu, B. Chen, E. K. Lee, and K. Nahrstedt. “Revisiting Disaggregated Large Language Model Serving for Performance and Energy Implications.” *Proceedings of the 2026 European Workshop on Machine Learning and Systems (EuroMLSys '26)*, 2026, pp. 397–406. https://doi.org/10.1145/3805621.3807662; https://arxiv.org/abs/2601.08833.
21. O. Basit, Y. Liu, Z. J. Kong, and Y. C. Hu. “DualScale: Energy-Efficient Disaggregated LLM Serving via Phase-Aware Placement and DVFS.” arXiv preprint, 2026. https://arxiv.org/abs/2602.18755.
22. Z. Li et al. “Prima.cpp: Fast 30-70B LLM Inference on Heterogeneous and Low-Resource Home Clusters.” *The Fourteenth International Conference on Learning Representations (ICLR)*, 2026. https://arxiv.org/abs/2504.08791.
23. Y. Guo and S. Joshi. “SplitZip: Ultra Fast Lossless KV Compression for Disaggregated LLM Serving.” arXiv preprint, 2026. https://arxiv.org/abs/2605.01708.
24. K. Marzullo and S. Owicki. “Maintaining the Time in a Distributed System.” *Proceedings of the Second Annual ACM Symposium on Principles of Distributed Computing (PODC '83)*, 1983, pp. 295–305. https://doi.org/10.1145/800221.806730.
...
|---|---|---|---|
| prompt processing / prefill | 2. In-window calibration method | glossed-at-first-use | Prompt work through the first output token; the shorthand follows the physical phrase. |
| token generation / decode | 2. In-window calibration method | glossed-at-first-use | Later output-token emission; the shorthand follows the physical phrase. |
...
| powermetrics | 2. In-window calibration method | glossed-at-first-use | The macOS sampler and its shared interval-average CPU, GPU, and neural-engine record are stated at first use. |
| sampling record | 2. In-window calibration method | built-before | The preceding clause builds the one start-to-end average-power record before naming its use in phase assignment. |
...
| science window / measurement window | Bracketed pulse-train algorithm | glossed-at-first-use | One uninterrupted measurement session; Figure 2 later diagrams the same window. |
| SHA-256 / SHA-256 fingerprint | Bracketed pulse-train algorithm | glossed-at-first-use | A digest that identifies exact file bytes. |
...
| monotonic clock | Bracketed pulse-train algorithm | glossed-at-first-use | A counter that advances but is never corrected to civil time. |
...
| cell | Comparing the boundary-moved and point-only bounds | glossed-at-first-use | Runs sharing phase, workload, model, hardware, software, and processor-power definition. |
| false-difference components / false-difference | Comparing the boundary-moved and point-only bounds | glossed-at-first-use | The next two sentences split the cell's false-difference sources into absolute and comparative components. |
...
| independent-edge corner bound / corner | Comparing the boundary-moved and point-only bounds | glossed-at-first-use | Every joint lower-or-upper endpoint choice is enumerated and the complete formula recalculated. |
...
| threshold / exact equality | Comparing the boundary-moved and point-only bounds | audience-vocabulary | Plain-English comparison words; the fixed threshold is 2 and equality passes. |
| authenticated / unauthenticated | Comparing the boundary-moved and point-only bounds | glossed-at-first-use | Expected SHA-256 fingerprints and named inputs agree, or a required fingerprint, match, or check is absent. |
| registered rounding / registered | Comparing the boundary-moved and point-only bounds | glossed-at-first-use | Rounding fixed before collection in fingerprinted plan bytes. |
...
| local half-width / shared sign / local sign | Comparing the boundary-moved and point-only bounds | glossed-at-first-use | The member-local changes form one block half-width; enumeration uses one common sign and one per-block local sign. |
| half-width | Comparing the boundary-moved and point-only bounds | glossed-at-first-use | The local half-width is \(\ell_j=(r_{j1}+r_{j2}+r_{j3}+r_{j4})/2\). |
| shared-error ratio / \(R_{cm}\) | Comparing the boundary-moved and point-only bounds | built-before | The complete shared/local replay quotient is constructed immediately before it is named. |
...
| resolution bound / cell floor | Adding publication safeguards after the ratio | glossed-at-first-use | Largest false phase-energy difference the cell admits; artifact name for the maximum guarded component. |
...
| G2-a / hash-bound / prompt pin | Results | glossed-at-first-use | Fixed prompt-length shakedown; digest-bound selection record; retained prompt, token, tokenizer, and generation-rule record. |
...
| three-record minimum | Why the selected prompt length is not yet stated | glossed-at-first-use | A phase must overlap at least three sampler records to be reduced at all. |
...
| set membership | 11. References | built-before | The first literal occurrence is the Milanese–Vicino title in reference 19, a bibliography entry rather than an expository use; the first expository use is in Appendix A.3, where the exact set of anchor-rate pairs consistent with every evidence constraint is built before the phrase names it. |
