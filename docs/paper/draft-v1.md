...
3. A fail-closed collection protocol with admission gates, counterbalanced ordering, tamper-evident custody, pre-registration, and a refusal log.
...
6. An open tool and a planned release of hash-bound evidence artifacts.

## 2. Background and the gap

Energy is obtained by integrating power over a declared interval, so the validity of an energy value depends on both the power reading and the interval boundaries. Formal benchmark methods make this a run-level question: instrument uncertainty, clock synchronization, measurement duration, invalid samples, fixed ranges, and battery behavior must be checked for the interval that produced the result [1] [2]. Their central lesson is that a meter specification alone does not validate a measurement.

Software-visible counters make repeated experiments practical, but they remain measurement systems rather than ground truth. Prior validation work on Intel's Running Average Power Limit counters compares software readings with wall power, searches for lag, measures overhead and update behavior, controls thermal state, and limits claims when no reference instrument observes the quantity of interest [3] [4]. A wall meter can test whole-machine totals. It cannot by itself determine whether a software sample belongs to prompt processing or token generation, so phase attribution requires a separate timing experiment.
...
The custody chain continues after collection. The immutable corpus is backed up before extraction. The extractor re-authenticates member hashes, required files and cross-file consistency, calibration identity, environment admission, complete campaign membership, cooldown evidence, phase metric identity, and the whole-window drift allowance. It excludes or refuses a cell according to the frozen rules; missing uncertainty never becomes zero. The present boundary is narrower than an independently reusable floor file: a claim-bearing floor must be produced by protocol-controlled extraction and consumed by analysis in the same lead-controlled custody session. A standalone floor or result artifact is not independently sufficient to authorize a claim.
...
| Contrast | Point estimate | Interval [lower, upper] | Cell floor | Claim-side bound | Floor-gate outcome | Direction-gate outcome | Verdict |
|---|---|---|---|---|---|---|---|
| token generation, 7B − 1.5B | [PENDING] | [PENDING, PENDING] | [PENDING] | [PENDING] | [PENDING] | [PENDING] | [PENDING] |
| prompt processing, 7B − 1.5B | not registered under the adopted default | not registered | [PENDING] | not applicable | not evaluated | not evaluated | floors only |
...
The reusable contribution is the combination of in-window calibration, detection floors built from measured terms, prospective sizing, counterbalanced collection, fail-closed decisions, and tamper-evident custody. Those methods may transfer to other software counters, but the numerical floor will not; every new instrument, boundary, and stack must establish its own. Future work can add independent stacks, model families, an external meter for total-energy scale, and richer phase structures such as speculative decoding, mixture-of-experts routing, and split inference.
