...
The frozen P06 result specification contains the predicates below. D-152 supplies the ruled values: the phase-overcount tolerance is \(10^{-6}\) J, six held-out reference probes are registered, and an unavailable claim-anchored limit has no absolute substitute. In that last case the row is indeterminate; the artifact's outcome name for it is `characterization_operative_floor_unavailable`.
...
The two inferential properties—the null-mean containment and prefill-invariance containment—use Holm \(\alpha=0.05\) with \(m=2\). Those two form a Holm family of their own; it is a different family from the two-contrast family of Section 6, and the two families are never pooled. Every other cell is a deterministic predicate.
...
2. **Direction and registered inference:** both the metrology-aware interval and the final decision interval lie wholly on the registered side of zero, and the registered statistical test passes. The decision interval is the metrology-aware interval widened on both sides by the contrast's whole deterministic total, the artifact's `deterministic_bounds.total` field; the clock-anchor shift bound (the artifact's `E_clock_anchor_shift_bound_j` field) is one term inside that total, not the whole of it. That deterministic total is not the claim-side bound of the Section 6 sizing disclosure: the claim-side bound is a separately registered quantity (registry `B_decode_claim_J`, row DS-29) whose supplier is not built, and neither gate compares against it.
...
For practical sizing, Section 6 discloses \(F_{\mathrm{cell}}+B_{\mathrm{claim}}\), where \(B_{\mathrm{claim}}\) is the separately registered claim-side bound named above, whose supplier is not built. This sum estimates the effect size likely to clear both gates, but it is not an acceptance threshold: the point estimate is compared only with \(F_{\mathrm{cell}}\), while the intervals are compared only with zero.
...
The rule is mechanical. A *powermetrics* sampling record has a **record support**, its start-to-end time interval. The sampling record counts when the record support and the phase overlap for positive time:
\[
\min(t_{\mathrm{phase,end}},t_{\mathrm{record,end}})>
\max(t_{\mathrm{phase,start}},t_{\mathrm{record,start}}).
\]
A phase is resolvable only when at least three records count; fewer yields the not-resolvable outcome, whose artifact outcome name is `not_resolvable_sample_count`. Boundary records may count even when only part of their support lies inside the phase, because integration clips them to that overlap.

For a concrete case, retained bundle `p2015-df-ph-decode-abs-r03` supplies the times: its runtime events place prompt-processing start and end 0.121034145 s apart. Two different quantities describe the sampler here, and confusing them is the trap this example exists to close. The **sampling-record interval width** is how long one power record covers — the width of its record support in time — and for this bundle it is [PENDING] (DIAGNOSTIC-ERA VALUE: sampling-record interval width for p2015-df-ph-decode-abs-r03). The **record spacing** is the gap between the start times of consecutive records, which is longer than the width because the sampler pauses between records; for this bundle it is [PENDING] (DIAGNOSTIC-ERA VALUE: median record spacing for p2015-df-ph-decode-abs-r03). Neither number decides the rule. The **overlap count** is the number of record supports that retain positive overlap after each is clipped to the phase window. Here the overlap count is two, against a threshold of three, so **2 < 3** and the phase is not resolvable — even though the phase lasts 0.121 s, which is longer than a single record support. Dividing duration by either quantity would give the wrong answer, because alignment, not arithmetic, decides whether a third record support happens to overlap the window. The persisted audit supplies the population counts: 37 phases had two overlapping records and 13 had three.
...
27. J. Li, Y. Zhu, B. Chen, E. K. Lee, and K. Nahrstedt. “Revisiting Disaggregated Large Language Model Serving for Performance and Energy Implications.” *Proceedings of the 2026 European Workshop on Machine Learning and Systems (EuroMLSys '26)*, 2026, pp. 397–406. DOI:10.1145/3805621.3807662; arXiv:2601.08833.
