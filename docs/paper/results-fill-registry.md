...
| OR-01 — Refusal stop stage and issued reason in the Section-4 form and the Abstract, Section 7, and Section 10 Refusal paragraphs | `[FILL:OR-01]` | Registered professor-facing templates are exactly `before comparison: <model> — <issued stop reason>` and `at close-out: <issued reason>; affected: <components or 'none recorded'>`. Before comparison, `<model>` names the affected Qwen3 model; when an authenticated token-generation (`DS-32`) or prompt-processing (`PG-08`) verdict is absent, that slot instead names the absent verdict for the fixed Qwen3-8B-versus-Qwen3-1.7B pair. At close-out, the reason is the authenticated top-level `joulewise.d165_dominance_closeout.v1.refusal_reason`; `affected` lists refused ratio-record components in registered record order, or exactly `none recorded`. Governed stage order is before comparison then at close-out: a before-comparison stop wins, while a later close-out reason is retained only as secondary renderer metadata. Acceptance oracle `before_window`: `before comparison: Qwen3-1.7B — synthetic_window_excluded`. Acceptance oracle `before_verdict`: `before comparison: token-generation verdict for the fixed Qwen3-8B-versus-Qwen3-1.7B pair — required token-generation verdict absent`. Acceptance oracle `closeout_zero`: `at close-out: dominance_ratio_zero_denominator; affected: synthetic-floor-0-0 absolute`. Acceptance oracle `closeout_source`: `at close-out: floor_artifact_source_hash_mismatch; affected: none recorded`. Acceptance oracle `closeout_census`: `at close-out: replay_sidecar.cells: cell census does not match floor artifact; affected: synthetic-floor-1-1 comparative common-mode` | fixed Qwen3 pair / two-stage refusal | STOP_FILL until authenticated governing evidence issues; the renderer's professor-facing bytes are registered | SUPPLIERS_NAMED / RENDERER_ISSUED / VALUE_UNISSUED; `_v5` identity and revisions must match; no caller precedence channel; never infer a reason from ratio disposition | DRAFT, AUTH, D165 |
