...
| `[B_decode_claim_J]` | `_v5` `joulewise.claim_verdicts.v2` exact decode contrast `claim_side_bound.value_j`; required sibling names `E_clock_anchor_shift_bound_j`, is byte-equal to that one `deterministic_bounds.terms[]` bound, and is not `deterministic_bounds.total` (magistrate ruling Q-R2-1, `docs/process_traces/2026-09-04-paper-i/06-magistrate-contract-rulings.md`) | gamma / decode claim interval | MEASURED | KEY_FROZEN / VALUE_UNISSUED | TPL, CV, DF, AUTH |
...
| `[B_prefill_p[PREFILL_LENGTH]_claim_J]` | Selected prefill contrast `claim_side_bound.value_j` under the same Q-R2-1 exact named-term rule as decode | gamma / prefill-p[PREFILL_LENGTH] claim interval | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | CV, DF, AUTH, G2A |
...
| DS-29 — Table 3 decode contrast claim-side bound, line 289, col 6 under `Claim-side bound` | `[PENDING]`; row anchor `\| token generation, 7B − 1.5B \|` | `B_decode_claim_J` from required `joulewise.claim_verdicts.v2.contrasts[].claim_side_bound.value_j`, exactly the named `E_clock_anchor_shift_bound_j` term and not `deterministic_bounds.total` (magistrate ruling Q-R2-1) | gamma / decode contrast | MEASURED | KEY_FROZEN / VALUE_UNISSUED | DRAFT, TPL, DF, CV, AUTH |
...
| DS-32 — Table 3 decode verdict, line 289, col 9 under `Verdict`, and A/B outcome paragraphs | `[PENDING]`; row anchor `\| token generation, 7B − 1.5B \|`; repeated successor marker `[FILL:DS-32]` | `VERDICT_decode` from the authenticated decode `claim_evaluation`; use the ruled `supported`/`not supported`/`not evaluated` branches with model, token-generation phase, registered direction, and exact interval/floor relation, never D-165. On REFUSAL, retain the Table 3 slot: render the authenticated verdict if it issued; if this verdict's absence is the issued stop reason, render byte-exact `not evaluated — required token-generation verdict absent`; if an earlier before-comparison stop prevented evaluation, render `not evaluated — stopped before comparison: <issued reason>` from the governing evidence | gamma / decode contrast | DERIVE | KEY_FROZEN / VALUE_UNISSUED; every repeated placement uses identical bytes | DRAFT, TPL, CV, AUTH |
...
| PG-05 — Table 3 prompt contrast claim-side bound, line 290, col 6 under `Claim-side bound` | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `B_prefill_p[PREFILL_LENGTH]_claim_J` from required selected `joulewise.claim_verdicts.v2.contrasts[].claim_side_bound.value_j` under magistrate ruling Q-R2-1 | gamma / prefill-p[PREFILL_LENGTH] contrast | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DRAFT, TPL, CV, AUTH, V5GEN, G2A |
