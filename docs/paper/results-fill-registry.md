...
| Draft site | Exact marker or anchor | Intended supplier / binding token | Campaign / cell | Fill rule | Freeze status | Sources |
|---|---|---|---|---|---|---|
| OB-01 — Outcome-B failed-component list in the Abstract, Section 7, and Section 10 | `[FILL:OB-01]` | Authenticated `joulewise.d165_dominance_closeout.v1`: in registered stage order, render every `independent_ratios[]` record with `passes` false as exact template `<cell_id> <component>`, followed by every `comparative_common_mode_ratios[]` record with `passes` false as exact template `<cell_id> comparative common-mode`; retain source order within each stage. Join one item verbatim, two as `<item> and <item>`, and three or more as `<item>, <item>, and <item>` with every earlier item comma-separated. Acceptance oracle `branch_b`: `synthetic-floor-0-0 absolute, synthetic-floor-0-0 comparative common-mode, synthetic-floor-0-1 comparative common-mode, synthetic-floor-1-0 comparative common-mode, and synthetic-floor-1-1 comparative common-mode`. Successor seam: `joulewise.results_fill_outcome` accepts governed source paths plus expected digests and a digest-bound close-out validator receipt; substitute only `OutcomeFillResult.fills`, while `OutcomeFillRefusal.reason_code` and `.secondary_closeout_reason` are non-paper metadata and never fill values. | alpha and beta / all required absolute, comparative, and comparative shared-error components | STOP_FILL until an authenticated live close-out issues; the renderer's professor-facing bytes are registered | KEY_FROZEN / VALUE_UNISSUED; never render from an unauthenticated, wrong-identity, incomplete, or unsafe-label close-out | DRAFT, D165, AUTH |
...
| OR-01 — Refusal stop stage and issued reason in the Section-4 form and the Abstract, Section 7, and Section 10 Refusal paragraphs | `[FILL:OR-01]` | Registered professor-facing templates are exactly `before comparison: <model> — <issued stop reason>` and `at close-out: <registered reason sentence>; affected: <components or 'none recorded'>`. Before comparison, `<model>` names the affected Qwen3 model; when an authenticated token-generation (`DS-32`) or prompt-processing (`PG-08`) verdict is absent, that slot instead names the absent verdict for the fixed Qwen3-8B-versus-Qwen3-1.7B pair. Future acceptance oracle `before_window` (blocked on `WHOLE-WINDOW-STOP-RECEIPT-01`): `before comparison: Qwen3-1.7B — synthetic_window_excluded`. Future acceptance oracle `before_verdict` (blocked on `CLAIM-NONISSUANCE-RECEIPT-01`): `before comparison: token-generation verdict for the fixed Qwen3-8B-versus-Qwen3-1.7B pair — required token-generation verdict absent`. At close-out, the authenticated top-level `joulewise.d165_dominance_closeout.v1.refusal_reason` is a code and only the following byte-exact map may supply its sentence. Reason sentence `closeout_input_malformed`: `the dominance close-out inputs were malformed`. Reason sentence `closeout_input_malformed: replay.block_ids`: `the dominance replay block identities were malformed`. Reason sentence `closeout_input_malformed: closeout.independent_ratios`: `the dominance close-out ratio records were malformed`. Reason sentence `closeout_input_malformed: source.census_or_block_membership`: `the dominance close-out source census or block membership was malformed`. Reason sentence `dominance_ratio_zero_denominator`: `a required attribution-dominance ratio could not be evaluated because its repeatability floor was zero`. Reason sentence `floor_artifact_source_hash_mismatch`: `the detection-floor source did not match the finalized campaign record`. Reason sentence `cell_not_common_mode`: `a required comparison lacks its registered common-mode replay`. Reason sentence `common_mode_replay_authenticated_operative_bound_invalid`: `the common-mode replay lacks its authenticated timing bound`. Reason sentence `common_mode_replay_block_count_invalid`: `the common-mode replay has an invalid block count`. Reason sentence `common_mode_replay_input_invalid`: `the common-mode replay inputs were invalid`. Reason sentence `common_mode_replay_window_domain_invalid`: `a common-mode replay window fell outside the registered domain`. Reason sentence `common_mode_replay_zero_point_divergence_out_of_domain`: `a common-mode replay zero point fell outside the registered tolerance`. Reason sentence `common_mode_replay_zero_point_membership_invalid`: `a common-mode replay zero point was absent from its registered sweeps`. Reason sentence `d165_mint_adapter_input_invalid`: `the dominance replay inputs could not be adapted from the detection-floor record`. Reason sentence `dominance_ratio_nonfinite_or_negative_denominator`: `a required attribution-dominance denominator was invalid`. Reason sentence `dominance_ratio_nonfinite_or_negative_numerator`: `a required attribution-dominance numerator was invalid`. Reason sentence `dominance_ratio_nonfinite_result`: `a required attribution-dominance ratio was not finite`. Reason sentence `finalized_manifest_id_mismatch`: `the finalized campaign record did not match its content-derived identity`. Reason sentence `floor_cell_unresolved`: `a required detection-floor cell could not be resolved`. Reason sentence `floor_member_census_mismatch`: `the replay membership did not match the detection-floor membership`. Reason sentence `manifest_lacks_replay_sidecar`: `the finalized campaign record lacks the required dominance replay`. Reason sentence `point_floor_parent_nonfinite_or_negative`: `a required repeatability-floor input was invalid`. Reason sentence `replay_sidecar_digest_mismatch`: `the dominance replay did not match its registered digest`. Reason sentence `replay_sidecar_identity_mismatch`: `the dominance replay did not match its registered identity`. Any other value, including a diagnostic containing a path, index, or quoted representation, is unregistered and refuses out of band. `affected` lists refused ratio-record components in registered record order, or exactly `none recorded`. Governed stage order is before comparison then at close-out: a before-comparison stop wins, while a later close-out reason is retained only as secondary non-paper metadata. Acceptance oracle `closeout_zero`: `at close-out: a required attribution-dominance ratio could not be evaluated because its repeatability floor was zero; affected: synthetic-floor-0-0 absolute`. Acceptance oracle `closeout_source`: `at close-out: the detection-floor source did not match the finalized campaign record; affected: none recorded`. Acceptance oracle `closeout_census`: `STOP_FILL`. Successor seam: `joulewise.results_fill_outcome` accepts governed source paths plus expected digests and a digest-bound close-out validator receipt; substitute only `OutcomeFillResult.fills`, and on `OutcomeFillRefusal` substitute nothing, emit the refusal on stderr, and exit 2. The refusal reason and secondary close-out reason remain non-paper metadata. | fixed Qwen3 pair / two-stage refusal | STOP_FILL until authenticated governing evidence issues; the renderer's professor-facing bytes are registered | SUPPLIERS_NAMED / VALUE_UNISSUED; TOKEN_MISSING for the two future before-comparison renderings; `_v5` identity pins must match; no caller precedence or bytes channel; never infer or print an unregistered reason | DRAFT, AUTH, D165 |
