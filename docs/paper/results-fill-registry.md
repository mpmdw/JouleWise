...
`ctr-d117-decode-qwen3-1p7b-vs-qwen3-8b`. R2 registers the complete symmetric
prefill token family below; `[PREFILL_LENGTH]` is replaced only from an
authenticated selected G2-a record joined to prompt-pin v2. The family exists,
but every prefill value remains `UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED`; a
refused or incomplete G2-a record is `STOP_FILL`, never a 4096 selection.
...
| `[B_decode_claim_J]` | Digest-bound `joulewise.claim_side_bound.v1` row for the production v1 decode contrast; `claim_side_bound.value_j` is byte-equal to that contrast's one `E_clock_anchor_shift_bound_j` `deterministic_bounds.terms[]` bound and is not `deterministic_bounds.total` (Q-R2-1 and R2 addendum) | gamma / decode claim interval | MEASURED | KEY_FROZEN / VALUE_UNISSUED | TPL, CV, DF, AUTH |
...
| `[S_decode_joint_J]` | `F_claim_decode_armwise_max_J + B_decode_claim_J`; disclosure only, never an acceptance gate | gamma / decode contrast | DERIVE | DERIVATION_FROZEN / VALUE_UNISSUED | TPL, DF, CV |
| `[C_decode_sizing_signed_clearance_J]` | `M_decode_contrast_abs_J_per_request - S_decode_joint_J`; signed sizing disclosure, never the floor-only gate clearance | gamma / decode contrast | DERIVE | DERIVATION_FROZEN / VALUE_UNISSUED | TPL, DF, CV |
| `[OUTCOME_decode_floor_gate]` | From unrounded `M_decode_contrast_abs_J_per_request > F_claim_decode_armwise_max_J`, cross-checked to the authenticated claim verdict; render exactly `passes — \|estimate\| > armwise cell floor`, `does not pass — \|estimate\| ≤ armwise cell floor`, or `not evaluated — <issued reason>` | gamma / decode contrast | DERIVE | RENDERER_FROZEN / VALUE_UNISSUED | TPL, CV, AUTH |
| `[OUTCOME_decode_direction_gate]` | From the fully composed interval lying wholly on the registered positive side, cross-checked to the authenticated claim/Holm state; render exactly `passes — the fully composed interval lies wholly above zero`, `does not pass — the fully composed interval does not lie wholly above zero`, or `not evaluated — <issued reason>` | gamma / decode contrast | DERIVE | RENDERER_FROZEN / VALUE_UNISSUED | TPL, CV, AUTH |
| `[VERDICT_decode]` | Exact decode `claim_evaluation`; `direction_supported` may render `supported — Qwen3-8B used more token-generation energy per request than Qwen3-1.7B under the registered comparison` only when the registered direction, L2+ readiness, both gates, and Holm rejection agree; other ruled outcomes use `not supported`/`not evaluated` phrases and authenticated ordered reasons; `equivalent` is `STOP_FILL` | gamma / decode contrast | DERIVE | RENDERER_FROZEN / VALUE_UNISSUED | TPL, CV, AUTH |
| `[E_prefill_p[PREFILL_LENGTH]_contrast_signed_J_per_request]` | Selected `_v5` production `joulewise.claim_verdicts.v1` prefill contrast `estimator.estimate`; orientation is condition B minus condition A and the exact contrast ID is joined to authenticated G2-a plus prompt-pin v2 | gamma / prefill-p[PREFILL_LENGTH] contrast | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | CV, V5GEN, G2A |
| `[E_prefill_p[PREFILL_LENGTH]_contrast_lower_J]` | Same contrast, `deterministic_bounds.decision_interval.lower` | gamma / prefill-p[PREFILL_LENGTH] contrast | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | CV, V5GEN, G2A |
| `[E_prefill_p[PREFILL_LENGTH]_contrast_upper_J]` | Same contrast, `deterministic_bounds.decision_interval.upper` | gamma / prefill-p[PREFILL_LENGTH] contrast | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | CV, V5GEN, G2A |
| `[M_prefill_p[PREFILL_LENGTH]_contrast_abs_J_per_request]` | `abs(E_prefill_p[PREFILL_LENGTH]_contrast_signed_J_per_request)` | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, CV, G2A |
| `[F_claim_prefill_p[PREFILL_LENGTH]_armwise_max_J]` | Maximum of the two exact selected alpha/beta prefill `floor_gate_j` fields; verify against the selected claim artifact's ordered A/B `floor.arm_gates`, max never sum | gamma consumer of alpha and beta prefill-p[PREFILL_LENGTH] floors | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DF, CV, MINT, G2A |
| `[B_prefill_p[PREFILL_LENGTH]_claim_J]` | Selected prefill row from the digest-bound `joulewise.claim_side_bound.v1` sidecar under the same exact named-term rule as decode | gamma / prefill-p[PREFILL_LENGTH] claim interval | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | CV, DF, AUTH, G2A |
| `[C_prefill_p[PREFILL_LENGTH]_floor_clearance_J]` | `M_prefill_p[PREFILL_LENGTH]_contrast_abs_J_per_request - F_claim_prefill_p[PREFILL_LENGTH]_armwise_max_J`, only after floor-gate passage | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, CV, G2A |
| `[S_prefill_p[PREFILL_LENGTH]_floor_shortfall_J]` | `F_claim_prefill_p[PREFILL_LENGTH]_armwise_max_J - M_prefill_p[PREFILL_LENGTH]_contrast_abs_J_per_request`, only on floor-gate refusal | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, CV, G2A |
| `[R_prefill_p[PREFILL_LENGTH]_effect_x_floor]` | `M_prefill_p[PREFILL_LENGTH]_contrast_abs_J_per_request / F_claim_prefill_p[PREFILL_LENGTH]_armwise_max_J`; exact nonzero denominator required | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, CV, G2A |
| `[S_prefill_p[PREFILL_LENGTH]_joint_J]` | `F_claim_prefill_p[PREFILL_LENGTH]_armwise_max_J + B_prefill_p[PREFILL_LENGTH]_claim_J`; disclosure only, never an acceptance gate | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, DF, CV, G2A |
| `[C_prefill_p[PREFILL_LENGTH]_sizing_signed_clearance_J]` | `M_prefill_p[PREFILL_LENGTH]_contrast_abs_J_per_request - S_prefill_p[PREFILL_LENGTH]_joint_J`; signed sizing disclosure | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, DF, CV, G2A |
| `[OUTCOME_prefill_p[PREFILL_LENGTH]_floor_gate]` | Same exact `passes`/`does not pass`/`not evaluated` floor-gate phrases and unrounded rule as decode, from the selected prefill contrast | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, CV, AUTH, G2A |
| `[OUTCOME_prefill_p[PREFILL_LENGTH]_direction_gate]` | Same exact fully-composed-interval direction-gate phrases and cross-checks as decode, from the selected prefill contrast | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, CV, AUTH, G2A |
| `[VERDICT_prefill_p[PREFILL_LENGTH]]` | Selected prefill `claim_evaluation`; support phrase names Qwen3-8B, Qwen3-1.7B, prompt-processing energy per request, and the registered comparison; retain authenticated ordered reasons and D-166's count split exactly | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | TPL, CV, AUTH, G2A |
...
| DS-28 — Table 3 decode sizing sum and signed clearance, line 289, col 5 under `Sizing sum F+B; signed clearance` | `[PENDING]`; row anchor `\| token generation, 7B − 1.5B \|` | `S_decode_joint_J` and `C_decode_sizing_signed_clearance_J`; print `F+B` plus signed sizing clearance, never floor-only clearance or shortfall | gamma / decode contrast | DERIVE | KEY_FROZEN / VALUE_UNISSUED | DRAFT, TPL, CV |
| DS-29 — Table 3 decode contrast claim-side bound, line 289, col 6 under `Claim-side bound` | `[PENDING]`; row anchor `\| token generation, 7B − 1.5B \|` | `B_decode_claim_J` from the required `joulewise.claim_side_bound.v1` row digest-bound to the production v1 artifact, exactly the named `E_clock_anchor_shift_bound_j` term and not `deterministic_bounds.total` (Q-R2-1 and R2 addendum) | gamma / decode contrast | MEASURED | KEY_FROZEN / VALUE_UNISSUED | DRAFT, TPL, DF, CV, AUTH |
| DS-30 — Table 3 decode floor-gate outcome, line 289, col 7 under `Floor-gate outcome` | `[PENDING]`; row anchor `\| token generation, 7B − 1.5B \|` | `OUTCOME_decode_floor_gate`; exact ruled phrases are `passes — \|estimate\| > armwise cell floor`, `does not pass — \|estimate\| ≤ armwise cell floor`, and `not evaluated — <issued reason>` | gamma / decode contrast | DERIVE | KEY_FROZEN / VALUE_UNISSUED | DRAFT, TPL, CV, AUTH |
| DS-31 — Table 3 decode direction-gate outcome, line 289, col 8 under `Direction-gate outcome` | `[PENDING]`; row anchor `\| token generation, 7B − 1.5B \|` | `OUTCOME_decode_direction_gate`; exact ruled phrases state whether the fully composed interval lies wholly above zero, cross-checked to registered direction and claim/Holm state | gamma / decode contrast | DERIVE | KEY_FROZEN / VALUE_UNISSUED | DRAFT, TPL, CV, AUTH |
| DS-32 — Table 3 decode verdict, line 289, col 9 under `Verdict`, and A/B outcome paragraphs | `[PENDING]`; row anchor `\| token generation, 7B − 1.5B \|`; repeated successor marker `[FILL:DS-32]` | `VERDICT_decode` from the authenticated decode `claim_evaluation`; use the ruled `supported`/`not supported`/`not evaluated` branches with model, token-generation phase, registered direction, and exact interval/floor relation, never D-165. On REFUSAL, retain the Table 3 slot and render an authenticated issued verdict. Contrast absence is non-issuance and therefore `STOP_FILL`; the byte-exact `not evaluated — required token-generation verdict absent` sentence can issue only from future `CLAIM-NONISSUANCE-RECEIPT-01`. An earlier stop likewise remains `STOP_FILL` unless governing evidence issues its reason | gamma / decode contrast | DERIVE | KEY_FROZEN / VALUE_UNISSUED; every repeated placement uses identical bytes | DRAFT, TPL, CV, AUTH |
| DS-33 — Table 3 prompt floor, line 290, col 4 under `Cell floor` | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `F_claim_prefill_p[PREFILL_LENGTH]_armwise_max_J`, the unrounded maximum of the two exact selected alpha/beta `floor_gate_j` fields, cross-checked to the claim artifact's ordered A/B arm gates | gamma consuming alpha/beta prefill-p[PREFILL_LENGTH] floors | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DRAFT, TPL, DF, CV, AUTH, V5GEN, G2A |
...
| PG-01 — Table 3 prompt point estimate, line 290, col 2 under `Point estimate` | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `E_prefill_p[PREFILL_LENGTH]_contrast_signed_J_per_request` from exact selected `_v5` contrast `estimator.estimate`, condition B minus condition A | gamma / prefill-p[PREFILL_LENGTH] contrast | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DRAFT, TPL, CV, AUTH, V5GEN, G2A |
| PG-02 — Table 3 prompt interval, line 290, col 3 under `Interval [lower, upper]` | `[PENDING, PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `E_prefill_p[PREFILL_LENGTH]_contrast_lower_J` and `E_prefill_p[PREFILL_LENGTH]_contrast_upper_J` from the selected contrast's fully composed `deterministic_bounds.decision_interval` | gamma / prefill-p[PREFILL_LENGTH] contrast | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED; one marker site contains two semantic fills | DRAFT, TPL, CV, AUTH, V5GEN, G2A |
| PG-04 — Table 3 prompt sizing sum and signed clearance, line 290, col 5 under `Sizing sum F+B; signed clearance` | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `S_prefill_p[PREFILL_LENGTH]_joint_J` and `C_prefill_p[PREFILL_LENGTH]_sizing_signed_clearance_J`; print `F+B` plus signed sizing clearance, never floor-only clearance or shortfall | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DRAFT, TPL, CV, V5GEN, G2A |
| PG-05 — Table 3 prompt contrast claim-side bound, line 290, col 6 under `Claim-side bound` | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `B_prefill_p[PREFILL_LENGTH]_claim_J` from the required selected row in `joulewise.claim_side_bound.v1`, digest-bound to production v1, under Q-R2-1 and the R2 addendum | gamma / prefill-p[PREFILL_LENGTH] contrast | MEASURED | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DRAFT, TPL, CV, AUTH, V5GEN, G2A |
| PG-06 — Table 3 prompt floor-gate outcome, line 290, col 7 under `Floor-gate outcome` | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `OUTCOME_prefill_p[PREFILL_LENGTH]_floor_gate`, using the same exact ruled `passes`/`does not pass`/`not evaluated` phrases and unrounded relation as decode | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DRAFT, TPL, CV, AUTH, V5GEN, G2A |
| PG-07 — Table 3 prompt direction-gate outcome, line 290, col 8 under `Direction-gate outcome` | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|` | `OUTCOME_prefill_p[PREFILL_LENGTH]_direction_gate`, using the same exact fully-composed-interval phrases and claim/Holm cross-check as decode | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED | DRAFT, TPL, CV, AUTH, V5GEN, G2A |
| PG-08 — Table 3 prompt verdict, line 290, col 9 under `Verdict`, and A/B outcome paragraphs | `[PENDING]`; row anchor `\| prompt processing, 7B − 1.5B \|`; repeated successor marker `[FILL:PG-08]` | `VERDICT_prefill_p[PREFILL_LENGTH]` from the authenticated selected prefill `claim_evaluation`; use the ruled `supported`/`not supported`/`not evaluated` branches with model, prompt-processing phase, registered direction, and exact interval/floor relation, never D-165. On REFUSAL, retain the Table 3 slot and render an authenticated issued verdict. Contrast absence is non-issuance and therefore `STOP_FILL`; the byte-exact `not evaluated — required prompt-processing verdict absent` sentence can issue only from future `CLAIM-NONISSUANCE-RECEIPT-01`. An earlier stop likewise remains `STOP_FILL` unless governing evidence issues its reason. D-166 remains split: count below 3 renders `not_resolvable_sample_count`; resolvable count 3–4 renders byte-exact `not supported — below the pre-registered count floor of 5 (reducer result remained resolvable at observed overlap count <n>)` | gamma / prefill-p[PREFILL_LENGTH] contrast | DERIVE | UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED; every repeated placement uses identical bytes | DRAFT, TPL, CV, AUTH, V5GEN, G2A |
...
|---|---|---|---|
| Gamma prefill contrast | RESOLVED as to the token and supplier contract by R2: the symmetric family is registered and the gamma renderer validates production v1 + digest-bound sidecar + selected G2-a + prompt-pin v2 | Keep every prefill value `UNRESOLVED-UNTIL-G2A / VALUE_UNISSUED`; a refused G2-a record is terminal `STOP_FILL`, and production publication wiring remains lead-owned outside this supplier mission | DRAFT, TPL, AUTH, V5GEN, G2A |
...
| Generic draft table outcomes | RESOLVED as to the machine renderer contract by R2: DS-30..32 and PG-06..08 bind the exact gate/verdict strings above | Keep the frozen publication adapter unchanged until its separately governed integration; do not infer strings from variant headings or contrast absence | DRAFT, TPL, CV |
...
- Preserve the digest-bound `joulewise.claim_side_bound.v1` join and its exact
  named-term projection; never substitute `deterministic_bounds.total`.
- Keep the registered prefill family value-unissued until G2-a selects a rung;
  preserve D-166's two distinct exhausted-ladder refusal renderings verbatim.
