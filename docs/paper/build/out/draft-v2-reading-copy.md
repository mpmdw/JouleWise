...
reasoning disabled.  Greedy generation—choosing
the highest-probability next token at every step—forces 512 output tokens for
one rendered prompt in each run.  G2-a is the fixed
candidate-length selection step, and a prefill prompt pin is its record fixing
the selected prompt and generation inputs. The selected prefill prompt pin is omitted: its G2-a-bound `joulewise.prefill_prompt_pin.v2` record has not issued. 
...
independent coverage guarantee. 
...
edge endpoints gave 3.153 J, 2.922 J, and 2.184 J. 
...
the calibration corpus—is 9.724 ms.  The same screen is rendered
...
results.  The superseded calibration corpus
contained nineteen captures; the current corpus contains seventeen.
 Separately, the historical short-prefill diagnostic
population contained 50 bundles. 
...
rounding**)
([not yet measured: prompt processing point floor]–[not yet measured: short prompt-processing ratio]). They show the numerical effect of recalculating
...
square root, \(se_{\mathrm{total}}=\sqrt{se_{\mathrm{repeat}}^2+ se_{\mathrm{metrology}}^2}\). Divide the mean \(\bar d\) by
...
Figure 3 summarizes these paths. It must show three separate paths from the
...
![Figure 3. Evidence exclusion, magnitude, and direction paths.](../../figures/fig3_decision_gates.svg)
...
|---|---|---:|---:|---:|---|---:|---|
| prompt processing; [not yet measured: selected prompt length] | Qwen3-1.7B | [not yet measured: prompt processing 1.7-billion-parameter model floor cell] | [not yet measured: prompt processing at the selected prompt length absolute independent-edge ratio] | [not yet measured: prompt processing at the selected prompt length comparative independent-edge ratio] | [not yet measured: prompt processing at the selected prompt length absolute-component shared-error ratio] | [not yet measured: prompt processing at the selected prompt length comparative-component shared-error ratio] | [not yet measured: prompt processing 1.7-billion-parameter model gross cell] [not yet measured: prompt processing 1.7-billion-parameter model per-token cell] [not yet measured: prompt processing 1.7-billion-parameter model count cell] |
| prompt processing; [not yet measured: selected prompt length] | Qwen3-8B | [not yet measured: prompt processing 8-billion-parameter model floor cell] | [not yet measured: prompt processing at the selected prompt length absolute independent-edge ratio] | [not yet measured: prompt processing at the selected prompt length comparative independent-edge ratio] | [not yet measured: prompt processing at the selected prompt length absolute-component shared-error ratio] | [not yet measured: prompt processing at the selected prompt length comparative-component shared-error ratio] | [not yet measured: prompt processing 8-billion-parameter model gross cell] [not yet measured: prompt processing 8-billion-parameter model per-token cell] [not yet measured: prompt processing 8-billion-parameter model count cell] |
| token generation | Qwen3-1.7B | [not yet measured: token generation 1.7-billion-parameter model floor cell] | [not yet measured: token generation absolute independent-edge ratio] | [not yet measured: token generation comparative independent-edge ratio] | [not yet measured: token generation absolute-component shared-error ratio] | [not yet measured: token generation comparative-component shared-error ratio] | [not yet measured: token generation 1.7-billion-parameter model gross cell] [not yet measured: token generation 1.7-billion-parameter model per-token cell] [not yet measured: token generation 1.7-billion-parameter model count cell] |
| token generation | Qwen3-8B | [not yet measured: token generation 8-billion-parameter model floor cell] | [not yet measured: token generation absolute independent-edge ratio] | [not yet measured: token generation comparative independent-edge ratio] | [not yet measured: token generation absolute-component shared-error ratio] | [not yet measured: token generation comparative-component shared-error ratio] | [not yet measured: token generation 8-billion-parameter model gross cell] [not yet measured: token generation 8-billion-parameter model per-token cell] [not yet measured: token generation 8-billion-parameter model count cell] |
...
|---|---:|---:|---:|---:|---|---|---|
| token generation, Qwen3-8B − Qwen3-1.7B | [not yet measured: token generation point estimate] | [not yet measured: token generation interval] | [not yet measured: token generation floor] | [not yet measured: token generation sizing sum and signed clearance] | [not yet measured: token generation floor-gate outcome] | [not yet measured: token generation direction-gate outcome] | [not yet measured: token generation verdict] |
| prompt processing, Qwen3-8B − Qwen3-1.7B | [not yet measured: prompt processing point estimate] | [not yet measured: prompt processing interval] | [not yet measured: prompt processing floor] | [not yet measured: prompt processing sizing sum and signed clearance] | [not yet measured: prompt processing floor-gate outcome] | [not yet measured: prompt processing direction-gate outcome] | [not yet measured: prompt processing verdict] |
...
[not yet measured: selected prompt length]
...
shortest passing length became [not yet measured: selected prompt length] Qwen3-8B probes were recorded to discover whether the larger
...
\(0.030067931757111657\) s.  This is a calibration value, not a bound on real inference. The concrete closing check is the inserted-gap
...
bound for the CPU or neural-engine channel is issued. 
...
their stochastic measurement terms is zero. 
...
sums match; unequal runtimes or cooldowns break that balance. 
...
would remain after an authenticated result issued. 
...
limit remains open until that check is available. 
...
and-after calibration bracket, and every refusal record. 
...
capture's pulse-derived bound.  The registered row does not prescribe an acceptance
...
12. B. Ruf and M. Detyniecki. “The Cost of Context: Profiling the Energy Footprint of Input Tokens in Large Language Models.” *HotCarbon '26*, 2026.
...
15. M. Dauner, M. Steinberg, A. Brunnert, B. Schicker, and B. Zönnchen. “Evaluating the Influence of Measurement Frequency on Energy Readings Using Intel RAPL and NVIDIA NVML.” *HotCarbon '26*, 2026.
...
| Figure 3 summarizes these paths | Outcome sentence forms | glossed-at-first-use | The following sentence enumerates the exclusion, magnitude, and direction paths the required figure must show. |
