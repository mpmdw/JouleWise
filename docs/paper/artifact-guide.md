...
Paths are relative to `/Users/edr/code/JouleWise-wt-r4`. `pf` = `joulewise/powermetrics_fiducial.py`; `ue` = `joulewise/uncertainty_evidence.py`; `pm` = `joulewise/adapters/powermetrics.py`; `vs` = `scripts/validate_powermetrics_fiducial.py`; `clk` = `joulewise/clock.py`.
