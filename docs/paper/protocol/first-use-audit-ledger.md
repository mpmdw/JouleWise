...
Abstract, Sections 1 through 11, and appendices), followed by the prospective
...
| independent unit | Comparing the moved-edge limit and point-only value | glossed-at-first-use | One separately admitted bundle, not one sampler record within it. |
...
| independent units | Comparing the moved-edge limit and point-only value | glossed-at-first-use | Singular first use defines one separately admitted bundle; later the component-specific units are stated. |
...
| directional comparison / directional comparisons | P.3 Directional comparison and claim gates | glossed-at-first-use | The expected direction is fixed before collection. |
| Holm step-down correction / raw probability | P.3 Directional comparison and claim gates | glossed-at-first-use | Applied at nominal family-wise level 0.05 to two model-based tests, conditional on distributional and dependence assumptions; the later worked ordering supplies mechanics. |
| repeat standard error | P.3 Directional comparison and claim gates | glossed-at-first-use | Repeat-to-repeat scatter divided by \(\sqrt n\). |
...
| null hypothesis / tail area | P.3 Directional comparison and claim gates | audience-vocabulary | Textbook statistics vocabulary for zero mean difference and the resulting two-sided probability. |
| effective sample size / n_{\mathrm{eff}} | P.3 Directional comparison and claim gates | glossed-at-first-use | Number of independent blocks giving the same repeat scatter. |
| AR(1) model | P.3 Directional comparison and claim gates | glossed-at-first-use | Adjacent block errors are treated as serially correlated. |
| serially correlated | P.3 Directional comparison and claim gates | glossed-at-first-use | Adjacent block errors are treated as serially correlated. |
| decision-interval sign check / direction gate | P.3 Directional comparison and claim gates | forward-pointer-next-paragraph | The first use explicitly says the direction gate is in the next paragraph, which defines both intervals and the fixed direction. |
| magnitude check / direction check | P.3 Directional comparison and claim gates | glossed-at-first-use | Absolute estimate must clear the cell floor; both complete intervals must remain on the fixed direction. |
| not resolvable | Printed negative result: short prompt processing has too few overlapping records | glossed-at-first-use | The estimate does not clear the cell floor; it is not zero. |
...
| signed clearance or shortfall | P.3 Directional comparison and claim gates | glossed-at-first-use | Absolute point estimate minus cell floor, with positive clearance and nonpositive shortfall. |
...
| fail-closed | 5. Evidence validity | built-before | Missing, malformed, out-of-limit, or inconsistent evidence stops collection and records why. |
...
| third-party provenance / provenance | Printed negative result: short prompt processing has too few overlapping records | glossed-at-first-use | Evidence that would convince someone who does not trust the operator. |
...
| record support / positive overlap / overlap count | 3. Scope of the reported evidence | glossed-at-first-use | Positive overlap is defined by the positive-time inequality; overlap count and record support both name the number of sampling records that pass it. |
...
| reducer | P.3 Directional comparison and claim gates | glossed-at-first-use | Program that integrates native interval-average records over their overlap durations and later turns retained bundles into assigned phase energies. |
...
| run bundle | 9. Evidence and code availability | audience-vocabulary | Plain-English software packaging vocabulary; Appendix A.2 later enumerates its files. |
...
| Clock stamps / paired stamp | 5. Evidence validity | glossed-at-first-use | Monotonic-wall-monotonic readings; their half-width covers bracket timing plus reported clock resolution. |
...
| admissible | Adding publication safeguards after the ratio | built-before | The exact admissible interval for \(A\). |
...
| custody | 9. Evidence and code availability | glossed-at-first-use | Each named input's fingerprint still matches its recorded bytes. |
