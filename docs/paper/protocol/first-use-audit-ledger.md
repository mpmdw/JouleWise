...
The reading order tested here is the methods/diagnostic article (title,
Abstract, Sections 1 through 9, and appendices), followed by the prospective
comparison protocol. The ledger lives outside the article and excludes itself.
...
| pulse-derived limit | 1. Introduction | glossed-at-first-use | Largest displacement between the commanded times and every edge position in the fit’s model-defined accepted regions, plus the clock-anchor bound. |
| interval-overlap allocation / interval-overlap-assigned phase energy | 1. Introduction | glossed-at-first-use | Split each record's energy in proportion to the interval on each side of the phase boundary; the Conclusion states average power times overlap duration. |
...
| synthetic enclosure diagnostic | 3. How the method quantifies assigned-energy sensitivity | glossed-at-first-use | A labelled 0.9-s, ten-record example comparing the timing envelope with a nonnegative partial-record enclosure; it is never composed into a bound. |
| component | 3. How the method quantifies assigned-energy sensitivity | glossed-at-first-use | One separate same-model repeat or same-model null A/B/B/A floor calculation. |
| permitted edge movement | 3. How the method quantifies assigned-energy sensitivity | glossed-at-first-use | Every lower-or-upper edge position allowed by the pulse calibration and clock mapping. |
| independent-edge ratio / four-run comparison | 3. How the method quantifies assigned-energy sensitivity | glossed-at-first-use | Moved-edge limit divided by the point-only value with separate movement per run; a four-run comparison can be a same-model null or a two-model science contrast. |
| moved-edge limit / independent-edge corner bound / point-only value | 3. How the method quantifies assigned-energy sensitivity | glossed-at-first-use | The first is \(U_{\mathrm{corner}}\) after every allowed edge choice; the second is its single artifact alias, declared at first use; the point-only value is \(U_{\mathrm{point}}\) at the stored boundary times. |
| decision rule | P.1 Prospective campaign identities and ratio census | glossed-at-first-use | A direction prints only when the measured difference clears its cell floor and the lowest-to-highest range after known measurement errors stays on the direction fixed before collection. |
| twofold boundary contribution | P.1 Prospective campaign identities and ratio census | glossed-at-first-use | The moved-edge limit is at least twice the point-only value. |
...
| \(U_{\mathrm{point}}\) / \(U_{\mathrm{corner}}\) | 3. How the method quantifies assigned-energy sensitivity | glossed-at-first-use | An independent component bound at recorded edges versus that component after all allowed lower-or-upper choices are evaluated jointly and the largest result is retained; shared movement uses the separately defined \(U_{\mathrm{cmp,shared}}/U_{\mathrm{cmp,point}}\). |
| A/B/B/A block | 1. Introduction | glossed-at-first-use | Four runs in the order A, B, B, A. |
| energy-allowance sign | Combining shared movements and local widths | glossed-at-first-use | Says which direction a nonnegative block-level allowance moves assigned energy. |
| twelve required ratios | P.1 Prospective campaign identities and ratio census | glossed-at-first-use | Eight independent-edge ratios over same-model repeat and null floor components and four comparative \(R_{cm}\) diagnostics over same-model null components. |
| reasoning disabled | P.1 Prospective campaign identities and ratio census | glossed-at-first-use | Qwen3's optional chain-of-thought output is switched off. |
...
| science window / measurement window | Abstract | glossed-at-first-use | One uninterrupted measurement session; Appendix Figure A2 later diagrams the same window. |
| SHA-256 / SHA-256 fingerprint | Moving edges and enumerating endpoints | glossed-at-first-use | A fingerprint that identifies exact file bytes. |
| applied chat-template | P.1 Prospective campaign identities and ratio census | glossed-at-first-use | The formatter that turns a prompt into model input. |
...
| first-record endpoint | Bracketed pulse-train algorithm | glossed-at-first-use | Wall-clock time assigned to the end of the first native sampling record. |
...
| gross energy / idle-subtracted energy | Directional comparison | glossed-at-first-use | Recorded processor energy versus that amount after mean idle power times duration is removed. |
...
| Workload response | P.2 Instrument characterization | glossed-at-first-use | Whether request and token-generation energy increase with realized output length in the registered way. |
| Identical-condition null | P.2 Instrument characterization | glossed-at-first-use | Whether an A/B/B/A comparison manufactures a difference when both conditions are the same. |
| independent unit | Comparing the moved-edge limit and point-only value | glossed-at-first-use | One repeated run for the absolute component or one complete A/B/B/A block for the comparative component; statistical independence is assumed, not established by admission. |
| workload-response slope | P.2 Instrument characterization | glossed-at-first-use | Fitted change in energy per output token. |
| workload level | P.2 Instrument characterization | glossed-at-first-use | One output-token count fixed before collection. |
| workload magnitude | P.2 Instrument characterization | glossed-at-first-use | One target size fixed in the identical-condition ladder. |
| per-token conversion | P.2 Instrument characterization | glossed-at-first-use | Fitted joules per output token. |
| fitted residual | P.2 Instrument characterization | glossed-at-first-use | One observed energy minus the straight-line value predicted for its output length. |
| null-test blocks | P.2 Instrument characterization | glossed-at-first-use | Identical-condition blocks whose allowed difference should contain zero. |
| interval of allowed differences | P.2 Instrument characterization | glossed-at-first-use | Minimum and maximum A/B/B/A difference over the fixed member-energy endpoint combinations. |
| mean interval | P.2 Instrument characterization | glossed-at-first-use | Mean of the five block lower endpoints through mean of their five upper endpoints. |
| resolution band | P.2 Instrument characterization | glossed-at-first-use | Symmetric allowed slope range made from one admitted prefill half-width across the output span. |
| floor band | P.2 Instrument characterization | glossed-at-first-use | Analogous symmetric slope range made from an independently issued prefill floor. |
| unphased gap | P.2 Instrument characterization | glossed-at-first-use | Recorded interval between the end of prefill and the start of decode. |
| shared session timing term | P.2 Instrument characterization | glossed-at-first-use | One bracket-capture timing bound common to all session members. |
| member-local timing | P.2 Instrument characterization | glossed-at-first-use | Each run's own local and edge-span timing contribution. |
| timing flags | P.2 Instrument characterization | glossed-at-first-use | Mark a member whose clock bound exceeds a quarter of its window. |
| sampling flags / cadence ratio | P.2 Instrument characterization | glossed-at-first-use | Mark too few in-window records, an unrecorded cadence ratio, or cadence below the fixed phase-rate multiple. |
| package power | P.2 Instrument characterization | glossed-at-first-use | Summed CPU, GPU, and neural-engine power. |
| reference roles | P.2 Instrument characterization | glossed-at-first-use | Separate opening, midpoint, and closing allowance builders from held-out probes. |
| passing cooldown exit | P.2 Instrument characterization | glossed-at-first-use | First post-hold gate evaluation meeting the frozen rolling-power, coverage, and thermal checks. |
...
| admitted energy | Comparing the moved-edge limit and point-only value | glossed-at-first-use | Energy from a run that passed entry checks; validity alone does not establish statistical independence or authorize a new comparison. |
| independent units | Comparing the moved-edge limit and point-only value | glossed-at-first-use | The calculation counts runs or complete blocks as specified for each component; admission does not establish statistical independence. |
...
| independent-edge ratio \(R\) / dominates | Moving edges and enumerating endpoints | glossed-at-first-use | Moved-edge limit divided by the matching point-only value; dominates means \(R\ge2\). |
...
| authentication / evaluation | Benchmark and metrology lineage | glossed-at-first-use | Authentication matches inputs to named source-file contents; evaluation requires a nonzero second value. |
| registered rounding / registered | Abstract | glossed-at-first-use | The Abstract defines the registered timing domain as the set of edge movements fixed before collection; the later registered-rounding use names a rule fixed in fingerprinted plan bytes. |
...
| repeat standard error | Directional comparison | glossed-at-first-use | Repeat-to-repeat scatter divided by \(\sqrt n\). |
| total standard error | P.5 Campaign dependence and custody limitations | glossed-at-first-use | For the gross phase-energy path, this is the standard error of the block differences because the builder supplies no additional stochastic metrology variance. |
| null hypothesis / tail area | Directional comparison | audience-vocabulary | Textbook statistics vocabulary for zero mean difference and the resulting two-sided probability. |
| effective sample size / n_{\mathrm{eff}} | Directional comparison | glossed-at-first-use | Number of independent blocks giving the same repeat scatter. |
| AR(1) model | Directional comparison | glossed-at-first-use | Adjacent block errors are treated as serially correlated. |
| serially correlated | Directional comparison | glossed-at-first-use | Adjacent block errors are treated as serially correlated. |
| decision-interval sign check / direction gate | Directional comparison | forward-pointer-next-paragraph | The first use explicitly says the direction gate is in the next paragraph, which defines both intervals and the fixed direction. |
| magnitude check / direction check | Directional comparison | glossed-at-first-use | Absolute estimate must clear the cell floor; both complete intervals must remain on the fixed direction. |
| not resolvable | Record support in two historical model stacks | glossed-at-first-use | Here the phase has fewer than three overlapping sampling records; this is insufficient record support, not a below-floor energy estimate. Protocol P.3 defines the separate cell-floor sense. |
...
| request total | 1. Introduction | glossed-at-first-use | Energy assigned over the whole request; moving the dividing time reallocates energy between its parts without changing that total. |
...
| Figure P1 | Evidence refusal and claim gates | glossed-at-first-use | The prose and caption enumerate the refusal lane, measured-contrast input, magnitude and direction gates, yes/no arrows, and four outcomes. |
| fail-closed | Evidence validity | built-before | Missing, malformed, out-of-limit, or inconsistent evidence stops collection and records why. |
...
| idle baseline / nearest-rank p95 | Large-language-model energy measurement | glossed-at-first-use | Thirty-second no-run sample; sort \(n\) values and select item \(\lceil0.95n\rceil\). |
...
| third-party provenance / provenance | Record support in two historical model stacks | glossed-at-first-use | Evidence that would convince someone who does not trust the operator. |
...
| record support / positive overlap / overlap count | 1. Introduction | glossed-at-first-use | Positive overlap is defined by the positive-time inequality; overlap count and record support both name the number of sampling records that pass it. |
| interquartile range / IQR | Record support in two historical model stacks | glossed-at-first-use | Upper edge minus lower edge of the middle half of sorted values. |
| resolvability / not_resolvable_sample_count | Record support in two historical model stacks | glossed-at-first-use | Here the not-resolvable verdict names record support below the three-record minimum and the printed label identifies that reason; Protocol P.3 uses the same verdict words for failure to clear a cell floor. |
| record width | Record support in two historical model stacks | glossed-at-first-use | Record width is the duration of one sampling record's interval. |
| prospective demonstration | P.1 Prospective campaign identities and ratio census | glossed-at-first-use | Comparison fixed before collection; outside this submission's empirical scope. |
...
| reducer | Directional comparison | glossed-at-first-use | Program that integrates native interval-average records over their overlap durations and later turns retained bundles into assigned phase energies. |
| variance multiplier | P.5 Campaign dependence and custody limitations | glossed-at-first-use | The displayed definition says how much the selected dependence model multiplies repeat variance. |
| within-arm variation | P.5 Campaign dependence and custody limitations | glossed-at-first-use | Variation among runs of the same A or B condition. |
...
| external-meter study | P.7 Future validation designs | glossed-at-first-use | A proposed, unrun whole-request comparison against a meter on the wall side of the power supply. |
...
| admission predicates | P.11 Fresh collection prerequisites | glossed-at-first-use | The pass/fail checks a machine's own calibration must satisfy before its runs are admitted. |
| run bundle | 7. Evidence and code availability | audience-vocabulary | Plain-English software packaging vocabulary; Appendix A.2 later enumerates its files. |
...
| whole-window verdict / floor extraction / claim verdict | P.8 Comparison archive objects | glossed-at-first-use | Successive artifacts bind admitted membership, each cell floor, and the final contrast decision. |
...
| Clock stamps / paired stamp | Evidence validity | glossed-at-first-use | Monotonic-wall-monotonic readings; their half-width covers bracket timing plus reported clock resolution. |
...
| model/stack | Record support in two historical model stacks | built-before | The two named Qwen inference populations and their retained software stacks; this is not the affine clock relation. |
| The clock model | A.3 Formal calibration algorithms | glossed-at-first-use | Appendix A.3.3 labels the affine wall-versus-monotonic relation and its joint anchor, offset and rate unknowns. |
...
| median absolute deviation / robust scale | Bracketed pulse-train algorithm | glossed-at-first-use | Median distance from the median, scaled to a Gaussian standard-deviation equivalent with a 1-mW floor. |
...
| robust SNR | A.3 Formal calibration algorithms | glossed-at-first-use | Fitted pulse height divided by robust baseline scale. |
...
| Huber loss | A.3 Formal calibration algorithms | glossed-at-first-use | Quadratic for small residuals and linear for large ones; the loss remains unbounded, so a large discrepancy can still dominate the total. |
