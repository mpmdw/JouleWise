...
| MDE/n sizing + predeclared top-up rule | n>=5 measured validation repetitions for headline placement cells; n>=3 for non-headline cells. Top up to n=10 when regret CI crosses the active floor gate, when measured best changes under leave-one-out, or when prediction error is near the success threshold. |
