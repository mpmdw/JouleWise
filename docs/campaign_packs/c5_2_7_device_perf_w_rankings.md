...
| MDE/n sizing + predeclared top-up rule | n>=5 per headline device x workload cell. Top up to n=10 before L2 rank wording when an adjacent rank gap confidence interval touches the active floor gate, when leave-one-out changes rank order, or when cooldown flags cluster by device. |
