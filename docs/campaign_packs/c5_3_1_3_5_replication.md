...
| Estimator/formula | Estimate unit delta `delta_unit = metric(unit_b) - metric(unit_a)` and variance relative to the active P2-015 floor. Source-claim replication succeeds only when the original direction/verdict survives on the second unit or the difference is declared `not resolvable` under the frozen rule. |
...
| MDE/n sizing + predeclared top-up rule | Match the source campaign n or n>=5 per replication cell, whichever is larger. Top up to n=10 when the replication verdict changes under leave-one-out, when the unit delta CI touches the floor, or when source direction is near the threshold. |
...
| MDE/n sizing + predeclared top-up rule | First cross-lab target uses the smallest suite with n matching the transferred bundle pattern. Top up or rerun only under a frozen corrective plan when strict validation fails, agreement CI touches the active floor, or source verdict changes under leave-one-out. |
