...
| claim_role | primary |
| selection_scope | Frozen split matrix for `<<DEVICE_PAIR>>`, `<<MODEL_SET>>` default `{Qwen2.5-1.5B, Qwen2.5-7B if feasible}`, prompt lengths `{512,2048,8192}`, decode length `256`, links `<<LINK_MBPS_SET>>`, split mode `offline_replay` unless explicitly frozen as analytical composition, and both monolithic references for the same model/runtime/artifact where available. |
...
| Floor gate | pending-P2-015: use `max(floor_abs_j, floor_cmp_j)` for the same backend, metric, and window class. Existing exact floor rows consumed: `DF-RQ-GROSS-MID` (`gross_energy_j`, gross request), `DF-RQ-IDLE-MID` (`energy_request_j`, `idle_subtracted_energy_j`, idle-sub request), `DF-RQ-GROSS-LONG-PROMPT` (`gross_energy_j`, gross request), `DF-RQ-IDLE-LONG-PROMPT` (`energy_request_j`, `idle_subtracted_energy_j`, idle-sub request), and `DF-CMP-ABBA-RQ` (`gross_energy_j`, `energy_request_j`, request windows). Stage descriptors consume `DF-PH-PREFILL` (`phase_energy_j.prefill`, phase window), `DF-PH-DECODE` (`phase_energy_j.decode`, phase window), and `DF-CMP-ABBA-PH` (`phase_energy_j.prefill`, `phase_energy_j.decode`, phase windows). No exact P2-015 row currently exists for `serialize`, `transfer`, or `deserialize`; standalone L2/L3 claims for those stages are capped until P2-015 adds rows or this AP freezes an accepted AP-specific bound. |
...
| Floor gate | pending-P2-015: request-level split-total link sensitivity consumes `DF-RQ-GROSS-MID`, `DF-RQ-IDLE-MID`, `DF-RQ-GROSS-LONG-PROMPT`, `DF-RQ-IDLE-LONG-PROMPT`, and `DF-CMP-ABBA-RQ` for `gross_energy_j` / `energy_request_j` request windows. Transfer-stage energy has no exact P2-015 transfer-window floor row yet; standalone transfer-energy L2 wording is capped until that row or AP-specific bound exists. |
...
| Floor gate | pending-P2-015: energy axis consumes `DF-RQ-GROSS-MID`, `DF-RQ-IDLE-MID`, `DF-RQ-GROSS-LONG-PROMPT`, `DF-RQ-IDLE-LONG-PROMPT`, and `DF-CMP-ABBA-RQ` for request windows. Latency has no P2-015 energy floor; latency uncertainty follows D-014 contrast CI and run-bundle timing quality fields. Stage-energy captions consume `DF-PH-PREFILL`, `DF-PH-DECODE`, and `DF-CMP-ABBA-PH` only for prefill/decode descriptors. |
...
{
  "schema_version": "0.1",
  "run_id": "split-ref-<<DEVICE_PAIR.PREFILL_TARGET_ID>>-qwen25-15b-p2048-d256-r1",
  "model": {
...
```json
{
  ...
  "run_id": "split-offline-<<DEVICE_PAIR>>-<<LINK_MBPS>>mbps-qwen25-15b-p2048-d256-r1",
  "model": {
    "name": "Qwen2.5-1.5B-Instruct-4bit",
    "family": "qwen2.5",
...
python3 -m joulewise validate-bundle --strict runs/<<COMPOSITE_SPLIT_RUN_ID>>
python3 -m joulewise reduce runs/<<COMPOSITE_SPLIT_RUN_ID>>
```

Operator sequence:

1. Fill placeholders and freeze DRAFT AP rows into the analysis registry with
   complete `contrast_id` enumeration and manifest hash.
2. Run `kv-size` and the prompt-cache spike; record feasibility verdicts in
   `docs/phase_3/kv_feasibility.md`.
3. Validate monolithic configs, run monolithic references, strict-validate and
   reduce each bundle.
4. Execute transfer bench and split offline runs once planned commands exist.
5. Package strict-valid bundles and verify the pack before analysis.
