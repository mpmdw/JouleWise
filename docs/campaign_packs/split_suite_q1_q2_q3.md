...
| MDE/n sizing + predeclared top-up rule | n is FROZEN at registry freeze per D-062: n=10 for headline pairing cells expected near-floor per Window-A evidence (D-048 prediction error near floor, or the contrast MDE within 2x of the floor gate), n>=5 otherwise; n>=3 minimum for non-headline cells. No outcome-dependent growth: if an observed CI touches the floor or a crossover verdict changes under leave-one-out at the frozen n, the verdict is reported at that n (`not resolvable`/`unresolved`) — post-outcome additions demote to exploratory (D-062 clause in `docs/contracts/analysis_plans.md`). Technically invalid runs follow the predeclared replacement ruldent growth). |
...
| MDE/n sizing + predeclared top-up rule | n follows the Q1/Q2 source cells' FROZEN sizes (D-062). Pareto-frontier language requires dominance verdicts stable under leave-one-out at the frozen n and energy contrasts clear of the floor gate — otherwise the frontier claim is withheld (`unresolved`), never topped up post-outcome; additions demote to exploratory (D-062 clause in `docs/contracts/analysis_plans.md`). |
...
- EXECUTION GATE (SPLIT-AP): headline Q1/Q2 split cells may not execute until
  ...
  composite `split_total_energy_j` and for the `serialize`, `transfer`, and
  `deserialize` windows on the frozen backend/boundary. Executing split bundles
  without these rows produces exploratory-only data; the AP rows may not be
  frozen while this gate is open.
