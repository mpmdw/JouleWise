...
| MDE/n sizing + predeclared top-up rule | n>=5 for headline pairing cells; n>=3 minimum for non-headline cells. Top up to n=10 before Q1 L2 wording when either split-vs-monolithic-reference CI touches the active floor gate, when a crossover verdict changes under leave-one-out, or when D-048 prediction error is near the floor. |
