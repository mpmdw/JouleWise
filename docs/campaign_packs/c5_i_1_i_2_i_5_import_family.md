...
| MDE/n sizing + predeclared top-up rule | HumanEval smoke may remain L0-L1 with n dictated by plumbing. Preserve n>=5 repeated bundles or an accepted repeated-block design for L2 family energy signatures. Sizing authority: D-062 + `configs/analysis_registry/<<C5I1_ANALYSIS_REGISTRY>>.json` (frozen n; no outcome-based top-ups without demotion). |
...
| MDE/n sizing + predeclared top-up rule | L1 summaries may use HumanEval smoke evidence. Preserve preplanned n>=5 per stratum/block or an accepted repeated-block design for L2 contrasts. Sizing authority: D-062 + `configs/analysis_registry/<<C5I2_ANALYSIS_REGISTRY>>.json` (frozen n; no outcome-based top-ups without demotion). |
...
| MDE/n sizing + predeclared top-up rule | Preserve n>=5 paired repeated bundles for L2 template sensitivity. Sizing authority: D-062 + `configs/analysis_registry/<<C5I5_ANALYSIS_REGISTRY>>.json` (frozen n; no outcome-based top-ups without demotion). |
