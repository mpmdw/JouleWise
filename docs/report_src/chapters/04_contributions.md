...
- **Rung 2 — scoped empirical coverage (conditional):** per-request energy
  ...
  support it.
- **Rung 3 — scientific findings (contingent):** any comparative or scaling
  statement requires the frozen-contrast and claim-verdict machinery
  (P2-042/P2-037) and is not claimed in this slice.

An unresolved or boring matrix is not a failed instrument result: if the
production runs show no resolvable contrast, Rung 1 still stands, and the
