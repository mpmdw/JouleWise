...
energy removes the run's own measured idle baseline. Idle-subtracted
per-request energy is the primary basis; gross is co-reported as context.

The measurement boundary is named on every artifact — for the legacy corpus,
Apple SoC CPU + GPU + ANE package power via powermetrics. Wall power, memory,
and peripherals are outside this boundary and no claim extends past it.
