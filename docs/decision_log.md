...
| D-147 | R2 RULING — MINT-LANE FAN-OUT COMPOSITE (magistrate, 2026-08-19, under D-144): generation-indexed mint-policy resolver (registry-authoritative, operatives-crosswire refusal) PLUS immutable `_v3` pack family bound to r5 AT BIRTH; `_v2` packs READ-ONLY including their generators (frozen pack content); freeze-0003 chained to freeze-0002 (parked step 6 AMENDED — no freeze-0002 re-mints); `_v3` produced by the unedited `_v2` generators then draft-retargeted; genesis digest RENAMED only; no new tracked tooling; binding sequencing S0-S6 with freeze-0003 as the last acceptance-bearing step. ONE home: `docs/process_traces/2026-08-19-r1-r2-codesign/14-r2-ruling.md`. | ratified (magistrate) |
