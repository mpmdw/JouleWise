...
| D-144 | CO-DESIGN PROTOCOL (Ed process rule, 2026-08-18 evening): no design implemented without independent Sol/terra AND Opus designs -> bounded debate (2 rounds; 3 for big) -> Fable ruling (ratified spec, disagreements recorded) -> implementation gauntlet -> Fable final review (always); BIG designs (new method identities, schema/contract changes, D-138-class, family-superseding) additionally get one more two-seat pass over the implemented artifact pre-merge with a Fable ruling on its findings. Applies forward. First application: the R1/R2 rulings of 2026-08-19 (custody `docs/process_traces/2026-08-19-r1-r2-codesign/`) — both debates produced executed refutations of seat positions AND of the magistrate's briefs; protocol validated on first use. | adopted (Ed) |
| D-145 | D-079 ANCHOR-V3 GENERATION RECORD (r3/r4, 2026-08-18): r3 = `d079_calibration_acceptance_v2_n17_r3` (n=17, screens TIGHTENED) issued via the anchor-v3 arc (ratified science: `docs/process_traces/2026-08-18-anchor-v3-science-review/`); r4 = science-neutral capture-activation reissue (one pin moved; neutrality PROVEN by direct 19-member estimator replay). The reissue TOOL compares stored scalars and cannot check v3 generations — bespoke derive/build scripts are the r3/r4 route. EXTENDED 2026-08-19: r5 (production capture flip; three pins) and r6 (claim-barrier/taxonomy fix round; two pins) issued the same way, each neutrality-proven by full 19-member replay with zero mismatches; r6 is LIVE (sha 0227bca3…); r5/r4/r3 retained; amendment record `docs/process_traces/2026-08-19-r1-r2-codesign/15-amendment-r6.md`. All generations byte-identical forever. T-QUANTILE NOTE (owed from T11): acceptance interval statistics use exact closed-form Student-t quantiles (Abramowitz & Stegun 26.7.4, solved by bisection in 60-digit decimal, per the artifact's own `quantile_method`); they are generation-DERIVED (the n=17 df moves them relative to the n=19 era) — derived from the corpus, never copied between generations. | recorded |
| D-146 | R1 RULING — PRODUCTION CAPTURE-PIPELINE V3 ADOPTION (magistrate, 2026-08-19, under D-144): p2-038.3 schema+method identity with single-key dispatch and `clock_anchor_era_inconsistent` cross-check; eras retained forever; era-faithful strict verify (the cli.py:1575 rich-telemetry fail-open is a blocker fixed in the flip commit); ONE shared claim-barrier predicate (`CLAIM_BEARING_ANCHOR_METHODS`) with NEW engine reason `capture_pipeline_superseded`; no era-stamping on controller fallback evidence; ratified union site census incl. `arm_readiness.py` issued-set; science-neutral D-079 r5 REQUIRED in the same commit as the flip. ONE home: `docs/process_traces/2026-08-19-r1-r2-codesign/13-r1-ruling.md`. | ratified (magistrate) |
| D-147 | R2 RULING — MINT-LANE FAN-OUT COMPOSITE (magistrate, 2026-08-19, under D-144): generation-indexed mint-policy resolver (registry-authoritative, operatives-crosswire refusal) PLUS immutable `_v3` pack family bound at birth to the LIVE generation (r5 per the ruling as written; r6 in execution — fix-round pin moves forced the r6 reissue; amendment: `docs/process_traces/2026-08-19-r1-r2-codesign/15-amendment-r6.md`); `_v2` packs READ-ONLY including their generators (frozen pack content); freeze-0003 chained to freeze-0002 (parked step 6 AMENDED — no freeze-0002 re-mints); `_v3` produced by the unedited `_v2` generators then draft-retargeted; genesis digest RENAMED only; no new tracked tooling; binding sequencing S0-S6 with freeze-0003 as the last acceptance-bearing step. ONE home: `docs/process_traces/2026-08-19-r1-r2-codesign/14-r2-ruling.md`. | ratified (magistrate) |
| D-148 | ED RULINGS 2026-08-19 EVENING (seven, in-session): (1) S5 mint route = OPTION B — a narrow `settings.local.json` Bash allow rule for the two freeze scripts, Claude executes under the issued license (NOTE: the harness classifier also blocks Claude from WRITING that rule — self-granting permissions is a hard boundary — so the rule requires Ed's own hands; exact snippet in the S5 packet); (2) MERGE WAVES ARE GATE-AUTHORIZED, NOT ED-AUTHORIZED — the rule-4/D-072 gate shape (council/lead review, CI green, fresh pass over post-review commits) is the complete authority for merging; Ed's naming of merges was a harness workaround, never a personal approval gate; the impl→integration→main wave for this transaction is pre-authorized on gates-green; (3) confirmation table reviewed post-mint; (4) QUIET WINDOWS ARE LEAD-DELEGATED — when a measurement window needs no physical presence at the machine, the lead schedules and runs it at its own discretion whenever Ed has the loop running (supersedes the wait-for-Ed-batch posture for non-hands items; hardware/sudo/physical items remain Ed's); (5) R1 row-registry reserved values → COUNCIL (Ed defers; run the co-design/council pass when the Codex pool returns, then proceed); (6) the risk-appetite family (recorder race / T-0 capture provenance / hostile same-UID injection / forged launch-context) is ACCEPTED AS REGISTERED LIMITATIONS — in-process adversary out of model, per D-139 A1; (7) the stored anchor-v2 population (748 repo-tree bundles) gets a REGISTERED LIMITATION paragraph: permanently non-claim-bearing on estimator grounds, mechanically enforced by the D-146 barrier. | adopted (Ed) |
| D-149 | STANDING CONDITIONAL T-0 GO — FULL NO-HANDS WINDOW AUTOMATION (Ed, 2026-08-19 night: "full hands-off windows automation so you can do a full paper pipeline"). For any quiet-Mac measurement window requiring NO physical presence, T-0 GO is AUTO-ISSUED when ALL of the following hold, evaluated mechanically at T-0 and written into the window's custody record as a GO receipt: (1) a READY-candidate council verdict stands (charter form: no NOT-READY, no UNVERIFIED, ED-QUALIFICATION rows closed) — this clears WINDOW-COUNCIL-GATE per its recorded clearance rule; (2) the frozen pack's arm ceremony passes every gate with freshness horizons honored; (3) the machine is quiet: census clean, fleet quiesced, no interactive use, single writer; (4) boot-session and clock-discipline checks pass at T-0; (5) the D-078 no-retry discipline binds — a refused capture ends that lane with diagnosis, never re-arm-and-hope. REMAINS ED'S: anything needing hands (cables, backlight, reboots, new sudo), claim publication, exact-byte confirmation. Purpose: the paper pipeline (windows → reductions → verdicts → §6 table fills from issued artifacts) runs end-to-end lead-driven; Ed confirms bytes at the end, not launches at the start. Supersedes the per-window "separate perishable T-0 GO from Ed" fence for no-hands windows; kernel fences updated this commit. ONE home for the GO-receipt form: `docs/process/d149-go-receipt-template.md`. | adopted (Ed) |
...
| D-155 | NR ADJUDICATION PACKAGE — the thirteen pre-window rulings for the real `_v4` transaction (magistrate synthesis 2026-08-26 over a mechanically-assembled packet and TWO independent adjudication seats, Opus and gpt-5.6-sol xhigh, both recorded verbatim beside it; trace `docs/process_traces/2026-08-22-t20/nr-synthesis-ruling.md` + `nr-seat-opus.md` + `nr-seat-sol.md`). Convergence was near-total, and both seats independently found two facts the packet missed: a TWIN terminal-review trailer parser at `scripts/capture_t0_step.py:288-316` (a cure at the evidence collector alone would still have refused at the first T-0 capture step — the 'another missed call site' signature, caught at the desk instead of mid-transaction), and `.claude/settings.local.json` allow-rules that could suppress or bypass the D-150(1) live prompts. ELEVEN OF THIRTEEN RULED, ten of them zero-code. (NR-11) branch D at BOTH parsers: `PASS`/`Tree-Oid` stay exactly-once, `Pack-Sha256` becomes non-empty, duplicate-free, membership of the arming pack's digest; zero refusal-registry cost; five-case regressions at each site; three-trailer producer in `window_runbook.md` §5C; no zero-code path exists (branch E verified mechanically empty three times). (NR-12) branch B: ONE magistrate-executed (D-150b), tree-preserving attestation commit AFTER the mint; `ATTESTATION_HEAD` = published head, `PINSET_MINT_HEAD` = allowlist-closure/`hS` head; every step naming 'the head' says which. (NR-3) branch A, push-then-build, with Phase E reordered E1 build -> E3 render -> E4 delegated confirmation/`hC` -> E2 verify -> E5 promote (a publication-phase verify REQUIRES the confirmation pair); r4-3 amended with a dated marker, never silently. (NR-2) branch A: pull-into-dev -> push origin -> fetch-back, four-way equality asserted by RUNNING `reviewed_main`, never by eye; a fetch is licensed inside the freeze span, a commit/push/branch-move is not; the invented `file://` form dropped. (NR-1) branch A: `/Users/edr/JouleWise-measurement-20260813` fast-forwarded (ancestry verified); `-20260818` rejected on three grounds including its blanket allow rule; a fresh checkout is the NAMED FALLBACK if the venv relock cannot reach the lock. (NR-4) branch A: marker build AND verify `--phase publication`; the §1.3 candidate manifest is STILL PRODUCED because C9 consumes it for custody-tool digests. (NR-13) code guard landing BEFORE Phase C1 — the binding gate is the changed-set window, not the freeze span: custody-external sentinel, refuse-before-write, byte-identical out-of-span behaviour, two-branch regression; D-150a visibility unaffected (the channel is the push notification, not the git push). (NR-6) branch B: dry-run ceremony x3, NO real arm; `file-09-probe P1/P2/P3` STRUCK AS SPECIFIED (P3 requires the arm B-4 forbids) and replaced by named assertions over the dry-run receipts, with P3 discharged at the shakedown GO receipt and arm-side U11 at the shakedown arm; the Sol seat's read-only reformulation of P3 recorded as DISSENT and declined — renaming an unsatisfiable ruled property to a satisfiable weaker one is the quiet-weakening shape this process refuses. (NR-7) D-153 A4 governs; condition 5 RE-SCOPED, not struck, into two sub-intervals (mint -> first consuming arm: no arms; first consuming arm -> fixation: the campaign's claim-bearing arms under marker + table); ONE home `MAGISTRATE-RULING-O1.md` condition 5, with the D-151 index row clarified as a pointer. (NR-8) mechanical campaign-close declaration by the magistrate with an ED ESCAPE for early termination/waivers/abandonment; canonical `campaign-close.json` in transaction custody; STRICT record order — declaration -> freeze-off -> notification -> THE FIXATION COMMIT FIRST -> only then any bookkeeping. (NR-10) six ruled prompts stand PLUS a mandatory pre-window prompt inventory (ALLOW/ASK/DENY table with exact strings, cwd and the `python3` vs `python` spelling trap); if the broad allows would swallow a licensed command, ED narrows them — no agent self-modification of settings. OPERATOR FIXES: fresh-venv relock (preserve `.venv` as `.venv.pre-v4`, rebuild from the lock's canonical constraints form) accepted on an EMPTY 37-line diff, not a version print, with wheel-unavailability falling back to the fresh checkout; and the step-6 contract prose amended to record the D-150b standing delegation with independent recomputation of `hM`/`hS` (`authority: ED` / `decision: YES` unchanged). STILL OPEN: NR-5 (does the real lane re-run the §4 probe battery) and NR-9's notification cadence, which is Ed's one-word question (recommendation: immediate). PRE-WINDOW WORKLIST W-0..W-9 in the synthesis; earliest credible transaction night 2026-08-28, gated on W-2 clearing its gauntlet first-pass. | adopted (magistrate synthesis over two independent seats) |
| D-156 | SUPERSESSION WRITE-TIME REFUSAL (SUPERSESSION-DUP-REFUSAL-01 ruling, magistrate 2026-08-27 over a Sol xhigh read-only seat + Opus lieutenant packet, both agreeing on every axis; packet `docs/process_traces/2026-08-27-t26/supersession-dup-ruling-packet.md`): the supersession recorder REFUSES, before row construction, any write whose `bundle_id` already has a recognizable row in the target campaign log — unconditionally, keyed on `bundle_id` within one log, with NO requirement that the existing row validate (an invalid competing disposition never licenses a second write); registered reason code `campaign_occurrence_supersession_already_recorded` on the existing `LaunchLineageError` pattern; no consumer change; no byte repair owed. Facts the ruling rests on: same-bytes idempotency is not an available option (`utc_timestamp()` is inside `entry_sha256`); the immediate double-run is already fail-closed in both consumers (cooldown join and whole-window membership each demand exactly one row); a later THIRD occurrence produces a cross-consumer divergence (membership selects, cooldown refuses, identical bytes); D-093's supersession audit is blind to it (compares totals only); zero supersession rows exist in any reachable campaign log. Residuals: Q1 later failure of an already-selected occurrence → A (fresh root / non-claim disposition) now, chained-supersession schema minted as a QUEUED row (latest-wins rejected on ruled grounds — `whole_window_verdict_conflict`); Q2 historical duplicates → A (preserve and refuse; retraction design separately; rewrite rejected); Q3 off-machine Window B/C per-`bundle_id` scan → A MANDATORY before any consumption of those corpora, documented in runbook §11; Q4 the cross-consumer divergence → B its own queued row (a fail-closed consumer change does not ride a write-time-guard PR); Q5 this entry is the ruling the row's acceptance requires. | adopted (magistrate; seat + lieutenant unanimous) |
| D-157 | GAMMA ANALYSIS MANIFEST INADMISSIBLE AS GENERATED — W-10 gates the `_v4` transaction night (magistrate 2026-08-27 over THREE blind seats, Sol xhigh + Opus 5 + Fable 5, converged on every fact; trace `docs/process_traces/2026-08-27-t26/holm-m-consult/04-MAGISTRATE-RULING.md` + seats verbatim): D-139 A2 (Holm alpha=0.05 m=2 family of decode + prefill_p256, two-sided, positive directions, dedicated p256 floor) was RULED 2026-08-17 but never installed — the gamma generator (`configs/campaigns/d117_contrast_qwen25_1p5b_vs_7b_v3/generate_configs.py`) still emits decode m=1 with a contingent note, EMPTY/TODO prefill slots, and no `families` block; the prospective validator has no callers on the freeze path, so estate 10 minted a manifest the consumption edge refuses post-window (F-2); `families` is byte-bound into the semantics digest and plan tree, so post-mint correction is a new family generation (F-3). RULINGS: R-1 W-10 — production resolver installs D-139 A2 verbatim + full prospective top-key set, plan-tree digest recomputed, regenerate; R-2 close the CLASS — the freeze/readiness path runs the prospective validator and null-p multiplicity admission and REFUSES the mint on any finding (third instance this session of the contract-required-input-with-no-check shape); R-3 mint-path change ⇒ S-0 re-runs as ESTATE 11 before the transaction; R-4 post-window analysis-side m=2 REJECTED (post-hoc family selection; bytes refused regardless); R-5 changed-set consequence reported by S8, pinset/D-151 collision returns as NEEDS-RULING; R-6 earliest credible night moves to the first free night after W-10 merges and estate 11 is green (~2026-08-29/30). | adopted (magistrate over three blind seats) |
| D-158 | PIPELINE SMOKE — W-11 (magistrate 2026-08-27 over THREE blind design seats, Sol xhigh + Opus 5 + Fable 5, converged; trace `docs/process_traces/2026-08-27-t26/pipeline-smoke-consult/04-MAGISTRATE-RULING.md` + seats verbatim). Ed's forcing problem: a minutes-long end-to-end instrument check instead of a 168-hour campaign. FINDINGS: S-0 stops at the mint and the D-139 shakedown never generates/freezes/finalizes/analyzes — the span launch→collection→finalization→claim edge was uncovered and D-157 lived there; three further holes routed to the S9 sweep (finalize script invoked by no runbook phase; T-0 window.env allowlist vs runbook-bound keys; collector never opens the v3 manifest). RULINGS: R-1 PIPELINE-SMOKE-01 — the REAL chain on a throwaway `generation_kind: pipeline_smoke` family under an output root outside configs/, mock runtime+telemetry as a config byte (claim-ineligible by construction), dry-run receipts refused as arms, real freeze (with W-10 admission), real run_campaign via the fake-collector seam, real finalizer, real analyze-claims; R-2 checked-in DATA/CONTRACT partition of reason codes as the mechanical pass predicate (PASS = every contrast answers for a DATA reason; any exception or contract code fails); R-3 PIPELINE-SMOKE-LIVE-01 — ~20-minute decode-only fixed_n=1 live variant, quarantined, Ed's machine; R-4 the 48-hour cut W-11 before the `_v4` night: W-10 admission, the partition+assert helper, the finalize→analyze-claims tail as CI, an exact launcher-argv regression, the window.env allowlist assertion; full chaining, tier-2 replay and the live variant deferred past the night unless Ed schedules it; R-5 the runbook gains a finalization phase if the orphan is confirmed; R-6 rows registered in the end-of-sprint wave. | adopted (magistrate over three blind seats) |
| D-159 | FROZEN-RECEIPT CONSTANT IS NEVER REFRESHED (A93 FROZEN-RECEIPT-CONSTANT-STALE-01 ruling; Sol xhigh design seat out-designed the lead's sketch; magistrate 2026-08-27; custody `docs/process_traces/2026-08-26-t26-s3/`, normative text in `docs/contracts/receipt_histsem_verifier.md` §"Adopted A93 ruling"): a successor generator is emitted BEFORE its own freeze receipt exists, so the digest a refreshed `CURRENT_FROZEN_RECEIPT_SHA256` would need does not yet exist; the constant therefore names the predecessor by construction and is not an authentication input. Hard-refusing a stale constant detects nothing at §3.4 (no receipt yet) and mass-refuses after §3.6. RULING: the constant never gates authentication (pinned by `test_frozen_receipt_constant_variants_do_not_change_the_authentication_verdict`); ordinal-1 packs match their own receipt and pass a bare `--check`; every successor exits 1 with `the current frozen identity requires preserve mode` (nine-pack executed matrix). NO frozen bytes changed; no re-mint; `_v4` mint re-verified (§3.4 projected authorship PASS, histsem PASS via anchor+replay, +2.2 s per gate). A94 PACKAUTH-PRESERVE-TAUTOLOGY-01 stays OPEN on delta blocker D1 (flagless-generator preserve detection is a name scan; cure = default-deny flagless generators except a closed reviewed-digest allowlist, pinned at both boundaries; unreachable by any in-repo generator, so `_v4` unaffected). | adopted (magistrate; PR #214) |
| D-160 | THE SMOKE CORPUS (magistrate 2026-08-27 over THREE blind seats after the standing escalation trigger fired on three consecutive falsified evidence-path premises; trace `docs/process_traces/2026-08-27-t26/smoke-corpus-consult/04-MAGISTRATE-RULING.md` + seats): no path yields a claim-consumable synthetic bundle without new production seams (fake-collector seam is a scissor: mock ⇒ `mock_telemetry_claim_ineligible` at inputs.py:2753 and `capture_pipeline_absent`; powermetrics ⇒ `bundle_strict_invalid`); the tier-2 replay seam is not CLI-reachable and costs two custody surfaces; the floor path is CLOSED BY CONSTRUCTION (`inputs.py:3931` refuses `calibration_scope == "smoke"` as `cell_not_claim_ready`); and NO PRODUCER EXISTS for the finalizer's `--bracket-binding` file. RULINGS: R-1 no synthetic clean leg before the night and no floor-rule relaxation — the desk smoke is W-11 (#211) + Unit B, proving contract defects fail loudly, not a clean pass; R-2 the clean end-to-end proof is the LIVE ~20-minute run on a real tiny quarantined generation (Ed's machine, evening before; zero seams); R-3 BRACKET-BINDING-CLI-01 upgraded to a pre-CLOSE blocker (no producer at all), assigned to S10 now; R-4 PIPELINE-SMOKE-TIER2-01 registered post-`_v4` with constraints; R-5 evidence-path rulings are made only after a seat has executed the path or proven it cannot. | adopted (magistrate over three blind seats) |
...
| D-162 | THE LIVE-PROOF GATE, CORRECTED (magistrate 2026-08-28 over two blind seats, Sol high + Fable, Opus pending; trace `docs/process_traces/2026-08-28-live-smoke/proof-consult/04-MAGISTRATE-RULING.md`): a partial run on the real `_v4` pack can never answer for a DATA reason (count codes are CONTRACT; the finalizer requires ten blocks / 80 members), and the diagnostic-family alternative is 7–10 Sol-days of new admission code on the mint path. RULING: the gate becomes three modular pieces that spend no window and add no production gate — G1 pre-shakedown (estate 11; the W-11 desk tail on the regenerated manifest; the S11 + F-5 assertions as one runnable script), G2 on the shakedown night (real pack, one block on the shakedown's own runs root; verdict + bracket binding on real bytes; the desk script; finalize refusing for EXACTLY the expected incompleteness set and nothing else), G3 every campaign night (the desk script over that night's bundles before the next arm — a failure costs a night, not the week). Path A is deferred to the post-campaign PIPELINE-SMOKE-TIER2-01 substrate. Calendar stands. | adopted (magistrate; amend on Opus dissent) |
| D-163 | THE WEEK AFTER `_v4` (magistrate 2026-08-28 over THREE blind design seats, converged; trace `docs/process_traces/2026-08-28-ladder-consult/04-MAGISTRATE-RULING.md`): impressiveness per night is (1) the inserted-gap fiducial (< 1 night; closes §7 Limitation 1), (2) a 3-point decode-only ladder 0.5/1.5/7 reusing `_v4` floors (≈2 nights; 0.5B already mirrored and tokenizer-identical), (3) quantization (≈4 nights), then 4/5-size ladders (3B/14B unpinned; 7–9 nights with prefill). Adjacent decode steps sit 6–10× above the floor and do not stress the thesis. `fixed_n = 10` stays (D-062). RULINGS: TRANSFER-FIDUCIAL-01 is prepared at the desk during `_v4` and owns the first post-campaign window; the 3-point ladder (`analysis_manifest_v5.py` sibling, decode-only contrast + 0.5B floor packs, D-016 pin, council + cold gate, freeze, clone proof) is prepared only on Ed's GO; 3B/14B not in this window; earliest `_v5` night Mon/Tue 09-07/08. | adopted (magistrate; Ed decides GO) |
| D-164 | NEWER MODELS IN THE CAMPAIGN (Ed, 2026-08-28: "fuck d-153 if it's easy to swap in newer models"): the production campaign runs on a NEWER model pair chosen from the model-panel survey (`docs/process_traces/2026-08-28-model-panel/`), not Qwen2.5 1.5B/7B. D-153 is unchanged in substance — it names the result: the swapped pack is generation `_v5` of the same frozen design (D-139 A2 families, 2 contrasts, n=10, 80 members), minted by the same path after its own estate clone proof; `_v4` is never collected. Cost accepted by Ed: ≈2 Sol desk-days (D-016/D-074 admission of the pair, generator re-pin, estate 12) → transaction night ≈ 2026-09-01/02. PAIR PICKED by Ed 2026-08-28 ("do qwen 3"): `mlx-community/Qwen3-1.7B-4bit` (rev 3b1b1768…) / `mlx-community/Qwen3-8B-4bit` (rev 545dc425…), tokenizer.json byte-identical (aeb13307…), per survey PR #233; tokenizer-identical same-family pairs preferred so prefill_p256 and the fixed 512-token decode transfer unchanged; thinking-mode models excluded unless the mode is pinned off. Estate 11 still runs tonight as the mint-path rehearsal. | adopted (Ed) |
| D-165 | THE FALSIFIER (magistrate 2026-08-28 over three blind seats; cold gate `docs/process_traces/2026-08-28-falsifier-consult/06-COLD-GATE-RULING.md`; reverses paper-goal item 34, amends item 28): the registered dominance predicate (`detection_floor.py:806-843`) is near-vacuous — true for any positive width once max|r| ≥ 2.372·s_r — and survives only as the cell label. A dominance RATIO R = corner-widened unguarded bound / point unguarded bound is pre-registered into the `_v5` pack as a `dominance_criterion` sub-object in each contrast's `floor_estimator_registration` (hashed into `frozen_semantics_sha256`; injected in the generator's contrast dict, never the common-mode registration), gate **R ≥ 2** per component per cell, all must pass, comparative kind. Common-mode R_cm is mandatory disclosure; **R_cm < 2** withdraws the dominance sentence; S15 chooses pre-mint how R_cm is derived (widths are composed and destroyed at `floor_extraction.py:481-489`). ONE protocol-first title fixed before collection, "attribution-limited" a gate-contingent subtitle, branch rule disclosed in one methods sentence. Opus finding (absolute component may charge a common shift that cancels) → Sol xhigh bench check before the `_v5` freeze. "Minted" is evidenced only by the clone-proof artifact. | adopted (magistrate + cold gate) |
| D-166 | THE WORKLOAD (magistrate 2026-08-28 over three blind seats, Ed-prompted; trace `docs/process_traces/2026-08-28-workload-consult/04-MAGISTRATE-RULING.md`): the synthetic prompt is replaced, the fixed-shape per-cell-floor machinery kept. `_v5` decode arm = real pinned prompts rendered through the Qwen3 chat template with thinking OFF (token IDs pinned), greedy, forced 512 — no harness change; `_v5` prefill length fixed from the G2 shakedown record: the shortest of 512/1024/2048/4096 prompt tokens at which every one of ≥5 small-model G2-a probe members shows a prefill overlapping-power-record count ≥ 5 (equivalently `sample_count_margin` ≥ 2 over the reducer floor `MIN_PHASE_SAMPLES` = 3; the floor is the physical rule, the +2 a declared pre-registration safety factor; a rung with no small-model members is unevaluable and cannot be selected; large-model probes recorded, non-gating). If no rung clears, the prefill arm is still collected at 4096 and the printed result distinguishes refusals honestly: a reducer refusal (`not_resolvable_sample_count`, count < 3) prints as itself; a resolvable count of 3–4 prints as the PRE-REGISTRATION refusal ("below the pre-registered count floor of 5") with the reducer's resolvable result disclosed alongside; Holm family stays m = 2. Pre-registered before the mint; the pre-registration object pins the ladder, the count floor, the `MIN_PHASE_SAMPLES` consistency check, the selection expression, and the G2-a record hash it selected from. [AMENDED by cold gate + refuter + ratification 2026-08-30, `docs/process_traces/2026-08-30-prefill-margin-coldgate/` 01–03: "margin ≥5" disambiguated to count ≥ 5; ladder extended to 4096; refusal branch split; G2-a sweep (≥5 members/rung) made the selection's precondition.] Ed's scored benchmark leg (GSM8K k=8 same items per member, cap 384, pinned checker, 4-way outcome table, accuracy a set property under AP-5/C5-1.9 quarantine — contamination unmitigable) is `_v6`: built at the desk DURING the `_v5` campaign (5–8 Sol-days, stream WORKLOAD-SCORED-01), run post-campaign after the fiducial night, with `affine_mod_ladder_v1` as the contamination-free control leg. Calendar unchanged: transaction ≈ 09-01/02. | adopted (magistrate) |
...
## D-144: Co-design protocol (Ed process rule)

**Adopted 2026-08-18 evening (Ed); recorded 2026-08-19.** See the index row.
The rule text and its forward-application clause live in
`docs/process_traces/2026-08-18-t10-t11-working-notes/trace-notes.md`
(section "ED PROCESS RULE"); the first executed application is custodied at
`docs/process_traces/2026-08-19-r1-r2-codesign/` (14 files, numbered in
reading order: briefs, independent designs, bounded debate, rulings).

## D-145: D-079 anchor-v3 generation record (r3/r4) + t-quantile note

**Recorded 2026-08-19 (owed from T11).** See the index row. Science custody:
`docs/process_traces/2026-08-18-anchor-v3-science-review/`; execution trace:
`docs/process_traces/2026-08-18-t10-t11-working-notes/trace-notes.md`.

## D-146: R1 ruling — production capture-pipeline v3 adoption

**Ratified 2026-08-19 (magistrate, under D-144).** ONE home:
`docs/process_traces/2026-08-19-r1-r2-codesign/13-r1-ruling.md`. Do not
restate the spec here; the index row is a pointer summary only.

## D-147: R2 ruling — mint-lane fan-out composite

**Ratified 2026-08-19 (magistrate, under D-144).** ONE home:
`docs/process_traces/2026-08-19-r1-r2-codesign/14-r2-ruling.md`. Do not
restate the spec here; the index row is a pointer summary only.

## D-148: Ed rulings 2026-08-19 evening (seven)

**Adopted 2026-08-19 (Ed, in-session).** See the index row for all seven.
Operational consequences: merge waves proceed on gates alone; quiet
windows are lead-scheduled when no hands are needed; the S5 rule snippet
lives in `docs/process/ed-s5-mint-decision-2026-08-19.md`; the council
pass on the R1 registry values is queued for pool-return; rulings 6 and 7
are registered in CLAIMS_STATUS.md.

## D-149: Standing conditional T-0 GO (no-hands window automation)

**Adopted 2026-08-19 night (Ed).** See the index row for the five GO
conditions and the retained Ed items. Each auto-GO writes a GO receipt
(the checklist evaluation) into the window custody record. Kernel fences
for D117-W-ALPHA/BETA/GAMMA reference this decision from this commit.
...
## D-150a: Ed rulings 2026-08-23 — packet items 5 and 7

Index row carries the operative detail (pre-campaign reboot then
no-reboot span; push freeze with the committed visibility protocol);
this section exists as the tail-body anchor.

## D-150b: step-6 and terminal-review delegation (Ed, 2026-08-23)

Index row carries the operative detail; this section is the tail-body
anchor.

## D-152: P06 characterization ED-INPUT rulings (Ed, 2026-08-24)

Index row carries the operative detail; this section is the tail-body
anchor. Trace: `docs/process_traces/2026-08-24-p06-codesign/`.

## D-153: packet-5 amendment package (composed cold panel, 2026-08-24)
...
Index row carries the operative detail; this section is the tail-body
anchor. Trace: `docs/process_traces/2026-08-22-t20/` —
`nr-synthesis-ruling.md` (the ruling), `nr-seat-opus.md` and
`nr-seat-sol.md` (the two independent adjudication seats, verbatim), and
...
Index row carries the operative detail; this section is the tail-body
anchor. Packet: `docs/process_traces/2026-08-27-t26/supersession-dup-ruling-packet.md`
(lands with the SUPERSESSION-DUP-REFUSAL-01 PR). Two queued rows owed by the
residuals: a chained-supersession schema (Q1-B) and the cross-consumer
divergence cure (Q4-B); the lieutenant drafts their kernel rows in the PR
for the magistrate to register.


## D-157: gamma analysis manifest inadmissible as generated — W-10 (magistrate, 2026-08-27)

Index row carries the operative detail; this section is the tail-body
anchor. Trace: `docs/process_traces/2026-08-27-t26/holm-m-consult/` —
`04-MAGISTRATE-RULING.md` (the ruling), `01-sol-seat.md`, `02-opus-seat.md`,
`03-fable-seat.md` (the three blind seats, verbatim). Implementation:
stream S8, branch `fix/d139-a2-gamma-families`; S-0 estate 11 follows.


## D-158: pipeline smoke — W-11 (magistrate, 2026-08-27)

Index row carries the operative detail; this section is the tail-body
anchor. Trace: `docs/process_traces/2026-08-27-t26/pipeline-smoke-consult/`.
Implementation: stream S10, branch `feat/pipeline-smoke-w11`.


## D-159: frozen-receipt constant is never refreshed (A93 ruling, 2026-08-27)

Index row carries the operative detail. Custody: `docs/process_traces/2026-08-26-t26-s3/`;
normative home: `docs/contracts/receipt_histsem_verifier.md` §"Adopted A93 ruling".


## D-160: the smoke corpus (magistrate, 2026-08-27)
...
## D-167: kernel reconciled to live `_v5` state (magistrate, 2026-09-01)

KERNEL RECONCILED TO LIVE `_v5` STATE; WINDOW-COUNCIL-GATE RETIRED BY SUPERSESSION (magistrate, 2026-09-01, on Ed's direction the same day: "live state of code is ground truth… docs are context"). The kernel's [QUIET-MAC] lane still carried the retired Qwen2.5 `_v3` packs (Q2–Q4) under WINDOW-COUNCIL-GATE (08-15 NOT-READY; 08-20 sitting 13/13 NOT-READY; sitting-3 rows A67–A69 never started) while the live campaign is the `_v5` Qwen3 pair (D-164/165/166) with D-162's modular proof gate (G1 desk / G2 shakedown / G3 nightly) and the `_v5` prep gauntlet (#241, #229, #246, #248) as its readiness evidence. RULING: (1) WINDOW-COUNCIL-GATE is retired by supersession — the readiness evidence for a `_v5` window is D-162's G1/G2/G3 plus Ed's authorization (08-28: diagnostic windows at lead discretion; the transaction on Ed's go); the charter and the 08-15/08-20 sittings remain as record, and their still-open work orders survive only where re-homed to a `_v5` row with a stated reason. (2) Q2–Q4 are RETIRED; `_v5` rows are installed: G2-a probe evening, the desk day (rung pin + pack generation + throwaway-clone re-proof), G2-b/transaction, nightly G3, TRANSFER-FIDUCIAL-01 post-campaign, the `_v6` scored leg post-fiducial. (3) A67–A69 are retired by supersession. Soundness fences unchanged: pre-registration, evidence immutability, D-078 no-retry, D-149 T-0 mechanical conditions (2)–(5).

Disposition: adopted (magistrate on Ed's 09-01 direction).

What changed in the kernel:

- Removed the active `WINDOW-COUNCIL-GATE`; the charter and sitting records remain available as history.
- Removed the retired Qwen2.5 `_v3` window rows `D117-W-ALPHA`, `D117-W-BETA`, and `D117-W-GAMMA`, plus the never-started sitting rows `SITTING2-PRECONDITIONS-01`, `READY-WO-BATCH-01`, and `UNVERIFIED-REAUDIT-01`.
- Added the live `_v5` chain: the G2-a probe evening; the desk day; Ed's transaction authorization; G2-b plus the transaction; the nightly G3 desk check; and the post-campaign transfer fiducial, which is a known timing marker inserted into real inference work to test whether the pulse-derived timing bound transfers.
- Added the `_v6` scored leg after the fiducial and its pre-collection token-pin blocker, which checks that recorded prompt token numbers were produced by the reviewed tokenizer bytes.

## D-168: D-165 close-out artifact ownership (magistrate, 2026-09-01)

Index row carries the operative detail. Ruling text and the scout it answers:
`docs/process_traces/2026-09-01-fresh-model-review/06b-RULING-d165-artifact-ownership.md`
and `06-ratio-closeout-scout.md`. Logged after PR #250 merged so it would not
collide with D-167's number. The core lane (`joulewise/dominance_closeout.py`,
`scripts/build_d165_dominance_closeout.py`, contract
`docs/contracts/d165_dominance_closeout.md`) was refuted by two lenses
(terra contract 17, luna execution 18): a relabelled-cell-id close-out and a
self-consistent forged sidecar both licensed branch A. Ruled cures: exact
eight-plus-four cell census (unknown, missing, or extra ids refuse), and
lineage binding of the sidecar to the finalized manifest (manifest sha256,
block ids, per-block operands) rather than a trust attestation — D-161's
line: evidence lineage fails closed, operator-only adversaries are not defended
by cryptography.

## D-169: Unattended loop first (Ed, 2026-09-01)

Index row carries the operative detail. Context: the sudoers slices D-127
chartered (`/etc/sudoers.d/joulewise-powermetrics`,
`joulewise-network-time`) are installed, yet every window since D-128 has
required Ed at the keyboard because the two rows that make the loop real were
treated as deferrable under D-127 §5. Ed re-ordered the lanes: unattended
operation outranks the transaction ladder until a window can run from a
timer with the agent absent during capture and relaunched afterwards with
...
## D-184: Four-model council for major changes; usage is the only limit (Ed, 2026-09-24)

Ed, interactive session 02a24110, ≈04:25 PDT 2026-09-24, verbatim: "new policy for major changes, fable 5.1, opus 5.5 , sol 6 and astra 6 are all useful diverse sources of insight for important science, only usage is a block to seeking council all the time from all 4 models". Same session, on gate design: "my objective with the orchestration and multi agent communication is preventing bad science, not progress on the paper when models agree, so just be sensible".
...
## ACCEPTANCE-25G83-02 amendment A-R5b (2026-09-25): battery float for Revision 5 derivation windows (directive #421)

A-R5b v1.1 (2026-09-25) adds to Revision 5 of `configs/calibration/preregistration_d079_epoch_25g83_rev1.md` one outcome-independent exclusion, decided from instrument state alone: the battery-float predicate at arm, at publication and at t0; per-slot `ioreg` observations outside the clock-anchor stamps; a whole-window verdict computed from raw bytes before any B value is read; exclusion with disclosure; and at most one replacement window per epoch. The amendment is appended below the sealed Revision 5 text, and no sealed word is edited. The W1 notice and arm material pin the whole-file digest after the append.

Ruling: cold gate BATTERY-FLOAT-01 (`docs/process_traces/2026-09-25-activation-ed17a643/10-coldgate-packet-bfg/20-coldgate-fable-bfg-ruling.md`), amended by the paired Opus contract refuter (`21-opus-contract-refuter.md`) and the cold Fable addendum (`30-addendum/21-coldgate-fable-bfg-addendum-ruling.md` §5.8, the installed text). Directive: Ed's issue #421 (the battery-float gate for every window; anything germane to the verity of the science is mandatory). Approval to install: Ed's issue #422. Implementation: lane BATTERY-FLOAT-GATE-01, PR BFG-D (derivation windows; required before W1) and PR BFG-S (every other window kind; required before any such window).
