...
A **zero-capture refusal** is a final night-gate refusal on machine state for which the driver's result and receipt agree and no measurement chain started or capture was recorded. Once `courier.sent`, the courier's completed-delivery marker, exists, the watchdog ends that plan's agent-free hold on its next tick instead of waiting through the planned measurement window. The cost is then delivery, harvest (inspection and preservation of the night's records), cleanup and preparation of a fresh plan, not the unused window. The **successor** is that fresh plan: it needs a new identity and notice, at least 60 seconds after the refusal's final write, installation before its own cutoff, and the usual owner veto and checks at its own scheduled start. The current next-start planner puts that start at least 40 minutes ahead of preparation, so the 20-minute recovery target is not a guarantee of a new measurement within 20 minutes. Registration and class refusals, or any night whose chain started, keep the full hold and cold-gate review.
