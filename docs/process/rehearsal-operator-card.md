...
E-10 carries the **confirmation pair**. `--step6-confirmation-table` is the path to `d117_step6_confirmation_table_v4.json`, the file recording Ed’s `YES` over this pack family and the SHA-256 of the successor-pinset file at the reviewed head; `--expected-confirmation-digest` is `hC`, the SHA-256 **of that file’s own bytes**. The launcher replays the arm receipt before consuming, and that replay hits a gate which hashes the table’s bytes and compares them to `hC` before reading any field out of it. The digest is mandatory: without it the gate refuses. The path flag is optional — omitted, the gate looks for the table at `…/arm-readiness-custody/family_publication/d117_step6_confirmation_table_v4.json` — but state it anyway, so the file under test is explicit. **`hC` is carried, never recomputed:** export it into the shell first, pasted from the freeze transaction’s `085-ed-step6-confirmed-sha256.txt` custody transcript. It is deliberately absent from this card, because `hC` never enters a repository path. Running `shasum -a 256` over the table instead would hash the file under test to produce the value that same file is checked against — it would agree whatever the file contained, which is the one use the gate’s contract forbids.

```sh
export ED_STEP6_CONFIRMED_SHA256=''   # paste the 64-hex digest from 085-ed-step6-confirmed-sha256.txt
```

```sh
cd /Users/edr/JouleWise-measurement-20260818 && ARM_RECEIPT=/Users/edr/JouleWise-window-custody/ed-qual-20260817/rehearsal/arm-readiness-custody/d117_floor_qwen25_1p5b_v2/arm_readiness.receipts/arm-0001.json && LAUNCH_MANIFEST=/Users/edr/JouleWise-window-custody/ed-qual-20260817/rehearsal/arm-readiness-custody/d117_floor_qwen25_1p5b_v2/arm_readiness.t0.inputs/launch-manifest.json && STEP6_CONFIRMATION_TABLE=/Users/edr/JouleWise-window-custody/ed-qual-20260817/rehearsal/arm-readiness-custody/family_publication/d117_step6_confirmation_table_v4.json && test -n "${ED_STEP6_CONFIRMED_SHA256:-}" && .venv/bin/python scripts/launch_window.py --pack-root /Users/edr/JouleWise-measurement-20260818/configs/campaigns/d117_floor_qwen25_1p5b_v2 --arm-receipt "$ARM_RECEIPT" --arm-readiness-custody-root /Users/edr/JouleWise-window-custody/ed-qual-20260817/rehearsal/arm-readiness-custody --launch-manifest "$LAUNCH_MANIFEST" --step6-confirmation-table "$STEP6_CONFIRMATION_TABLE" --expected-confirmation-digest "$ED_STEP6_CONFIRMED_SHA256"
```
