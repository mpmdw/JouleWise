...
anything not verifiable in the checkout is marked `ASSUMED:` inline. Register: a systems researcher who has not
...
**custody root** (the directory that retains the night's records), and the chain's SHA-256. **Arming** is the act of
publishing that plan where the supervisor will find it and installing two launchd jobs from it: `com.joulewise.night`
(fires at t0, runs `scripts/run_night.py`) and a **dead-man** job, `com.joulewise.night.deadman` (fires after the
planned completion and refuses or stands down anything still alive). "Fully automatic" means no human types anything
between the decision to run and the harvested result. The plan is authored, noticed, armed, executed, harvested and
retired by two unattended agent sessions: the **magistrate**, a headless supervising session (`claude -p`, launched by
a user LaunchAgent) that owns planning and adjudication, and the **courier**, the session the night driver spawns
...
1. **No capture with an agent active.** The night gate runs a **process census** at t0, a scan of the process table
   for agent runtimes (`pgrep -lf 'codex|claude|t3'`) plus load, HID-idle, boot-clock and custody guards; any hit
   refuses the night and no chain starts (`docs/phase_2/derivation_night_runbook.md` §0.6). The magistrate must
   therefore be gone before t0. The **plan span** is the agent-free interval from `t0 − PLAN_LEAD_S` through
   completion; inside it the watchdog refuses to launch a magistrate (`docs/process/MAGISTRATE_WATCHDOG.md` §Fence and
   deadlines).
...
Two more project terms recur below. A **lane** is one queued work item in `TASK_QUEUE.md`, named by an id such as
`INSTALL-WINDOWS-MULTI-01`; a **seat** is one delegated model session (implementer, reviewer or consultant) run
against a brief. A **cold gate** is an adjudication by a fresh model session with no loop context, ruling on a
mechanically assembled packet; its rulings are numbered by packet in the trace directories.

## 2. Dated timeline

### 2.1 Charter (2026-08-08)

The autonomous-window decision (`docs/decision_log.md` D-127) chartered the loop (harvest → mint → judge → build next
pack → launch → exit for the capture → relaunch a fresh headless session) with the zero-agent-during-capture fence
unchanged and a relaunch harness shaped as preflight → launch → liveness proof → bounded retries → independent launchd
...
invocation, the custody digest step, the transaction GO and the watchdog install; stand-down margin 5 min before t0
...
300 s, resident supervisor at 10 s resolution) was designed by a three-seat consult, built, refuted twice and
cold-gated (hands-free-week trace records 02–20), landed on PR #284 (`TASK_QUEUE.md` WATCHDOG-BUILD-01 row) and
installed by the interactive magistrate on 2026-09-06, which was reaped by design (`RUN_STATE.md` checkpoint T36). The
first watchdog-owned activation came only at 2026-09-08 00:51:55: the tick had sat in `CLOCK_UNCERTAIN` since 09-06
...
checkout, email with pins before the move, no other agent alive), and the pack-night GO receipt and custody step were
...
- **rehearsal-20260909** (fired 02:56 09-09): verdict `REHEARSAL_ONLY`, chain exit 0, results branch
...
- **n1-20260913**, the first science night (an epoch-equivalence check ordered by issue #316): fired 02:56:02, REFUSED
...
- **n1-20260915**: Ed's directive (issue #336) authorised an evening install; armed 16:45:46 09-14 after Ed closed his
...
the 09-10 arm was blocked by two idle interactive sessions): install spans recur within a day; a pre-authorised retry
class exists for non-physics arm aborts (idle interactive session, stale notice hash, watchdog
`CLOCK_UNCERTAIN`/`NETWORK_UNCERTAIN`, transport failure); idle interactive sessions are not foreign at a stub's
...
02:45–03:30 belt and the calendar-day install span were "a mechanism limit, not a scientific one"; three lanes went to
...
matched Apple's always-present Safari XPC services and could never pass; a cold gate anchored the patterns (cold gate
...
- **Threat-model prune (Ed, 2026-08-27, "yes to all 3. right instincts"; `docs/decision_log.md` D-161).** The
  ...
  states a single trusted operator, so custody mechanisms whose only defended-against actor is that operator touching
  a file were over-engineering: that week they cost three hand edits and blocked the mint twice. Rulings: the reviewed
  pinset gets a REVIEWED REFRESH LANE (`--refresh-row`, re-derived by the verifier's own code path, diff printed, lands
  as a PR) and "no update lane" becomes "no UNREVIEWED update"; a prune lane (THREAT-MODEL-PRUNE-01) enumerates every
  refusal whose only actor is the trusted operator and downgrades it to warn-and-record or retires it; fail-closed
  STAYS where the failure is physics/evidence or pre-registration (missing calibration, unresolved anchor, absent
...
- **Sensible gates (Ed, 2026-09-10 ~04:20).** "make sure there are no silly gates on accepting numbers … recall that
  ...
  measurement". Every tolerance on the claim path is sized to the instrument, attribution limit ≈ 1 J and claim bar
  ≈ 5 J (`docs/decision_log.md` D-078 cl.11), never to decimal places; physics/evidence refusals stay. The gate sweep
  (lane GATE-SENSIBILITY-SWEEP-01) inventoried 299 gates, merged repairs R1/R3/R4 and the G2-a `idle_seconds 75` in
  ...
  per member, not G2-a-blocking (cold gate 47; the directive is recorded in the assistant memory file
...
- **The census fence that survived (2026-09-13).** The question was whether the t0 census should stop refusing on the
  ...
  survived the prune because it protects the measurement (a helper that can start inference or a build inside the
  window), not the operator.
...
prune, a fixture-orphan sentinel (queue rank A208), temp-dir hygiene.
...
will wake me" and lost all three seats within minutes. The watchdog logged both as `clean activation exit` (exit 0,
...
~20 GPT-6 (Astra) seats in one activation and the Claude 5-hour limit fired at 09:46 under Opus fan-out; Ed restored
...
preconditions time-varying (install only inside a listed span and before `install_close_epoch`) and every mutation is
machine state a fence reads: `installed_agent_fence` decides from the two plist files whether a plan is armed
...
`set -e`-skipped EXIT trap → post-gate `rm -rf` → `--uninstall`. A cold gate measured the shell fact under it: zsh 5.9
...
executed the round-3 head: a failed `rm -rf <backup>` after the commit gate tore down a verified arm (rc 1, both labels
...
"no fix seat, no round 4" (delta-3 stop record). At 11:48 a blind three-seat consult (GPT-6 Astra at xhigh effort,
Claude Opus, Claude Fable) was asked one question: keep zsh and fix the predicate at three sites, or a Python
transaction module. All three chose the module (consult records `33a/33b/33c`). Its four devices, from the Opus seat:
...
frame, the mask was left blocked, teardown was skipped. The round-6 cold gate licensed round 6 as the LAST same-shape
round and wrote the STOP CONDITION: any further signal-class defect in the delta re-audit means no round 7 but a blind
consult on one question, replace raise-from-handler with record-and-poll
...
interpreter, because a signal queued after its original disposition was re-installed was delivered at the final
`SIG_SETMASK` with that disposition and killed the process AFTER the result was complete (trace record `08f`). The
signal-redesign consult convened 22:58; by 23:10 two of three seats had converged with executed probes on the same
...
`SIG_BLOCK` in the handler was inherited by every launchctl child spawned afterwards, making the subprocess unkillable
(`13-consult-signal-redesign/03-magistrate-adoption.md`). Round 8b fixed the one self-contradiction (`_commit` polled
twice) into: predicate on the clock read → the last poll (latch) → direct assignment of COMMITTED; a signal during the
...
→ SUCCESS) → `bootout` rc 0 ×2 → `print` rc 113 ×2 → uninstall rc 0 → `SMOKE VERDICT exit=0: PASS`, pinning the
launchctl "service absent" wire signature (rc 113 plus its stderr line) on macOS 25G83 twice (trace record `08k`, log `08m-…-PASS.log`). PR #341 merged as `2944a45d`
...
Four lanes, in the order the any-time-windows ruling set, after the installer:

- **Arm-retry class** (lane ARM-RETRY-CLASS-01, rank A172; rulings 20:55): the enumeration lives in one executable
...
- **Idle-interactive census exemption** (lane ARM-CENSUS-IDLE-INTERACTIVE-01, rank A173; rulings 21:05): an
  enumerated-inert-set design was rejected because Claude Code spawns a `zsh -c` child per tool call, so no real idle
  session would ever pass; adopted the workload-positive classifier: a session is BUSY iff any descendant matches the
  workload table (`unittest`/`pytest`/`shard_tests.py`, `powermetrics`, `run_night.py`, `run_campaign.py`,
  `chain.zsh`, `vllm`/`mlx` serve, `codex … exec`, `claude -p`…), else idle; UNKNOWN reads as IDLE (a wrong idle costs
  one stub, no physics); the exemption applies to `REHEARSAL_STUB` only, and the t0 plan-span refusal is untouched
  (rulings record `04b` blockers 1–3, R1; `joulewise/arm_census.py:230-235`). Discovery is PID-only so multi-line seat
  argv cannot masquerade as hits (`arm_census.py:23-27`). PR #343, `b8b4406d`.
- **Lead margin** (lane LEAD-MARGIN-01, rank A210): `INSTALL_CLOSE_MARGIN_S` 3600 → 120 s (`scripts/run_night.py:75`;
  round-1 RED was `AssertionError: 3600 != 120`, trace record `18`), and the resident ladder PLAN/REQUEST/TERM/KILL
  ...
  before d01: quiet-to-first-capture ≥ 300 + 10 + 600 s ≈ 15.2 min (runbook §1.3). Arm-to-t0 floor:
  `t0 − 8 min − 2 min = t0 − 10 min`. PR #344, `e79eac14`.
- **Refusal fast retry** (lane REFUSAL-FAST-RETRY-01, rank A212): Ed at ~00:05, "how have we not made this a lower cost
  to pay". Mechanism: a refusal at t0 in the first second still held the magistrate away for the full span
  `t0 + 9000 + 300` s as if the night had run, then harvest → §2.5 → fresh plan → arm lead: ≈ 3 h per refusal. Cure:
  (a) the span ends at the refusal record when no capture started (`refusal.json` present, no receipt, no
  `chain.started`); (b) a zero-capture machine-state refusal is a pre-authorised retry in the arm-retry shape; (c) a
...
Integration found two cross-lane defects single-lane gates cannot: a retry-class test pinned `install_close_epoch = t0
− 85 min` by construction and broke under the lead-margin lane's 10-min floor (fixed to derive the lead from
`run_night.install_close_epoch`), and a fixture-sentinel checklist sentence tripped the docs-freshness fence's literal
...
`ci-postmerge-ruling.md`). Test-speed levers that did not land: the 3.8× fitter (bit-identical but it changes source
bytes pinned by the issued calibration acceptance; deferred to the next acceptance re-issue, trace record `12a`); the
calibration-exits pool (17 cases must stay serial, `12c`); receipt-history semantics lever H1 (`git cat-file --batch`, CLI
73.5 → 59.7 s) landed as PR #347, H2 dropped (`12e`).
...
integration tree's version) → `quick_suite.py --tier quick` on the lane head → `gh pr merge`: retry class #342
`cb70e97f`, idle-interactive census #343 `b8b4406d`, lead margin #344 `e79eac14`, CI trim + quick tier #340
`bf870fed`, fixture sentinel #345 `e41cb4c2`, render-only fix #346 `a77067cc`, receipt-history semantics lever #347 `68e5dce6`
...
The interactive session handed the installer lane to a headless magistrate at ~01:55 with step 0 "wait for a green CI
...
(`26-merge-wave-record.md` Addendum 02:12). The headless magistrate re-cut `rehearsal-20260916` at handback commit
`cf249594` (the **handback** is the record the interactive session leaves for its successor): clone
...
KILL 02:55, window end 03:15, dead-man 04:20; `--render-only` rc 0; arm census rc 3 on the peer's six detached
...
channel are unchanged; email stays primary; lane NOTICE-TRANSPORT-FALLBACK-01 (memory file
...
- **Headless connector re-auth.** The claude.ai Gmail MCP is an OAuth session a running `claude -p` cannot re-acquire;
  ...
  costs the window. The issue-transport fallback of directive #349 is ruled but its lane (NOTICE-TRANSPORT-FALLBACK-01)
  is not landed; the relaunch prompt still says email-then-arm only.
...
- **Refusal cost cure not landed.** The fast-retry lane (A212) is ruled and queued; until it lands a t0 refusal still
  holds the full span.
...
- **Physics gates unchanged by design:** load > 2.0 refuses (that is what refused 09-15); XProtect/mds/backupd
  > 5 % CPU refuses; the 25F84 → 25G83 macOS acceptance-epoch mismatch still returns `check` rc 3 and gates real
  numbers on the derivation nights of the acceptance re-issue lane (ACCEPTANCE-EPOCH-25G83-01).
- **Stale prose on main:** the first-use block rendered by the retry-class lane in
...
Memory files below live under `~/.claude/projects/-Users-edr-code-JouleWise/memory/`.

- A headless turn end is a process exit; hold with bounded waits and launch seats detached (memory file `headless-turn-end-kills-seats.md`).
...
- Two consecutive fix rounds on one signature → consult, not round three (orchestration rule 11 in `/Users/edr/code/JouleWise/CLAUDE.local.md`; the round-6 cold gate ruling Q3 and the delta-3 stop record cited in §2.6).
...
- Fail-closed only for physics, evidence and pre-registration; operator-only-adversary refusals retire; tolerances are sized to the instrument (≈ 1 J / ≈ 5 J) (threat-model prune `docs/decision_log.md` D-161; memory file `sensible-gates-directive.md`; the `codex|claude|t3` census kept in PR #334).
- Unknown liveness means loaded; a fake that cannot express unknown tests the bug, not the contract (Opus design seat record Q3 in §2.6).
- Seats run in linked worktrees, never main; briefs under enforced `WRITE_SCOPE` (memory file `codex-seat-launch-rules.md`).
...
- Decided ≠ done: a ruling names its implementation lane and the prior mechanism's limit stands until that lane lands (preambles of `docs/decision_log.md` D-180 and D-181).
