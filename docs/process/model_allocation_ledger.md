...
|---|---|---|---|---|---|---|---|---|---|---|
| L-001 | 2026-07-07 | C-006 | Refuter/verifier tier across the six-stream parallel batch | Opus subagents | `unknown` | blocker refutation | tier DROPPED from the default roster | **0** unique catches, and none since C-001 | over-powered/redundant *as a redundant lens*; later re-scoped, not reversed, by C-045 | `docs/council_log.md` §C-006 "Layer yield + spend" (l.562-563) and "Meta-review C-006 verdicts adopted" (l.590-593) |
...
| L-026 | 2026-07-22 | C-043 (D-078 close-out) | 3 fresh read-only lenses over a shared packet → 8 refuter verdicts (blockers get 2 distinct lenses) | Sol | xhigh | independent audit + refutation | round-8b delta re-audit caught what two audited rounds missed | lenses: **A1** (v3 claim-eligibility contract divergence), **B1** (ClockStamp physical-sanity gap → understated `B_fiducial` ~3 µs), C1/C2, C3/C4. Refuters: killed A2 and B2 outright (both plausible, both wrong), narrowed C1 to a nit, split A1 | right instrument; **3 of 8 refuters were killed mid-run by an upstream content filter** on adversarial phrasing — recovered by data-quality rephrasing | `docs/council_log.md` §C-043 (2026-07-22) "Layer catches (unique)" and "Failure modes recorded" (l.1676-1699) |
| L-027 | 2026-07-22 | C-043 (D-078 close-out) | Triage of the 8b audit's two "blockers"; L1 adjudication | lead-bench | `unknown` | severity adjudication | both "blockers" were the lead's own authorized bench edits | **2 false-attribution triages + the L1 registered-limitation adjudication** | fixed constant; a lens structurally cannot see edit provenance | `docs/council_log.md` §C-043 "Lead unique" (l.1688-1689) |
| L-028 | 2026-07-24 | C-044 (NEG-8 estimand debate) | One peer round on the clause-10 ruling, under an invited-disagreement debate brief (steelman each option, demand failure modes + examiner view) | Sol | xhigh | design consult / adjudication debate | peer's position adjudicated CORRECT; Ed ratified the amended design | **1 structural correction adopted** (the anomaly screen must not erase drift from the claim budget) — **second recorded case of invited peer design judgment beating a lead ruling** | strong. Calibration note in the entry: invited-disagreement briefs produced markedly higher design yield than review-shaped prompts | `docs/council_log.md` §C-044 (2026-07-24), l.1707-1723 |
...
| L-043 | 2026-09-24 | A291 cold gate A291-CONTRACT-01 (same packet as L-044) | Cold judge on 20 contract questions for the re-cut packer | Fable 5.1 (`claude -p --model fable`) | `high` | cold-gate design ruling | 20 questions ruled with exact texts, none refused | **2** MATERIAL the Opus/Sol charge missed (refusing minima at requeue exits; insertion-ordered model list breaks replay) | strong for design rulings; its cure texts then needed L-044's repair | `docs/process_traces/2026-09-24-activation-a65fb4fa/15-coldgate-packet-a291-contract/10-coldgate-fable-ruling.md` |
| L-044 | 2026-09-24 | A291-CONTRACT-01 (same packet as L-043) | Paired contract refuter on the judge's cure texts | Opus 5.5 (`claude -p --model opus`) | `high` | contract refutation | 12 findings; the Fable addendum accepted all, reversed none | **1 BLOCKER** (culprit scope across stages; executed counter-example) + **8 MATERIAL** (incl. a false numeric bound in standing text; replay must re-derive decisions) | strong; carries the implementability catches on Fable's texts, as at gate 45 | `docs/process_traces/2026-09-24-activation-a65fb4fa/15-coldgate-packet-a291-contract/11-opus-contract-refuter.md`; addendum `20-addendum/21-coldgate-fable-addendum-ruling.md` |
| L-045 | 2026-09-24 | A282 cold gate A282-AP5M-01 (same packet as L-046) | Cold judge on 18 analysis-plan questions | Fable 5.1 | `high` | cold-gate policy-text ruling | 18 questions ruled, 23 final texts; 3 BLOCKERs cured by text | 3 BLOCKER resolutions (denominator guard, smoke predicates, floor/anchor split), from lens findings, not new catches | strong for rulings | `docs/process_traces/2026-09-24-activation-a65fb4fa/30-coldgate-packet-a282-ap5m/10-coldgate-fable-ruling.md` |
| L-046 | 2026-09-24 | A282-AP5M-01 (same packet as L-045) | Paired contract refuter on the 23 final texts | Opus 5.5 | `high` | contract refutation | 17 findings; the Fable addendum accepted all, reversed no verdict | **1 BLOCKER** (the ruled smoke gate compared a block with one problem's worst case: unpassable) + **12 MATERIAL** | strong; same pattern as L-044 | `docs/process_traces/2026-09-24-activation-a65fb4fa/30-coldgate-packet-a282-ap5m/11-opus-contract-refuter.md`; addendum `21-coldgate-fable-addendum-ruling.md` |
| L-047 | 2026-09-24 | A291 independent checker | Oracle review of the independent checker before it gated the implementation | Opus 5.5 subagent | `high` (inherited) | oracle/contract lens | checker fixed in one round; delta clean (0/1,500 mismatches) | **2 BLOCKER** + 1 MATERIAL + 8 surviving mutations; B2 also found independently by the Sol implementer (report 36), B1 only here, and B1 later caught the implementer's identical misreading | strong; an unreviewed oracle would have certified a wrong packer | `docs/process_traces/2026-09-24-activation-a65fb4fa/37-a291-checker-opus-lens.md`, `44-a291-checker-delta.md`, `36-a291-implementer-i1-report.md` |
| L-048 | 2026-09-24 | A294+A295 PR #403 | Cold final pass on the merge candidate | Fable 5.1 | `high` | final-pass code gate | MERGE; six real-git scenarios executed at base and head | **0** new defects (lenses 20/21 and delta 34 had them); value was executed verification and the bench-fix ruling | adequate; confirms, does not discover, on a well-lensed small diff | `docs/process_traces/2026-09-24-activation-a65fb4fa/41-a294-a295-fable-final-pass.md` |
| L-049 | 2026-09-24 | A291 contract v3 | Installation-fidelity lens on the ruled contract and the magistrate's representation rulings | Sol 6.0 (`gpt-6-sol`) | `high` | contract lens | 2 BLOCKER + 5 MATERIAL; all accepted | **2 magistrate over-reaches** caught (a mandatory extra gate column, a model-id domain restriction presented as representation) + a dropped ruled argument | strong; the cross-family check on the magistrate's own rulings | `docs/process_traces/2026-09-24-activation-a65fb4fa/27-a291-v3-installation-lens.md`, `29-a291-v3-installation-rulings.md` |
| L-050 | 2026-09-24 | A291 escalation consult (same brief as L-051) | Blind design consult on the structural cure after the same-signature trigger | Sol 6.0 (`gpt-6-sol`) | `high` | design consult | C3 ADOPT, C1/C2/C4 ADOPT WITH CHANGES; measured replay cost | correct C2 correction (a single population re-creates lens-37 B1); missed that the checker's derived code is a transcription | adequate; strong on measured cost, weaker on the oracle | `docs/process_traces/2026-09-24-activation-7370d0fb/02-a291-consult-sol.md` |
| L-051 | 2026-09-24 | A291 escalation consult (same brief as L-050) | Blind design consult on the structural cure | Opus 5.5 (`claude -p --model opus`) | `high` | design consult | same C1–C4 verdicts plus seal reorder, backstop, fuzz differential | **unique catch:** the checker's `_derived` is the packer's with renamed variables (bench-verified; judge E3 0.88), so the oracle was not independent at the derived layer | strong; the load-bearing catch of the consult | `docs/process_traces/2026-09-24-activation-7370d0fb/03-a291-consult-opus.md` |
| L-052 | 2026-09-24 | A291 cold gate A291-FIX2-01 (same packet as L-053) | Cold judge on the fix-round-2 plan (B1–B10) | Fable 5.1 (`claude -p --model fable`) | `high` | cold-gate design ruling | 10 questions ruled with executed probes in ~8 min | **1 BLOCKER** (P/K scope collision on the stress module) + 6 MATERIAL the plan missed (`_derived` exit sites outside the boundary; `AttributeError` from `_digest`; a third derived copy in a test helper; executed-spread clause; R4a not on a production entry; no checker-accepts/packer-refuses property) | strong for design rulings; its cure texts then needed L-053's repair | `docs/process_traces/2026-09-24-activation-7370d0fb/07-coldgate-packet-a291-fix2/10-coldgate-fable-ruling.md` |
| L-053 | 2026-09-24 | A291-FIX2-01 (same packet as L-052) | Paired contract refuter on the judge's Final texts | Opus 5.5 (`claude -p --model opus`) | `high` | contract refutation | 14 findings; addendum pending | **1 BLOCKER** (ruled regression R2b can never raise `inv_12`; executed) + 8 MATERIAL (two INV-11 readings, handler-list mismatch, executed arithmetic outside the boundary, a vacuous AST guard, vacuous fuzz properties, an unstable similarity ratio) | strong; third gate in a row where the paired Opus refuter finds a BLOCKER in Fable's cure texts | `docs/process_traces/2026-09-24-activation-7370d0fb/07-coldgate-packet-a291-fix2/11-opus-contract-refuter.md` |
| L-054 | 2026-09-24 | A280 PR B | Scout of remaining idle-only call sites, then a decomposition consult | Sol 6.0 (`gpt-6-sol`) | `high` | scout + design consult | inventory with file:line; B0–B4 split adopted | **NEW** idle-only sites beyond lens-29 S3 (preparation, generator, gate registration, installer, driver, courier, zero-capture) | strong; fast (7 + 3 min) | `docs/process_traces/2026-09-24-activation-7370d0fb/05-a280-prb-scout-report.md`, `docs/process_traces/2026-09-24-activation-7370d0fb/09-a280-prb-decomposition-consult.md` |
| L-055 | 2026-09-24 | A291-FIX2-01 addendum (same packet family as L-052/L-053) | Cold judge on the refuter's 14 findings | Fable 5.1 (`claude -p --model fable`) | `high` | cold-gate addendum ruling | all findings ruled, Final texts v2 issued, ~9 min | confirmed the BLOCKER by its own probe; 4 texts rewritten better than the refuter's (F3, F6, F7(e), F9 with a validated similarity control); **2 new MATERIAL** (reseal must not refresh derived fields; relation check independent of lever nullness) | strong; the Fable-rules / Opus-refutes / Fable-addendum loop converged in one round | `docs/process_traces/2026-09-24-activation-7370d0fb/07-coldgate-packet-a291-fix2/20-addendum/21-coldgate-fable-addendum-ruling.md` |

**Row count: 49.** Fully evidenced (no `unknown` field): **31**. Rows
...
| Independent adversarial audit | **Sol**, fresh read-only session per round | L-018, L-026, L-030, L-037; C-045 records real mechanisms in all four rounds, and C-038's single audit produced a first-ever executable demonstration of registered limitation L1 | **strong on yield, weak on severity** — auditor severity inflation is recorded as systematic, not incidental (`~/.claude/skills/adversarial-review/SKILL.md` §C-045). L-037 refines this: the *finding* was correct-as-found and the *pricing* was still the lead's, against the primary source |
...
2× Sol at equal cost?** C-045 was a **single-arm** result: the pairing ran,
...
**Q2 — Is `xhigh` justified over `high` for audits?** C-045 ran rounds
...
**Q5 — Which layer should own severity pricing?** C-045 shows the auditor
...
against C-045's 3–4 of 7 baseline.
...
RESOLVED 2026-08-03 (sweep finding PD-1, commit c908b3e): the later
holders were renumbered — D-078 close-out → C-043, NEG-8 estimand debate
→ C-044, screen+budget gauntlet → C-045 — and the earlier holders keep
their original IDs; this ledger's session-column values above reflect
the renumbering.
...
**A-5 — The C-045 Opus refuters' effort tier is unrecorded.** The entry
...
`~/.claude/skills/adversarial-review/SKILL.md` §C-045 rules that `high` is
...
before roster change"). C-045 changed the default refuter roster on one
...
notes; `~/.claude/skills/codex-delegation/SKILL.md` C-045 field notes).
