...
| `desk.current_pack` | The generator check, validators, sidecars, extraction specification, attempt policy, and committed pack digest all describe the same completed GAMMA pack. |
...
| `desk.identity_pin_projection` | Freeze and arm verification preserve the four ordered model, runtime, and configuration identity units listed above. |
...
| `desk.terminal_review` | The context-holding lead's final review binds this exact commit tree and GAMMA pack digest; any later change invalidates it. |
...
| `t0.offline_inputs` | The local model, tokenizer, configurations, scripts, runtime environment, and all four live identity derivations byte-match the GAMMA pack's frozen inventory, and no network fetch occurs. |
| `t0.passwordless_powermetrics` | The exact passwordless measurement probe frozen in the GAMMA pack exits zero before the final settle. |
...
| `t0.storage_backup_capacity` | The machine meets the GAMMA pack's frozen free-space minimum, and both frozen backup destinations exist, are distinct and writable, and meet their frozen capacity minimum. |

No row in this page currently authorizes GAMMA. Authenticated receipts must
