...
| `campaign_occurrence_supersession_already_recorded` | The target campaign log already contains a recognizable supersession row for this bundle; or contains a recognizable supersession row whose bundle cannot be identified; or cannot be read at all by the supersession consumer (for example a torn final line that is itself a partial supersession row, which the provenance loader tolerates and the consumer reader does not). | Preserve the log and stop; no row was appended. Do not retry against this log. If the selected occurrence later failed, use a fresh runs root or keep the member non-claim-bearing; same-root chained recovery is not supported. |
...
consumer can use, so the recorder refuses under the same reason code. Preserve
the log and repair custody; do not retry the recorder against it.
