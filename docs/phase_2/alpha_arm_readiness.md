...
For the current 2026-08-13 frozen pack, the D-134 freeze receipt and
plan-tree pin are authoritative over the byte-frozen legacy
...
| Common-mode contrast estimator identity | **RESOLVED — TIGHTER FLOOR LIVE (2026-08-12, PR #144 `dc162bc`).** D-133 clause 4's conditional fired on its own ruled terms at #140's merge and Ed ratified EXECUTE (decision log, T6 entry): all six shared-edge comparative cells plus the contrast pack's consuming cells select `d124_two_shared_edge_common_mode.v1`; the A5 replay derives **1.8695016260131627 J** (vs the 8.611855 J default). [Superseded record: D-133 (2026-08-11) decoupled the estimator from the freeze via the #132 fallback (`2b43de8`, worst-case default, Q7 resolved-by-reversal), with re-spec-back conditional on ALT-D120 + the terminal delta + WO-MINT-ESTIMATOR-VOCAB landing pre-freeze-wave — all three landed 2026-08-12 and the conditional fired.] | Nothing — preserve the re-specced (tighter-floor) pack state through the freeze. | Lead | `docs/decision_log.md`, D-133 (`f0e7cf6`); PR #132 (`2b43de8`); freeze-plan Q7. |
...
- **Current relationship — estimator:** AMENDED 2026-08-12 (#144): the
  shared-edge comparative cells select the registered common-mode estimator
  (D-133 cl.4 EXECUTE, Ed-ratified); packs freeze at the tighter 1.869502 J
  floor. [was: D-133 returned this pack cycle to the worst-case default.]
...
| `desk.estimator_identity` | AMENDED 2026-08-12 (#144): the shared-edge comparative cells select the registered common-mode estimator (D-133 cl.4 EXECUTE, Ed-ratified); the exact estimator identity is derived from the frozen plan and admitted mint registry. [was: D-133 selected the worst-case default for this cycle.] |
...
| `t0.display_thermal_idle` | Not established by committed evidence. The reviewed preparation and completed wait must prove display, screensaver, thermal pressure, and idle state. JW-MET-2 also requires the System Settings keyboard-backlight census: level zero, automatic adjustment false, inactivity never, and `verification=operator_visual`; `ioreg` capability inventory cannot prove the level. |
| `t0.fresh_roots_waivers` | Not established by committed evidence. Absolute claim, bound, arm-readiness custody, fresh window-context custody, and quarantine roots must be distinct and fresh; quarantine must be outside the context custody root inspected for emptiness, and the actual attempt waiver bytes must decode exactly to `[]`. |
| `t0.ledger_reservation` | Not established by committed evidence. The live diagnostic and real reservation command must bind the plan SHA and use the `--ledger` / `--head-pin` / `--plan` argv superset, recording the pre-reserve authorization event plus `status: reserved`. |
| `t0.machine_readiness` | Not established by committed evidence. The frozen current-alpha command is `scripts/prewindow_check.sh --wait --timeout-min 45 --window alpha`; it must inspect the alpha claim/bound roots under the measurement checkout and return `READY`. It is **not** the arming gate and cannot substitute for one; the calibration-ledger diagnostic and real reservation command with `--execute` are separate and mandatory. No automated word — `READY`, `ready`, or `clean` — licenses arming. Even a complete `GO` arm receipt is necessary but not sufficient: lead verification and the operator's separate physical launch action are still required. |
