...
| Mint trust bar | **CLOSED.** PR #122 landed the trust behavior; #129 split the decisive proof; hosted runs `31541829071` and `31518739878` succeeded. D-130's second-independent-execution condition is discharged and its temporary citation discipline has expired. | Preserve the trust behavior and run its proof automatically on push to `main` and pull requests. | Discharged | PR #122 (`ae6af48`); PR #129 (`7a76a29`); D-130 closure note. |
...
| Receipt-oracle freshness | **PASS in the 2026-08-13 freeze receipts; successor derivation pending.** The replay-derived oracle remained byte-identical after PR #122. | Re-derive if the final ledger read/write surface changes and bind the result at successor freeze. | Lead | PR #125; row `desk.receipt_oracle` in the three freeze receipts. |
| Three-window regression | **PASS in the 2026-08-13 freeze receipts; successor replay pending.** The current committed ALPHA receipt records `freeze-three-window-regression-v1` as passing evidence. | Re-run and bind the regression against the final successor family and reviewed head. | Lead | Row `desk.three_window_regression` in the three `freeze-0001.json` receipts. |
...
| Reason-code plumbing | **IMPLEMENTED; final successor rehearsal pending.** The prior freeze receipt passed the row, and #152 added the T-0 acquisition refusal vocabulary. | Verify registry coverage and emitted refusal fields at the final successor rehearsal. | Lead | PR #116; PR #152; row `desk.reason_code_plumbing`. |
