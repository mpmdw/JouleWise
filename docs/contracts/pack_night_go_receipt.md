...
| §10 B1: plan root of trust and byte digest | G §§1–3, 10 B1 | `scripts/run_night.py:1105` (`_prepare_pack_night`); `joulewise/night_gate.py:698` (`_authenticate_pack_records`) | `tests/test_run_night.py:2052` (`test_each_plan_record_digest_and_pinned_plan_swap_refuse`); `tests/test_run_night.py:1987` (`test_gate_checks_authorization_fields_and_confirmation_bytes`) | Changed pinned plan bytes, changed authorization bytes, or a different persisted authorization attempt passes producer authentication |
...
| §10 S6: exact author evidence set and digest | G §2, 10 S6 | `joulewise/night_gate.py:787` (`_pack_evidence`); `scripts/run_night.py:1173` (`_produce_pack_go`) | `tests/test_run_night.py:2119` (`test_t0_inventory_cannot_omit_add_or_substitute_author_or_capture_bytes`); `tests/test_run_night.py:1864` (`test_driver_self_authors_arm_before_go_and_pins_all_eight_flags`) | Missing/extra/substituted author paths or changed receipt/capture bytes enter GO, or its set digest differs from canonical sorted evidence bytes |
| §10 N1: numeric epochs, vocabulary/order/uniqueness | G §§2, 5–6, 8.3, 10 N1 | `joulewise/night_gate.py:698` (`_authenticate_pack_records`); `scripts/run_night.py:1173` (`_produce_pack_go`) | `tests/test_run_night.py:1864` (`test_driver_self_authors_arm_before_go_and_pins_all_eight_flags`); `tests/test_run_night.py:1987` (`test_gate_checks_authorization_fields_and_confirmation_bytes`) | Producer emits non-float issuance epoch, non-integer monotonic times, reordered/duplicate conditions, non-PASS status/non-null basis, or accepts a string confirmation epoch |
...
Seat-2 §10.3 implementation pins (fixture evidence only; consumer/live gates remain lead-owned):

| Synthesis item | Contract installation | Production site | Biting assertion | Counterfactual |
|---|---|---|---|---|
| §10.3 1 | G §§2–6 and §10.3 | `joulewise/arm_readiness.py:248` (`production_custody_roots`) | `tests/test_arm_readiness_schemas.py:1708` (`test_frozen_derivations_resolve_every_role_without_existence_filter`) | Literal or incomplete census replaces frozen derivations |
| §10.3 2 | G §§2–6 and §10.3 | `joulewise/arm_readiness.py:248` (`production_custody_roots`) | `tests/test_arm_readiness_schemas.py:1724` (`test_backup_override_cannot_shrink_census_and_three_script_literals_stay_pinned`); `tests/test_arm_readiness_schemas.py:1733` (`test_inventory_each_entry_counts_and_invalid_or_empty_census_refuses`); `tests/test_arm_readiness_schemas.py:1694` (`test_shipped_inventory_pins_all_four_retained_deployments`) | Environment override shrinks census or a retained deployment disappears |
| §10.3 3 producer/G6 | G §§2–6 and §10.3 | `joulewise/t0_rehearsal.py:749` (`evaluate_g6`); `joulewise/night_gate.py:830` (`_pack_rehearsal_roots`) | `tests/test_run_night.py:2135` (`test_rehearsal_plan_and_arm_context_roots_follow_sibling_child_rule`); `tests/test_rehearse_t0_unattended.py:51` (`test_sibling_child_rejects_parent_nested_and_wrong_basename`) | Nested/equal-to-parent/wrong-name/symlinked rehearsal root passes |
| §10.3 4 | G §§2–6 and §10.3 | `scripts/rehearse_t0_unattended.py:118` (`load_evidence_bundle`) | `tests/test_rehearse_t0_unattended.py:19` (`test_stale_census_cannot_false_pass_g6`); `tests/test_rehearse_t0_unattended.py:34` (`test_inventory_must_equal_head_bytes`) | Stale or omitted census evidence produces false PASS |
| §10.3 5 | G §§2–6 and §10.3 | `joulewise/night_gate.py:675` (`_pack_digest`); `scripts/run_night.py:1105` (`_prepare_pack_night`); `scripts/run_night.py:1173` (`_produce_pack_go`) | `tests/test_run_night.py:2035` (`test_pack_digest_mismatch_at_preparation_and_go_refuses_without_go`); `tests/test_run_night.py:2111` (`test_pack_root_must_match_the_written_arm_root_and_digest`); `tests/test_night_plan_writer.py:101` (`test_pack_root_absolute_basename_and_symlink_refusals`) | Substituted pack root/digest passes preparation or GO; consumption point remains seat 3 integration |
| §10.3 6 | G §§2–6 and §10.3 | `scripts/run_night.py:1111` (`_author_pack_arm`); `joulewise/night_gate.py:765` (`_pack_no_retry`) | `tests/test_run_night.py:2064` (`test_selected_old_arm_and_higher_receipt_and_consumption_refuse`); `tests/test_run_night.py:2017` (`test_no_go_on_arm_refusal`) | Select existing ARM, ignore supersession/consumption, or GO on NO_GO |
| §10.3 7 | G §§2–6 and §10.3 | `scripts/run_night.py:1149` (`_pack_launch_references`) | `tests/test_run_night.py:2077` (`test_second_manifest_missing_symlink_and_attested_digest_refuse`) | Second manifest wins a fallback or changed digest passes |
| §10.3 8 | G §§2–6 and §10.3 | `scripts/run_night.py:1269` (`run_night`); `scripts/run_night.py:1173` (`_produce_pack_go`); `scripts/run_night.py:1252` (`_pack_launcher_argv`) | `tests/test_run_night.py:1864` (`test_driver_self_authors_arm_before_go_and_pins_all_eight_flags`); `tests/test_run_night.py:2100` (`test_machine_refusal_and_refused_receipt_never_publish_go`) | GO before ARM, GO on refusal, mutable census citation, interactive stdin, or any of eight flags omitted |
| §10.3 8 evidence | G §§2–6 and §10.3 | `joulewise/night_gate.py:787` (`_pack_evidence`) | `tests/test_run_night.py:2119` (`test_t0_inventory_cannot_omit_add_or_substitute_author_or_capture_bytes`) | Subset/extra author files or changed capture bytes enter GO |
| §10.3 9 | G §§2–6 and §10.3 | `joulewise/night_gate.py:648` (`_pack_bytes`); `scripts/run_night.py:1105` (`_prepare_pack_night`); `scripts/run_night.py:1149` (`_pack_launch_references`) | `tests/test_run_night.py:2052` (`test_each_plan_record_digest_and_pinned_plan_swap_refuse`); `tests/test_run_night.py:2077` (`test_second_manifest_missing_symlink_and_attested_digest_refuse`) | Missing locator or disagreeing digest silently falls back |
