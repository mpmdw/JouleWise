...
with `launch_go_receipt_invalid`, detail `receipt_class`. The rehearsal's own
...
(2026-09-08). The bundle's closed record set gains `g7_control: {path, sha256}`;
the loader authenticates its external bytes and G7 re-reads and validates them.
...
| §10 B4: consumer order and G7 artifact | G §§3, 6, 10.5 | `joulewise/arm_readiness.py::_admit_pack_launch_go`; `scripts/run_night.py::produce_g7_control`; `joulewise/t0_rehearsal.py::evaluate_g7` | `tests/test_launch_window.py::PackNightLaunchBoundaryTests.test_g7_control_refuses_both_presentations_before_missing_arm` exercises the producer and real launcher, emits exact artifact JSON, asserts 0600/create-once and preserved source bytes; `test_valid_rehearsal_class_refused_by_production_entry_and_consumer` and `tests/test_arm_readiness.py::PackNightConsumerTests.test_pre_arm_admission_rejects_rehearsal_go_and_authenticates_digest_first` pin digest/class/ARM order; `tests/test_rehearse_t0_unattended.py::ProductionCensusLoaderTests.test_g7_locator_authenticates_external_bytes_and_acceptance_rechecks` pins locator and PASS recomputation | Missing ARM masks class refusal; forged PASS, wrong digest or completed-night consumption substitutes for the control |
...
| §10 S1: authenticated ARM window id | G §§2, 6, 10 S1 | `joulewise/arm_readiness.py::_authenticate_go_purpose`; `scripts/run_night.py::_produce_pack_go` | `tests/test_arm_readiness.py::PackNightConsumerTests.test_four_case_purpose_root_table_through_consumer` changes the authenticated ARM window id independently of plan_id; `tests/test_run_night.py::PackNightProducerTests.test_go_producer_enforces_two_by_two_purpose_window_table` uses the ARM window id at issuance | Trust an unauthenticated window label |
...
| §10 S4: shared frozen roots and complete purpose table | G §6, 10 S4 | `scripts/run_night.py::_produce_pack_go`; shared gate and consumer purpose/root predicates | `tests/test_run_night.py::PackNightProducerTests.test_go_producer_enforces_two_by_two_purpose_window_table`; `tests/test_arm_readiness.py::PackNightConsumerTests.test_four_case_purpose_root_table_through_consumer` (production id/rehearsal purpose; prefixed id/production roots; prefixed id/disjoint roots; normal production) | Either purpose/id mismatch passes; the named direct child or its own ARM paths refuses; equal/nested/wrong-name custody or DISJOINT equality/containment passes; stale manifest census passes |
...
| §10 N1: numeric epochs, vocabulary/order/uniqueness | G §§2, 5–6, 8.3, 10 N1 | `scripts/run_night.py::_produce_pack_go`; `joulewise/t0_rehearsal.py::evaluate_g5` | `tests/test_run_night.py::PackNightProducerTests.test_go_producer_enforces_two_by_two_purpose_window_table` validates the emitted 26-key GO and non-claim purpose; `tests/test_t0_rehearsal.py::PackGoReplayTests` rebinds hashes while independently breaking C1 authorization, C2 ARM/T0, C3 census, C4 time bounds and C5 consumption ; `tests/test_t0_rehearsal.py::PackGoReplayTests.test_g5_numeric_types_condition_order_and_vocabulary_are_exact` refuses numeric-type, order, duplicate and vocabulary mutations | Producer emits non-float issuance epoch, non-integer monotonic times, reordered/duplicate conditions, non-PASS status/non-null basis, or accepts a string confirmation epoch |
