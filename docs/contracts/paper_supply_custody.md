...
| X16 | CP-X16-challenge | DS-04 deliberate small-difference challenge | No issued challenge result | characterization_projection | EMPIRICAL | UNRESOLVED | UNRESOLVED | UNRESOLVED | DO_NOT_START; explicit inclusion and design gates required | Remain excluded; no silent restoration | PROPOSED_STOP_FILL |
| X16 | CP-X16-between-session | DS-07 retired between-session stability | No authorized successor field | fixed_limitation_text | LIMITATION | NONE | NONE | NONE | DO_NOT_START; retired result remains retired | Retain exclusion; historical Window C is no supplier | PROPOSED_STOP_FILL |
