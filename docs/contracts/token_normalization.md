...
|---|---|---|
| "more efficient per token" | Cross-tokenizer or cross-model-family ranking. | "lower `J/token` under tokenizer [name/revision/class/vocab size], with request energy [direction/value] under [boundary]." |
| "better J/token" | Cross-family efficiency conclusion. | "different tokenizer-scoped `J/token`; request energy is [direction/value] under [boundary]." |
...
| Tokenizer-blind "energy per token" leaderboard language | Ranking stacks without naming tokenizer identity and request energy. | "tokenizer-scoped companion metric for [stack/tokenizer], reported beside request energy." |
...
Every reader-facing figure and table governed by
`docs/contracts/claims_ladder.md` must carry stack identity. The table below
defines the minimum fields and the expected bundle/provenance surface when it
is already known.
...
| Kernel/library where known | Kernel, attention implementation, library backend, graph/capture mode, or equivalent runtime kernel identity when exposed. If unavailable, say unavailable rather than omit. | Runtime adapter metadata; `metadata.adapters.runtime.prepare_metadata` when captured. |
...
| Tokenizer identity | Tokenizer name, revision, class, and vocabulary size where available; token-ID hash domain and hash when the caption cites prompt-token identity. | `metadata.workload_provenance` (D-033); single-prompt domain `joulewise.prompt_token_ids.v1`; suite rollup domain `joulewise.suite_prompt_token_ids.v1`; per-suite item token hashes in `outputs/suite_items.jsonl`. |
| Sampler/output policy | Sampler settings, stop condition, runtime stop reason, output cap/policy label, and batching/concurrency policy when applicable. | `metadata.workload_provenance.output_policy`; `outputs/suite_items.jsonl` item `output_policy` and `stop_reason`; suite `metadata.workload_provenance` sampler provenance per `docs/contracts/run_bundle_layout.md`. |
