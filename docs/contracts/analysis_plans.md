...
| Floor gate | Rule v1, single-block claims: `max(floor_abs_j, floor_cmp_j)` from the P2-015 calibration artifact for the same metric/window class. Rule v2, mean/envelope claims: estimate-scale floor `F_est`, calibrated on same-epoch envelope-mean null contrasts in the estimand's unit (CG-1). The `pending_p2_015` specifications retain v1 until prospectively re-registered. The rule version and floor class are frozen with the claim. Open item (magistrate ruling 97): shared-term aggregation is deferred; cross-envelope sharing or unspecified independence refuses with `not_estimable: envelope_term_scope_unknown` until a registered independence field is ruled. |
