...
| C-057 | 2026-08-11/12 | T5 window session (~20:35 PT 08-11 → ~08:01 PT 08-12 Ed stop order (checkpoint 72b8427 @ 15:01:29Z; the checkpoint header’s “~17:45Z” label is wrong); Ed's 12h autonomous window): the T4-late merge queue cleared **6/6** — PRs #132, #133, #131, #127, #134, #129 — plus three same-session PRs opened, gated and merged (#136 D-135 advisory site budgets, #137 p2038 clock-phase flake, #139 calexits mutation classifier), **ten merges** in all (#138 Q8 p256 floor cells landed after this entry was drafted); **D-136** minted on main (Ed: the site lane is retired from all automatic processes, `f4aa138`); the WO-MINT-ESTIMATOR-VOCAB **COLD GATE** sat and custodied its full record for the first time (`docs/process_traces/2026-08-12-mintvocab-coldgate/`: packet + three rulings) — option A authorized under a self-contained F1–F12; the §5C arm-readiness generator (D-134) ran a three-lens gauntlet at `3a140bb`, a fix round, a FIX-1 delta re-audit, a bench repair and an integration tree; two PRs open at stop (#135 crash-matrix exclusive, #140 mintvocab gate-complete) | Layer catches: the cold gate's **paired** reader is the entry's headline — the fresh Fable adjudicator found the binding-seam fail-open that both prior condition sets would have certified as closed, and its paired Opus contract-lens refuter then proved the adjudicator's own cure **INERT** (v2 forces one `evidence_root_id`, so the absolute and comparative refusal messages are byte-identical and the suffix match closes nothing) and restated C1 against a main that had moved under the packet (`bf628eed…` → `79229aa2…` at `14879e4`); ruling-3 records that *neither* the provisional five *nor* ruling-1's C1–C10 would have caught it; the attestation layer then caught **B1** — `test_common_mode_full_cli_path_writes_bound_exact_artifact` replaces the pinned binder with a stub returning `{}`, so its name claims a binder-verified full mint it never performs — and flagged **G3**, the decision-log's "terminally clean" FCM record standing against an authority that directs D-133 cl.3 be reported open; on §5C, lens A executed a **live** derive-never-enter forgery (operator-attested evidence → `validator_status=PASS` with `operator_source=OPERATOR_ATTESTATION`), lens C found 7 documentation blockers, lens B's mutation census killed 21 of 25 with 4 regression gaps, and the FIX-1 delta then found **2 of 35** predicate transcriptions WEAKER than contract — both on rows that mix live T-0 facts with frozen pack facts; the bench repair of those two exposed **nine** end-to-end/integration fixtures that had been minting reservation receipts bound to nothing, passing only because the code was exactly as weak as the fixture (D-132 guard held the occurrence count at TWO — converging instrument, not doom loop); the **integration tree** caught what no branch could — the doctrine branch minted a second **D-136** from a base predating main's, and its own "exactly one D-136" proof was correct in-branch (`5a80e39`); direction-layer: an unfilled `[LEAD FILLS THIS SLOT BEFORE LAUNCH]` returned `needs_ruling` instead of invented shipped behavior, the docs branch asserted reboot-fence machine behavior ~45 min before the code that enforces it was committed, and requiring exact `file:line` from every lens turned doctrine triage into grep (two FIX-E targets found MISSED); infrastructure: four Sol runs died at 59.8–60.0 min against a subagent-shell kill cap, one of them mid-canonical-suite |
| C-058 | 2026-08-13/15 | T6-late→T7→T8 span: readiness-tooling union **#149 merged** (#146/#147/#148 CLOSED-not-merged by design — the integration-tree merge vehicle), charter v2 rebuilt against its own refusing consult, audit baseline pinned `694442c`, and the **full readiness council**: eleven-seat fleet (11/11, 2.41M tokens, 46.7 min; raw 1 READY / 10 NOT_READY) → nine C-028 refuters + one ordered second-lens (kills: L8-B4 wrong-path artifact, WO-L2-4 phantom, F4 timing premise; **L2's only READY felled** by the falsely-clean attack; new defects found: doubled plan-path, terminal-review-trailer gap) → rule-11 cold pairing (cold Fable + Opus contract refuter) → **VERDICT: NOT-READY 0/11**, L2 dual NOT-READY+UNVERIFIED, four-phase work-order program NOT certified complete, M-2 REMANDED to its own cold gate; custody `bd7f81c`+`15d00d2` | Layer catches: the cold pairing found **four blockers in the sitting itself** (no custody — the whole evidentiary base in ephemeral /tmp; packet omitted six untriaged sweep blockers; M-2 unadjudicable as submitted — reviewed party wrote the charge sheet; coverage undischarged 10/11) — all cured pre-recording; the refuter layer killed two blockers and found two new defects the fleet missed; the sweep scout's top blocker (pack digests) was itself a FALSE POSITIVE (wrong algorithm — both cold seats verified the lead's refutation); the drafting mechanic's ⚑-flags surfaced both stop-signal misses that became the **same-signature structural finding** (rule-11 escalation trigger MET: M-2 + drift-disposition acted on before cold review); infrastructure: 5/9 refuter relay agents wedged ~7h (rc-74 SCOPE_CAPTURE_FAILED class root-caused: subagent-shell launches lose their audit trail; lead-shell relaunches all OK within 13 min). **+ ADDENDUM (post-verdict, recorded 2026-08-16):** Phase 0 COMPLETE (R1-R4 + two further rule-11 cold gates + 3 WO contracts), #150/#151/#152 merged, the recorder-race gate REJECTED the magistrate's own proposed mechanism on executed evidence, wake-source failure class minted |
| C-059 | 2026-08-15/16 | T9 session (T8 successor order executed; ~19h): Phase-1 CODE WAVE merged — 4 PRs after full C-028 gauntlets + a pre-merge integration tree (#153 should-fix batch `8035bf2`; #154 T-0 F4 honest contract `a59c795`; #155 WO-CONSUMPTION-EDGE `d54db78`; #156 WO-LAUNCH-BINDING stages 1+2 `f392ff6`); WO-DETECT-PULSES-BUDGET + calexits flake fix gauntlet-complete MERGE-STAGED Phase 2 (`5449e58`, R-t9-4 D-079 staleness); WO-L2-REAUDIT delivered — Coverage VERIFIED 251/251; **rule-11 COLD GATE on the launch-binding F3 third failure** (design AFFIRMED, round 3 licensed under the refuter-amended bar A0-A11, custody `docs/process_traces/2026-08-16-launch-f3-coldgate/`); 7 magistrate rulings R-t9-1..7 incl. the D-130 trigger deferral on runnability evidence | Layer catches: the paired cold-gate refuter caught a SCHEMA-IMPOSSIBLE anchor in the adjudicator's own ruling (4th consecutive gate the pairing materially amended the adjudicating seat) + restored the dropped path binding + pre-decided the exception-family stall; delta layers killed a forged caller-frame guard, foreign-context consumption, a catch-all misclassification, and unbounded custody reads; THREE review layers caught MAGISTRATE-authored content (refuter on R-t9-3 wording+dates, E's final-head pass on the bench cure, the cold-gate pairing); the local-date convention bit three times (folded); integration tree: all cross-stream seams clean, 2 findings (third crash-matrix timeout case registered; handback prose reconciled pre-merge). Full record: docs/run_reports/2026-08-16-t9-session.md |
| C-060 | 2026-08-23 | T0-UNATTENDED-01 two-seat BLIND co-design + debate (D-144 class; Sol xhigh seat + Opus seat, one critique round each way, magistrate bench-verifying every load-bearing measurement; custody `docs/process_traces/2026-08-23-t22/t0-unattended/`, ruling MAGISTRATE-RULING-T0-UNATTENDED.md the ONE home): seats CONVERGED — machine SNTP attestation (3-server roster, agreement-interval intersection, R0+R1), separate 5 ms CLOCK_MONOTONIC_RAW anchor gate, in-place PROBE branch (no registry change), merged 10-gate rehearsal, 6 h horizon untouched with issuance-time numeric recency; rows UNATTENDED-LAUNCH-01 + T0-CLOCK-ROW-RENAME-01 registered (86→88) | Layer catches: the debate round did what neither blind seat could — Opus's measured kill of Sol's continuity term (Python `monotonic_ns` IS sleep-blind `CLOCK_UPTIME_RAW`, 9.41 d asleep on this machine; the summed 0.5 s bound PASSES the 110 ppm incident a separate 5 ms gate refuses), Sol's self-retraction of that term plus its 1200 s horizon (a ruled-number reversal traveling as an implementation detail — rule-11 catch), Opus reversing its own anchor-publication deferral once the debate located the arm-side consumer, and Sol's honest demotion of "Marzullo" to an agreement-interval gate; magistrate bench killed the r4-6 privileged-scope premise itself (prior_network_time zero readers; prior_systemsetup_state_captured a producer/predicate tautology) — the D-127 amendment is a scope CLOSURE; 4 Ed-hands items flagged incl. the exact-Off string bench check no seat had until debate |
...
## C-058: T6-late into T7 — the charter its own consult refused to ratify, three PRs that held each other's cures, and an eleven-seat fleet whose single READY did not survive the only attack anyone ran (2026-08-13/15)

Session: Fable magistrate with an Opus lieutenant and mechanic corps, covering the record from **T6's coverage cutoff** — declared in the T6 run report's magistrate attestation as "T6 through the overnight arm-author build state" (`docs/run_reports/2026-08-13-t6-session.md`, §Coverage cutoff), which the artifacts place at **2026-08-13 ~20:40 PT**, the moment `armauth-out` started (`started_at: 2026-08-14T03:41:06Z`, `head_start 8915ec1`) — through the **readiness council sitting whose verdict was custodied at `bd7f81c`, 2026-08-15 07:43 PT**. The T6 report names the successor record's opening state itself: "registry-test reconciliation #146, arm-author gauntlet closure #147, chain-fix batch #148, charter v2 ratification, M-1/M-2 rulings under the 2026-08-14 interaction contract, the paper boundary fix, and the readiness-audit program." This entry is that record, plus the three things the T6 report could not see because they landed after it: the **#149 union merge**, the **audit-baseline pin**, and **the audit itself**.

Lanes: the instrument-readiness audit charter, drafted and then rebuilt against a design consult that refused to ratify it (`a13b01c` → `6a7849c`); WO-ARM-EVIDENCE-AUTHOR-01 through a full gauntlet in `wtARMAUTH` (PR #147); the post-freeze registry-test reconciliation in `wtSTALE` (PR #146); the chain-fix batch in `wtCHAIN` (PR #148); the readiness-tooling integration tree in `wtINTEG` (PR #149, the merge vehicle); the keyboard-backlight metrology investigation that minted JW-MET-1/2/3; Ed's interaction contract with the M-1 and M-2 rulings taken under it; the ED-QUALIFICATION session script; the audit-baseline manifest; the eleven-seat fleet's launch, clean stop and fresh relaunch; the nine-run C-028 refutation; and the rule-11 cold pairing that ruled on all of it.

Full records: main commits `503b676`, `91e5da7`, `bccd427`, `8a25b78`, `a13b01c`, `6a7849c`, `91d7e64`, `4f85dba`, `18ef535`, `29a4ad0`, `31ccef5`, `95362e2`, `880271a`, `f99582e`, `ac3fe1d`, `694442c`, `d279a7c`, `8937dec`, `bd7f81c`; the integration chain `7e8c3fd` → `225988a` → `dc2ba8b` → `0e1bbb4` → `ab32589`/`763bb75`/`e4240d2` → `2683959` → `dff318f` → `95c890c` → `f468b98` → `346ff4d`; the custodied consult at `docs/process_traces/2026-08-14-readiness-charter-consult/consult.md` (`6a7849c`); the charter at `docs/process/instrument-readiness-audit-charter.md` and the baseline at `docs/process/audit-baseline-manifest.json`; **the complete sitting record at `docs/process_traces/2026-08-15-readiness-council/`** (`bd7f81c`, 28 files, +4,928 — sealed packet, eleven seat reports, `triage.json`, nine refuter outputs, the full consistency sweep, the pre-sitting dispositions, both cold rulings, the verdict, and the packet index); and the scratchpad run sets under sessions `2cc5ce62-…` (implementation) and `e972460c-…` (refutation).

### What shipped and what did not

**One PR merged: #149** (`ac3fe1d`, merged 2026-08-15T05:20:28Z = 2026-08-14 22:20 PT), the readiness-tooling union: 17 files, +5,173/−157. **#146, #147 and #148 are CLOSED, not merged** — by design. The #149 body states the reason plainly: "The three readiness-tooling PRs … mutually held each other's cures — every individual merge ref was red on tests another branch fixed, so this integration branch is the merge vehicle (the §5C-validated pattern). Each constituent PR retains its full review record and closes on this merge." Any later count of "merges this stretch" that reads the GitHub merge state literally will see **one**, not four; the reviewed work is four PRs' worth.

Also landed on main, none of it through a PR: the **charter v2 ratification** (`6a7849c`, +97 charter lines, +320 consult lines), the **window-gating directive and the interaction contract with M-1/M-2** in the decision log (`8a25b78`, `29a4ad0`), the **paper boundary narrowing at five sites** (`31ccef5`, `docs/paper/draft-v1.md`, +5/−5 — five insertions for "all five sites", the count checks), the **ED-QUALIFICATION session script** (`95362e2`), the **audit-baseline manifest** (`694442c`), the T7 checkpoint pair (`d279a7c`, `8937dec`), and the **council custody + verdict** (`bd7f81c`). Four README activity-blurb refreshes landed in the span (`91d7e64` minting the living blurb, `8a25b78`, `4f85dba`, `880271a`, plus the checkpoint's own README edit) — Ed's standing per-work-block instruction, honoured.

**Not shipped: readiness.** The council returned NOT-READY on ten of ten gating seats. No measurement, arming, calibration or claim publication occurred anywhere in this span, and none may occur until the four-phase work-order program below closes and a fresh sitting is convened.

### The charter its own consult refused to ratify

The magistrate wrote an instrument-readiness audit charter draft — "nine components, council mechanics, verdict form" (`a13b01c`, `…-charter-DRAFT.md`, 67 lines) — and then, before ratifying it, spent a rule-2 pre-decision design consult on it. The consult came back **`DO_NOT_RATIFY_AS_WRITTEN`** with **six blockers and two should-fix findings**:

- **F1** — "The authority and state-kernel control plane is outside the audit."
- **F2** — "Component lenses do not audit producer-consumer seams or obligation completeness."
- **F3** — "READY-WITH-CONDITIONS and arm-time ED-SESSION closure form a fail-open path."
- **F4** — "A magistrate-alone READY verdict is not a trustworthy council decision."
- **F5** — "The post-collection path stops before claim consumption is proven."
- **F6** — "Operator-error and environmental-control completeness lack dedicated discriminating lenses."
- **F7** (should-fix) — lens boundaries overlap and dilute ownership.
- **F8** (should-fix) — "The mechanics verify raised findings but do not discriminate a falsely clean audit."

Its own summary is the sharpest line in the span: the draft "omits the authority plane, producer-consumer seams, operator/environment failure surfaces, and full post-collection claim path, while CONDITIONAL GO and magistrate-only GO adjudication are fail-open."

The magistrate adopted **all thirteen amendments in full** (`6a7849c`'s subject; the charter's own §Ratification: "magistrate adoption of the design consult's thirteen amendments IN FULL"; the consult document carries thirteen numbered amendment headings, count verified). The concrete deltas are visible in the ratified text: **READY-WITH-CONDITIONS is DELETED** (F3), **every council READY verdict requires the rule-11 cold pairing over a sealed mechanically-assembled packet** (F4), the fleet grows from nine components to **eleven seats** including the authority plane (F1), two independent producer-consumer seam readers (F2), a **sacrificial full-lifecycle** seat that drives a synthetic collected window through reduce → verdict → mint → claim consumption "proving the AFTER-window path BEFORE a window is spent" (F5), dedicated operator/environment seats (F6), and the **anti-ritual discipline** — evidence universe, coverage numerator/denominator, executed positive *and* negative probes with a mandatory READY-falsification attempt, unexecuted obligations listed, per-finding failure scenarios; "A zero-finding report without the full packet is UNVERIFIED, not READY" (F8).

Every one of those amendments paid. F1's authority-plane seat produced two of the sitting's confirmed blockers. F4's mandatory cold pairing found four blockers against the sitting itself. F5's sacrificial seat found the funded window's claim edge unbuilt. And F8's anti-ritual packet is the machinery that made the falsely-clean attack on L2 a scheduled act rather than an inspiration. This is rule 2's amended form working exactly as written: the design consult was run *before* the decision, with explicit license to disagree, and it out-designed the magistrate's own draft wholesale. The draft the consult rejected is retained beside the charter rather than deleted — the record keeps the thing that was wrong.

### The arm-author gauntlet: the TOCTOU the fix round could not see

WO-ARM-EVIDENCE-AUTHOR-01 closed the §0.6 launch-blocker registered at the T6 arm slip: fifteen `ARM_ONLY` rows with no evidence producer — "the arm-side mirror of the freeze-side X-1" (#147 body). The design template is the #145 freeze-side author's cold-gate-hardened patterns. The gauntlet ran the C-028 shape end to end:

**Implementation** (`armauth-out`, xhigh, 54.8 min, `head_start 8915ec1`, WRITE_SCOPE three files, all three touched, no violations) → **paired lenses** at the same commit → **fix round** → **delta** → **round 2** → **focused delta**.

**Lens A (contract)** returned two should-fix findings, both about the seam between the tested path and the operator path: **F1**, "The acid test bypasses `scripts/generate_arm_readiness.py arm`" (`tests/test_arm_readiness_evidence_t0.py:707`) — "A regression in the arm subparser, `--arm-context` decoding, or CLI dispatch can leave this test green because it calls `readiness.generate_arm_receipt` directly"; and **F2**, absent runbook artifacts refusing only under a generic kind-level code (`joulewise/arm_readiness_evidence_t0.py:546`) — "The detail names the file, but the machine-readable reason does not." Its V2 probe drove the real CLI and recorded `{"arm_disposition": "GO", "authored_rows": 15, "cli_exit": 0, "reason_codes": [], "status": "PASS"}`.

**Lens B (mutation/test-audit)** returned the harder verdict: **`DO_NOT_AUTHORIZE_LIVE_WINDOW_ARMING`**, mutation summary **4 killed / 3 survived**, with two blockers — **F1** "Every census test bypasses the real command-execution seam" (`:402`) and **F2** "No fault-injection test covers failure between the two publication renames" (`:667`) — plus F3 (the acid test stubs a substantive identity-verification link) and F4 (the principal success oracle reuses author-owned row and predicate authorities instead of independently checking all fifteen derivations). Its summary states the reviewer's job in one sentence: "Do not use this test suite alone to authorize live-window arming."

**The delta re-audit (`armauth-delta-out`) returned `REJECT`** — and it found what neither lens nor the fix round could, because it was the first reader to *run the world forward in time*. **F1, blocker: "Volatile census can become stale before arm and consumption."** Its evidence is an executed sequence, not an argument: "After author PASS, a real `/usr/bin/caffeinate` process was started. The subprocess arm CLI returned PASS/GO and the subprocess consume CLI returned CONSUMED while PID 2975 remained alive." A forbidden process started *after* authoring survived both arm and consume as GO. **F2, blocker:** the claimed CLI-driven acid path is not the operator/runbook path — "The runbook contains no `author_arm_evidence_t0.py` step and no producer for `arm_readiness.t0.inputs`." **F3, should-fix:** publication is exception-recoverable but not crash-atomic — "Killing the child immediately after the first successful rename left 15 public source files and the staging directory, with no evidence directory."

Round 2 answered F1 with **bounded volatile-evidence horizons** — 20-minute monotonic validity on the nine live-state kinds, 6 hours on the four procedure-backed kinds, with an import-time completeness assertion so a new kind cannot be added unclassified — and F3 with **crash-detectable three-step publication**: ordered renames, completion marker written last, marker-absence as the recovery signal, demonstrated against a kill-injection matrix. The **focused delta (`armauth-delta2-out`) returned `ACCEPT`, `findings: []`**, having independently re-derived the volatile/non-volatile split per kind and driven an independent kill-injection at an untested point. F2's runbook half was handed to the chain-fix batch rather than patched in place, and #148 carries it as the T-0 E-step.

The doctrinal point: the fix round was competent and the lenses were competent, and **the defect that would have shipped a forged GO was found by the delta** — the layer whose entire job is to read the fix round rather than the implementation. Fix rounds introduce defects; delta re-audits catch them; this is now the third recorded instance in the T-series. The postscript is harsher: the fleet's L6/L8 seats later found that the *whole* T-0 authoring path this gauntlet hardened has **no producer for its own nine inputs** — the gauntlet audited the consumer to production standard while nothing anywhere wrote what it consumes. A gauntlet is only as wide as the artifact handed to it.

### The union: three PRs that held each other's cures, and five integration catches

#146 (`7e8c3fd`) reconciled four registry tests that were "stale pre-freeze expectations" — the packs legitimately froze on 08-13, so the reworked invariant is that the slot is "null (unfrozen) OR a fully verified pinned freeze receipt (path shape, sha format, bytes byte-identical to the committed blob via `git show`, schema-valid, GNU sidecar correct)" with an exact-key census proving the future arm-receipt namespace is declared but never populated. #148 (`0e1bbb4`) carried F-A..F-H launch-procedure corrections, the T-0 E-step, **M-2 as ruled** (freeze-aware forward-only generators), the keyboard-backlight census control (JW-MET-2), the D-8′ post-CH-1 rewrite, and the two executable ED-QUALIFICATION scripts.

None of the three could merge alone. The integration tree at `integration/readiness-tooling` produced **five fixes after the three-way merge**, and they are five *different* classes — the RUN_STATE checkpoint's phrase "five integration catches" is accurate as a count of integration-fix commits, but only the first is a true cross-branch union defect:

1. **`2683959` — the cross-PR defect, the one no branch could see.** #148 threaded the mutable pack status into generator artifacts; the ruled-frozen p256 prompt artifact's own byte-stability pin fired under #146's synthetic-freeze regeneration test. Fixed surgically: all 20 threading sites audited, byte-stability-pinned artifacts reverted to the stable literal with a disposition table, freeze-aware status retained only on unpinned descriptive surfaces. The #149 body calls this "the integration tree's specialty, third time in the record."
2. **`dff318f` — stale-by-construction pins.** The generator fix necessarily moved the generator shas; plan-test aggregate and per-file sha maps refreshed; "union focused set 95/95 green."
3. **`95c890c` — a CI-only failure invisible on the bench.** T0 tests platform-split into darwin-only real-command variants (explicitly "unweakened") plus portable mocked variants, "hosted runners lack `kern.bootsessionuuid`."
4. **`f468b98` — the live real-boot-session acid.** Synthetic-repo `.gitignore` + `PYTHONDONTWRITEBYTECODE` in child envs, and a coherent-world variant with freeze-side evidence patched to the live session: "the X-3 fence demanded exactly this; real acid to GO now passes on darwin." This is the claim the T7 checkpoint summarises as "authored evidence through the real arm generator to GO on this machine" — it is **attested by the commit message; no separate test-output artifact was located** **[UNVERIFIED-BY-MECHANIC]**.
5. **`346ff4d` — clock hermeticity.** "Every time read flows from the injected context clock (single-clock coherence for horizons/staleness); TZ-shift + future-wall-clock acid regressions; both acid variants bench-verified post-change." Note what this closes: the round-2 horizons from the delta's F1 are only as trustworthy as the clock they are read from, and until this commit the module read time from more than one place.

⚑ **Anomaly (documentation, not defect):** the #149 body was written at PR-creation time (2026-08-14T23:33:55Z) and describes catches 1 and 2 only. Catches 3, 4 and 5 landed at 18:07, 19:15 and 20:45 PT — *after* the body was authored — and the body was never refreshed before the 22:20 PT merge. The merged PR's own description therefore understates its integration round by four commits and by three defect classes. The commits carry the record; the PR body does not.

### The rulings taken under Ed's interaction contract

`29a4ad0` records Ed's 2026-08-14 morning instruction: wait on him **only** for physical-hardware and sudo actions, minimise his appearances, batch all Ed-needs into as few machine sessions as possible. Consequently "the magistrate now rules directly on items previously queued as Ed-owed unless they touch hardware/privilege." Two rulings were taken immediately under it:

**M-1 RULED:** `BRACKET_SESSION_ID` for Window ALPHA = `window_alpha_YYYYMMDD-calibration`, date bound at arm. The runbook:177 convention reads `<WINDOW_ID>-calibration`, which would yield the plan-id literal; the window-letter form was adopted for operator legibility after the packet mechanic verified the operator binding passes the path-component check and that the margin recorder reads the plan-id as window ID independently. The arm packet and §12 record **both** the literal and the plan-id "so no consumer ambiguity survives."

**M-2 RULED:** the frozen packs' `draft_status: "unfrozen_draft"` and README "not armable" lines are "GENERATOR-OWNED DESCRIPTIVE TEXT that predates the freeze machinery; the freeze receipts and plan-tree pins are the AUTHORITATIVE state." The remedy is forward-only generators (landed in #148, with generator `--check` byte-identity against the frozen packs executed ×3). The ruling's second half is the load-bearing part: "until that lands, the freeze receipt's presence governs and **the §5C gate's placeholder-text NO-GO reading is OVERRIDDEN for exactly this field by this ruling** (scoped, recorded — the packet cites it)."

**M-2 — RAISED BY THE DRAFTING MECHANIC, AND REMANDED.** The mechanic's pre-sitting draft flagged that rule 11 lists "any reversal or reinterpretation of a stop signal or verdict" as a **mandatory** cold-pairing trigger, not a discretionary one; M-2 reinterprets a gate's NO-GO reading; no cold-pairing artifact for M-2 existed on disk; and the trigger as written does not exempt the magistrate, whose whole design premise is that the loop-immersed seat is the one that cannot judge when to stop. The magistrate **confirmed the finding rather than arguing it** ("the trigger was missed — this is acknowledged as a process defect") and submitted M-2 to the sitting's cold pairing as Disposition 2.

The pairing then **split on adjudicability**, and the conservative composition prevailed. The cold Fable adjudicator completed a full §C analysis — M-2 UPHELD on the merits ("reading placeholder prose as NO-GO would have deadlocked: the text cannot be regenerated on a frozen pack without rotating the committed digests and voiding the very receipts that prove the freeze"), remedy sound as engineering but **incomplete as recorded** ("a ruling scoped as transitional is now the standing operative instrument for the current packs, and the policy home never says so"), no backward reliance re-review needed. The Opus contract refuter ruled the question **not adjudicable on this packet at all** (B3): the packet supplied the M-2 fact pattern "only as the magistrate's own prose," with the decision-log entry, the overridden §5C gate text, the #149 generator diff and L5's independent finding all absent — "the reviewed party wrote the charge sheet, set the scope, and characterised the exposure, while the one document that contradicts the remedy narrative (sweep-S3) was left out."

**Recorded disposition: M-2 is REMANDED, not adjudicated.** It will be re-submitted as its own cold-gate artifact with primaries attached (decision-log entry, the overridden §5C gate text, the #149 generator diff, L5's independent finding, sweep-S3); the cold adjudicator's completed §C analysis is custodied as **ADVISORY INPUT to that gate, not as a verdict**. Operationally M-2 remains in force on its own scoped terms until that gate rules, and the successor arm packet must cite it until the re-freeze retires it. The magistrate adopted the stricter of the two cold positions over the one that would have closed the item — recorded here because the cheaper reading was available and was not taken.

Also under the contract: ED-QUALIFICATION batching — all sudo/hardware rows assembled into **one scripted session**, prepared before Ed is next pinged, with a window arm chainable onto the same session so "one appearance can close qualification AND start a window." The script landed at `95362e2`; the two executable pieces (sampler checklist, ABBA rail probe, dry-run verified, tired-operator prompts, everything tees to custody) landed inside #148. The sitting expanded that session substantially — see the work-order program.

### The metrology investigation nobody asked for

The `kbd-power` run — a question about whether keyboard-backlight LED power contaminates the corpus — returned a boundary verdict of `keyboard_backlight_outside_consumed_cpu_gpu_ane_subsystem_proxy`, "high for operational inclusion; exact M3 Max board-level LED supply rail remains unresolved," and explicitly "a9/a10 validity is not affected for the presently declared CPU+GPU+ANE boundary." The valuable part is what it found *while* answering: **JW-MET-1**, "Paper boundary wording is broader than the implementation" — the paper said system-on-chip / whole-system-on-chip where the implementation measures a CPU+GPU+ANE subsystem proxy. That is a claim-side overstatement in the paper, found by a hardware-scope question. It was fixed at all five sites in `31ccef5` (+5/−5) and flagged for the next review train. **JW-MET-2** — quiet-machine procedures omit keyboard-backlight control and census — became the #148 census control (§5A control step, census literals, `quiet_mac_prep` capability inventory). **JW-MET-3** — an optional five-minute sudo rail probe — became an ED-QUALIFICATION row and the `rail-probe.sh` script.

### The baseline pin, the fleet, and the drift question

Charter amendment 2 requires an immutable audit-baseline manifest committed **before any lens launches**, binding HEAD + origin/main, all three pack digests, the row-registry sha, the acceptance-artifact sha, the runbook and chain-template shas, the state-kernel sha, and the governing decision IDs, with the rule that "any drift from it invalidates affected lens results (amendment 12's final-head invalidation)." `694442c` committed it at `docs/process/audit-baseline-manifest.json`, schema `joulewise.audit_baseline_manifest.v1`, `head_commit` = `origin_main` = `ac3fe1d`, one minute after the #149 merge — satisfying the charter's §Sequencing requirement that "the fleet launches after the arm-author lands and the chain-fix batch merges." Governing decisions listed: D-117, D-118, D-121, D-124, D-131, D-132, D-133, D-134, D-136, D-137, plus "M-1/M-2 rulings 2026-08-14" and "interaction contract 2026-08-14" — the two rulings above are cited as governing authority by the audit's own baseline.

The fleet is eleven seats per charter §The fleet — L1 authority plane, L2 calibration acquisition, L3 capture + telemetry, L4 quantitative claim pipeline, L5 pack/readiness/custody, L6/L7 the two independent producer-consumer seam readers (contract and execution, "independent from audit start"), L8 operator + recovery human factors, L9 environmental controls census, L10 sacrificial full lifecycle, L11 (non-gating) retained characterization basis — each in its own worktree, with schema-forced findings (`lens_id`, `verdict` ∈ {READY, NOT_READY, UNVERIFIED}, `coverage {examined, total}`, `falsifiers`, `findings`, `work_orders`, `ed_qualification_rows`, `unexecuted_obligations`, `report_markdown`) so a seat cannot return prose where the charter demands a packet.

It launched, ran, and was **stopped cleanly at Ed's pause order with all eleven seats started and zero completed** (`d279a7c`). The checkpoint's first version pointed the successor at a resume; `8937dec`, 36 seconds later, corrected it: `resumeFromRunId` is same-session-only and does not survive `/clear`, so the fleet relaunches **fresh** — "zero seats completed so nothing is lost," and a fresh launch is preferable for audit freshness anyway. The correction is worth recording as a positive: a checkpoint that was wrong for less than a minute because the author re-read his own resume instruction before walking away.

**Baseline drift — RAISED BY THE DRAFTING MECHANIC, ADJUDICATED AT THE SITTING, ZERO LENS RESULTS VOIDED.** The mechanic's draft flagged the live version of the problem: the manifest pins `head_commit: ac3fe1d`, two commits landed after it (`d279a7c`, `8937dec`), and the relaunched fleet's eleven worktrees are checked out at `8937dec` — so every seat was being told to cite a manifest whose head does not equal the tree it is reading, against an invalidation rule that is mechanical and contains no doc-only exemption. The magistrate wrote it up as Disposition 1 (zero results affected) and — this is the finding, not the outcome — **acted on it by letting the fleet run before the cold pairing reviewed it**.

The pairing accepted the conclusion and rejected the reasoning. The cold adjudicator verified mechanically that `git diff --stat ac3fe1d..8937dec` is exactly `README.md`, `RUN_STATE.md` and the manifest itself, and that the commit freeze held through harvest and seal. But the disposition's rationale point 3 — "README.md and RUN_STATE.md are session-state surfaces in no lens's evidence universe" — **is false on the packet's own face**: L1-B2 cites `RUN_STATE.md:3433` as its evidence location, verified at HEAD to be exactly the P2-006 READY row the finding rests on. The Opus refuter put it most sharply (S5): the disposition "applied the wrong test ('is the file in a universe?') instead of the right one ('did the lens read pre- or post-drift bytes?'). It survived by luck." The corrected proof, executed and strictly stronger, was substituted into the record: **all eleven lens worktrees audited `8937dec`, which is `main` == `origin/main` — the final head, so no lens result can be stale against a head it read — and every manifest binding recomputes byte-identical at that head, including all three pack digests via `joulewise.arm_readiness.committed_pack_tree_sha256`.** Rationale point 3 is stricken from the record; point 4's "manifest-after-its-own-head is charter-by-construction" is recorded as over-reaching (it licenses exactly one post-manifest commit, its own).

Conditions attached: the council record **pins the effective audit baseline as `8937dec` = `ac3fe1d` + {manifest, README, RUN_STATE}** so no successor re-derives the question; and the manifest's own defects — no `pack_digest_algorithm` field, no chain-template coverage note, no paths for three bindings — are fixed at the next cut, which the charter requires be worded as **SUPERSESSION, never re-pin** (the charter calls the manifest immutable). The magistrate's earlier "the manifest requires NO fix" is rejected as too generous by both cold seats.

⚑ **Anomaly (briefing text):** the fleet script's shared preamble tells every seat "You are one seat of the **ten**-seat readiness fleet" while eleven seats (L1–L11) launch. The charter's own heading is "ten launch-gating seats + one non-gating," so ten is the gating count and the preamble is defensible shorthand — but L11 reads that sentence too, and L11 is precisely the seat whose non-gating status the sentence silently encodes. Harmless; worth one word.

### The fleet outcome: 11/11 returned, and the single READY did not survive

The relaunched fleet returned complete. Run `wf_29eee330-1db`: **11/11 seats returned, 2,411,618 subagent tokens, 1,083 tool uses, 46.7 minutes wall.** Raw aggregate at harvest: **1 READY (L2) / 10 NOT_READY**, with **16 blocker findings, 32 should-fix, 22 nits, 68 executed falsifiers, 62 unexecuted obligations declared, and 23 ED-QUALIFICATION rows minted.** Aggregate self-declared coverage 219/253 artifacts (see §Source cautions — this number is arithmetic over self-nominated denominators and is not an audit-coverage figure).

The blockers concentrate where the charter's new seats were pointed. **L8 (operator/recovery) returned seven** — the largest single-seat yield in the T-series — led by B1: the T-0 evidence author "consumes nine byte-canonical JSON inputs that no tool, no runbook step, and no packet step produces; the only 2am path forward is hand-fabricating canonical JSON with invented monotonic_ns values, which the receipts cannot distinguish from honest capture." **L6 (seam reader A) independently found the same gap** from the producer-consumer side, and added B2: every one of the frozen packs' generic evidence receipts is already past its 24h monotonic horizon, so "any arm receipt issued now is expired at birth." **L1 (authority plane)** confirmed the expiry against all 33 receipts and added B2, the kernel fail-open: `active_global_gates: []` while `RUN_STATE` renders "READY — Q2 P2-006" for a campaign D-117 superseded — "a quiet night gets spent on a campaign whose outputs do not trace to the current claim path, bypassing the council gate entirely." **L10 (the sacrificial lifecycle seat the consult's F5 bought)** executed the after-window path and found it unbuilt: `analyze-claims` refuses the packs' own `v3.prospective` manifests, the designed prospective builder/validator do not exist, "the window is spent and its REQUIRED OUTPUT cannot trace through a claim consumer without landing new code post-hoc." **L4** produced the charter's named type specimen: the margin recorder REFUSEs the re-specced frozen spec with `authoritative_input_invalid`, while "the committed census tests model 'real floor pack cell shapes' WITHOUT the estimator vocabulary, so the suite is green while the real seam is broken." **L9** found both arm-time census probes unpassable on the real machine — permanently resident Apple daemons match the maintenance census, and the browser/monitor regexes match nine always-resident Safari LaunchAgents and `watchdogd` — with the diagnosis of why CI never saw it: "tests fake the probe executor with exit_code=1."

Ten of ten gating seats therefore self-falsified. **The eleventh, L2, returned READY** at coverage 15/16 — and it was the only seat subjected to an external falsely-clean attack.

### The refuter round: nine xhigh lenses, three kills, two new defects, and seven hours lost to relays that never launched

The blockers went to a C-028 refutation of **nine Sol xhigh read-only runs** (`reasoning_effort: xhigh`, `sandbox: read-only`, `write_scope: []`, `head_start 8937dec` — verified in the manifests), organised as distinct contract/execution lens pairs per blocker cluster (A = arm/freeze expiry, B = T-0 producer, DG = authority plane + launch ceremony, ECF = consumption edge/recorder/census) plus **one falsely-clean attack on L2's READY**.

**The relay wedge is the process finding.** The first wave launched four runs at 2026-08-14 23:25 PT; the other **five refuter relay agents wedged for roughly seven hours reading bridge-contract documentation without ever launching their Sol runs**, and were killed at 07:15 PT the following morning and relaunched from the lead shell per the T5 lesson. The relaunched five — `A-execution`, `B-contract`, `B-execution`, `ECF-contract`, `ECF-execution` — started within 37 seconds of each other at 14:15Z and **all five finished inside 13 minutes** (423 s to 760 s each, manifest-verified). Seven hours of wall clock bought thirteen minutes of work that a lead-shell launch delivered immediately. The standing rule is now third-time-confirmed: runs that matter launch from the lead shell, and `.status` is not liveness.

**Adjudication tally: 19 blocker-level claims examined (16 fleet + 3 raised by the L2 attack).**

**Three died.** **L8-B4** (committed freeze receipt stale → every arm refuses `readiness_freeze_receipt_mismatch`) was **REFUTED by both lenses with an executed canonical-path counter-probe**: the receipt binds the canonical measurement-checkout absolute path, `identity_matches=True` for all three packs at that path, and the mismatch reproduces only from the audit's scratch path — "correct fail-closed wrong-checkout refusal, not a defect." **WO-L2-4** (an idempotent-marker work order) was refuted as **phantom** — the runbook forbids re-reserving, so the proposed reprint would mislead — and dropped. **F4's timing premise** died while its substance survived: the sudo-cache-cold argument is wrong at the current ~1-minute E-7b, and becomes true only once F3's 10-minute dwell lands; the privilege gap itself survives inside the T-0 work order.

**L2's READY did not survive.** The falsely-clean attack returned "READY DOES NOT SURVIVE. L2 → NOT-READY" on three grounds, each new: **L2-1**, `detect_pulses` region projection has **no finite work budget** and the frozen chain calls it synchronously **under the writer lease** (`validate_powermetrics_fiducial.py:846` acquire, `:1509` call, `:1037` release; `runbook:1017` no watchdog; `powermetrics_fiducial.py:555` unbounded loop) — a hang at that site holds the lease indefinitely on a funded night; **L2-COV-1**, the coverage denominator is refuted outright — the self-selected 16-artifact universe omitted contracts, bootstrap/backfill scripts and a 23-test three-window lifecycle module, the crash matrix is 13 tests not 16, and the **real direct test universe is 251**; and **L2-EDQ-1**, the seat declared READY with ED-QUALIFICATION rows open, which the charter forbids. Two of its should-fix findings were confirmed and one was raised from nit to should-fix ("can mechanically abort every correct pre-slot session"). Lead-side replay corroborated the crash-matrix count of 13, the 23-test module, and the reproduced traceback.

**Two new defects were found by refuters that no seat had raised.** **The doubled plan-path** (B-contract, confirmed independently by B-execution): the author joins `plan_tree.json`'s repo-relative plan path onto `pack_root`, producing a nonexistent `pack_root/configs/campaigns/.../calibration_plan.json` — **production-only, and masked by the suite because the test fixture uses a bare filename.** A real-pack regression test is now mandatory in its work order. **The terminal-review-trailer producer gap** (B-execution): the audit baseline itself "lacks the three `JouleWise-Terminal-Review*` commit trailers the T-0 author demands (`arm_readiness_evidence_t0.py:918-930`)" — the terminal-review evidence needs an operational producer too.

Everything else was **CONFIRMED, several with executed refusals**: the 33/33 expiry refusals reproduced at live monotonic; `load_manifest` refusing `v3.prospective` verbatim; the margin recorder's `authoritative_input_invalid` on the forbidden `estimator_registration` key at the pack-pinned spec cell; the prewindow dwell measured at **60.09 s against the author's required 600 s**; all four env/chain mismatches. Several remedies were corrected rather than accepted — in-place re-author is **not** contract-valid under D-131 (a successor pack + custody root is required), the 24h horizon is implementation policy rather than contract text, F1+F2 merge into a single shipped T-0 acquisition tool, and F3's remedy is a minimum dwell in `--wait`, **not** a lowered author threshold. Refuters earned their spend on both edges: they killed three claims that would have generated work orders against nothing, and they found two defects that would have reached the night.

### The cold pairing: four blockers against the sitting itself, all cured before recording

Charter amendment F4 made the rule-11 pairing mandatory for any council verdict: a **cold Fable adjudicator** (fresh session, no loop context) plus an **Opus contract-lens refuter** for cross-model diversity, both over the sealed mechanically-assembled packet (sha16 `e68c7fb9fe88ed0b`, 500 lines) with read-only primary-evidence spot-checks at `8937dec`. Both rulings are custodied verbatim; the magistrate edited neither.

The pairing's four blockers are all against the **sitting**, not the instrument:

- **B1 — the packet was not in custody.** Charter:88 requires sealed-packet custody under `docs/process_traces/<date>-readiness-council/`; no such directory existed. The packet, eleven seat reports, `triage.json`, the sweep and the dispositions lived only in an ephemeral scratchpad: "**2.4 M tokens of fleet output and the entire basis for ~20 work orders are one `rm` from unreconstructable**, and the verdict would be unauditable by anyone who was not in the session." Cured at `bd7f81c` before anything was recorded.
- **B2 — mandatory packet contents were incomplete.** A 23-finding consistency sweep (7 blockers / 10 should-fix / 6 nits) was compressed into one line plus a sha16 of an uncustodied file, and "the only one shown is the one the reviewed party killed." Three omissions bore directly on the sitting: sweep-B1 (a doc anchored at T4-late asserting NO-GO for closed gates — a **false-dirty** generator inside the pinned baseline, the exact mirror of the false-clean risk the sitting was ordered to adjudicate), sweep-S3 (the M-2 remedy "describes a regeneration that deliberately did not happen" — withheld from the pairing asked to rule on M-2), and sweep-B5 (stale kernel — independent corroboration of L1-B2 that the packet did not record as corroborated). "A packet whose visible triage of an input consists of the single finding the reviewed party refuted is not 'mechanically assembled' in the rule-11 sense." Cured: the full sweep is custodied with per-finding triage state.
- **B3 — M-2 was not adjudicable on this packet.** See above; the remand is the recorded outcome.
- **B4 — the coverage adjudication was undischarged for 10 of 11 seats, and the one seat tested failed it.** "One for one against the method." The verdict direction is not at risk — more coverage only finds more defects — but the *work-order program's completeness* is, and "a NOT-READY record that implies 'close these and we are READY' would be false."

The cold adjudicator's independent ruling reached the same place from the other side (§E): all ten gating NOT-READY verdicts stand as NOT-READY rather than being downgraded to UNVERIFIED, because **NOT-READY is conservative with respect to coverage error** — undiscovered universe can only add defects. But the falsely-clean risk **transfers to closure time**: closing a component's work orders does not entitle a component READY, the eventual re-audit must re-enumerate every universe independently and run the adversarial coverage attack as a **standing packet element**, and L2 additionally receives a full re-audit against the 251-test universe because "its report's clean areas are ruled void, not merely thin."

It also ruled the one split the refuters left open. **L1-B3** (bifurcated work-selection authority) drew `should_fix` from the contract lens and `blocker` from the execution lens; the adjudicator ruled **should-fix as an independent finding, with its remedy subsumed into the blocker-gated kernel-reconciliation transaction** — "the execution lens's concern is thereby fully honored: the fix ships launch-gated regardless of the label" — plus a standing instruction that P2-006 be retired by formal ruling rather than silent deletion, "a superseded campaign leaving the kernel without a recorded disposition would be a fresh instance of the same defect."

Its closing note is worth preserving verbatim: "The packet is one of the better-assembled adjudication instruments I have reviewed: adverse findings against the magistrate's own launch decision, its own missed trigger, and its own reference manifest were all placed in the sealed record rather than smoothed over. The verdict is nonetheless unambiguous."

### The verdict: NOT-READY, 0/11, and a program that is not certified complete

**COUNCIL VERDICT: NOT-READY. 0 READY / 11 NOT-READY** (ten gating seats plus the non-gating L11 basis seat). **L2 additionally carries UNVERIFIED on coverage** — charter amendment 11 treats the two as distinct verdicts with different remedies, so the NOT-READY carries the work orders and the UNVERIFIED carries the mandatory re-audit; recording only one would have made the re-audit obligation disappear. **No funded window may be armed.** T-0 GO is a separate later closure and is not reached.

**The work-order program is NOT CERTIFIED COMPLETE** (Opus B4, cold §E concurring): every seat's evidence universe was self-nominated, and the one denominator adversarially tested fell. Closing all listed work orders does not entitle READY.

Two structural blockers dominate the program: **the T-0 producer gap** (nothing writes the nine inputs the arm-time author demands) and **the freeze-evidence expiry lifecycle** (the frozen packs' receipts are past their monotonic horizon and no operative document names a refresh lane). Between them, "any funded window is either unlaunchable or unconsumable at the audited baseline."

The ratified program runs in four phases. **Phase 0 (rulings and consults before code):** R1 freeze-evidence lifecycle — durable-freeze-evidence vs successor-pack tool, rule-2 design consult mandatory, and its own cold gate if it amends contract text; R2 `FROZEN_PLAN` identity; R3 P2-006 retirement; R4 the M-2 execution-note amendment plus the remanded M-2 cold gate; design consults for the prospective-manifest validator/finalizer and the margin-recorder authorization; L8-B7's launcher binding recorded as contract-bearing. **Phase 1 (parallel code):** WO-KERNEL-RECONCILE first and magistrate-supervised (it edits the declared sole work-selection authority — lieutenant-forbidden alone); the integrated WO-T0-PRODUCER (nine-input capture tool, ≥10-min dwell, the D-127 privileged clock route landed at last, env/chain/manifest plus the doubled-plan-path fix with a real-pack regression, and the terminal-review-trailer producer with its second lens first); WO-LAUNCH-BINDING; WO-CONSUMPTION-EDGE; WO-MARGIN-RECORDER-AUTHZ; WO-CENSUS-SEMANTICS (hard-gated on ED-Q-L9-3's quiet-state fixture); WO-DETECT-PULSES-BUDGET; WO-L2-REAUDIT; and a should-fix batch carrying the sweep's verify-and-fix items including B7, the paper's 8.611855 J vs 1.869502 J floor-regime contradiction, registered as a **live P1 claim-bearing queue row**. **Ed-required, one batched session:** D-127 sudoers install with explicit ratification (it amends D-004's privilege posture), the **full dress rehearsal E-4→E-9 plus author→arm→verify→consume against scratch custody — "the program's single most valuable Ed hour"** — the sampler checklist, SIGTERM relay and rail probe, the keyboard-backlight rows, ED-Q-L9-3 early because it gates the census work order, production-machine freeze-chain replays, the §5C live verification, the a9/a10 desk replay, and the decisive network replay. **Phase 2 (sequential):** the re-freeze via the R1-ruled route, **once, atomically, LAST among pack-byte changes** — this is Opus W2, "the sharpest omission," because the re-freeze rotates the pack digests and therefore destroys the very baseline manifest that produced the audit; irreversible, so magistrate + Ed — regenerating truthful freeze-aware status text and thereby retiring M-2; then the successor arm packet, only after the T-0 repair passes end-to-end at the exact reviewed head. **Phase 3:** baseline-manifest supersession with the ruled fields, focused re-audit of the pack/custody-bearing seats, adversarial coverage re-enumeration of every universe, delta re-audits per C-028 on every fix round. **Phase 4:** a reconvened READY-CANDIDATE sitting with a fresh cold pairing.

**Four findings are labelled SINGLE-LENS** and may not drive implementation until a second distinct lens reads them: L2-1, L2-COV-1, L2-EDQ-1 (all from the one falsely-clean run, which itself had an environment gap) and the terminal-review-trailer gap (B-execution only). "Lead relay corroboration of counts is not a distinct lens." That second refuter was launched by the magistrate at verdict recording (`run_started` 2026-08-15T14:44:05Z, xhigh, read-only) and **completed eight minutes after the custody commit** — rc 0, `OK`, valid envelope, 472 s, overall `NOT_READY`. It read an independent EXECUTION lens over **exactly the four SINGLE-LENS claims** and returned **three CONFIRMED, one PARTIAL** — which discharges Disposition 5 on all four, pending custody. **L2-COV-1 CONFIRMED** and hardened: the crash matrix is 13 tests not 16, the omitted three-window module holds 23, "the eight-module direct universe is exactly 251 tests." **L2-EDQ-1 CONFIRMED**: three 600-second loaded-host failures are durably recorded, WO-CRASHMATRIX-RELIABILITY is open, live writer/sudo qualification is unexercised. **The terminal-review-trailer gap CONFIRMED**: the author requires three exact trailers, `ac3fe1d` carries none, and repository search finds consumers and tests but no operational producer. **L2-1 PARTIAL** — the `detect_pulses` projection genuinely has no enforced cell, deadline or cancellation budget and genuinely runs synchronously under the writer lease, but the *literal* non-convergence framing is refuted: "the unbudgeted tree is finite but intractably large." Remedy sharpened rather than overturned — a deterministic evaluation budget with registered fail-closed finalization, plus a bypass of full projection when the clock anchor is already unresolved. **CUSTODIED at `15d00d2`** (magistrate, same day): the report landed in `refuter-outputs/sol-refuter-singlelens.md` with a council-verdict addendum discharging Disposition 5 and clearing all four SINGLE-LENS labels — the custody lag was eight minutes, not a recurrence of the B1 defect class.

### Process rulings entered at this sitting

**A charter amendment, adopted at the pairing — the correct venue for a proposed process rule.** Opus S12 found charter:77-78 ("only T0 rows may remain open at the sitting") both violated (23 ED-QUALIFICATION rows open, none performed) and **unsatisfiable as written**: the fleet is what *enumerates* the ED-QUAL universe, so those rows could not have pre-existed it. The rule is impossible on a first sitting — a charter defect, not a compliance failure. Amending it is "any proposed process rule," a rule-11 mandatory item, so the pairing itself was the correct venue. Adopted: sittings divide into **ENUMERATING** sittings (first-pass; output includes the ED-QUALIFICATION universe; charter:77-78 does not bind) and **READY-CANDIDATE** sittings (charter:77-78 binds). This sitting is recorded as an ENUMERATING sitting.

**The same-signature structural finding — the entry's most important process fact.** Opus S13, with cold §G.3 concurring: **twice in one cycle a stop-signal question was ruled unilaterally under packet-finalization pressure and referred after the fact.** M-2 overrode a gate's NO-GO reading with the mandatory cold-pairing trigger unnoticed until a drafting mechanic flagged it; the baseline-drift disposition interpreted an invalidation rule — itself a stop signal — not to fire, and **the fleet was allowed to run on that interpretation before the cold pairing reviewed it.** Same shape, twice, in one span. Rule 11's **STANDING ESCALATION TRIGGER is MET and is recorded as structural**, not as two unrelated misses. (The refuter is precise about the boundary: the referrals were proper, and "continuing past an escalation trigger" is enumerated on the lieutenant's forbidden list rather than the magistrate's mandatory-trigger list — **the acting-before-ruling is the finding**.) The cold adjudicator adds a third instance of the same disposition from a different angle: the relay wedge left the session "in a waiting state on a scarce open resource" for seven hours, which is itself a mandatory trigger that raised no consult, "a pattern, not two accidents."

Three consequences adopted:

- **(a)** The drafting-mechanic checklist gains a **mechanical rule-11 trigger enumeration over every decision-log ruling** — any entry containing an override, reversal or reinterpretation of a stop signal or verdict raises a flag that **blocks packet finalization** until a cold-gate artifact exists or the magistrate records written dissent for Ed.
- **(b)** Refuter fleets get **output-keyed liveness checks with a bounded first-checkpoint deadline** as standing protocol.
- **(c)** **Stop-signal questions arising at packet time are ruled AFTER cold review, not before** — unless delay itself destroys evidence, in which case written dissent to Ed accompanies the action.

**Overreach accepted into the record** (cold §G, Opus concurring): the magistrate's self-performed digest refutation (outcome verified correct by both cold seats; practice flagged — "never self-grade" exists precisely for this shape, and there was no independent refuter at the time); the overbroad drift rationale; the relay-monitoring lapse; and the benign-environment self-rulings on two vanished refuter worktrees (accepted as low-risk **because both affected verdicts were adverse to readiness** — the adjudicator notes explicitly that "a vanished worktree on a refuter that had confirmed a READY would demand replay, not a benign ruling"). **No written dissent is filed**: the magistrate accepts both cold seats' rulings in full, with the single synthesis noted at the M-2 remand, where it adopted the stricter position.

### Unique catches by layer

- **The design consult (rule-2 pre-decision, on a process artifact):** the entry's headline, and the layer whose catches compounded through everything below. It refused to ratify the magistrate's own charter — six blockers — and the magistrate adopted all thirteen amendments in full. The catches that matter are structural, and each one produced a downstream finding at the sitting: the audit's own control plane was outside the audit (F1 → L1's two blockers), the verdict form contained a fail-open (F3, READY-WITH-CONDITIONS, deleted — with a fleet that returned 1 READY, a "READY-WITH-CONDITIONS" option was a live hazard), the magistrate was going to adjudicate its own readiness alone (F4 → the cold pairing that found four blockers against the sitting), the post-collection path was unproven (F5 → L10 finding the claim edge unbuilt), and the audit had no way to distinguish a clean report from a lazy one (F8 → the falsely-clean attack that felled L2). **Rule 2's amendment earned its keep on a decision that was about process, not code.** (Evidence: `charter-consult-out.md` F1–F8; `6a7849c`.)
- **The falsely-clean attack (the F8 mechanism, executed):** the only seat that returned READY was the only seat attacked, and its READY did not survive. Three new blockers, including a **self-nominated coverage denominator of 16 against a real universe of 251** and an unbounded detector called under the writer lease. One for one against the method — which is precisely why the work-order program is recorded as not certified complete. (Evidence: `refuter-L2-out.md`; verdict Disposition/§E.)
- **The C-028 refuters (nine xhigh lenses):** they earned their spend on both edges. **Kills:** L8-B4 (both lenses, executed canonical-path counter-probe — a wrong-path artifact, and the refusal it triggered is *correct* fail-closed behaviour), WO-L2-4 (phantom work order that would have made the runbook mislead), and F4's timing premise. **New defects no seat found:** the **doubled plan-path** — production-only and *masked by the test fixture's bare filename*, the second occurrence this span of a suite green over a broken real seam — and the **terminal-review-trailer producer gap**, discovered by noticing that the audit's own baseline commit lacks trailers the T-0 author demands. Plus remedy corrections that would otherwise have shipped wrong: in-place re-author is not D-131-valid, and F3's cure is a minimum dwell rather than a lowered threshold. (Evidence: `refuter-outputs/refuter-verdicts.md`; `sol-refuter-B-contract.md` F6; `sol-refuter-B-execution.md`.)
- **The fleet lenses (eleven seats, 46.7 minutes):** ten of ten gating seats falsified their own readiness. The catches no single-plane review would have produced: **L8** found that the machinery the arm-author gauntlet hardened has **no producer for its own nine inputs** ("the only 2am path forward is hand-fabricating canonical JSON"); **L6 and L7, independent from launch, converged on the same seam from opposite sides**; **L10**, the sacrificial seat, spent a synthetic window and found the claim edge unbuilt before a real window paid for the discovery; **L4** produced the charter's own type specimen — "the suite is green while the real seam is broken"; **L9** found both arm-time census probes unpassable on the real machine and named why CI never saw it (the tests fake the probe executor). The seat-level lesson: **the fleet's yield came from seats that execute against the live machine and the frozen bytes, not from seats that read code.**
- **The cold pairing (fresh Fable adjudicator + Opus contract refuter):** four blockers, none of them about the instrument. **Custody** — the entire evidentiary base for a verdict and ~20 work orders sat in an ephemeral scratchpad, "one `rm` from unreconstructable." **Packet completeness** — six untriaged sweep blockers excluded, and "the only one shown is the one the reviewed party killed," including the one document (sweep-S3) that contradicts the remedy narrative in the very disposition the pairing was asked to rule on. **M-2 adjudicability** — the reviewed party wrote the charge sheet; referral proper, adjudication impossible on these contents. **Coverage transfer** — NOT-READY is conservative under coverage error, so the verdict stands, but the falsely-clean risk moves to closure time and the program must be recorded as not certified complete. **W2 program ordering** — the top-priority remedy (re-freeze) mechanically destroys the baseline manifest that produced the audit, so the program must be sequenced or "the next sitting will discover its baseline is already dead." Four of the five are invisible from inside the loop by construction; the fifth is a scheduling fact nobody executing a fix list would have looked for. (Evidence: `opus-contract-refuter-findings.md` B1–B4, W2; `cold-fable-ruling.md` §§B, C, E, G.)
- **The consistency-sweep scout — and the scout's own false positive.** The sweep found 23 findings, of which sweep-B1 (a T4-late-anchored doc asserting NO-GO for closed gates, sitting *inside* the pinned audit baseline as a false-dirty generator), sweep-B5 (stale kernel — independent corroboration of L1-B2) and sweep-S3 (the M-2 remedy describing a regeneration that deliberately did not happen) are all material and all fed the sitting. But **its top blocker was itself refuted**: sweep-B4 claimed the baseline manifest's three pack digests "reproduce under no algorithm and match no revision," having recomputed under six candidate algorithms; the digests reproduce **exactly** via `joulewise.arm_readiness.committed_pack_tree_sha256`, verified by the magistrate and re-verified independently by both cold seats. The scout's proposed remedy — overwrite the manifest's digests — would have corrupted the audit's reference artifact. Recorded as the scout layer's cost of doing business: a review layer that reaches for a canonical algorithm and picks the wrong one produces the highest-severity finding shape in the audit, pointed at the audit itself. The cold ruling's counter-catch is the durable part: the false blocker exists **because the manifest names no digest algorithm**, so the fix is a `pack_digest_algorithm` field at supersession, not a scolding. (Evidence: `consistency-sweep-findings.md` B4 + magistrate verification note; cold §G.1; Opus N4.)
- **The drafting mechanic (this entry's own layer):** the two ⚑ OPEN flags. Both became the sitting's two formal dispositions; both were **confirmed by the magistrate rather than argued down**; both were then materially corrected by the pairing (drift survived on a substituted proof, M-2 was remanded rather than closed); and together they are the evidence base for the same-signature structural finding that produced three standing process consequences. A read-only agent verifying a magistrate's dictation against primary evidence found the magistrate's two open process defects. The dictated-fills pattern also produced the span's smaller catch: the T6 merge-count correction from eight to seven (#138 belonged to T5/C-057), landed at `18ef535`. (Evidence: `c058-draft.md` ⚑ items; `magistrate-dispositions-for-sitting.md`; Opus S13.)
- **The delta re-audit (arm-author):** the executed TOCTOU. A real `caffeinate` started after author PASS survived arm as GO and consume as CONSUMED, PID alive throughout — reproduced, not argued. Neither lens found it; the fix round could not have, because it was reading the implementation rather than the world's clock. (Evidence: `armauth-delta-out.md` F1 with PID 2975.)
- **Lens B (mutation/test-audit):** the verdict `DO_NOT_AUTHORIZE_LIVE_WINDOW_ARMING` with 4 killed / 3 survived, and the two blockers that name the exact seams the suite never crossed. The value is in the three survivors, each converted into a named gap. (Evidence: `armauth-lensB-out.md`.)
- **Lens A (contract):** the acid test that calls the library rather than the mandated CLI (`:707`) — the same defect class as T5's `test_common_mode_full_cli_path_…` stub, a test whose name promises a path it does not take. **The class now has its third and fourth occurrences this span** (L4's green census tests over a broken recorder seam; the doubled-plan-path fixture using a bare filename). It is a class, not a coincidence — see follow-ups. (Evidence: `armauth-lensA-out.md` F1.)
- **The integration tree:** `2683959`, the cross-PR defect provably invisible from any single branch — #148's pack-status threading firing #146's byte-stability pin. Third time in the record per the #149 body.
- **The integration tree, second-order:** three further defect classes the union round found that were nobody's branch defect — a CI-only failure hidden by the bench's platform (`95c890c`), an acid test that had never run in a coherent world (`f468b98`), and a module reading wall-clock time from more than one place under a design whose safety depends on horizons (`346ff4d`). **The integration tree is where the environment gets audited, not just the merge.**
- **The metrology investigation (JW-MET-1):** a paper-side claim overstatement — system-on-chip wording over a CPU+GPU+ANE measurement — found by a question about keyboard LEDs. The sitting produced its sibling: L11's paper-integrity findings and sweep-B7's floor-regime contradiction (8.611855 J vs 1.869502 J), now a live P1 claim-bearing queue row. Claim-bearing prose is audited by no code lens, and it has now yielded a finding in three consecutive spans. (Evidence: `kbd-power-out.md` JW-MET-1; `31ccef5`; cold §F.)
- **The checkpoint author, on himself:** `8937dec`, correcting `d279a7c` 36 seconds later on the resume semantics. A trivial catch with a non-trivial property — it happened before the session ended rather than after the successor tried the wrong command.

**The pattern worth binding forward:** C-057's lesson was *a second reader of the same bytes*. This span sharpens it — **the highest-yield readers were the ones reading a different plane than the one under construction** — and the sitting supplies the strongest instance yet. The consult read the audit's authority structure while the magistrate designed its component list. The delta read the clock while the fix round read the code. The integration tree read the CI environment while three branches read their own diffs. The metrology run read the paper while everyone read the harness. And **the cold pairing read the sitting's own paperwork — custody, packet assembly, adjudicability, program ordering — while every other layer read the instrument.** Ten gating seats produced sixteen blockers about the machine; two cold seats produced four blockers about whether the verdict about the machine was worth anything. Both were necessary; only one of them was scheduled by anybody's instinct.

### Deliberation resolutions and dissent

The charter consult's rejection was accepted **in full and without negotiation** — thirteen amendments adopted, no partial adoption, no recorded magistrate dissent, and the rejected draft retained on disk rather than deleted. Where the consult and the magistrate's draft disagreed on the verdict form, the consult won outright: READY-WITH-CONDITIONS was deleted rather than tightened, and magistrate-alone READY was replaced by a mandatory cold pairing — the magistrate voluntarily surrendering adjudication authority over its own readiness verdict, months before the verdict existed to be surrendered.

On the arm-author delta's F2 (the runbook has no author step), the magistrate **declined to patch in the implementation lane** and routed the documentation obligation to the chain-fix batch, which is where the runbook lives — a scope-discipline call recorded in #147's body and discharged in #148.

At the sitting, three disagreements were resolved rather than averaged. **L1-B3's split verdict** (contract lens: should_fix; execution lens: blocker) was **synthesized, not majority-voted**: should-fix as a label, remedy subsumed into the blocker-gated kernel transaction, so the fix ships launch-gated either way — with an explicit instruction that the shipped work orders must not be re-registered and that P2-006 be retired by ruling rather than deletion. **The two cold seats split on M-2** — the adjudicator completed a merits analysis, the refuter ruled it not adjudicable on the packet — and the magistrate adopted **the stricter position**, remanding M-2 to its own future cold gate and demoting the completed analysis to advisory input. **The magistrate's own dispositions were partly overturned**: rationale point 3 of the drift disposition is stricken as false, point 4 recorded as over-reaching, and "the manifest requires NO fix" rejected by both seats. **No written dissent is filed**; the magistrate accepts both cold rulings in full.

The clean stop at Ed's pause order remains worth recording: eleven seats were in flight, the sunk cost was real, and the session stopped, checkpointed, and accepted a fresh relaunch rather than contriving a resume. That is the disposition rule 11 exists to enforce, exercised without needing to be enforced — which makes its twin failure (twice acting on a stop-signal question before referring it) the more instructive, and is why it is recorded as structural rather than as two misses.

### Process findings and follow-ups

**The ~60-minute subagent-shell kill cap did not recur.** C-057's dominant tooling loss — four runs dying at 59.8–60.0 minutes with a `run_started` and no `run_finished` — has **zero occurrences** in this span: all eight manifested implementation runs recorded a `run_finished`, and the longest (`armauth-out`, 54.8 min) sat just under the cap. The mid-session operating rule adopted in T5 (long runs launch from the magistrate's own shell) is consistent with this outcome, but the improvement is **correlational at this sample size** and one run came within five minutes of the old failure mode **[UNVERIFIED-BY-MECHANIC]**.

**A new envelope class: `SCOPE_CAPTURE_FAILED` (rc 74), caused by the launcher's own worktree vanishing.** Three refuter runs launched from *agent* worktrees (`.claude/worktrees/agent-…`) returned rc 74 with `head_end: null` — the harness deleted the worktree under the running Sol process, so post-run scope capture had nothing to compute against. All three had `scope_action: capture_failed_preserved`, and one (`refuter-L2-out`) additionally returned `report_parse: invalid`, `semantic_status/completion: unknown/unknown`. **All three were harvested from disk anyway, and their content drove the sitting's single most consequential change — the felling of L2's READY.** The five runs relaunched from the lead shell into scratchpad trees returned `OK` uniformly. This is the strongest instance yet of the standing rule that **`run_status` is a statement about the envelope, not about the work**; it is also a concrete argument for launching any run whose output must outlive its launcher from the lead shell.

**Report-parse acceptance failures remain the dominant implementation-envelope class.** Three of eight manifested implementation runs returned rc 65: `armauth-out` and `armauth-fix2-out` both with `error_stage: report_parse`, `report_parse: invalid`, `semantic_status/completion: unknown/unknown` and `pathspec_report_mismatch: true` (`reported_changed_count: 0` against actual 3 and 2 — the work was done and in scope; the envelope's changed-paths list came back empty), and `integ-fix-out` with a *valid* parse but `semantic_status: blocked`, `completion: partial`. **Both `report_parse: invalid` runs are ones whose work was accepted and shipped** (`225988a`, `dc2ba8b`), and neither PR body mentions that its originating run returned ACCEPTANCE_FAILED.

**A scope-expansion request was raised and not granted.** `integ-fix-out` returned `scope_expansion` naming five test files ("The authorized source edits necessarily change exact generator and pack SHA pins, while two acceptance tests only consume committed bytes"), `scope_expansion_valid: false`, `scope_action: acceptance_failed`. The work was then done at the bench in `dff318f` (pins refreshed, 95/95 green) — the rule-9 bench-vs-session threshold applied correctly.

**Follow-ups:**
- **Phase 0 first, and nothing else.** R1 (freeze-evidence lifecycle, rule-2 consult mandatory, own cold gate if it touches contract text) blocks everything in Phase 2; R2 blocks the B-cluster parser/prose work; R3 and R4 are cheap rulings that unblock the kernel transaction and the M-2 remand.
- **The remanded M-2 cold gate**, with primaries attached (decision-log entry, overridden §5C gate text, #149 generator diff, L5's finding, sweep-S3). M-2 stays operative until it rules; the successor arm packet must cite it until the re-freeze retires it.
- **Tenth refuter custody: DONE** (`15d00d2`, verdict addendum + `refuter-outputs/sol-refuter-singlelens.md`). Remaining residue: carry its L2-1 remedy correction (deterministic budget + anchor-unresolved bypass) into WO-DETECT-PULSES-BUDGET when that WO's contract is cut.
- **Adopt consequence (a) into the drafting-mechanic checklist now** — the mechanical rule-11 trigger enumeration over the decision log, blocking packet finalization. It is the one consequence that would have caught this cycle's failure prospectively.
- **Sequence the re-freeze last** (W2). Any earlier pack-byte change forces an extra baseline supersession and re-audit round.
- **The consistency sweep for this span has now run** (23 findings, custodied) and its blockers are inside Phase 1's should-fix batch — including **B7, the paper's floor-regime contradiction, which is claim-bearing and P1**. The sweep's own findings remain formally UNVERIFIED; the work order is verify-and-fix.
- **JW-MET-1's five-site boundary narrowing is still unreviewed** by the paper train, and L11 adds three further paper corrections.
- **FLAKE-CALEXITS-311-REDERIVE** — fix shape registered, not implemented, 4 occurrences. **WO-CRASHMATRIX-RELIABILITY** — bench canonical suites still carry the known 3-test load-pathology trio (inherited from C-057).
- **"The test name overstates its proof" is now a confirmed class, not a coincidence** — T5's B1, this span's lens-A F1, L4's green census over a broken recorder seam, and the doubled-plan-path fixture's bare filename. Four occurrences across three spans. It wants a **mechanical check** (real-artifact fixtures for any test whose name asserts a full path), not a reviewer's attention. This is the single highest-leverage process item in the follow-up list.

**Yield/spend:** **Implementation lane:** 8 `codex-run-v3` manifests dated 2026-08-14/15 UTC, recording **8 `run_started` attempts at 2 xhigh / 6 high** and **8 `run_finished` — 5 at rc 0, 3 at rc 65, 0 at rc 79, zero attempts leaving no `run_finished`**; all attempt 1, no retries; aggregate recorded run time **16,804 s** (≈4 h 40 m), longest `armauth-out` 3,288 s. **Six further Sol reports carry no manifest**, so the span's true Sol implementation/review count is **at least 14** and that aggregate is a floor. **Audit lane:** the eleven-seat fleet (`wf_29eee330-1db`) — **2,411,618 subagent tokens, 1,083 tool uses, 46.7 minutes wall, 11/11 returned**; **nine C-028 refuter runs at Sol xhigh, read-only, `write_scope: []`, `head_start 8937dec`**, of which eight are manifested (aggregate recorded refuter run time **4,828 s**, ≈80.5 min; five of those ran concurrently from the lead shell and all finished within 13 minutes) and one (`A-contract`, relay-launched) left no manifest; plus a **tenth xhigh refuter** (the ordered SINGLE-LENS second reader) launched at verdict recording, completed rc 0 in 472 s, custodied at `15d00d2`. **Adjudication lane:** two cold seats (fresh Fable adjudicator + Opus contract refuter) at ~184k tokens, and two Opus drafting mechanics at ~322k **[UNVERIFIED-BY-MECHANIC — magistrate-dictated; the Agent tool leaves no on-disk spend artifact]**. Magistrate context spend likewise leaves no artifact **[UNVERIFIED-BY-MECHANIC]** — the known custody gap, now flagged in four consecutive entries (C-056, C-057, the T6 report's attestation item 4, and here).

### Source cautions

- **The sealed packet's §2 seat table is STALE against its own §9 and against the verdict.** It still shows L2 READY and the aggregate "1 READY / 10 NOT_READY." The authoritative aggregate is **0 READY / 11 NOT-READY** in `council-verdict.md`; the packet index says so explicitly. A reader quoting the sealed packet's table will get the wrong verdict.
- **The fleet's aggregate coverage (219/253 across eleven seats) is arithmetic over self-nominated denominators and is NOT an audit-coverage figure.** The one denominator adversarially tested went from a claimed 16 to a real 251. Both cold seats ruled every other denominator suspect for exactly this reason. Do not quote 219/253 as coverage; the honest statement is that coverage is unquantified pending the Phase-3 re-enumeration.
- **The manifest gap narrowed but did not close, and it moved.** The pre-fleet span has **six** unmanifested Sol reports — `armauth-lensA-out`, `armauth-lensB-out`, `armauth-delta-out`, `armauth-delta2-out`, `charter-consult-out`, `kbd-power-out` — all review/delta/consult runs, effort tier, duration, WRITE_SCOPE and rc unrecoverable **[UNVERIFIED-BY-MECHANIC]**. In the audit lane the ratio inverts: **8 of 9 refuter runs are manifested**, with only relay-launched `A-contract` missing. The class is therefore not "review runs don't manifest" but "**runs launched from inside subagent shells lose their audit trail**" — the same root cause as the rc-74 worktree-vanishing class above.
- **Fleet effort tiers are recorded but not evidenced.** `packet-index.md` records "5 xhigh + 6 high per charter tiers; zero ultra"; `council-verdict.md` phrases the same set as "5 xhigh + 4 high gating, 2 high seam/basis." The drafting mechanic's earlier reading of the launch script ("seven xhigh and four high") is superseded by both. But **Opus N3 records that per-seat effort tiers and the charter's "no ultra sessions" claim are unevidenced anywhere in the packet** — no per-seat manifest exists, because the seats are Agent-tool subagents **[UNVERIFIED-BY-MECHANIC]**.
- **Two refuter worktrees vanished mid-run and the rulings on them were self-graded.** `refuter-DG-out` and `refuter-L2-out` (and `sol-out-refuter-DG`) lost their trees to harness cleanup; the magistrate ruled the gaps benign and the probes continued read-only on main at the same HEAD. The cold adjudicator accepted this **only because both affected verdicts were adverse to readiness** and stated the counterfactual explicitly: a vanished worktree on a refuter that had *confirmed* a READY would demand replay.
- **Several refuter probes were sandbox-blocked and their conclusions rest on static analysis.** Live `pgrep` was denied (so L9's "~20 matches observed live" exists only in the L9 seat's own run — single-source, flagged S8), stateful L2 tests were blocked, and no network meant **PR #149's CI was never re-queried**.
- **The magistrate refuted a blocker in its own sitting's reference artifact with no independent refuter present** (sweep-B4). The outcome was later verified correct by both cold seats, but the practice is flagged in the record as a "never self-grade" violation, and its closing claim ("the manifest requires NO fix") was rejected.
- **`.status` files remain unreliable as run state** (C-057's finding, unchanged, now with a new instance): `armauth-out.status` and `armauth-fix2-out.status` read `OK` beside manifests recording `ACCEPTANCE_FAILED`, and all three rc-74 refuter `.status` files open with `OK` on their first line.
- **The T6 run report was stale at landing on one item.** Its "Owed as bookkeeping" says the `RUN_STATE` merge count needs correcting from eight to seven — but `18ef535` made that correction roughly eighteen hours *before* the report itself landed at `f99582e`. Harmless, and the exact shape of the staleness class this project keeps catching in other people's branches.
- **The task brief's span was too narrow.** `git log f99582e..HEAD` returns five commits, but T6's declared cutoff is the *overnight arm-author build state* (~2026-08-13 20:40 PT). The true span is **23 commits** (`git log --since='2026-08-13 20:40'`), and the charter ratification, window-gating directive, interaction contract, M-1/M-2, the paper fix and the ED-session script all land between the cutoff and `f99582e`. A reviewer checking this entry against `f99582e..HEAD` will find two thirds of it apparently unsourced; check against `--since='2026-08-13 20:40'` instead.
- **Date naming, third occurrence.** `docs/process_traces/2026-08-14-readiness-charter-consult/` is named by its UTC date; its file is dated 2026-08-13 22:14 PT. The sitting directory `2026-08-15-readiness-council/` happens to agree in both zones (the sitting ran 2026-08-14 23:25 PT → 2026-08-15 07:43 PT), but the sealed packet's own header reads "mechanically assembled 2026-08-14" — the sitting crosses midnight in both calendars, and the two dates are both correct for different parts of it.
- **"Five integration catches"** (RUN_STATE) is a count of five integration-fix *commits* spanning four defect classes; exactly **one** (`2683959`) is a cross-branch union defect. The #149 body's "third time in the record" refers to that one, not to five.
- **The "LIVE real-boot-session acid … to GO on this machine"** claim is attested by `f468b98`'s commit message and by `armauth-lensA-out`'s V2 probe — but the lens-A probe ran at `8915ec1`, *before* the coherent-world variant existed. **No separate artifact for the post-`f468b98` real acid was located** **[UNVERIFIED-BY-MECHANIC]**. The fleet's L8 seat subsequently found the operator-path half of that claim hollow regardless (no producer for the author's inputs).
- **Mutation counts differ between sources.** Lens B reports "killed: 4, survived: 3"; #147's body says "fix round FIX-1..6 (demonstrated mutant kills)" — the fix round's claim about its own mutants, not the same census. Do not add them.
- **The 42-worktree checkpoint figure is time-varying** and reconciles only at its own drafting moment (53 listed − 11 fleet seats = 42).
- **The #146/#147/#148 review records** are cited from their PR bodies, which are self-descriptions written by the executing lane. **#146's "Module 17/17 ×2 interpreters" and #148's "50/50 ×2 interpreters, generators `--check` byte-identical ×3" are body-attested only** — `stale-fix-out` records `verification: 5 pass / 1 fail` and `chain-out` records `7 pass / 1 fail`, neither of which resolves to those totals **[UNVERIFIED-BY-MECHANIC]**.

### C-058 ADDENDUM (2026-08-15, recorded 2026-08-16): what the verdict's own day produced

The entry above closes at the recorded verdict. The **same day** carried the
repair program from ruling to merged code, and the record of that is here rather
than in a new entry: it is the same span, the same sitting's consequences, and
the same instrument. Full session record: `docs/run_reports/2026-08-15-t8-session.md`.

**Verification debt discharged first.** The verdict's Disposition 5 ordered a
second distinct-lens refuter before any SINGLE-LENS claim drove implementation.
It ran, **CONFIRMED all four** (L2-1 refined but standing verbatim, L2-COV-1's
251-test universe re-enumerated exactly, L2-EDQ-1, the terminal-review-trailer
gap), and was recorded as an **ADDENDUM to the verdict rather than an edit of
it** (`15d00d2`). C-058 itself landed at `af7c23b` with its seven UNVERIFIED
markers retained.

**Phase 0 COMPLETE, all custodied under `docs/process_traces/2026-08-15-*/`.**
R1 freeze-evidence lifecycle — content-bound design adopted **with a composed
amendment set the consult had not proposed** (`RE_DERIVABLE`/`EXECUTION_BOUND`
split, changed-set gate, no grandfathering), through its own cold gate
(`193e533`). R2 FROZEN_PLAN identity (`9dd3c34`). R3 P2-006 retired by formal
supersession (`cd4c136`). R4 M-2 execution note (`c0b7068`) plus **the remanded
M-2 gate** (`6760a9b`), where the stricter seat prevailed: engineering core
upheld, but the premise and the universal citation duty **stricken**, the
retirement clause **corrected as unreachable on landed code**, scope **pinned to
three receipt hashes** — and the **non-author packet-assembly protocol** adopted,
curing the cold pairing's original charge that the reviewed party had written its
own charge sheet. Three WO contracts adopted (`05b6b44`, `462f8fd`, `a2d4d04`)
plus the WO-LAUNCH-BINDING F2 lineage-locator mechanism (`5fe977d`), each on a
custodied Sol design consult; the T-0 F4 provenance ruling deferred its
paper-scope half to Ed/advisor (`7b31213`).

**Both post-verdict cold gates are in the record for the same reason — the
paired refuter beat the seat that convened it.** The R1 gate amended the design
it was asked to ratify. **The recorder-race gate rejected the magistrate's own
proposed mechanism, on executed evidence**
(`docs/process_traces/2026-08-15-recorder-race-coldgate/`, verdict `5cdec0c`):
the proposed content-bind was proved **inert** and the fd-identity alternative
**hardlink-defeated** by an uninstrumented race and a silent-success attack the
adjudicator's static series never surfaced; the two seats' failure to converge
on the adjudicator's own alternative was itself the proof that **no caller-side
patch can be atomic** while the callee re-resolves the caller's path. The gate
then produced, none of it the magistrate's proposal: the concurrent local writer
**not** ruled out of model (contract silent ≠ exclusionary), the race registered
as a **limitation** with receipt integrity intact, the real cure re-scoped to a
future authority-plane gate (**WO-RECORDER-GRANT-IDENTITY**), F-5 landed
unconditionally (an S1-table violation **both prior rounds missed**), and an Ed
risk-appetite note surfaced rather than decided. This is the gate shape working
exactly as designed, and the first instance in the record where the rejected
proposal was the convening seat's own.

**Three Phase-1 PRs merged, all D-121-verified at the audited head.** **#150**
`47d2645` — the **WINDOW-COUNCIL-GATE is LIVE on main** (no quiet-mac window
selectable until a READY-candidate verdict) with P2-006 retired per R3, in one
transaction. **#151** `00ec3b7` — close-out blocker L4-B1 cured, the
check-to-grant race carried as a registered limitation. **#152** `a61ac92` — the
T-0 acquisition producer, merged at checkpoint close with head `9e8936a`
re-verified in the same turn and the checkpoint **amended** rather than left
stale (`07d1a78`). WO-LAUNCH-BINDING checkpointed WIP at `345bfbb` (does **not**
cure L8-B7; launch stays NO-GO); WO-CONSUMPTION-EDGE stopped early in its build
for the clear order — its manifest has a `run_started` and no `run_finished`, the
branch carries no commits, nothing was lost, and a fresh relaunch is ordered.

**Process finding minted: the wake-source failure class.** Never end a turn with
zero live background work. The session re-invokes only on background-task
notifications, so a turn ending in a summary plus a promised queue has **no wake
source** and the loop dies until Ed taps. **Two occurrences in T8, hours each:**
the ~7 h relay wedge already recorded above, and a Phase-0 wrap that ended with
"then Phase 1 launches" and nothing in flight. A block boundary is a **launch
point**, not a stop. Recorded as memory rule `turn-end-wake-source-rule`
(2026-08-15); it extends `background-jobs-must-be-tracked` and
`proactive-inflight-polling` from "track what you launch" to "never hold zero
launches," and pairs with the verdict's already-ratified consequence that refuter
fleets get output-keyed liveness checks with a bounded first-checkpoint deadline.

**Open at the addendum's cutoff:** the program is **not** certified complete and
no funded window may be armed; Phases 2–4 stand in the verdict's order; the Ed
batch is owed once Phase 1 nears close, carrying **three risk-appetite calls the
