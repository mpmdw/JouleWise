...
>
> **Addendum 09:25 PDT (record 57):** Codex (gpt-6-astra) is at its usage limit until 2026-09-15 00:52 PDT — every `codex-run-v3` seat fails; use Opus agents (Agent tool, model "opus") for implementation and review, in linked worktrees, and review each diff against its WRITE_SCOPE yourself. The ACCEPTANCE-EPOCH-25G83-01 mechanism is ruled (cold gate 46 + addendum 11) and its seats live on `feat/2026-09-10-epoch-s*` branches with `feat/2026-09-10-epoch-integration` as the integration tree; read the newest records (57 onward) for their state before touching them. The derivation-night runbook does not exist yet; do not arm any night before the mechanism lands, the pre-registration is committed with Ed's affirmative acknowledgment of V3, and the corpus-night runbook is written.
