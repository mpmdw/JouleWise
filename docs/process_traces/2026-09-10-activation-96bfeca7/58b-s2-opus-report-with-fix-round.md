...
| historical/production parse (OBSERVATION layer only — corrected in fix round 1, finding E4) | `test_production_ledger_fixture_parses_and_reserializes_byte_identically` (`tests/fixtures/d117_v2_production/issued/calibration_observation_ledger.jsonl`: 76 rows, zero refusal reasons, canonical re-serialization equals the file bytes) and `test_kindless_historical_open_receipt_reads_as_the_bracket_pair`. **That fixture is 38 `historical-import-v1-reservation` + 38 `historical-import-v1-finalization` observation receipts and contains ZERO rows carrying a `session_id`** (re-counted this round), so it witnesses the OBSERVATION layer, not the session layer. Session-layer byte identity rests on the two witnesses in §Fix round 1 → E4 instead. | — |
...
## Fix round 1 (execution refuter E1–E5)

Verdict addressed: MERGEABLE AFTER FIXES. **No production change.** `git status` after this
round shows exactly one modified path, `tests/test_calibration_ledger.py` (+275 lines, 0
deletions); `joulewise/calibration_ledger.py` is byte-identical to the reviewed head
(sha256 `807f340d92863015d1ccec3efb4ce39cef12fcbcb61bb73834d0d167bf71dda2` before and after
every mutation cut below).

The refuter's diagnosis is correct and worth restating in one line, because it explains why
these three checks were unwitnessed and why they matter: **the writer shadows the reader.**
`claim_bracket_session_slot` and `finalize_bracket_session_slot` refuse an out-of-turn slot
with `SLOT_ORDER_CONFLICT` before a byte is written, and a second open session is refused by
the writer's head-pin check — so no test that goes through the writing API can ever reach the
assembler's own recomputation of `declared[len(finals)]` or its "exactly one open session"
tolerance. Those three checks are the AUTHENTICATING READER's half: the defence against a
ledger file that was hand-edited, truncated, or corrupted between windows. All three new
regressions therefore build receipts DIRECTLY and hand them to the reader.

### E1 — claim-order term (`joulewise/calibration_ledger.py:1656-1670`)

New: `DerivationSessionSlotTests.test_out_of_order_declared_claim_is_refused_by_the_session_assembler`.
A 3-slot derivation session is opened, then TWO claims are forged with
`_new_bracket_session_record` — one for `d01` (whose turn it is, since nothing is finalized)
and one for `d02` (which skips d01's turn). Both are asserted to pass
`_valid_session_receipt_shape` and to name a slot the open receipt actually reserved, with its
reserved `attempt_id`, so every OTHER term of the claim guard is satisfied and only the turn
differs. `_bracket_sessions_and_observations` is called with each: the in-turn list is clean,
the out-of-turn list produces refusal reason **`calibration_ledger_bracket_session_conflict`**
(the assembler has one conflict reason; it does not distinguish order from identity).

### E2 — finalization-order term (`:1674-1690`)

New: `test_out_of_order_declared_finalization_is_refused_by_the_assembler`, same shape. Each
forged finalization reproduces its own reservation exactly — `attempt_id`, `custody_locator`,
`identity_epoch` and `t1_bindings` all copied from the open receipt's slot entry, with real
custody bytes and a real `content_id` — so the four identity terms and the shape check all
pass and only `slot != expected_slot` can fire. In-turn `d01` clean; out-of-turn `d02` yields
`calibration_ledger_bracket_session_conflict`.

### E3 — the "exactly one open session" tolerance (`:401-448`)

New: `test_two_open_sessions_are_not_a_governed_open_bracket_extension`, in two halves.

*Half one (physically realizable).* A second open session is appended straight to the ledger
file, bypassing the writer: its own `session_id`, slots and attempt ids, plus the durable
`append-intent` control row the physical scan pairs with every business row (without that
intent the forged open is discarded as tail residue and never reaches the assembler at all —
found empirically this round). The reader then reports **two** open sessions, and
`is_governed_open_bracket_extension` is False.

*Half two (isolates the tolerance).* Half one alone does NOT witness `len(open_sessions) != 1`,
and this is worth recording: the property's trailing `all(...)` requires every tail row to
belong to the single candidate session, and the second session's rows carry a different
`session_id`, so the predicate already returns False for a different reason. Half two therefore
assembles the same two-open state directly — `dataclasses.replace` on the REAL
`CalibrationBracketSession` (not a hand-rolled double) to make a second open session, and
`replace` on the snapshot to seat both — leaving every receipt in the tail belonging to
session one. In that state the count is the only thing that can refuse, and the test asserts
the predicate is False. The single-open snapshot is asserted True first, as the positive
control.

### Cut results (each new test run alone, then the term it witnesses deleted)

Harness `/tmp/s2_fix1_mutate.py`: apply cut → `python3 -m unittest <single test>` in a
subprocess → restore the file from the in-memory original in a `finally` block.

| Test | Alone, unmutated | Term cut | Alone, mutated |
|---|---|---|---|
| `test_out_of_order_declared_claim_is_refused_by_the_session_assembler` | `Ran 1 test in 0.015s` **OK** | `or slot != expected_slot` deleted from the CLAIM branch | rc=1 **FAILED (failures=1)** |
| `test_out_of_order_declared_finalization_is_refused_by_the_assembler` | `Ran 1 test in 0.015s` **OK** | `or slot != expected_slot` deleted from the FINALIZATION branch | rc=1 **FAILED (failures=1)** |
| `test_two_open_sessions_are_not_a_governed_open_bracket_extension` | `Ran 1 test in 0.015s` **OK** | `len(open_sessions) != 1` → `len(open_sessions) < 1` | rc=1 **FAILED (failures=1)** |

```
PRE  sha256 807f340d92863015d1ccec3efb4ce39cef12fcbcb61bb73834d0d167bf71dda2
CUT  E1 claim-order term: rc=1  FAILED (failures=1)
CUT  E2 finalization-order term: rc=1  FAILED (failures=1)
CUT  E3 exactly-one-open tolerance: rc=1  FAILED (failures=1)
POST sha256 807f340d92863015d1ccec3efb4ce39cef12fcbcb61bb73834d0d167bf71dda2
RESTORED IDENTICAL: True
```

**A methodology trap worth carrying forward.** The FIRST run of that harness reported E2 as
`rc=0 OK` — a false "survives". Cause: the E1 and E2 cuts delete the same 41-byte line, so the
mutated files have IDENTICAL SIZE, and CPython's `.pyc` validation is `(mtime_seconds, size)`.
Both writes landed in the same integer second, so the E2 subprocess imported the E1-mutated
bytecode and the finalization term was never actually cut. Re-running with
`PYTHONDONTWRITEBYTECODE=1` and a cleared `__pycache__` gave the table above, and the E2 cut
was independently re-confirmed in a standalone run. **Any mutation harness that applies
same-size edits in a tight loop must disable bytecode caching**, or it will silently report
survivors.

### E4 — the production-fixture evidence claim, corrected

The refuter is right: `tests/fixtures/d117_v2_production/issued/calibration_observation_ledger.jsonl`
is 38 `historical-import-v1-reservation` + 38 `historical-import-v1-finalization` observation
receipts, with **zero** rows carrying a `session_id` and no session events at all (re-counted
this round). It is byte-identity evidence for the OBSERVATION layer only. The per-change table
row above has been corrected in place. The session layer's byte identity rests on these two
witnesses instead:

**(i) Bracket reservation with the OLD flags writes byte-identical ledger bytes, old tree vs
new tree.** Run by the execution refuter: `scripts/reserve_calibration_window_bracket.py`
invoked with the pre-change flag set on both trees, both writing into the same absolute temp
path, ledger sha256 `21ab2c7a900ed6a606f465c643bb1d08d19dd75481d3154e0d152ef1060453ed` on
both. Cited here as the refuter's result; I did not re-run it (E4 asked me to re-run (ii)).

**(ii) The pre-change test files at `7c4366ec` pass against the NEW library, except the one
SimpleNamespace double.** Re-run by me this round. Method: the two test files were checked out
from `7c4366ec` into their real module positions (so subprocess- and `__file__`-dependent tests
behave normally), run, then restored from an in-memory copy in a `finally` block; both restored
sha256s were asserted equal to their pre-run values. Verbatim tail:

```
PRE  tests/test_calibration_ledger.py 96da84212d1f202042927ab67515da3ad6ddc1aa943b636b0b92076c0259ca35
PRE  tests/test_calibration_ledger_custody.py f27ec1a3f0c756c67d1b33ff9f60426ddec747d56307323e1cead87fe82f932e

--- OLD tests.test_calibration_ledger vs NEW library: rc=0
Ran 72 tests in 3.486s
OK (skipped=1)

--- OLD tests.test_calibration_ledger_custody vs NEW library: rc=1
Ran 29 tests in 3.295s
FAILED (errors=2)
   ERROR: test_readiness_forwards_resolution_mode_to_snapshot_and_state (... CustodyProbeTests ...) (enforcing=True)
   ERROR: test_readiness_forwards_resolution_mode_to_snapshot_and_state (... CustodyProbeTests ...) (enforcing=False)
POST tests/test_calibration_ledger.py 96da84212d1f202042927ab67515da3ad6ddc1aa943b636b0b92076c0259ca35 IDENTICAL: True
POST tests/test_calibration_ledger_custody.py f27ec1a3f0c756c67d1b33ff9f60426ddec747d56307323e1cead87fe82f932e IDENTICAL: True
```

All 72 pre-change `test_calibration_ledger` tests pass unchanged against the new library
(72 = the current 88 minus this seat's 16 new `DerivationSessionSlotTests`). The only two
errors in the whole pre-change corpus are E5's double.

*Caveat on method, recorded because it nearly produced a wrong number.* A first attempt loaded
the old files from `/tmp` via `importlib` under an alias module name and reported
`FAILED (failures=8, errors=1)`. Every one of those was a harness artifact (writer-lease and
other tests that re-invoke themselves as subprocesses by module path). The in-place run above
is the trustworthy one.

### E5 — the SimpleNamespace session double

Already fixed in the reviewed head, and the E4 (ii) run above is its proof: the pre-change
double raises `AttributeError: 'types.SimpleNamespace' object has no attribute 'next_slot'`
at `calibration_ledger.py:5224` (`next_slot = session.next_slot`), twice; the current
`tests/test_calibration_ledger_custody.py:149-153` gives the double `session_kind`,
`declared_slots` and `next_slot`, and the module is green. No further change was needed this
round.

### Six-module sweep after the round (rc captured in a variable, never a pipe)

```
# for m in ...; do python3 -m unittest $m > /tmp/s2fix_$m.log 2>&1 ; RC=$? ; echo "$m RC=$RC ..." ; done
tests.test_calibration_ledger            RC=0  Ran 92 tests in 4.012s   OK (skipped=1)
tests.test_calibration_ledger_custody    RC=0  Ran 29 tests in 3.137s   OK
tests.test_calibration_bracketing        RC=0  Ran 50 tests in 0.242s   OK (skipped=1)
tests.test_calibration_live_three_window RC=0  Ran 23 tests in 1.713s   OK (skipped=3)
tests.test_receipt_oracle                RC=0  Ran 3 tests in 0.220s    OK
tests.test_mint_floor_artifact_generalized RC=0 Ran 83 tests in 24.169s OK (skipped=2)
```

`tests.test_calibration_ledger` is 92 tests, up from 89: the three regressions above.

### E6

No action, as the refuter directed: only the dry-run payload gains `declared_slots` and
`session_kind`; the `--execute` payload is unchanged.

### Not done in this round

No git state was changed (no commit, checkout, stash, reset or push) — the lead commits.
`tests/test_calibration_writer_crash_matrix.py` (233 s) and `tests/test_calibration_exits.py`
(434 s) were NOT re-run this round: this round adds test-only code to one module and touches
no production byte, so neither can have moved. Both were green in the seat's sweep 2 and should
be re-run by CI on the committed head.
