#!/bin/zsh
# Pre-notice block: attempt dir (§1.4a steps 4-5), schedule + boundaries (§1.3), --render-only validation (§1.4), arm census REPORT (§0.6 3b; the binding census runs in 40-arm).
...
def show(label, epoch):
    print(label, datetime.fromtimestamp(epoch).astimezone().isoformat(), int(epoch))
...
show("window end", plan.t0_epoch_s + plan.window_max_s)
...
echo "=== §0.6 step 3b arm census REPORT (real class: idle interactive sessions are foreign)"
set +e
"$PY" -B -m joulewise.arm_census --plan "$STAGED_PLAN" 2>&1 | tee "$ATTEMPT_DIR/arm-census-preflight.txt"
rc=${pipestatus[1]}
set -e
echo "arm census rc=$rc (report only here)"
