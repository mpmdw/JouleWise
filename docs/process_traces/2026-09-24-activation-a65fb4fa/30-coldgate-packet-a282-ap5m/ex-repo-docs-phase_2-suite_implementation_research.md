...
**2026-08-28/30 — D-166 R-3 implementation:** `suite_manifest.v2` admits an exact, validated top-level `benchmark_import` block and exact per-item `scoring` metadata for the `_v6` scored leg; a benchmark import requires scoring on every item and one matching prompt-template identity. Expected answers remain quarantined in a manifest-bound sidecar. `import_fields`, `pair_id`, and `holdout_role` remain deferred, while legacy v1 parsing, serialization, and canonical hashes remain unchanged. This is a targeted successor to the historical P2-010a deferrals in §8, not a retroactive change to `suite_manifest.v1`.

The adopted offline producer/scorer is `joulewise/benchmark_import.py`, invoked by the canonical thin entry point `scripts/gen_gsm8k_scored.py`; it authenticates the pinned 1,319-line / 749,738-byte GSM8K test JSONL by SHA-256 and Git-blob SHA-1, applies the order-independent SHA selection rule at k=8, requires both local Qwen3 mirrors to agree on chat-template bytes, tokenizer bytes, tokenizer-file identity, rendered prompt text, and token IDs with thinking disabled, then writes canonical manifest and quarantine-sidecar JSON. The stall-era monolith `scripts/gsm8k_import.py` and its duplicate test module were deleted after their unique assertions moved to the canonical chain; the sole public checker requires the exact pinned k=8 set, accepts only an exact final `#### N` line, compares canonical rational values, emits the exhaustive `correct / incorrect / truncated / malformed` table (`truncated` means cap hit without a parsable final answer), and labels accuracy as a property of the pinned set rather than a capability claim.

The NEEDS-RULING-01 contract amendment is installed alongside that producer: `joulewise.condition_family_definition.v2` selects the exact suite workload shape `{name, suite_manifest_sha256, suite_item_count, output_cap_tokens, output_policy, repetitions, warmup_runs}` while v1 keeps the integer-pair shape. Analysis-input identity dispatches only on the presence of `suite_manifest_sha256`: suite evidence binds the metadata manifest hash and the uniform manifest budget `item_count × output_cap_tokens`; every non-suite config keeps the prior scalar `requested_tokens == output_tokens` comparison.
