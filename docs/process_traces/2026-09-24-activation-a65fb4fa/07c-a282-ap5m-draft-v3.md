# 07c — A282 AP-5M draft, version 3: the proposed MATH energy-per-correct analysis plan and D-166 addendum (PROPOSAL, gate-ruled text installed)
...
replaces version 2 (record 07b in this folder) completely. It installs the cold-gate ruling on version 2 (record
30/10, below) and fixes the writing defects that ruling's charge and the teaching-quality re-review (record 18)
listed. Read-only everywhere else.
...
with no loop context) has ruled on version 2 (record 30/10); its texts are installed here, and a paired refuter is
checking that ruling in parallel. Ed (the project owner) alone decides adoption, through his answer to question E2
...
items, which outrank consults, which outrank design packets. Among cold gates the later ruling prevails: 30/10,
then 45/21 §7, 45/10, 21/10 with 21/11. Blockquoted text is copied byte for byte from the source named under it
(marked VERBATIM). Texts marked **T-n** are the gate's final texts, pasted byte for byte from 30/10 §3; §5 maps each
...
- **Instrument floor and anchor bound.** Two different error terms come from the instrument. (i) The *floor*: the smallest energy difference the instrument can distinguish from zero for a given window class, published as `floor_gate_j = max(floor_abs_j, floor_cmp_j)` by the calibration manifest (`docs/phase_2/detection_floor.md`, "Floor Artifact Semantics"): `floor_abs_j` is the larger of the largest absolute residual and the prediction bound `t_0.975 · s_r · √(1 + 1/n)` over repeated identical cells; `floor_cmp_j` is the same construction over same-condition A-B-B-A contrast deltas. Floors are calibrated per *window class* (request, phase, level, block …). AP-5M's window class is the block window; its floor row does not yet exist and is a registration prerequisite (row, Floor gate). The published per-phase figures are historical planning context only: an anchor envelope of about 0.7–1.0 J per phase boundary (D-078 clause 11) and operative phase floors of 3.823787 J prefill and 7.377086 J decode gate (D-079 as amended by D-084). No block-window value may be inferred from them. (ii) The *anchor bound* `anchor_j` (`E_clock_anchor_shift_bound_j`): the most energy that can be assigned to the wrong window because the marker clock and the sampler's clock may be offset; it is the only instrument term inside the interval. Under D-078 clause 11 the floor and the anchor bound play separate roles and neither may be dropped as a double count: the floor gates each block window; the anchor bound widens the interval.
  In the floor formulas, n is the number of strict-valid bundles (defined below) in the calibration cell, s_r the
  sample standard deviation of the residuals, and t_0.975 the 97.5 %
  ...
  separate roles of floor and anchor bound. A *residual* is one measurement minus its cell mean; the *prediction bound*
...
- **Group status, three-way test.** Each tested group gets one of four statuses: *8B cheaper* (code E8, letter
  ...
  *three-way test* (S2) gives a direction only when Holm rejects, p gives the direction, and the interval lies
  wholly on that side of 1. `interval_disagrees` flags a group whose Holm test rejected but whose interval still
...
2. **Select counted windows.** Exclude voided windows. A terminal `ceiling_violation` attempt's window is recorded
   but never enters a cell sum (K17). Refuse any block window at or below `floor_gate_j` for the block-window class (row, Floor gate); the refusal is `below_floor` and the window is not counted.
3. **Form each parent's energy per model.** g = the sum of the parent's counted windows for that model; k = the
   ...
   the parent (K13 (a)). Each problem's correctness and tokens come from its counted attempt (§2.1).
...
7. **Point estimate per tested group.** R = (Σ E_8B ÷ Σ correct_8B) ÷ (Σ E_1.7B ÷ Σ correct_1.7B) over the
   group's levels; point bounds with U = Σ k·anchor_j per model (K11).
8. **Bootstrap, B = 20,000.** Per replicate: draw parents within each level of the group, then problems within each
   ...
   add bound u = k·anchor_j·s; form the low-side and high-side ratios (K10, K11; worked in §2.5).
...
*Anchor bound.* With, for illustration only, `anchor_j` = 1 J per window, each model's point bound is U = 5 parents ×
1 window × 1 J = 5 J, giving `point_low` 1.0187 and `point_high` 1.0193 around 1.0190. (Each block window must also
...
A toy level with three parents (A, B, C) of three problems each. `anchor_j` = 1 J; each parent is one window (k = 1).
...
`point_low` 1.4552 and `point_high` 1.4589 (U = 3 J per model), reported beside the interval.
...
per model, per draw:  s = drawn tokens / parent tokens;  e = g * s;  u = k * anchor_j * s;
...
|---|---|---|---|---|---|---|---|---|---|---|
| #1 B (1,1,3) | 7,600 | 1.02703 | 2,850.00 | 1.0270 | 3 | 11,600 | 0.96667 | 1,067.20 | 0.9667 | 2 |
| #2 C (2,3,3) | 7,300 | 0.97333 | 2,735.07 | 0.9733 | 3 | 11,900 | 0.99167 | 1,090.83 | 0.9917 | 3 |
| #3 B (2,3,1) | 7,400 | 1.00000 | 2,775.00 | 1.0000 | 2 | 12,000 | 1.00000 | 1,104.00 | 1.0000 | 1 |
| Sum | | | 8,360.07 | 3.0004 | 8 | | | 3,262.03 | 2.9583 | 6 |

R\* = (8,360.07 ÷ 8) ÷ (3,262.03 ÷ 6) = 1.9221. Low side = ((8,360.07 − 3.0004) ÷ 8) ÷ ((3,262.03 + 2.9583) ÷ 6) =
1.9197. High side = ((8,360.07 + 3.0004) ÷ 8) ÷ ((3,262.03 − 2.9583) ÷ 6) = 1.9246. This replicate adds one to the
...
(illustrative values; the registered values come from the shakedown night as in K12):* δ_upper = 2 J per block per
slot, budget_j = 50 J, P = 5 parents per cell give max_gap = 50 ÷ (2 × 5) = 5 slots; 2.5 ≤ 5, so the roster is
accepted. Under the registered drift model (linear in envelope index, one slope shared by both models) the bias is at
most 2 × 5 × 2.5 = 25 J against a 23,552 J cell (0.1 %).
...
(S7). One recapture is allowed: on a later census-clean night, P3.a is run once by each model as a single, in
...
| Retry and overrun rules *(added)* | K13 (M12 as amended), K14–K16, K8, K22, K23. |
...
dollar signs, a trailing degree or percent sign, a unit word and a leading "x =", parses as an integer, a decimal, a grouped integer (thousands separated only by the TeX forms `{,}`, `,\!` or the thin space `\,`, which the importer removes before parsing), a/b, or `\frac{a}{b}`: `5`, `-\frac{3}{4}`, `0.25`, `10\%` and `1{,}000` qualify; `\sqrt{2}`, `3\pi` and `(1,2)` do not. A reference containing a bare comma, such as `1,000`, is a *plain-comma* reference and is excluded, because a bare comma could separate thousands or list two answers. Eligible: 5,001 − 2 − 954 − 5 = 4,040, by level 381/437, 733/894, 924/1130, 967/1214, 1035/1324 of the
...
candidate cap values, fixed in the registration packet. The "registered window budget" is a number of capture windows (a *window* is what this file calls a night); the value is fixed in the registration packet (O-14). Ed's decision E4 is whether that budget is
acceptable (O-18).
...
**K9 — denominator guard, merge order, multiplicity.** The operative guard is the observed count of §2.1 (T-1) and
the row's "Denominator provenance requirement" (T-2), ruled by 30/10 Q1. The AP-5 wording it replaces for this plan:
...
parent generated zero tokens). Replicates use Python's `random.Random(seed)` with the seed fixed in the registration packet; percentiles are linear interpolation between order statistics. Both are registered so the interval can be rebuilt exactly.
...
*Source: 21/10 §Q2 D5a. VERBATIM; bracket superseded.* The operative text (30/10 T-5):

u = k·anchor_j·s, with k the block's counted windows for that model and s its drawn-token share; point bounds use s = 1. This departs from gate 21 D5a's bracket `(floor_j + anchor_j)` because D-078 clause 11 and `detection_floor.md` assign the floor to a separate gate; D5a's per-replicate share-scaled structure is kept.

In the next carry, E₈ and E₁.₇ are each model's replicate energy and U₈, U₁.₇ its summed bound (now Σ k·anchor_j·s).
...
*Source: 21/10 §Q2 D5b. VERBATIM.* The registered derivation (30/10 Q7, T-10):
...
*Source: 45/21 §7 (P). VERBATIM.* The full precedence (30/10 Q7 (d), T-11):
...
**K19 — ruled constants.** Levels [1, 2, 3, 4, 5]; merge order per S2 `merge`; denominator guard: observed count ≥ 3
correct per model per level or group (T-1); Holm m = 5 per family; α = 0.05 two-sided; minimum parent blocks per cell
...
**K22 and K23.** K22 is the flag recapture bound (T-9, second part; ruled 30/10 Q6):
...
K23 is the rule for retries across the night (T-13; ruled 30/10 Q9), also given in §2.1 "Nights and retries":

Retries and reschedules extend the same night: a "fresh envelope" is appended to that night's schedule up to its registered maximum envelope count. Any block unfinished when the night ends is the terminal refusal `night_exhausted`; its cell is unresolved until the K22 recapture. Envelope indices start at 0 in every night. A level's drift lever is computed within each night over that night's counted windows, and the level's lever is the largest of its nights' levers.

### 2.10 Change log vs version 2
...
group; S7 adds `ceiling_violation_unresolved`; K12, K15, K22 and K23 add `drift_exceeded`, `spread_exceeded` and
`night_exhausted` as NR reasons and terminal states.
...
*Source: 21/11 R6 cure. VERBATIM.* Worked in §2.6 panel 2. A failed check on the night's other blocks (30/10 Q11):
...
## 5. Gate texts T-1 to T-23: where each is installed

Every T-n is pasted byte for byte from 30/10 §3 (for T-5, T-7, T-18 and T-20, the quoted fragments the ruling
dictates). Coverage is total.

| T-n | Ruling target | Installed at in v3 |
|---|---|---|
| T-1 | §2.1 sparse/merge bullet | §2.1 "Denominator guard, sparse, merge, group, pooled" |
| T-2 | row "Denominator provenance requirement" | §2.8 row; "pending O-2" and "(O-2)" deleted (§2.4, S2 reading note) |
| T-3 | K21; row points to K21; D-166 carries it | §2.9 K21; §2.8 row "Inclusion/exclusion"; §4 ruling (9) |
| T-4 | §2.1 floor text | §2.1 "Instrument floor and anchor bound" |
| T-5 | step 2, steps 7–8, K11, §2.5 | §2.2 steps 2, 7, 8; §2.9 K11; §2.3 and §2.5 recomputed with u = k·anchor_j·s |
| T-6 | row "Floor gate" | §2.8 row |
| T-7 | interval term; step 11; K10 | §2.1 bootstrap bullet; §2.2 step 11; §2.9 K10 ("proposed text pending O-4" removed) |
| T-8 | step 10 | §2.2 step 10 |
| T-9 | spread/drift flag term; K22 | §2.1 last Estimation bullet; §2.9 K22; flagged at step 4 |
| T-10 | drift threshold sentences; O-7; K12 | §2.1 drift bullet; §2.9 K12; O-7 closed (§7) |
| T-11 | step 12; K16 | §2.2 step 12; §2.9 K16 |
| T-12 | drift bullet from "If a block's energy shifts" | §2.1 drift bullet |
| T-13 | after the Night term; K23 | §2.1 "Nights and retries (K23)", placed after the drift bullet so that its terms (retry, terminal refusal, drift lever, K22) are built first, with a pointer from the Night term; §2.9 K23; flagged at step 5 |
| T-14 | after the voided/counted windows term | §2.1 "Counted attempt" |
| T-15 | S8 failed check | §3 S8 |
| T-16 | O-8 | §7 O-8 |
| T-17 | S7 source line | §3 S7 |
| T-18 | S6 template rules | §3 S6 "Combined sentence"; "PROPOSED" label deleted |
| T-19 | row "Order/blocking/covariates"; K4 | §2.8 row; §2.9 K4 |
| T-20 | K2 budget sentence; O-18 | §2.9 K2; §7 O-18 |
| T-21 | K1 predicate | §2.9 K1 |
| T-22 | scorer term | §2.1 "Scorer, correct, malformed" |
| T-23 | instrument term, last sentence | §2.1 "Power boundary and sampler" |
...
| 62 | T-1 to T-23 | §5 map | 30/10 §3 | VERBATIM |
...
pseudocode count once each), 23 gate texts (row 62), 6 ADAPTED (rows 33, 63–67). Version 2's faithful excerpt of the
...
**Closed by ruling 30/10** (each with the question that closed it):

- **O-1 (reason for withholding L\*): closed by Q13**, T-17 (S7).
- **O-2 (denominator guard): closed by Q1**, T-1 and T-2: observed count ≥ 3, replacing AP-5's wording for AP-5M
  (part of Ed's E2 adoption).
- **O-3 (claim sentence): closed by Q14**, T-18 (S6).
- **O-4 (interval and draw): closed by Q4**, T-7: percentile-only ends, point bounds reported beside; K10's draw,
  RNG and quantile rule adopted as registration text.
- **O-5 (Holm membership): closed by Q5**, T-8 (step 10).
- **O-6 (flags in merged groups; recapture count): closed by Q6**, T-9 (§2.1 and K22).
- **O-7 (drift threshold): closed by Q7**, T-10 (K12) and T-11 (step 12, K16); drift model by Q8, T-12.
...
- **O-9 (failed recapture check): closed by Q11**, T-15 (S8).
- **O-10 (tails across nights): closed by Q9**, T-13 (K23).
- **O-11 (smoke gate): closed by Q2**, T-3 (K21, and §4 ruling (9)).
- **O-12 (floor and anchor): closed by Q3**, T-4 to T-6. The block-window floor row is a registration prerequisite.
- **O-13 (idle covariate): closed by Q15**, T-19.
- **O-14 (registration numbers): closed by Q16**: envelope length, cap ladder, capture-window budget and seed are left
  to the registration packet, which must be hashed before any test problem runs; the budget unit is T-20.
- **O-15 (problem draw): closed by Q17**, K1 adopted with T-21.
- **O-16 (added row fields): closed by Q18**, kept.
- Not previously an open item, ruled by Q10: which attempt counts (T-14).

**Left to the registration packet (not claim policy):** envelope length, interior, offset and guard margin; the cap
ladder values; the capture-window budget; the seed; the block-window `floor_gate_j` row; δ_upper, budget_j and
`max_gap` computed as K12 prescribes from the shakedown night.

**Open for Ed:**

- **O-17 (Ed, E2).** Adopt AP-5M and the §4 addendum, amend, or reject. Every NEW RULE of 30/10 (K22's one-recapture
  bound, K12's 1 % budget, the observed-count guard's replacement of AP-5's wording, K21's smoke predicates) is text
  for that adoption, not policy. Packet C's proposed choices, narrowed since: (1) a sibling row, not a rewrite of AP-5;
  (2) two Holm families of m = 5, not one of m = 10; (3) rung L2's "n ≥ 5" met by five parents in five envelopes, not
  by amending the ladder; (4) J/token descriptive only; (5) capped = incorrect even when an answer parses; (6) the
  20 % cap-bound threshold and the per-arm cap rule; (8) the residual fit exploratory; (9) claims say "MATH level", and
  "difficulty" appears only where defined.
- **O-18 (Ed, E4).** n per level is chosen mechanically by K2 (c): 128 if the pilot-projected capture fits the
  registered window budget, else 64. Ed's decision is whether the budget is acceptable; planning figures (record 18) are about 12 windows for n = 64 and about 23 for n = 128, at 4–5 windows a day. ("Record 18" in that sentence is record 18i.)
...
Findings the ruling flags without dictating text, and points this installation could not settle:
...
- **Shakedown design.** T-10 requires each model to run one fixed block of pilot problems twice in its first and last
  envelopes of the shakedown night. Packet B's shakedown plan (record 09 of activation 1d3796d5) does not yet include
  those runs; the runner and registration work items must add them, or registration refuses.
...
- **Refuter pending.** A paired refuter is checking 30/10 in parallel; an addendum amending any T-n would require a
  follow-up edit to the places §5 lists.
