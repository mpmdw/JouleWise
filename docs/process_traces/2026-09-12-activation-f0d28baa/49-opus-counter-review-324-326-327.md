...
Diverse lens after records 25/31/33/37/39, 08/18/27, 41/44. Read-only. All
four bases = `origin/main` ace4cc3c.

## PR #324 — A177, 74a547cd (test_controller.py, test_run_campaign.py)

**(a)** Near-smallest; consult 39's 33 cuts cover clauses 1–21. **One clause
has no killer:** only `capture = bounded_captures[0]` (test_controller.py:1677)
is inspected and nothing pins `len(bounded_captures)` —
`assertTrue(bounded_captures)` (:1673) admits any number, so a mutation adding
a *second, paced* bounded capture survives every assertion. Fix:
`assertEqual(len(bounded_captures), 1)` after :1673, or loop the
argv/timeout block. Nit: `100` in `for count in (None, 3, 7, 100)` (:1614) is
a vestige of the 100-sample pin delta re-audit 79 F1 refuted; no cut
distinguishes it from `7`.

**(b)** Not the CI-red class. Continuous pacing: `with patch.dict(os.environ,
...
`super()._run_bounded_capture(...)` (:1657-1661). **Production deadline
...
measured baseline reds this guard, not the defect; it fails loudly with its
own message, so brittle, not silent.

**(c) No host-dependent pin.** Count/interval/timeout are derived and
...
couples to the host. Nit: post-count formula in 3 homes, timeout formula in 2
(powermetrics.py:1468-1470 vs test_controller.py:1688-1689).
...
required `"git"` in literals or a runner callee. Built in memory against
...
`logger.info("init complete")`, `os.system("git status")`. Only three `"init"`
literals exist today (test_calibration_exits.py:2146,3650; git_fixture.py:26),
all legitimately exempt — prospective, not live. Should-fix: gate the
*bare*-`init` rule on the callee resolving to a runner name (`subprocess.run`,
`os.system`, `run`, `execute`, `_run`, `_run_fixture_command`), keeping the
unconditional rule when `"git"` co-occurs or `_shell_git_init` matches; all 13
hygiene fixtures still flag under it.

**(b) Airtight.** `_scope_name` (:52-59) yields fully-qualified
`Class.method`, matched exactly at :183, :190, :213; a rename, a move to
another class, or a nested `def` all miss the key and get flagged —
fail-closed, and their own
`test_named_maintenance_exceptions_do_not_exempt_other_tests` proves it. Nit:
nothing asserts the entries still resolve, so a *deleted* method leaves a
silent dead exception.

**(c) Right home, one stale artifact.** The mechanism has ONE home
(maintenance.py owns `_CommandLiterals`, `_git_init_violations`,
`MAINTENANCE_ON_EXCEPTIONS`); hygiene.py only imports it and adds adversarial
cuts. Acceptable; the smell is cross-module private imports, curable by moving
the census to `tests/git_fixture_census.py`. **Missed should-fix:** the PR
routes `tests/test_identity_pins.py` through `init_git_fixture(root, "-q")`
but leaves `ESTABLISHED_LOCAL_HELPERS["tests/test_identity_pins.py"] =
{"init_git"}` (:29). That entry is dead (no direct `init` call remains) yet
keeps a standing exemption, and
...
tuple through `tests/git_fixture.py`. Drop it.

## PR #327 — armfix, 100dfb2d (test_arm_readiness_dry_run.py)

**(a) No order-dependence or flakiness found.** Sampled once per call before
...
**(b) Scoped, context-managed, no leak.** `self.addCleanup(temporary.cleanup)`
(:277); `with mock.patch.object(time, "monotonic_ns",
return_value=self.origin):` (:280); `with mock.patch.object(time,
"monotonic_ns", return_value=self.origin + 1_000_000_000):` (:298-300); `with
mock.patch.object(time, "monotonic_ns", return_value=self.origin + 8 *
86_400_000_000_000):` (:312-314).

**(c) Duplicated — should-fix.** `self.origin = 1 + 15 * 86_400_000_000_000 //
2` (:279) and `self.origin + 8 * 86_400_000_000_000` (:313) hardcode 7.5 d /
8 d. ONE home: `configs/arm_readiness/d117_row_registry_v2.json` →
...
As written both tests keep passing if the registry horizon drops below 8 days.

## Merge-ability (#324, #325, #326, #327)

Pairwise **disjoint** (`git diff --name-only ace4cc3c..HEAD`): #324
`tests/test_controller.py`, `tests/test_run_campaign.py` · #325 (43c1ce95)
...
against each other worktree's `tests/`: `{}` for a177, a184, armfix, gitfix;
the census is per-file, so per-file clean over the union implies union clean —
no interaction today. **Integration replay must show**
...
- **#324 — MERGE.** Queue (non-blocking):
  `assertEqual(len(bounded_captures), 1)`; comment the `count >= 26` floor.
- **#326 — MERGE.** Queue (non-blocking): drop the dead
  `ESTABLISHED_LOCAL_HELPERS["tests/test_identity_pins.py"]` entry; re-gate
  the bare-`init` rule on a runner callee.
...
- wt-a177 `test_controller` + `test_run_campaign.IdleAdmissionCoreVerdictTests`
  → `Ran 147 tests in 154.715s` / `OK`, EXIT=0
- wt-gitfix `test_git_fixture_maintenance test_git_fixture_hygiene
  test_identity_pins` → `Ran 54 tests in 16.452s` / `OK`
- wt-armfix `test_arm_readiness_dry_run test_arm_readiness_evidence_t0
  test_arm_readiness_lifecycle` → `Ran 152 in 943.659s` / `OK (skipped=1)`,
  EXIT=0; `test_arm_readiness_integration` → `Ran 13 in 256.033s` / `OK`,
  EXIT=0
- census probes: `_git_init_violations` imported from wt-gitfix over synthetic
  trees in a `/tmp` TemporaryDirectory (no worktree written) — 8 cases, §326(a)
- cross-tree census `_git_init_violations(<wt>/tests)` — gitfix, a177, a184,
  armfix → `{}` ×4
- `git diff ace4cc3c..<head>` / `--name-only` ×4; `git status --short` ×4 → all
  empty; reads of powermetrics.py, arm_readiness.py,
  d117_row_registry_v2.json, fake_powermetrics_process.py, records 25/27/37/39/44
