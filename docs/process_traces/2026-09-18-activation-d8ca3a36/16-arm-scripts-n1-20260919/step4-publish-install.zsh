...
"$PY" -B "$ARM_SCRIPT_DIR/step4a-successor-evidence.py"
echo "== publication"
"$PY" -B - <<'PY'
import json,os,time
from pathlib import Path
from joulewise.arm_retry import successor_arm_allowed
...
evidence=json.loads((d/"successor-evidence.json").read_text())
...
r=successor_arm_allowed(time.time(), context, n,
    evidence["predecessor_result"], evidence["harvested_receipt"], evidence["delivery"])
print("successor_arm_allowed:", r, flush=True)
if not r.allowed:
