...
try:
    prepared = []
    epoch = int(time.time())
    for day in ('20260917',):  # BENCH DEVIATION 2026-09-18: 09-16 root RETAINED (it opened a ledger session; NIGHT_HANDBACK §Next lane; precedent: the 09-17 arm proceeded with it discoverable, A230)
        ...
        if not sums.exists():
            sums = Path(str(archive) + '.SHA256SUMS')  # BENCH: the harvest wrote the sums as a sibling file
...
# BENCH DEVIATION: exactly the retained 09-16 root may remain discoverable (see comment above)
(( ${#remaining} == 1 )) && [[ "${remaining[1]}" == /Users/edr/night-custody/d079-epoch-25g83-derivation-n1-20260916/night_plan.json ]] || exit 3
print -- 'STEP0 OK: 09-17 plan root retired; 09-16 root retained (session opened); clones untouched'
