...
## Addendum after delta re-audit round 6 (trace 18): S-1, S-3, S-4 CURED; S-2 narrowed

Residual F2 (round 6): a REPLACEMENT CLI tick (the resident supervisor died after spawning the magistrate) that finds a durable `HOLD_UNSAFE` does not adopt the surviving child and drain it. Specification **S-2b**: on every tick, if `state.json` records a live session (pid + start_time still match) and the tick's decision is any `HOLD_UNSAFE`, the tick adopts that session (records `resident_adopted{pid,start_time,activation}`) and runs the drain ladder exactly as a resident supervisor would (request → TERM after `STOP_COOPERATIVE_S` → KILL after `STOP_TERM_GRACE_S`), persisting the ladder stage in `state.json` so successive launchd ticks continue it rather than restart it; a recorded session whose start_time no longer matches is `already_gone`, never signalled. Tests: unit (dead supervisor + live recorded session + malformed plan → adoption + ladder continuation across two ticks with pinned constants) and CLI (stub resident recorded in state.json, plan truncated, `tick` without a supervisor → `resident_adopted` and the first ladder event). The auditor's same-signature statement (YES) is carried to packet 21 Q-SIG alongside round 5's.

## Magistrate apex read of the final head 3f6b6282 (2026-09-04) — one design-level finding, held for the Opus counter-review

Read: `is_retired_v1_plan`/`load_plans` (shape-based, fail-closed: correct), `decide`, `tick`/`main`, `handoff_inventory`, `adopt_recorded_session_for_drain`, `joulewise/night_plan_writer.py`, and `tests/test_magistrate_watchdog_cli.py` in full.

**A-1 (should-fix unless Opus shows it is covered): hold-drain timing is not bounded by an active plan's stand-down phase.** In `decide`, `snapshot.errors` returns `HOLD_UNSAFE` before the active-plan/STANDDOWN branch. With a VALID armed plan A inside its last 25 minutes and a torn sibling file (half-written future plan B), the resident is drained by the cooperative ladder (REQUEST now, TERM +540 s, KILL +60 s) — and when no supervisor is resident, one `supervisor.step()` per 300 s launchd tick — so the KILL can land after t0−15 min, inside the runbook's 10-minute idle. Cure: the drain ladder stage is `max(cooperative stage, standdown_phase(min active parseable plan))`, and a replacement tick under hold with an active plan in TERM/KILL phase signals that phase immediately. Test: A active at t0−12 min + torn sibling + recorded resident → first tick emits TERM (not REQUEST); at t0−15 or later → KILL.
