...
`scripts/night_chains/calibration_derivation_only.zsh` needs thirteen more variables and
...
observed_plan_sha256="$(/usr/bin/shasum -a 256 "$PLAN" | /usr/bin/awk '{print $1}')"
[ "$observed_plan_sha256" = "$PLAN_SHA256" ] || route_refuse 'frozen plan bytes do not equal the arm-time digest'
...
( cd "$REPO" && /usr/bin/shasum -a 256 --status -c '/Users/edr/night-custody/derivation-20260912/chain.zsh.chain-source.sha256' ) || route_refuse 'tracked derivation chain bytes do not match the arm-time digest'
