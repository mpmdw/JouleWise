...
| A2 routing addendum — superseded 2026-09-08 by [the plan-derived preflight contract](SHAKEDOWN-G2-RUNSHEET.md#plan-derived-measurement-variables) | The preceding historical A2 contract is retired: `preflight.sh <night_plan.json>` takes one absolute v2 plan filename, derives `MEASUREMENT_ROOT` / `MEASUREMENT_HEAD` and `<measurement_root>/.venv/bin/python`, sets `PYTHONPATH` to that root, and refuses checkout HEAD mismatch; inherited coordinates and the source interpreter are no longer inputs. | `preflight.sh`; [current desk routing commands](SHAKEDOWN-G2-RUNSHEET.md#phase-a--desk-preflight) |
