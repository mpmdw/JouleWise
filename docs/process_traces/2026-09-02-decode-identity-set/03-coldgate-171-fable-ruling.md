...
## Your deliverable
You are the COLD FABLE INSTANCE — fresh session, no loop context. RULE on the
draft: UPHOLD / UPHOLD WITH AMENDMENTS / REVERSE, on the draft's own terms
(D-044, D-131, D-165, U8, fail-closed). Answer Q-A, Q-B, Q-C in the draft.
For each amendment give the exact replacement text. Say explicitly whether
the cure is a soundness matter (physics/evidence/pre-registration) or an
engineering choice — D-161 forbids fail-closed machinery for operator-only
adversaries. Write your ruling to
<scratchpad>/out/182-coldfable-171.md
(under 120 lines, file:line for every factual claim), and return a 10-line
summary.
