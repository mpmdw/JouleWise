...
**UPDATE 2026-09-12 00:40 PDT (night courier session, pid 93256, launched by the night driver at 00:30:17; the watchdog held `HOLD_CENSUS` on it, so no magistrate was alive) — rehearsal-20260912 FIRED, HARVESTED, RETIRED.** Launchd started the driver 00:30:04 PDT under the pinned Python 3.13 at H′ `a7d1eb88`; gate verdict `REHEARSAL_ONLY`; stub chain exit 0; `result.json` `REHEARSAL_ONLY`; receipt C1/C3/C4/C5 PASS with measured keys, C2 `NOT_APPLICABLE` (`no_pack_by_design`), refusal null; `launchd.night.err` EMPTY; results branch `night-results/20260912` @ `e657f30f` (9/9 artifacts `cmp`-identical to custody); courier email `1a09487237fa6be2` sent 00:31 and read back (label SENT); watchdog alive (age 276.5 s FENCED at courier launch). **Ruling 06 C-7: every clause MET → NIGHT-REHEARSAL-01 item 6 MET** (`docs/process_traces/2026-09-12-courier-rehearsal-20260912/01-rehearsal-20260912-harvest-record.md`). **C-8 on disk** (record 02): both agents uninstalled FROM the stub checkout 00:35:23 (rc 0), stub worktree removed 00:37:36, plan root moved out of discovery 00:37:38 to `/Users/edr/night-archive/rehearsal-20260912-plan-root-retired-1789198658` (the `rm -r` was refused by the harness permission classifier; bytes identical to the verified 496 MB archive at `/Users/edr/night-archive/rehearsal-20260912-harvest-1789198485/`); `launchctl list` free of `com.joulewise.night*`; `find ~/night-custody -maxdepth 2 -name night_plan.json` EMPTY. **NOTHING ARMED; frozen-checkout list = canonical only.** Kernel row NIGHT-REHEARSAL-01: event REHEARSAL-20260911-HARVESTED satisfied; items 1/2/3/5/6 MET, item 4 (stage-1 email before the first DIAGNOSTIC_NO_PACK arm) OPEN. NEXT EXACT ACTION (first magistrate activation after this courier exits; the watchdog relaunches on an empty census): re-verify no NO on thread `1a0800cdb282c3f1` and no directive issue → equivalence-night install 03:00–06:30 PDT 09-12 for t0 02:56 09-13 per `docs/phase_2/derivation_night_runbook.md` §0–§1.5 (runbook 68 / runbook 99 rev 5), fresh checkout at its own `measurement_head`, both agents installed FROM it, stage-1 email (item 4) on the notice thread BEFORE arming; if the 03:00–06:30 span is missed, do NOT arm off-convention — record the miss and take the next 03:00–06:30 span. Ed may delete the retired plan-root copy above at leisure.

**UPDATE 2026-09-12 01:02 PDT (headless activation `b58fb582`, pid 94352, attempt 25, launched 00:40:45 after the courier's `HOLD_CENSUS` cleared).** Launch email `1a09492c4ac317b0` on thread `1a0800cdb282c3f1`; notice.ack written; pending notice `transition-103-hold_census` acknowledged (the night's own driver/courier); no directives, no stand-down request; thread re-read (METADATA_ONLY): no inbound from Ed since 2026-09-10 23:05Z, so no NO. Records 00/01 in `docs/process_traces/2026-09-12-activation-b58fb582/`. NOTHING ARMED. THIS COMMIT IS H for the equivalence night `d079-epoch-25g83-derivation-n1-20260913` (DIAGNOSTIC_NO_PACK, t0 2026-09-13 02:56:00 PDT = 1789293360, window 9000 s, install today 03:00–06:30 PDT): handback rewritten, inventory row `JouleWise-measurement-20260913-derivation` appended, pre-registration's five commit-time fields filled (DD 10, MLX 0.31.2, pin 76/08456d50…94d7, chain b8bf5b0a…ac8cf), inventory pin test raised to five deployments (Opus verification, record 01). Focused suites OK (night_gate, issuer, gen_derivation_night, rehearse_t0_unattended, arm_readiness{,_schemas}, t0_rehearsal, docs_freshness, magistrate_watchdog, gen_state); `gen_state --check` rc 0. NEXT EXACT ACTION: CI green at H → `/tmp/magistrate-b58fb582/20-clone.zsh` (fresh clone `/Users/edr/JouleWise-measurement-20260913-derivation` at H, venv from the lock, 76-record ledger restored + authenticated) → `30-desk.zsh` (runbook §0.3 check rc 3 with the pre-registered sampler line `match`, §0.8 desk inputs, staged plan at `/Users/edr/night-plan-staging/<PLAN_ID>/night_plan.json`, wrapper emitted + `--verify` + `zsh -n`, driver preflight) → stage-1 notice on thread `1a0800cdb282c3f1` (item 4) → `notice-evidence.txt` → directive re-check → `40-arm.zsh` inside 03:00–06:30 (begin by 06:05) → arm record 02 + evidence dir + this pointer → push → EXIT. If this activation dies first: nothing is armed until `40-arm.zsh` step 5 runs; the successor repeats from this line (remove `/Users/edr/night-plan-staging/<PLAN_ID>` and `/Users/edr/night-custody/<PLAN_ID>` only after confirming no `night_plan.json` was published there, and re-cut the clone if it exists — never rename or fast-forward it). Never install after 06:30; a missed span is recorded and the next 03:00–06:30 span taken.

**UPDATE 2026-09-12 03:05 PDT (activation `b58fb582`) — ARMED.** `d079-epoch-25g83-derivation-n1-20260913` (DIAGNOSTIC_NO_PACK, the epoch-equivalence night of issue 316 / pre-registration revision 2) installed 03:00:23 PDT from `/Users/edr/JouleWise-measurement-20260913-derivation` at **H = `f90cb8c016662f8af6faa73d905fc472443432ab`**; **frozen triple `(d079-epoch-25g83-derivation-n1-20260913, /Users/edr/JouleWise-measurement-20260913-derivation, f90cb8c016662f8af6faa73d905fc472443432ab)`**; plan at `~/night-custody/d079-epoch-25g83-derivation-n1-20260913/night_plan.json` (sha256 a6e83d14…e2131); wrapper `chain.zsh` sha256 569daeda…df45 pinning tracked chain b8bf5b0a…ac8cf; desk inputs b8a1094c…b607 / 8dcdfb00…9d98; pre-registration at H 84b820b9…0b5c; agents `com.joulewise.night` (02:56) + `com.joulewise.night.deadman` (07:00) loaded, `RunAtLoad` false, driver interpreter = the clone venv Python 3.13.1; CI at H green (run 34682174685). t0 2026-09-13 02:56:00 PDT (1789293360), window 9000 s, courier deadline 05:31:00, dead-man 07:00 09-13. Stage-1 notice (NIGHT-REHEARSAL-01 item 4) `1a094a59a84b53be` on thread `1a0800cdb282c3f1` sent 01:04 BEFORE publication; thread re-read 02:58 (no NO; Ed's last inbound 2026-09-10 23:05Z); directives `[]`. Arm record `docs/process_traces/2026-09-12-activation-b58fb582/02-equivalence-night-arm-record.md` + `02-arm-evidence/`. This activation EXITS now (no own background work; the watchdog is the wake source). NEXT EXACT ACTION (any activation alive today before 02:31 09-13): NO night work — observe the 07:00 09-12 dead-man stand-down line in `<night root>/night.log` if alive after 07:00, desk work only, exit on the watchdog's 02:31 request; never touch the frozen triple. (First activation after the night, earliest ~05:35 09-13 after `courier.sent`): harvest per runbook §2.0–§2.4 (verify wrapper sidecar first, preserve custody byte-exact + lstat inventory), then §2.5: `epoch_equivalence_check.py` twice, compare records, take exactly the one outcome action (INCONCLUSIVE → one more night; PASS → D-102 continuation addendum then real G2-a; FAIL → revision 1 nights 2/3); uninstall both agents FROM the clone after completion; RETAIN the clone and night root (production custody). A NO from Ed on the thread before t0 → uninstall FROM the clone (`--hour 2 --minute 56 --uninstall`), move the plan root out of discovery, record.

**UPDATE 2026-09-12 03:51 PDT (headless activation `f0d28baa`, pid 22790, attempt 27, launched 03:46:04 after `b02193d2` exited usage-exhausted mid-seat).** Launch email `1a0953b6f2c742bf` on thread `1a0800cdb282c3f1`; notice.ack written; `notice_pending` `[]`; directives `[]`; no stand-down request; thread re-read (no inbound since 2026-09-10 23:05Z, so no NO). **Night untouched:** frozen triple re-verified (`f90cb8c0` at `/Users/edr/JouleWise-measurement-20260913-derivation`, both night agents loaded); t0 02:56 09-13; exit on the watchdog's 02:31 request. Records in `docs/process_traces/2026-09-12-activation-f0d28baa/` (branch `bookkeeping/2026-09-12-activation-f0d28baa`). Desk seats (Astra high, `codex-run-v3`, scripts `/tmp/magistrate-f0d28baa/seat-0{1,2,3}.sh`): seat-01 = A184 RECOVER-SESSION-REFUSAL-WINDOW-EXHAUSTED-01 **resume+verify** over the inherited dirty draft in `/Users/edr/code/JouleWise-wt-a184` (brief 01 → report 02; contract change → paired refuters before PR); seat-02 = A177 FIXTURE-SENTINEL-CONTROLLER-01 fresh in `/Users/edr/code/JouleWise-wt-a177` (brief 03 → report 04); seat-03 = GIT-FIXTURE-MAINTENANCE-SWEEP-01 in `/Users/edr/code/JouleWise-wt-gitfix` (branch `fix/2026-09-12-git-fixture-maintenance-sweep`, brief 05 → report 06). NEXT EXACT ACTION: harvest each report → refuters (A184 paired; A177 + gitfix single) → fix rounds with delta re-audit → PRs → merge under the gate → kernel rows to done + `gen_state` → this pointer → push. If this activation dies first: the successor inspects the three worktrees' dirty state before touching anything and resumes from the reports if present (`*.status` DONE) or relaunches the seat otherwise; never touch the frozen triple.

**UPDATE 2026-09-12 05:08 PDT (activation `f0d28baa`, desk work in progress; night untouched).** Three PRs open, all gated in `docs/process_traces/2026-09-12-activation-f0d28baa/` (branch `bookkeeping/2026-09-12-activation-f0d28baa`): **#324** A177 `fix/2026-09-12-fixture-sentinel-controller` @ 8a5d1169 (landing c85a171d was CI-RED on both interpreters — whole-run sleep stress starved admission on hosted Linux; fix round 1 scopes the stress to the bounded sentinel, report 31; delta re-audit 33 RUNNING; CI on 8a5d1169 RUNNING); **#325** A184 `fix/2026-09-12-recover-window-exhausted` @ 43c1ce95 (code clean on paired refuters 13/14; the runbook paragraph hit rule 11 — two consecutive blockers in one paragraph — so COLD GATE 28 ruled the text, Opus pairing refuter 12 AMENDED it, magistrate synthesis 11 adopted the amendment + §8 row; applied verbatim at 43c1ce95); **#326** gitfix `fix/2026-09-12-git-fixture-maintenance-sweep` @ 87039749 (refuter 08 → fix 16 → delta 18 → fix 20 → delta 27 clean, zero surviving cuts). Opus counter-review 25: MERGE/MERGE (A177, A184 at earlier heads); Fable diff gate record 29. Integration tree `/Users/edr/code/JouleWise-wt-integ-f0d28baa` @ 6c5c602c = origin/main + 8a5d1169 + 87039749 + 43c1ce95; replay 2 RUNNING (`/tmp/magistrate-f0d28baa/replay-2-6c5c602c.log`, `REPLAY_RC=` line at the end). Leaked orphan fixture pid 22701 (from b02193d2's killed seat) and two abandoned replay-1 workers killed; census to re-check before exit. NEXT EXACT ACTION: harvest delta 33 → (fix round 2 only via consult if same signature) → replay 2 result → gate ledgers on the three PR bodies (12 rows: RUN <bookkeeping sha or path>, row 12 = PR head sha) → CI green → merge #324/#325/#326 → post-merge cross-unit review → kernel rows FIXTURE-SENTINEL-CONTROLLER-01, RECOVER-SESSION-REFUSAL-WINDOW-EXHAUSTED-01, GIT-FIXTURE-MAINTENANCE-SWEEP-01 → done + `gen_state` → RUN_STATE T38n → this pointer → push. If this activation dies: successor inspects the three worktrees (all committed and pushed; nothing dirty expected) and resumes from this line; never touch the frozen triple.

**UPDATE 2026-09-12 06:50 PDT (activation `f0d28baa`; night untouched).** Four PRs, all at final heads pending the last gates: **#324** A177 @ 6714ce96 (CI-red round 1 → delta 33 → round 2 → delta 37 → rule-11 consult 39 → delta 46 → COLD GATE 50 on the isolation rule's scope over terminal oracles: ruling 10 + Opus pairing 12 AMEND + synthesis 11 → applied; delta 54 CLEAN, same-signature NO); **#325** A184 @ 43c1ce95 (cold gate 28 applied; clean); **#326** gitfix @ 87039749 (delta 27 clean); **#327** armfix @ 61872ccf (new lane ARM-READINESS-FIXTURE-CLOCK-ORIGIN-01 from root cause 41: fixture authored evidence at monotonic origin 1 ns + 7-day horizon → hosts awake > 7 days read it expired; refuter 48 clean; Opus 49 fix-first applied — horizon read from the registry; delta 56 RUNNING). Opus counter-reviews 25 + 49; Fable diff gate 29. Integration tree `/Users/edr/code/JouleWise-wt-integ-f0d28baa` rebuilt at the four final heads; replay 4 RUNNING (`/tmp/magistrate-f0d28baa/replay-4-<head>.log`; replays 2/3 abandoned as stale — replay 2 record 42: only the seven pre-existing arm-readiness bench refusals, cured by #327). NEXT EXACT ACTION: delta 56 → replay 4 tail into a record → gate ledgers on the four PR bodies (rows RUN <bookkeeping sha|path>, row 12 = PR head) → CI green → merge #324/#325/#326/#327 → post-merge cross-unit review → kernel rows (FIXTURE-SENTINEL-CONTROLLER-01, RECOVER-SESSION-REFUSAL-WINDOW-EXHAUSTED-01, GIT-FIXTURE-MAINTENANCE-SWEEP-01 → done; ARM-READINESS-FIXTURE-CLOCK-ORIGIN-01 → registered+done) + `gen_state` → RUN_STATE T38n → this pointer → push. Never touch the frozen triple; exit on the watchdog's 02:31 request.

**UPDATE 2026-09-12 07:45 PDT (activation `f0d28baa`) — FOUR PRs MERGED; NIGHT UNTOUCHED.** main `a03a1b8a` = ace4cc3c + #326 (bdf3bf4a) + #325 (f2856ee3) + #327 (35dea9e4) + #324 (a03a1b8a); every ledger 12/12 RUN, CI 19/19 on each head, replay 4 record 58 (6047 tests, rc 0). Post-merge cross-unit review 60 RUNNING on main; main CI RUNNING. Kernel closed (three rows removed, four completed-queue rows; `gen_state --check` rc 0); RUN_STATE T38n; README blurb refreshed. Frozen triple re-verified after the merges (`f90cb8c0`, both night agents loaded). NEXT EXACT ACTION: harvest review 60 → merge this bookkeeping branch into main (docs only) → prune this activation's scratch worktrees (wt-a184-ref, wt-coldgate-a184, wt-coldgate-a177, wt-rc-arm, wt-integ-f0d28baa, wt-postmerge) → if usage remains, one more desk lane by kernel rank (never the two ruling-first rows) → exit on the watchdog's 02:31 09-13 request. Never touch the frozen triple.

**UPDATE 2026-09-12 07:58 PDT (activation `f0d28baa`).** Bookkeeping landed on main (37001624) and put main RED for ~12 minutes on `tests.test_gen_state.TestRefreshedStateFidelity.test_exact_live_id_set` (the test pins the kernel's live-ID set and count; closing three rows requires the matching edit — the docs-test batch had reported it before the push and the pipeline masked the exit code). Cured at b2606955 (175 → 172 with the ledger comment); `tests.test_gen_state` OK at the bench; main CI on b2606955 RUNNING. Lesson for the kernel-closure step: run `tests.test_gen_state` (not only `gen_state --check`) before pushing a closure. Scratch worktrees pruned; night untouched. NEXT EXACT ACTION: main CI green → next desk lane (CI-TRIM-01 / DOCS-THIN-01 resume notes or TEST-SPEED-01 via Opus hill-climb) or idle-hold; exit on the watchdog's 02:31 09-13 request.

**UPDATE 2026-09-12 08:35 PDT (activation `f0d28baa`).** main CI green at 0dc0668c. CI-TRIM-01 resumed per its note: main merged into `chore/2026-09-10-ci-trim`, Fable review 61 (FIX-FIRST), fix round 1 at 558a9054 (cancel only on pull_request; docs-resident code = code), docs-only skip PROVEN LIVE (run 34701979541: 1.4 runner-min, matrix skipped), draft PR #317 body rewritten with the two decisions for Ed (3.14 half of the matrix; `pr-fast` deletion vs the ratified TEST-SPEED-01 tier); PR stays DRAFT — the magistrate does not merge a CI-shape change that amends a ruled lever. CI on 558a9054 RUNNING. DOCS-THIN-01 NOT resumed (376 renames; its note says never merge without Ed + Fable; deferred to a session with usage headroom). NEXT EXACT ACTION: PR #317 CI green → note it in the stand-down email; then idle-hold with hourly polls (no more desk lanes without headroom) → exit on the watchdog's 02:31 09-13 request. Night untouched.

**UPDATE 2026-09-12 09:05 PDT (activation `f0d28baa`).** DOCS-THIN-01 resumed per its note: main merged into `chore/2026-09-10-docs-thin` (zero conflicts), Fable review 65 (FIX-FIRST: ten unrepaired links, two note claims), fix round 1 (report 67), DRAFT PR #329 opened for Ed's decision (merge the 309-rename archive as is / go further / paper-directory links); full-suite replay on the docs-thin tree RUNNING (`/tmp/magistrate-f0d28baa/replay-docsthin-29dbc537.log`; the result line goes into the PR body). PR #317 (CI-TRIM-01) is draft-ready for Ed's two decisions, CI 18/18 green at 558a9054. The magistrate merges neither. NEXT EXACT ACTION: docs-thin replay tail → PR #329 body → idle-hold (hourly heartbeat polls; no further desk lanes without usage headroom) → exit on the watchdog's 02:31 09-13 request; stand-down email lists the two draft PRs and their decisions. Night untouched.

**UPDATE 2026-09-12 09:40 PDT (activation `f0d28baa`) — IDLE-HOLD.** Dead-man observed (read-only): `night.log` line `2026-09-12T07:00:04 dead-man fired before the night's completion epoch 1789302660; standing down` — the expected no-op for the day before t0; both night agents remain loaded, frozen triple `f90cb8c0` intact, clone clean. DOCS-THIN-01 replay clean (record 68; PR #329 body updated; CI 18/18 green). Progress email to Ed `1a0967463c5b72ec` on the notice thread with the two draft-PR decisions (#317, #329). No codex children alive; census clean. From here: foreground idle-hold cycles (heartbeat refresh, stand-down/STOP poll, directive check; thread check hourly); exit within nine minutes of the watchdog's 02:31 09-13 request. Successor after ~05:35 09-13: harvest per `docs/phase_2/derivation_night_runbook.md` §2 → §2.5 verdict → exactly one action.

**UPDATE 2026-09-12 12:48 PDT (activation `f0d28baa`, idle-hold).** Census observation (not mine, not touched): an interactive `claude` session (pid 24974, started ~11:52 PDT) is running a Paper-N lane in `/Users/edr/code/JouleWise-wt-paper-n` (branch `feat/2026-09-12-paper-n`, Astra seat alive under it). It is Ed's or Ed-launched; the magistrate leaves it alone. Heads-up sent to Ed by email: any interactive session and its seats must be closed before the plan span (02:31 09-13) or the night's own census refuses t0. No directives; no comments on #317/#329; night untouched (`f90cb8c0`, both agents loaded).

**UPDATE 2026-09-12 13:05 PDT (activation `f0d28baa`).** GMAIL UNAVAILABLE: the Gmail MCP session expired at 12:54 PDT (two sends failed). The intended heads-up about the interactive Paper-N session is preserved verbatim at `/Users/edr/night-custody/magistrate/intended-email-f0d28baa-1254.txt`; the 09:30 progress email (`1a0967463c5b72ec`, the two draft-PR decisions) did go out. The stand-down email will be retried at the request; if Gmail is still down, the stand-down message is preserved the same way and this pointer says so. Idle-hold continues.

**UPDATE 2026-09-12 22:55 PDT (activation `f0d28baa`, idle-hold).** Gmail still down (expired since 12:54); the interactive-session heads-up went out as GitHub notice issue #333 instead (the interactive `claude` pid 24974 / Paper-N lane is still alive at 22:42; the plan span starts 02:31). No directives, no PR comments, census otherwise clean, night untouched (`f90cb8c0`, both agents loaded). Holding until the watchdog's 02:31 request.

**UPDATE 2026-09-13 02:32 PDT (activation `f0d28baa`) — STANDING DOWN on the watchdog's request** (`standdown.request` at 02:31:07, plan `d079-epoch-25g83-derivation-n1-20260913`, exit within 540 s, t0 02:56:00). Night untouched: `f90cb8c0`, both agents loaded, clone clean at every hourly check. No codex children or background processes of this activation alive; every worktree of this activation clean and pushed. Gmail MCP still expired: the stand-down message is preserved at `/Users/edr/night-custody/magistrate/intended-standdown-email-f0d28baa.txt` (a send is attempted at exit; if it fails, that file is the record) and the 12:54 heads-up at `intended-email-f0d28baa-1254.txt`; the interactive `claude` session pid 24974 (Paper-N) was STILL ALIVE at 02:05 — if it survives to t0 the night's census may refuse; notice issue #333 is open for Ed. Open for Ed: draft PRs #317 and #329. NEXT EXACT ACTION (first activation after ~05:35 09-13, once `courier.sent`): harvest per `docs/phase_2/derivation_night_runbook.md` §2.0–§2.4 (verify the wrapper sidecar first; preserve custody byte-exact with an lstat inventory), then §2.5: run `epoch_equivalence_check.py` twice (clone + a second checkout at the same head), compare records, take exactly the one outcome action (INCONCLUSIVE → one more night; PASS → the D-102 continuation addendum, then real G2-a; FAIL → revision-1 nights 2/3); uninstall both agents FROM the clone after completion; RETAIN the clone and night root. If the night refused at t0 (census), record the refusal reason from `night.log` and re-arm through NIGHT_HANDBACK only after the interactive session is gone.

**UPDATE 2026-09-13 06:00 PDT (activation `c5048879`) — NIGHT REFUSED AT t0; HARVESTED; UNINSTALLED; NOTHING ARMED.** `d079-epoch-25g83-derivation-n1-20260913` fired 02:56:02 and was REFUSED (`night_refused_agent_present`: pid 24974 Ed's interactive Paper-N session + its Codex MCP servers + the ChatGPT app's Codex helper); no chain, no session, nothing captured; courier `1a09a3319d602a37`; results branch `night-results/20260913`. Harvest record `docs/process_traces/2026-09-13-activation-c5048879/01` (sidecar/`--verify` OK, lstat inventory, byte-exact copy `/Users/edr/night-archive/d079-epoch-25g83-derivation-n1-20260913-harvest-20260913`, dry run session absent, equivalence tool rc 3 — no verdict). Both agents uninstalled FROM the clone 05:39:52 rc 0; clone `/Users/edr/JouleWise-measurement-20260913-derivation` at `f90cb8c0` and the night root RETAINED. Launch email `1a09ac6f074248c0` (Gmail alive again); issue #333 commented; kernel lane NIGHT-CENSUS-CHATGPT-APP-01 (ruling-first). Ed's 09-12 reply accepted PRs #317/#329 as proposed. NEXT EXACT ACTION: (this activation) merge bookkeeping → handback rewrite + inventory row for `d079-epoch-25g83-derivation-n1-20260915` as new H → fresh clone `/Users/edr/JouleWise-measurement-20260915-derivation` at H with venv + authenticated ledger → PRs #317/#329 through the gate. (Activation alive Mon 09-14 03:00–06:30 PDT) runbook §0.3–§1.5 + NIGHT_HANDBACK email-then-arm for t0 Tue 09-15 02:56:00 PDT (epoch 1789466160), ONLY if the arm-time ancestry census is clean (no foreign claude/codex process; pid 24974 gone; ChatGPT app quit); otherwise email Ed and notice the following night. Never install after 07:00.

**UPDATE 2026-09-13 06:30 PDT (activation `24b9d3dd`) — LAUNCHED; NOTHING ARMED; FIX SEATS RELAUNCHED.** Watchdog attempt 29 after `c5048879` exited at 06:02 (usage). Launch email `1a09ae4f5cedf1ac`; `notice.ack` written. Successor prep for `d079-epoch-25g83-derivation-n1-20260915` stands at runbook §0.5 (clone at `27957b60`, ledger authenticated); §0.6–§1.5 are Monday 09-14 03:00–06:30's. Ed's interactive `claude` pid 24974 and the ChatGPT desktop app are STILL ALIVE — both must be closed before Monday's arm (asked in the launch email). The previous activation's fix seats 11/12 and replay 1 died with it; seat 11's uncommitted `ci.yml` edit preserved (record 01) and relaunched as seat 03 (audit + complete), seat 04 relaunched from brief 10 verbatim, replay 1 re-run on `wt-integ-c5048879` `c63b5ec4`. NEXT EXACT ACTION: harvest seats 03/04 → lead diff read → commit by pathspec → delta re-audit (paired lenses for the #317 blocker fixes) → merge main into both heads → replay at final heads → twelve-row gate → PR bodies (#329 base `a4bb8838`) → `gh pr ready` → merge (D-072). Then NIGHT-CENSUS-CHATGPT-APP-01 cold-gate packet. Exit on the watchdog's request or usage; Monday's activation arms per runbook §0.6–§1.5 + NIGHT_HANDBACK.

**UPDATE 2026-09-13 06:55 PDT (activation `24b9d3dd`) — TWO COLD GATES RULED; THREE PRs IN THE GATE; NOTHING ARMED.** Census: cold gate packet 05 ruling 10 (option i, keep `codex|claude|t3`; desktop apps that bundle an agent runtime count as agents) + Opus pairing 12 AMEND → synthesis 13; installed as draft PR #334 (`fix/2026-09-13-night-census-desktop-apps` @ `92408a18`; refuter 20 fidelity/mutation PASS, first-use glosses added at the bench). New kernel lane ARM-CENSUS-BROWSER-PROBE-SYSTEM-SERVICES-01 (blocks the first pack night only). CI-TRIM: #317 escalated after deltas 07/08 (same signature) → design consult 14/15 (both: option A) → cold gate packet 17 ruling 10 (option A; fence unchanged; addendum after merge as bookkeeping; `pr_fast_tier` deleted) + Opus pairing 12 AMEND (addendum text, fences comment) → synthesis 14; round 2 at `8a733f5c`, comment `71b87e54`, main merged `80d0c110`; delta 24 running; PR title/body rewritten. DOCS-THIN #329: fix round 1 in four parts (`8727a84c` … `58bf4a23`), delta 16 CLEAN, Opus counter-review 18 AMEND applied; replay 3 running at `58bf4a23`; body rewritten. NEXT EXACT ACTION: harvest delta 24 + replay 3 → #329 ledger rows 7/9/11/12 + terminal review record → `gh pr ready 329` → CI → merge → post-merge review; #317: replay at `71b87e54` (after replay 3) → fresh-eyes on `71b87e54`/`80d0c110` → ledger → ready → CI → merge → bookkeeping commit carrying the ruled TEST-SPEED-01 addendum (refuter 12 §3 text) + kernel prose amendment; #334: ledger → ready → CI → merge. Then RUN_STATE T38q + memory checkpoint. Ed's pid 24974 and the ChatGPT app still alive (Monday's arm needs both closed).

**UPDATE 2026-09-13 08:30 PDT (activation `24b9d3dd`) — THREE PRs MERGED (#329 `64fc4e27`, #334 `8980f85a`, #317 `73bf1754`); TWO COLD GATES RULED (05 census, 17 CI gating); LANE NIGHT-CENSUS-CHATGPT-APP-01 CLOSED; LANE ARM-CENSUS-BROWSER-PROBE-SYSTEM-SERVICES-01 REGISTERED; NOTHING ARMED.** RUN_STATE T38q carries the account. Post-merge review 33 of the last two merges running. Successor-night prep unchanged at runbook §0.5 (clone at H `27957b60`; do not re-cut). Ed's interactive `claude` pid 24974 and the ChatGPT app still alive at 08:30. NEXT EXACT ACTION: hold resident (directive/stand-down polls) or exit on usage; Monday 03:00–06:30 activation arms per runbook §0.6–§1.5 + NIGHT_HANDBACK on a clean census only.

**UPDATE 2026-09-13 09:40 PDT (activation `24b9d3dd`) — FOURTH PR IN THE GATE; NOTHING ARMED.** Cold gate packet 34 (ARM-CENSUS-BROWSER-PROBE-SYSTEM-SERVICES-01: the arm-time browser AND monitor probes refuse on macOS's own services) ruled option (i) anchored patterns; pairing 12 AMEND adopted (synthesis 13 + addendum replacing the grep control with a decoy-child control, bench-proven). Installed as PR #335 (`7909cf2e`): seats 36/38, bench run 40 (75 tests OK, real probes), refuter 41 + counter-review 42 (LANDABLE), fix round 1, delta 43 CLEAN (one accepted nit). Replay 6 at `7909cf2e` running; ledger rows 9/11 pending; merge → post-merge review → bookkeeping (kernel acceptance A3 clause via `/tmp/magistrate-24b9d3dd/kernel-arm-census-merge.py`, lane stays OPEN for the Ed-hands desk proof). Main `4b9a3411` unchanged since. NEXT EXACT ACTION: harvest replay 6 → fill rows 9/11 → `gh pr ready 335` → CI → merge → post-merge review → bookkeeping commit + fast-forward main → then hold resident or exit on usage/watchdog; Monday's activation arms per T38q.

**UPDATE 2026-09-13 10:20 PDT (activation `24b9d3dd`) — PR #335 MERGED (`58edfa29`); FOUR PRs TODAY; THREE COLD GATES; NOTHING ARMED.** Lane ARM-CENSUS-BROWSER-PROBE-SYSTEM-SERVICES-01 installed, OPEN in `ed_external` for the Ed-hands desk proof. Post-merge review 46 running. Main `944eb856`. NEXT EXACT ACTION: bank review 46 → choose the next desk slice via agent_playbook M0 or hold resident; exit on usage/watchdog; Monday 03:00–06:30 activation arms per T38q (Ed must close pid 24974 and quit the ChatGPT/Claude desktop apps first).

**UPDATE 2026-09-13 10:55 PDT (activation `24b9d3dd`) — BOOKKEEPING CORRECTED (`e8b098a5`); COLD GATE 47 RECORDED; GATE-SENSIBILITY-SWEEP-01 CLOSED; NOTHING ARMED.** T38q correction: `cb7634f1` carried none of the #317/#334 bookkeeping (clean-tree guard tripped by an untracked seat manifest); all of it re-applied in `e8b098a5` with cold gate 47's record (D-124 addendum; GATE-B1-PROVENANCE-BAND-01 registered). Guard lesson recorded. Main `e8b098a5`+. NEXT EXACT ACTION: hold resident (directive/stand-down polls, hourly heartbeat) or exit on usage/watchdog; Monday 09-14 03:00–06:30 activation arms per T38q (Ed must close pid 24974 and quit the ChatGPT/Claude desktop apps first).

**UPDATE 2026-09-14 00:10 PDT (activation `24b9d3dd`) — ARM PREP DONE FOR n1-20260915; NOT YET ARMED; CENSUS FOREIGN.** Record 48: the refused 09-13 plan root retired to `/Users/edr/night-archive/…-plan-root-retired-1789369391` (20/20 checksums before and after); desk block rc 0 — night root written (wrapper `356a0810…`, VERIFIED), staged plan at `/Users/edr/night-plan-staging/d079-epoch-25g83-derivation-n1-20260915/night_plan.json` (NOT discoverable), preflight ok; scripts under `/tmp/magistrate-24b9d3dd/arm/` (copies in `48-arm-evidence/`); notice text drafted at `/tmp/magistrate-24b9d3dd/arm/notice.md`. Census at 00:02 FOREIGN (Ed's `claude` 24974 + MCP servers, ChatGPT app helpers, Claude updater, Codex Computer Use); reminder email `1a09eba418675d4a`. NEXT EXACT ACTION (this activation, inside 03:00–06:30): poll the census each cycle; when CLEAN (only own MCP servers by ancestry) and before 06:05: re-read directives + the thread → send `notice.md` → write `$STAGE/notice-evidence.txt` (send id, time, thread, NO relay) → run `40-arm.zsh` (foreground; census re-check, publish, install both agents, baseline) → record 49 (arm record) + handback §Executed line → push → exit before 02:31 09-15 (watchdog stand-down). If still foreign at 06:05: do NOT arm; remove this session's staged plan and night root (`rm -r` the staging dir and the 20260915 night root, both authored by this session; the clone stays); notice Ed for the next date (t0 Wed 09-16 02:56, install Tue 09-15 03:00–06:30); record; keep the clone at H (a 09-16 plan needs a new handback H with a `20260916` inventory row — desk work Monday).

**UPDATE 2026-09-14 06:10 PDT (activation `24b9d3dd`) — n1-20260915 NOT ARMED (census foreign through the 06:05 cutoff); NOTHING ARMED; own staged plan + night root removed (record 49).** Ed emailed 06:06. NEXT EXACT ACTION: desk work today for the successor `d079-epoch-25g83-derivation-n1-20260916` (t0 Wed 09-16 02:56, epoch 1789552560; install Tue 09-15 03:00–06:30): handback rewrite + inventory row + pin test → new H → fresh clone `/Users/edr/JouleWise-measurement-20260916-derivation` at H → §0.1–§0.5; then hold. Tuesday's 03:00 activation: census clean only → §0.6–§1.5 + email-then-arm. Ed must close pid 24974, the ChatGPT app and the Claude app.

**UPDATE 2026-09-14 06:15 PDT (activation `24b9d3dd`) — SUCCESSOR n1-20260916 PREPARED TO §0.5 (H `3c8bd220`; clone at H authenticated; record 50); NOTHING ARMED.** NEXT EXACT ACTION: hold resident (heartbeat, stand-down/STOP/directive polls, hourly thread reads) or exit on usage/watchdog; the activation alive Tue 09-15 03:00–06:30 PDT arms per record 50 on a CLEAN census only (`/tmp/magistrate-24b9d3dd/arm2/30-desk.zsh` then the §1.4 notice, then `40-arm.zsh`); Ed must close pid 24974, the ChatGPT app and the Claude app first.

**UPDATE 2026-09-14 15:00 PDT (activation `24b9d3dd`) — n1-20260916 STAGED (not discoverable), NOT ARMED; census still foreign.** Record 51: desk block rc 0 at 14:57 (wrapper `54a33c35…` VERIFIED; staged plan authored 1789423049, t0 − authored 35.98 h); notice drafted at `/tmp/magistrate-24b9d3dd/arm2/notice.md`. NEXT EXACT ACTION (activation alive Tue 09-15 03:00–06:30): census CLEAN only → directives + thread → send notice → `notice-evidence.txt` → `40-arm.zsh` → arm record + handback §Executed → push → exit before 02:31 09-16; if foreign at 06:05 → remove own staged plan + night root, notice for 09-17, repeat §0.1. Until then: hold resident with polls, or exit on usage/watchdog (a successor activation re-reads this line; the staged plan and night root survive an activation change and are re-verified by `40-arm.zsh` step 2).

**UPDATE 2026-09-14 16:50 PDT (activation `24b9d3dd`) — ARMED: `d079-epoch-25g83-derivation-n1-20260915`, t0 Tue 2026-09-15 02:56:00 PDT (1789466160), from the clone `/Users/edr/JouleWise-measurement-20260915-derivation` at H `27957b60`, under Ed's directive #336 (evening install, owner-authorized for this night only); published plan sha256 `207245e1…`; both agents installed from the clone; record 52.** The 20260916 staging/night root were retired unarmed (clone retained). Directive #337 received for after the night (acknowledged on the issue). NEXT EXACT ACTION: this activation does desk work only (#337 R3 prep, PR #330 gate) with every child stopped and exits on the watchdog's stand-down before 02:31 09-15; NEVER touch the frozen triple; after the night: runbook §2 harvest → §2.5 verdict → one action → uninstall from the clone → #337 lanes.

**UPDATE 2026-09-14 17:45 PDT (activation `24b9d3dd`) — NIGHT ARMED (untouched); PR #330 MERGED (`9e7ca12b`); cold gate 65 ruled the Ed-hands desk proof (kernel acceptance text replaced; command emailed `1a0a27b2b9d8024c`, run only before 02:20 tonight or after 07:00 tomorrow); D-181 = PR #338 in the gate.** NEXT EXACT ACTION: (this activation) merge main into PR #338, regenerate, terse compliant seat + replay + terminal review + ledger → merge → close #337 → stand down before 02:31 with every child stopped. (After the night) runbook §2 harvest → §2.5 verdict → one action → uninstall from the clone → release the kernel event `N1-20260915-HARVESTED-AND-S2-5-ACTION-TAKEN` → INSTALL-WINDOWS-MULTI-01 first (D-181 cl.1) → diff Ed's probe record against lines 57-58 at the named sha when he posts it.
...
**UPDATE 2026-09-15 02:17 PDT (activation `24b9d3dd`) — COOPERATIVE STAND-DOWN before the plan span (no watchdog request by 02:16; the span opens 02:31 and this session's processes are agent-census hits).** Everything is committed and pushed (main `6c8e0674` + this line); no child besides the session's own Codex MCP servers, which end with it; night `d079-epoch-25g83-derivation-n1-20260915` ARMED and untouched (t0 02:56, courier 05:31, dead-man 07:00); census clean of foreign agents at 02:16. NEXT EXACT ACTION (activation after the night): runbook §2 harvest → §2.5 verdict (`scripts/epoch_equivalence_check.py`) → its one action → uninstall both agents from the clone → release the kernel event `N1-20260915-HARVESTED-AND-S2-5-ACTION-TAKEN` → INSTALL-WINDOWS-MULTI-01 (D-181 cl.1) → PACK-ROOT-SUCCESSOR-V5-01 before any pack arm. Desk-proof clone `/Users/edr/JouleWise-desk-proof-20260915` and roots `/Users/edr/desk-proof-arm-census` are retained (no registry patch made); the 20260916 clone at `3c8bd220` is retained unarmed.

**UPDATE 2026-09-15 05:50 PDT (activation `1acf2aee`) — NIGHT n1-20260915 REFUSED at t0 (`night_refused_not_quiet`, load 2.55 > 2.0, census clean; C5 passed, ledger unchanged); HARVESTED per runbook §2 (record 01; archive `~/night-archive/d079-epoch-25g83-derivation-n1-20260915-harvest-20260915`, SHA256SUMS 0 non-OK); §2.5 nothing to judge → one action = re-plan, never re-arm; both agents UNINSTALLED from the clone 05:43:23; NOTHING ARMED; kernel event `N1-20260915-HARVESTED-AND-S2-5-ACTION-TAKEN` satisfied → INSTALL-WINDOWS-MULTI-01 QUEUED at agent rank 0.** Load source is `fseventsd` (pid 553) at ~184 % CPU, still running; identifying its client needs sudo (Ed; launch email `1a0a51594dc05767`). Clone `/Users/edr/JouleWise-measurement-20260915-derivation` (`27957b60`, clean) and the night root RETAINED; canonical root untouched. NEXT EXACT ACTION: INSTALL-WINDOWS-MULTI-01 seat pipeline in a linked worktree + PACK-ROOT-SUCCESSOR-V5-01 desk work; successor night only as a NEW plan via NIGHT_HANDBACK once the 1-minute load is below 2.0 with a clean census.

**UPDATE 2026-09-15 06:25 PDT (activation `decae362`) — LAUNCHED after `1acf2aee` exited on usage at ~06:06; NOTHING ARMED; load still 2.97 (`fseventsd` 186 %); the three dead seats (A implementation, D docs, PACK-ROOT scout) RELAUNCHED verbatim from briefs 07/08/04 at 06:19–06:21 into their clean worktrees (outputs `/tmp/magistrate-decae362/`).** Launch email `1a0a53815b4e8f97`; record 00 under `docs/process_traces/2026-09-15-activation-decae362/`. NEXT EXACT ACTION: harvest seat A + D reports → lead diff read → commit by pathspec on `feat/2026-09-15-install-windows-multi-01` (+ `-docs`) → test amplification → two refuters with distinct lenses on the integrated diff → delta re-audit → replay → twelve-row gate → PR; harvest the scout → PACK-ROOT-SUCCESSOR-V5-01 packet; successor night only as a NEW plan via NIGHT_HANDBACK once the 1-minute load is below 2.0 with a clean census. If this activation dies on usage, the seats die with it: re-check the three worktrees for uncommitted edits FIRST (preserve by committing on their branches), then relaunch from the same briefs.

**UPDATE 2026-09-15 06:40 PDT (activation `d6888966`) — LAUNCHED after `decae362` exited at 06:24; ROOT CAUSE of the last two exits = turn-end exit (headless `claude -p` exits when the turn ends and kills its background seats), NOT usage (5 h window at 29 %; the watchdog's `\brate_limit\b` pattern matched routine `rate_limit_event` telemetry — lane WATCHDOG-EXIT-CLASS-TELEMETRY-01 to register); NOTHING ARMED; load 3.50 (`fseventsd` 190 %); the three seats RELAUNCHED DETACHED (own sessions, parent pid 1) from briefs 04/07/08 at 06:35 (outputs `/tmp/magistrate-d6888966/`).** Launch email `1a0a54669089d7da`; record 00 under `docs/process_traces/2026-09-15-activation-d6888966/` (carries the dated timestamp correction for decae362's addendum). NEXT EXACT ACTION: hold resident until the seats report (never end the turn on a running seat) → harvest seat A + D → lead diff read → commit by pathspec on `feat/2026-09-15-install-windows-multi-01` (+ `-docs`) → test amplification → two refuters with distinct lenses → delta re-audit → replay → twelve-row gate → PR; scout report → PACK-ROOT-SUCCESSOR-V5-01 packet; successor night only as a NEW plan via NIGHT_HANDBACK once the 1-minute load is below 2.0 with a clean census. If this activation is gone, the detached seats keep running: harvest `/tmp/magistrate-d6888966/*.md` + `git diff` in the seat worktrees; do not relaunch a seat whose `.status` is still `RUNNING` with a live `codex exec` child.

**UPDATE 2026-09-15 06:41 PDT (activation `d6888966`; clock-read) — CORRECTION to the 06:40 line: no usage pattern fired; both exits were exit 0 → watchdog `clean activation exit` + its fixed 300 s cooldown (events 132/136); the `BACKOFF_USAGE` label and `state.json`'s `usage_exhausted` come from a STALE `last_exit_class` the clean path never resets (last real usage exit = sequence 37, 2026-09-09). Lane candidate corrected to WATCHDOG-STALE-EXIT-CLASS-01 (WATCHDOG-EXIT-CLASS-TELEMETRY-01 withdrawn). The turn-end cause stands.** Record 00 addendum 06:50. Seats still RUNNING detached. NEXT EXACT ACTION unchanged from the 06:40 line.

**UPDATE 2026-09-15 06:57 PDT (activation `d6888966`; clock-read) — FOUR SEATS HARVESTED (record 07); seat A `33bdc457` + seat D `d49c0b9b` integrated at `int/2026-09-15-install-windows` `1cc8db30` (seven modules OK, lead-run); lanes A197/A198 on main `664b3f6c`; GAMMA root-key repair seat RUNNING detached (brief 06); NOTHING ARMED; load ~2.8.** NEXT EXACT ACTION: Opus lieutenant runs the INSTALL-WINDOWS gauntlet on the integration tree (docs reconciliation → paired refuters → fix rounds + delta re-audits → replay → ledger → PR body; records `lt-*`); magistrate harvests the GAMMA seat → commit by pathspec → refuter → PR; then PACK-ROOT-SUCCESSOR-V5-01 packet from record 04 (BUILD waits for G2-a; roster is Ed's). If this activation is gone: seats are detached — harvest `/tmp/magistrate-d6888966/*.md`; the integration tree and every seat branch are pushed.

**UPDATE 2026-09-15 08:28 PDT (activation `d6888966`; clock-read) — GAMMA ROOT-KEY FIX IN THE GATE: PR #339 (`fix/2026-09-15-gamma-root-keys` @ `804eb394`) READY with all twelve ledger rows filled (records 11–20; `ci` green; `gate-ledger` re-check queued) → merge next (D-072); four lanes registered today (A197 WATCHDOG-STALE-EXIT-CLASS-01, A198 FIXTURE-FAKE-VLLM-LEAK-01, A199 PLAN-TREE-ROOTS-CONTRACT-01, A200 ROOT-NAMESPACE-FALLBACK-01; main `ee6065f8`+); the INSTALL-WINDOWS-MULTI-01 gauntlet is with the Opus lieutenant (reconcile, two refuters, fix round 1, delta all reported on `int/2026-09-15-install-windows`; records `lt-*` on `bookkeeping/2026-09-15-activation-d6888966-lt`); NOTHING ARMED; `fseventsd` still ~180 %.** Bench finding: the watchdog CLI test discovers plans in the custody root's PARENT, so stale `/tmp/*/night_plan.json` fixtures break it at the bench (four moved to `/tmp/magistrate-d6888966/stale-tmp-plans/`, record 19) → lane WATCHDOG-CLI-TEST-TMP-DISCOVERY-01 to register. NEXT EXACT ACTION: merge #339 when `gate-ledger` is green → post-merge review (row 11 second half) → harvest the lieutenant's packet → magistrate gate rows 7/9/12 on the INSTALL-WINDOWS integration head → PR → merge → then successor-night desk prep under the merged runbook (install spans) once load < 2.0 with a clean census.

**UPDATE 2026-09-15 08:44 PDT (activation `d6888966`; clock-read) — PR #339 MERGED (`e7cf1fe3`, GAMMA plan tree emits the T-0 reader's root keys) under the full twelve-row ledger; post-merge cross-unit review CLEAN (record 24); A201 WATCHDOG-CLI-TEST-TMP-DISCOVERY-01 registered (175 rows, main `6773df23`); PACK-ROOT-SUCCESSOR-V5-01 note updated; NOTHING ARMED.** NEXT EXACT ACTION: harvest the Opus lieutenant's INSTALL-WINDOWS-MULTI-01 packet (`int/2026-09-15-install-windows`, records `lt-*`) → magistrate rows 7/9/12 on the final integration head → PR → gate → merge → post-merge review → successor-night desk prep under the merged runbook once load < 2.0 with a clean census.

**UPDATE 2026-09-15 08:56 PDT (activation `d6888966`; clock-read) — INSTALL-WINDOWS-MULTI-01 ESCALATED TO COLD GATE 25 (mandatory: the lieutenant's delta auditor found the same signature after fix round 1 on two classes — the final dead-man bootstrap at `install_night_agent.sh:348` still crosses the span close; failure cleanup leaves a loaded job without plists — record `lt-05`); integration head `df86cee6` with the blocker OPEN, no PR; packet + exhibits A–I on main (`53e1d95e`); cold Fable judge convened 08:55 from the doctrine-free worktree `/Users/edr/code/JouleWise-wt-coldgate-iw` (script `~/.claude/jobs/d6888966/tmp/convene-coldgate-iw.sh`, stdout `coldgate-iw.stdout`, ruling file `…/25-coldgate-packet-install-windows/10-coldgate-fable-ruling.md` in that worktree); Opus pairing refuter writing `lt-12-…` on the lieutenant's bookkeeping worktree; NOTHING ARMED.** NEXT EXACT ACTION: harvest ruling 10 + pairing refuter 12 → magistrate synthesis 13 (dissent in writing if overruling) → if Q4/Q5/Q6 need Ed, email a decision brief with the arithmetic → resume the lieutenant for fix round 2 under the ruling (or land per Q4) → rows 6/7/10/11/12 → PR → merge. If this activation is gone: the judge and refuter files above are the resume point; do not re-convene a second judge on the same packet.

**UPDATE 2026-09-15 09:18 PDT (activation `d6888966`; clock-read) — COLD GATE 25 RULED AND ADOPTED (ruling 10 + Opus pairing 12 + synthesis 13, all on main): Q1 single commit gate at the installer's sole success exit; Q2 no new constant; Q3 one EXIT-trap teardown, bootout both labels, VERIFY not-loaded, only then restore/remove (Opus blocker amendment); Q4 close first (class 2 live under the default span); Q5 FIX-5 affirmed (kernel text recorded, main `3bcdfae8`); Q6 no conflict — the `:1297` sentence in this lane, the §3 cross-reference as Ed-visible lane A202. Lieutenant RESUMED for fix round 2 under the ruling (int head moving past `9dc36e53`); NOTHING ARMED.** Ed progress email `1a0a5d24d4b2ff94`. NEXT EXACT ACTION: harvest the lieutenant's round-2 packet (delta verdict, Opus counter-review, replay) → magistrate rows 7/12 → PR → gate → bare `gh pr merge` → post-merge review → successor-night desk prep under the merged runbook once load < 2.0 with a clean census.

**UPDATE 2026-09-15 10:19 PDT (activation `d6888966`; clock-read) — FIX ROUND 2 LANDED (int head `9a7bacdf`: commit gate + verified-bootout teardown + render-only exemption + runbook `:1297`; both isolated reversions prove the mechanisms) but DELTA 2 answered same-signature YES again (F1: failable backup `rm` after the gate and success print with the EXIT trap armed; F2: pre-existing `--uninstall` swallows bootout failures and deletes plists, outside every brief) → the lieutenant stopped; COLD GATE 28 convened 10:18 from `wt-coldgate-iw2` (packet on main `8f04645c`; ruling file `…/28-coldgate-packet-install-windows-round-3/10-coldgate-fable-ruling.md` in that worktree; Opus pairing → `lt-17-…` on the lieutenant worktree); NOTHING ARMED.** NEXT EXACT ACTION: harvest ruling + pairing → synthesis → round 3 only if authorized with the ruled stop condition (else land per the ruling) → rows 6/7/9/10/11/12 → PR → bare `gh pr merge` → post-merge review. If this activation is gone: the judge/refuter files above are the resume point; do not re-convene on the same packet.

**UPDATE 2026-09-15 10:34 PDT (activation `d6888966`; clock-read) — COLD GATE 28 RULED AND ADOPTED (ruling 10 + Opus 12 + synthesis 13 on main): ROUND 3 AUTHORISED as four dictated edits (delete `:374` + success branch in teardown; uninstall verified bootout under scope expansion; `:294` both labels; F3 `trap '' INT TERM HUP`), delta 3 with named survivor lists, and an OBSERVABLE STOP CONDITION (any executed class-1/class-2 case → no round 4 → three-seat redesign consult on a transactional installer, Ed emailed). Lieutenant RESUMED for round 3. NOTHING ARMED.** NEXT EXACT ACTION: harvest round 3 + delta 3 → if clean: rows 6/9/10 (lieutenant) → magistrate rows 7/12 → PR → gate → bare `gh pr merge` → post-merge review; if the stop condition fires: redesign consult, Ed email.

**UPDATE 2026-09-15 11:08 PDT (activation `d6888966`; clock-read) — ROUND 3 LANDED (int head `d74b5ff1`: post-gate rm deleted + teardown success branch; uninstall verified bootout + `:294` both labels; F3 trap; all four dictated mutations RED; nine-cell matrix unchanged); DELTA 3 pending one scope grant, now GIVEN: `tests/test_run_night.py` launchctl stub (it answered `print` rc 0 after bootout — an impossible launchd; bench-verified). Lanes A203 INSTALLER-BACKUP-WINDOW-01 and A204 WATCHDOG-INSTALLER-VERIFIED-BOOTOUT-01 registered (178 rows, main `6684dc76`). NOTHING ARMED.** NEXT EXACT ACTION: harvest delta 3 (stop condition = synthesis 13 §Q3 predicates) → if clean: rows 6/9/10 (lieutenant) → magistrate rows 7/12 → PR → gate → bare `gh pr merge` → post-merge review; if a predicate fires: redesign consult + Ed email.

**UPDATE 2026-09-15 11:48 PDT (activation `d6888966`; clock-read) — STOP CONDITION FIRED after round 3 (int head `073a9763`, green): delta 3 found class 2 real at all three liveness re-reads (`launchctl print … &&` reads ANY non-zero exit as not-loaded → plists deleted under a loaded job; record `lt-21`); class 1 over-fired on the ruled-benign backup removal. NO ROUND 4. REDESIGN CONSULT CONVENED 11:48 (record 33: three blind seats — Astra xhigh `/tmp/magistrate-d6888966/21-design-astra.md`, Opus `lt-22-…` on the lieutenant worktree, fresh Fable `DESIGN-33c-fable-seat-report.md` in `/Users/edr/code/JouleWise-wt-design-iw`); Ed emailed `1a0a666462e36d18` (override offered, none required). NOTHING ARMED.** NEXT EXACT ACTION: harvest the three design reports → magistrate adjudication (record 34, dictated-fills style) → implementation seats under the lieutenant on a fresh branch from `073a9763` (or main) → gauntlet → PR. If this activation is gone: the three report paths above are the resume point.

**UPDATE 2026-09-15 11:57 PDT (activation `d6888966`; clock-read) — REDESIGN ADJUDICATED (record 34; seats 33a/33b/33c all chose B): `joulewise/night_agent_install.py` transaction engine, three-valued liveness (rc 0 LOADED; rc 113 + exact not-found line ABSENT; else UNKNOWN ⇒ loaded), proof-carrying deletion, single commit predicate on a post-mutation clock read, single state-dispatched teardown, no TMPDIR, `.prior` sidecars, occupancy on both labels (no bootout on the success path), A204 as a separate PR. Implementation lane handed to the lieutenant on `feat/2026-09-15-install-windows-transactional` (worktree `wt-iw-txn`, from `073a9763`): seats T1 (engine+shell+tests) and T2 (docs D9) → refuters → deltas (stop condition of 28/13 unchanged, class 1 read per ruling 28 Q1) → Opus counter-review → replay + lt-21 reproduce NO/NO → ledger. NOTHING ARMED.** NEXT EXACT ACTION: harvest the lieutenant's packet → magistrate rows 7/12 → PR → gate → bare `gh pr merge` → post-merge review → A204 PR → successor-night desk prep under the merged runbook once load < 2.0 with a clean census.

**UPDATE 2026-09-15 15:53 PDT (activation `d6888966`; clock-read) — TRANSACTIONAL INSTALLER BUILT: `feat/2026-09-15-install-windows-transactional` @ `3b339ac9` (engine `joulewise/night_agent_install.py` 676 lines stdlib-only 3.9-compatible; shell 387→91 lines; new matrix `tests/test_night_agent_install.py`); acceptance `class_1: NO`, `class_2: NO` (lieutenant bench + delta: 473 attack cases, must-die 31/31 RED); contract refuter clean after fix round 1; delta caught the oracle re-importing the UNKNOWN⇒absent assumption (fix round 2). OPEN: execution lens (three Codex harness failures → now an Opus seat), fix round 3 (four stale `test_run_night` fixtures, scope granted), delta on rounds 2+3, rows 6/9/10, ledger; process finding: seats all day ran without bridge §7 baseline/lease (lane BRIDGE-BASELINE-COMPLIANCE-01 registering, council/cold gate to rule). NOTHING ARMED.** NEXT EXACT ACTION: harvest the lieutenant's packet → magistrate rows 7/12 on the final head → PR → gate → bare `gh pr merge` → post-merge review → A204 PR → successor-night desk prep.

**UPDATE 2026-09-15 19:50 PDT (activation `d6888966`; clock-read) — STANDING DOWN for Ed's machine RESTART (fseventsd ~61 GB RSS; peer session joulewise-31's request; nothing armed). TRANSACTIONAL INSTALLER at `feat/2026-09-15-install-windows-transactional` @ `0ba6ce54` (pushed): rows 1–5 and 9 done (replay 6135/0/0; acceptance class_1/class_2 NO), row 7/8 PASS (record 38), row 6 Opus counter-review LANDABLE with AMEND (`lt-31`: F1 should-fix = add a test + must-die line for `Target.validate()`'s retained-sidecar refusal `night_agent_install.py:133-135`; F2 ruling = adopt its replacement prose — the `.prior` sidecar is not resumable, uninstall discards it; F3 should-fix = 3-line signal-mask cure for the sliver between `finally` and `_unwind`'s `pthread_sigmask` at `:409`); delta on fix round 4 (record 37 brief, seat `25-delta4-astra`) INTERRUPTED by the restart if not finished — see `~/night-archive/magistrate-d6888966-tmp/25-delta4-astra.md`. NO PR OPENED.** NEXT EXACT ACTION (next activation): fix round 5 = F1 test + must-die line, F3 signal-mask cure, F2 prose (runbook/handback) — one Astra xhigh seat WITH bridge §7 baseline + lease (see record 37 for the field shape); delta re-audit (isolated reversions, the two predicates); re-run the delta on fix round 4 if `25-delta4-astra.md` is absent/partial; row 10 fresh-eyes on every post-review commit; ledger (`lt-92` draft) + PR body (`lt-93` draft, with the bridge §7 residual-risk paragraph) → open PR → CI → magistrate row 12 → bare `gh pr merge` → post-merge review → A204 (watchdog installer uninstall) as its own PR → successor-night desk prep under the merged runbook once load < 2.0 with a clean census. Seven lanes registered today (A197–A205). Cold-gate convene scripts archived under `~/night-archive/magistrate-d6888966-tmp/jobs-d6888966/`.

**UPDATE 2026-09-16 02:15 PDT (activation `08ca8197`; clock-read) — HANDOFF TAKEN; MERGE WAVE ON MAIN (installer #341, A172 #342, A173 #343, A210 #344 ladder 8/8/6/5 + 2-min pad, CI #340, A208 #345, render-only fix #346, #347); REHEARSAL_STUB `rehearsal-20260916` STAGED AND PREFLIGHTED at H `cf249594` (handback commit): clone `/Users/edr/JouleWise-measurement-rehearsal-20260916b`, staged plan sha `dff929bc…`, t0 03:00:00 PDT (1789552800), install close 02:50, render-only rc 0, arm census CLEAR at 02:05; ARM ATTEMPT 1 ABORTED, cause `arm_transport`: the Gmail MCP session is EXPIRED (send and search fail), so no notice, NO PUBLICATION, no installer run; intended notice preserved at `/Users/edr/night-custody/magistrate/intended-notice-08ca8197-0212.txt`; outcome.json in `$STAGE/arm-attempts/000001/`. Interactive session ruled the stub may arm on the integration replay with hosted CI (run 35076281462, still queued) recorded as pending. NOTHING ARMED.** NEXT EXACT ACTION: when Gmail transport is restored (Ed re-authorizes the claude.ai Gmail connector), attempt 2 = fresh notice for the SAME staged plan if `now < 02:38` (then 40-arm.zsh, record, exit before 02:52); otherwise re-plan t0 (new handback commit → new H → 20-clone + 21-ledger with a new root name → 30-stage → 35-preflight → notice → 40-arm). Record 10 + evidence in `docs/process_traces/2026-09-15-activation-08ca8197/`. The equivalence science night waits for a green CI run on its H.

**UPDATE 2026-09-16 02:18 PDT (activation `08ca8197`; clock-read) — EXITING CLEANLY FOR A WATCHDOG RELAUNCH: the Gmail MCP session in this process stayed EXPIRED after Ed re-authorized the connector (02:14, 02:19), so the notice cannot be sent from here; a fresh activation gets a fresh connection. Main `6b7e7b71` carries record 10 (attempt 1 aborted `arm_transport`), the interactive session's bookkeeping (five lane rows closed, 190→185) and this line; `enforce_admins` is OFF since 02:19 (direct pushes work; `gate-ledger` still required for PR merges; never `gh pr merge --admin`). NOTHING ARMED; zero discoverable plans; launchctl = magistrate only.** NEXT EXACT ACTION (successor): launch duties (if the launch email fails, preserve it and continue; ask the interactive session `b0ae8462` at `uds:/tmp/cc-socks/841.sock` to have Ed reconnect the Gmail connector from claude.ai settings); then `source /tmp/magistrate-08ca8197/stage/exports.zsh` and, if before ~02:40, ARM_ATTEMPT=2 → `35-preflight.zsh` → attempts.json = [000001/outcome.json] → notice (body 000001/notice-body.txt, "attempt 2; earlier abort arm_transport") → `notice-json.py` → `40-arm.zsh` → record → direct push → exit before 02:52; else re-plan t0 with a new handback commit (H) → new root name → 20-clone/21-ledger/30-stage/35-preflight/notice/40-arm. Message the interactive session on publication or any hold. Full successor instructions in memory checkpoint-2026-09-15-activation-08ca8197.md.

**UPDATE 2026-09-16 02:32 PDT (activation `736e2aed`; clock-read, corrected) — LAUNCHED 02:29 (attempt 35), HOLDING RESIDENT: the Gmail connector is DOWN on the claude.ai side (peer-confirmed: Ed's reconnect returns `not_found_error`), so the launch email is preserved unsent at `~/night-custody/magistrate/intended-launch-email-736e2aed-0232.txt`, no `notice.ack`, and arm attempt 2 is impossible; the 03:00 t0 of the staged stub `rehearsal-20260916` (H `cf249594`, plan sha `dff929bc…`, both re-verified) LAPSES unarmed at install close 02:50. Per the interactive session `b0ae8462` (Ed present): hold resident, do not exit for a relaunch, do not loop re-plans against the outage; Ed is being asked to rule whether the arm notice may be posted as a GitHub issue when Gmail is down (this activation asked for that ruling as an owner-authored `directive` issue). NOTHING ARMED; launchctl = magistrate only; launch record `docs/process_traces/2026-09-16-activation-736e2aed/00-launch-record.md`.** NEXT EXACT ACTION: on an Ed-authored ruling → re-plan the stub under the ruled transport (new t0 ≥ now + 25 min, NIGHT_HANDBACK three-section rewrite → new H → new clone root `…c` → 20-clone/21-ledger/30-stage/35-preflight → notice on the ruled transport → 40-arm → record + lane NOTICE-TRANSPORT-FALLBACK-01) and exit before that plan's REQUEST boundary; on a re-plan instruction without a ruling → wait for the connector, then the same re-plan via Gmail; on `standdown.request` → stand down per the launch contract.

**UPDATE 2026-09-16 02:34 PDT (activation `736e2aed`; clock-read, corrected) — EXITING CLEANLY FOR A WATCHDOG RELAUNCH (peer GO 02:33): Ed repaired the Gmail connector from the Claude desktop app at ~02:36 and went to sleep; this process spawned before the repair with NO Gmail tools and cannot re-acquire them, so the successor's launch email IS the probe. NOTHING ARMED; the staged stub `rehearsal-20260916` (H `cf249594`, plan sha `dff929bc…`) lapses unarmed at 02:50; launchctl = magistrate only.** NEXT EXACT ACTION (successor): launch duties; if the launch email SENDS → `notice.ack`, message the interactive session `b0ae8462` (joulewise-2e, `uds:/tmp/cc-socks/841.sock`), then re-plan the stub to the earliest whole-minute t0 the margins allow via the documented email-then-arm procedure (NIGHT_HANDBACK three-section rewrite → direct-push H → root `…c` → `source /tmp/magistrate-08ca8197/stage/exports.zsh` with the new H.txt/root/T0 → 20-clone/21-ledger/30-stage/35-preflight → notice → 40-arm → record), register lane NOTICE-TRANSPORT-FALLBACK-01 (GitHub-issue notice when Gmail is down), report publication to the peer, exit before the plan's REQUEST boundary; if it FAILS → preserve it, message the peer, hold resident (no relaunch loop) — Ed has a prepared `directive` body for the issue-transport ruling and files it when awake. Peer facts for the census: `b0ae8462` keeps no detached seats until publication; its in-session subagents are idle-class. Launch record: `docs/process_traces/2026-09-16-activation-736e2aed/00-launch-record.md`.

**UPDATE 2026-09-16 02:54 PDT (activation `83d93f5a`; clock-read) — `rehearsal-20260916c` (REHEARSAL_STUB) IS ARMED: published 02:50:45 PDT, both night agents installed rc 0 from the clone; t0 2026-09-16 03:25:00 PDT (1789554300), window 900 s, install close 03:15, REQUEST 03:17 / TERM 03:19 / KILL 03:20, completion / courier deadline 03:45:00 (1789555500), dead-man 04:45 daily; H `be221f6a` (handback commit on main, CI success); clone `/Users/edr/JouleWise-measurement-rehearsal-20260916c`; custody root `/Users/edr/night-custody/rehearsal-20260916c`; published plan sha256 `aa82ebd9…`. Notice transport EMAIL (Gmail worked in this activation; `1a0a99fa2717d749`, 02:50:06); directive #349's issue fallback not needed; lane NOTICE-TRANSPORT-FALLBACK-01 registered (kernel rank 217, record 02) and #349 closed. The lapsed candidate `rehearsal-20260916` (H `cf249594`) stays unpublished under `~/night-plan-staging/rehearsal-20260916` with its empty custody root; nothing else armed. Records: `docs/process_traces/2026-09-16-activation-83d93f5a/`. Exiting before 03:17.** NEXT EXACT ACTION (successor, after 03:45 and `courier.sent`): harvest per NIGHT_HANDBACK §Next lane (acceptance: `REHEARSAL_ONLY` or a recorded `night_refused_agent_present` hit with the stub completing, `chain_exit_code` 0, EMPTY `launchd.night.err`); honour any Ed NO on the notice thread; `--uninstall` FROM the clone (exit 0 required) → retire the plan root (record-48 procedure) and the stub clone; retire the lapsed `rehearsal-20260916` staging + empty custody root the same way (never published; cite record 10 of 08ca8197); then the equivalence science night (`DIAGNOSTIC_NO_PACK`, issue #316 ruling) under this same procedure with a clean all-agents-closed census.

**ADDENDUM 02:56 PDT (activation `83d93f5a`) — peer findings for the successor's bookkeeping (from the interactive session `joulewise-95`, verified by it against main `11ea57db`; not registered tonight for lack of time before the 03:17 boundary):** (1) the first-use block rendered by the arm-retry lane still says "plan span … beginning 25 minutes before t0" and "`install_close_epoch` … t0 − 85 minutes" at `docs/phase_2/derivation_night_runbook.md:1622-1626` and `docs/process/NIGHT_HANDBACK.md:55-59`, while `run_night.py:75`, `magistrate_watchdog.py:86-89` and runbook §1.3 say 8 and 10 min after LEAD-MARGIN-01 — register a docs-drift lane row (or fold into RUNBOOK-SOURCE-MAP-01) and fix; (2) A197 confirmed in code: the clean-exit path `magistrate_watchdog.py:1714-1723` never writes `last_exit_class`, only `apply_backoff` at `:1372` does, so a clean exit after a usage exit takes the 900 s usage backoff instead of the 300 s cooldown (this activation's launch paid it). The peer runs one Astra register seat after 03:45 in `/Users/edr/code/JouleWise-wt-register` (docs-only, nothing under night-custody or the clone).

**UPDATE 2026-09-16 03:08 PDT (activation `d7aba749`; clock-read) — relaunched 03:04:19 (attempt 37, after the 900 s usage backoff the stale `last_exit_class` forces, A197) while `rehearsal-20260916c` IS ARMED (t0 03:25:00 PDT, courier 03:45, dead-man 04:45; H `be221f6a`; night agents loaded). Notice thread `1a0a99fa2717d749` carries no NO; directives none; launch email `1a0a9ae7c1584531` sent, `notice.ack` written. Ed's interactive session (pid 7066) is alive and was flagged to Ed for the t0 census. This activation performs no night work and exits cleanly before the 03:17 request. NEXT EXACT ACTION is UNCHANGED from the 02:54 UPDATE above (harvest after 03:45 and `courier.sent`, uninstall from the clone, retire, register the two addendum findings, then the equivalence night). Record: `docs/process_traces/2026-09-16-activation-d7aba749/00-launch-record.md`.**

**UPDATE 2026-09-16 04:1x PDT (activation `0bd12d79`; clock-read) — `rehearsal-20260916c` HARVESTED, AGENTS UNINSTALLED, ROOT RETIRED; NOTHING ARMED.** Stub fired 03:25:00.82: gate `night_refused_agent_present` (Ed's session 7066 + MCP 7086/7094 + wait loop 9850, the expected stub outcome), stub ran (`chain_exit_code` 0, `REHEARSAL`), `result.json` `REHEARSAL_ONLY`, `launchd.night.err` EMPTY, courier `1a0a9c0fd1fc101e` 03:26:37, results branch `night-results/rehearsal-20260916c` @ `558d7b83`; acceptance MET. Harvest 03:53 (`~/night-archive/rehearsal-20260916c-harvest-20260916`, 15/15 SHA256 OK); uninstall FROM the clone 03:53:50 rc 0 (launchctl = magistrate only); plan root → `~/night-archive/rehearsal-20260916c-plan-root-retired-1789556074` (15/15 OK after the move; discoverable plans 0); lapsed `rehearsal-20260916` staging → `…-staging-lapsed-1789556131`; stub clones retained (rm refused by the classifier). Peer findings registered on WATCHDOG-STALE-EXIT-CLASS-01 (F2, live again: seq 160 clean exit vs stale `usage_exhausted`) and NIGHT-HANDBACK-GLOSS-01 (F3, 25/85-min first-use drift); PHASE-PARTITION-INVARIANT-01 registered (rank 218) from the peer's SIGARCH-article relay. Interactive `joulewise-95` alive (Astra register seat), exit estimate 05:00 PDT. Records: `docs/process_traces/2026-09-16-activation-0bd12d79/`. NEXT EXACT ACTION (this activation, record 02): equivalence night `d079-epoch-25g83-derivation-n1-20260916` — handback commit H (the next NIGHT_HANDBACK commit on main; inventory row `JouleWise-measurement-20260916-derivation` already present, note updated); `20-clone.zsh` fresh clone at H into `/Users/edr/JouleWise-measurement-20260916-derivation` (the unused `3c8bd220` clone moved to `~/night-archive/…-unused-3c8bd220-1789556506`); `21-ledger.zsh`; `30-desk.zsh` (check rc 3/3 expected, wrapper `--verify`, preflight); `35-preflight.zsh` (schedule, boundaries, render-only); hold for the peer's exit message → re-read directives + thread + `standdown.request` → NIGHT NOTICE by email (runbook §1.4 content) → `notice-json.py` → `40-arm.zsh` (census by ancestry must be clean, publish, install from the clone) → arm record 02 + handback §Executed line + pointer → push → exit before REQUEST 05:32:00 PDT (1789561920). t0 05:40:00 PDT (1789562400), window 9000 s, install close 05:30:00 (1789561800), completion / courier deadline 08:15:00 (1789571700), dead-man 09:15 daily. If the peer is still alive at 05:25: do NOT arm; remove this session's staged plan and night root (record-49 procedure), and re-plan with a later t0 under a new H. A t0 refusal on machine state → harvest, uninstall, retire, re-plan (REFUSAL-FAST-RETRY-01 not landed).**

**UPDATE 2026-09-16 05:3x PDT (activation `0bd12d79`) — 05:40 CANDIDATE LAPSED UNARMED; NOTHING ARMED; HOLDING FOR A CLEAN CENSUS.** H `82ea3eef` (fidelity-test pins fixed; `aef09471` superseded unpublished). Staged, desk rc 0, preflight rc 0 at 04:15; NOT noticed/published/installed: Ed's interactive `claude` 7066 + MCP 7086/7094 alive through 05:25 (terminal never closed; email `1a0a9f39f378ff97` 04:21 unanswered); candidate lapsed at install close 05:30; staging + night root → record 02 `02-arm-evidence/lapsed-82ea3eef/`; discoverable plans 0; launchctl = magistrate only. Own codex MCP pair stopped 04:15 (real-class census exempts only the caller's ancestors). NEXT EXACT ACTION (this activation, resident): poll census every 10 s; when clean → `python3 /tmp/magistrate-0bd12d79/arm/replan.py <T0>` (T0 = now + 20 min rounded up; same-day) in `JouleWise-wt-hb-0bd12d79` → commit + push → `H.txt` → clone `git fetch && checkout --detach H` (+ clean-tree/lock checks) → `30-desk.zsh` → `35-preflight.zsh` → re-read directives/thread/standdown → NIGHT NOTICE email (`notice.md`, update t0 tokens) → `notice-json.py` → `40-arm.zsh` → arm record + handback §Executed + pointer → push → exit before REQUEST (t0 − 8 min). A successor activation without /tmp rebuilds the scripts from `02-arm-evidence/`.**

**UPDATE 2026-09-16 09:28 PDT (activation `0bd12d79`; clock-read) — `d079-epoch-25g83-derivation-n1-20260916` (DIAGNOSTIC_NO_PACK, equivalence night) IS ARMED: published 1789576035.63 (09:27:15), both night agents installed rc 0 from the clone; t0 09:45:00 PDT (1789577100), window 9000 s, install close 09:35 (1789576500), REQUEST 09:37 (1789576620) / TERM 09:39 / KILL 09:40, window end 12:15 (1789586100), completion / courier deadline 12:20:00 (1789586400), dead-man 13:20 daily (1789590000); H `32243adca1bfc8822e8001e4bac58000d591aa4f` (handback re-plan commit on main, after `aef09471` and `82ea3eef` were superseded unpublished); clone `/Users/edr/JouleWise-measurement-20260916-derivation` at H (tree clean, lock diff empty, ledger 76 authenticated); custody root `/Users/edr/night-custody/d079-epoch-25g83-derivation-n1-20260916`; published plan sha256 `5cc4e11aa2e0b6e15ac6e1337f6247397a015aa800139c7156ae2551f7c258a2`; wrapper `f3a7b4d3862678fd7128a0fcb71858bdd3952c1c65723adae87e9747658b7e90`. Notice transport EMAIL `1a0ab0a61def58d3` 09:26:18 on thread `1a0a99fa2717d749`; Ed`s reply 09:25 ("i did /exit try to kill whatever is left") read before publication — not a NO; census empty at publication. Records: `docs/process_traces/2026-09-16-activation-0bd12d79/02-n1-20260916-arm-record.md` + `02-arm-evidence/`. Exiting before 09:37.** NEXT EXACT ACTION (successor, after 12:20:00 and `courier.sent`): harvest per NIGHT_HANDBACK §Next lane at H — runbook §2.0 rebuild coordinates from the frozen triple; §2.1 read `result.json`, receipt/refusal, `night.log`, `operator_logs/derivation-chain.log`, both launchd streams (`launchd.night.err` must be EMPTY); lstat inventory + byte-exact copy with `SHA256SUMS` to `~/night-archive/<plan id>-harvest-20260916` before anything moves; §2.2 `check --session-ids d079-epoch-25g83-derivation-n1-20260916` from the clone (counts/states only); §2.5 `scripts/epoch_equivalence_check.py --session-id d079-epoch-25g83-derivation-n1-20260916 --acceptance configs/calibration/calibration_acceptance_d079_v2_n17_r6.json --out <tmp>` from the clone, then the ONE action its outcome names (PASS → D-102 continuation addendum via the normal gate → real G2-a windows; FAIL → rev1 three-night derivation, this = night one; INCONCLUSIVE → one more equivalence night); `scripts/install_night_agent.sh --plan <custody root>/night_plan.json --uninstall` FROM the clone (rc 0 required); clone + night root RETAINED as production custody if a session opened; a t0 refusal (`agent_present` / `not_quiet` / `hid_idle`) → harvest, uninstall, retire the plan root (record-48), re-plan as a new plan (REFUSAL-FAST-RETRY-01 not landed); physics/evidence/pre-registration refusals → cold gate. Verify `night-results/d079-epoch-25g83-derivation-n1-20260916` on origin, never presume.**
