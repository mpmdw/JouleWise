...
4. Run the focused modules the PR touched plus every test module that imports the changed modules (grep -l): paste counts. Do NOT run the full suite (two replays are running on this host); note: a previous attempt at this review was cut off mid-run and saw tests.test_arm_readiness_evidence_author fail once on a timeout test that passed in isolation under host load — re-run that module and report both outcomes.
