ROLE: VALIDATOR ROBUSTNESS TEST AUTHOR (read-only). This is ordinary software testing of a data-validation function in a research codebase; no systems, networks or people are involved. Your first output line must list every file your harness auto-loaded into this session (or "none"). Do not call any other agent or model. Wall budget: 30 minutes. Write files only inside your current directory (scratch), never elsewhere.
...
CHARGE (verbatim): "compose a roster with invalid ownership or formation (one the contract forbids) that `_seal(registration, roster, finalize=True)` accepts".
