# A291 contract v4.1 — contract v4 (02d) with INV-11 replaced by the closed form

Produced 2026-09-24 by the resident magistrate (activation 278ebc9e) as step R4-0 of cold ruling A291-ESC2-02 (docs/process_traces/2026-09-24-activation-278ebc9e/82-coldgate-packet-a291-r3method/20-coldgate-fable-r4-ruling.md §4). The ONLY change from `docs/process_traces/2026-09-24-activation-a65fb4fa/02d-a291-contract-v4-self-contained.md` is the INV-11 predicate, replaced by the ruled closed form with the glossary pasted above its use. The source file is unchanged (historical record).
...
- Predicate (CLOSED FORM, v4.1, cold ruling A291-ESC2-02 R4-0 quoting A291-ESC2-01 K1; supersedes the v4 predicate): «for every (model, registered item): let LIVE be the list of (block, envelope) pairs over ALL blocks containing the item, superseded or not, whose block id is in that envelope's `blocks`; let TERM be the `terminal_refusals` entries naming (model, item). Exactly one holds: (a) (len(LIVE), len(TERM)) == (1, 0) and the one live block has `superseded == false` and none of its items has a `terminal_refusals` entry; (b) (len(LIVE), len(TERM)) == (0, 1).»
- Glossary used by this predicate (from §0 of this contract, pasted verbatim):
