PREAMBLE FROM THE LEAD (activation 24b9d3dd, 06:25 PDT 2026-09-13). The previous seat on this exact contract (run 20260913T130017Z-10148-11-fix-317-round-1-astra-report) was killed by its parent's exit before it reported. Its UNCOMMITTED edit to `.github/workflows/ci.yml` is present in this tree (`git diff` shows 76 insertions / 7 deletions) and appears to implement FIX-1, FIX-2, FIX-3 and FIX-5 below. Do NOT discard or revert it. Audit that edit line by line against the contract that follows, correct or complete whatever falls short (in particular: the docs-readers job's `if:` form, the shard partitioning, the exclusive-module exclusion, the regex on test sources, and the comment wording for FIX-1/FIX-5), then run every verification the contract lists and report as it specifies. If the edit is already complete and correct, say so explicitly per FIX id and still paste the verification evidence.
