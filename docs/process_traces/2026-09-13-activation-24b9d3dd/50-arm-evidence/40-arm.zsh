...
source /tmp/magistrate-24b9d3dd/arm2/exports.zsh
set -euo pipefail
...
start = datetime(2026, 9, 15, 3, 0, tzinfo=zone).timestamp(); end = datetime(2026, 9, 15, 6, 30, tzinfo=zone).timestamp()
print("arm clock:", now, datetime.fromtimestamp(now, zone).isoformat())
assert start <= now < end, "outside 03:00-06:30 PDT install span (2026-09-15)"
assert datetime.fromtimestamp(1789552560).strftime('%Y-%m-%d %H:%M') == '2026-09-16 02:56', 'system local time differs from the pinned launchd time'
assert 1789552560 + 9000 + 300 < 1789567200
