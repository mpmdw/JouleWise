...
replace 8–9 with the ruled sentence) and parsed with PyYAML. Result: **346
lines, parses clean**; the six surviving jobs are `fences`, `test`, both
exclusive calibration jobs, `build`, and `installed-wheel`, the last being the
only one with a `needs` (`build`) and none with an `if`. No dangling `needs`, no orphan `if`, no double blank line; no `changes` /
`docs-readers` / `cancelled` token survives. Every boundary is right on the
file: 15–18 detector comment, 19–90 `changes`, 91 blank; 125–127 fence comment,
...
`git grep -E 'ci\.yml|workflows/' 8819cb5f -- tests` → empty; same over
`scripts joulewise` → empty. Nothing parses the workflow.
...
`gh api repos/:owner/:repo/rulesets` → `[]`. No required check exists, so no
deleted job id can wedge a merge. Strike the "the lead confirms" clause.
...
*amending a process rule*; here Ed already ratified the amendment ("as
...
being amended.** Acceptance row 2 reads *"the fast tier **gates PRs**, the FULL
suite remains the gate for merges…"*. Shipped reality is additive
(`_what_this_is`: "It is NOT a gate"; no branch protection, §1). Both are true
of different objects — ratified intent vs shipped job — and the addendum must
say so, not assert only the convenient half.
...
implementation (349e06f4 touches both `ci.yml` and `test_timings.json`); Gmail
`1a0969ba0b31c2b2` (`…activation-c5048879/00-launch-record.md:34`, Exhibit K).
Charter recomputed `099de884…c95d81` = pin.
...
commit subject both use "lever 2" for the unit-atomic sharding and
crash-matrix split that landed the same day; those stay, and only the tier
split is retired here. As shipped the tier was additive and never a required
check (main carries no branch protection and no rulesets, verified
2026-09-13), although acceptance-evidence row 2 as ratified says "the fast
tier gates PRs"; that gating clause was never implemented and is withdrawn
with the tier. On 2026-09-12 Ed accepted PR #317 "as proposed" (Gmail
1a0969ba0b31c2b2), whose decision 2 proposed deleting `pr-fast`, retiring
`pr_fast_tier`, and amending this task's kernel text. The retirement deletes
zero tests. Row 2 is amended to read "Shard-runner implemented from the data;
...
**lines 6–13, an 8-line deletion**, not "two-line"; and PR #317 currently
touches only `ci.yml` plus two trace files, so this adds a second code file to
the diff — state that as scope.
...
at `test_claims_index_lint.py:382`) and `tests.test_gen_derivation_night`
(`GEN.RUNSHEET_PATH.read_text()` at `:1003`), while adding `test_env_locks` and
`test_run_night`, which Exhibit F never named. A third widening is round three
of the same class. Charter §9 quoted accurately (`coldgate_charter.md:136`).
...
Beyond T2, the 1.4-runner-min row and "60 test modules", also stale under A and
...
2. **Concurrency bullet** justifies FIX-1 by "a later docs-only push no longer
   cancels a code push's full matrix and then skips it; docs pushes … ~1.4
   runner-min". FIX-1 stays; its stated reason must become the generic one.
