...
ruling contradicts G I checked the file, not my earlier report.
...
what the row it amends contradicts, and its row-2 replacement is destructive.
Replacement text in §3; two further repairs in §6 and §7.
...
replace 8–9), and parsed with PyYAML: **346 lines, clean**, six jobs
(`fences`, `test`, both exclusive calibration jobs, `build`, `installed-wheel`),
only `installed-wheel` carrying a `needs` (`build`) and none an `if`. No
dangling `needs`, no orphan `if`, no doubled blank line; no `changes` /
`docs-readers` / `cancelled` token survives. Every boundary is right on the file
(15–18 and 125–127 are the two comments; 19–90 `changes`, 128–193
`docs-readers`, 91 and 194 blank; `fences` 92–123; `build` 424;
`installed-wheel` 442): **no mis-numbered deletion.** FIX-3's `!cancelled()` is
moot and its removal strictly safer: with `needs: changes` gone, `test` and both
exclusive jobs cannot be skipped by a dead upstream at all.
...
`…/rulesets` → `[]`. No required check exists, so no deleted job id can wedge
a merge; strike the "the lead confirms" clause.
...
satisfied; every push and PR runs four shards × two Pythons plus both exclusive
jobs. True on all three clauses; no narrowing needed.
...
sharding, crash-matrix split, PR-fast tier"** (349e06f4). A reader of "lever 2
is retired" cannot tell whether unit-atomic sharding and the crash-matrix split
— live and load-bearing — went too: the first-use test failing on a term doing
technical work.
...
retired 2026-09-13 by owner acceptance" deletes the shard-runner criterion,
which nobody retired.
...
One condition to add: the kernel edit and its `gen_state.py --check`
regeneration must land in the **same commit** — that check runs in `fences` and
...
`scripts/shard_tests.py` and `tests/test_shard_split.py` load the JSON, neither
asserting on the top-level key set (`:152` reads only
`unknown_module_weight_seconds`). Deletion is safe. Corrections: the block is **lines
6–13, an 8-line deletion**, not "two-line"; and PR #317 touches only `ci.yml`
plus two trace files today, so this adds a second code file to the diff.
...
never named. A third widening is round three of the same class. The ruling's
charter §9 quote is accurate (`coldgate_charter.md:136`).
...
reads docs" is false; strike it. Exhibit F's R2 is correct.

## 6. Writing standard — one defect the ruling creates

The line-8 replacement is accurate against lines 10–12 and plain. But the ruling
keeps "the `fences` job and its comment (92–123)", which reads *"deliberately
**ungated** on every push and pull request, **including docs-only changes**"*.
Under A nothing is gated and docs-only is not a category, so it asserts a
contrast the operator's checkout lacks — true on 8819cb5f, false on main after
the merge. AMEND to:

```
  # F2 state generation, F3 docs freshness, F4 historical receipt semantics and
  # F5 CLI/strict mock smokes ran identically inside every matrix job; they run
  # once here instead. Every job below runs on every push and pull request.
```
...
1. **Title** "trim the hosted CI matrix without losing a fence": A trims no
   matrix. Use "hoist once-per-run fences, delete `pr-fast`, guard zsh".
2. **Concurrency bullet** justifies FIX-1 by "a later docs-only push no longer
   cancels a code push's full matrix and then skips it". FIX-1 stays; restate
   its reason generically (no push may evict a queued push).
...
5. **"Known gaps"** is all detector behaviour; delete it whole.
...
extract; `git show` of `ci.yml`, the four test modules and `claims_lint.py`
cited above, `test_timings.json` and `state_kernel.json@27957b60`; `git grep`
for `pr_fast_tier`, `pr-fast` and workflow readers; `git log -S` for the origin
commit; `gh api` protection + rulesets; charter `shasum`. No test run; no
tracked file edited.
