...
source /tmp/magistrate-24b9d3dd/arm/exports.zsh
set -euo pipefail
...
start = datetime(2026, 9, 14, 16, 26, tzinfo=zone).timestamp(); end = datetime(2026, 9, 14, 23, 30, tzinfo=zone).timestamp()  # directive #336: one-night owner authorization, evening install from the notice send until 23:30 PDT
print("arm clock:", now, datetime.fromtimestamp(now, zone).isoformat())
assert start <= now < end, "outside the directive-#336 authorized evening install interval (2026-09-14 16:26-23:30 PDT)"
assert datetime.fromtimestamp(1789466160).strftime('%Y-%m-%d %H:%M') == '2026-09-15 02:56', 'system local time differs from the pinned launchd time'
assert 1789466160 + 9000 + 300 < 1789480800
