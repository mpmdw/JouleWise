...
{"schema":"claude-codex-report/v1","genre":"scout","status":"findings","completion":"complete",
 "summary":"Recommend A: no path-based matrix skip. Runtime open()-tracing cannot see subprocess, git-object, or not-yet-existing-file reads; the fail-closed repair pulls in 120/230 modules (66% of suite seconds) and the saving collapses to ~36%.",
...
concurrency group (FIX-1). Delete the `changes` job, the `if:` guards on `test`
and the two exclusive jobs, and the `docs-readers` job.
...
worktree root; script at `/tmp/opus317-trace.py`), run with
`/Users/edr/code/JouleWise/.venv/bin/python3 -B`, cwd `JouleWise-wt-ref-317`:
...
git plumbing, or scans a directory is unconditionally in the docs-only set.
...
Both exclusive modules spawn subprocesses, so both return to every docs-only
run (delta-07 R2 independently requires `test_calibration_exits` back anyway).
A *sound* docs-only run is therefore ≈ 119 min of single-version work + 1.5 min
fences ≈ **121 runner-min against the full matrix's 190** — a 36% cut, not the
87% the current shape advertises (1.5 + 23 = 24.5).

Build estimate for the design B actually needs — audit hook in
`scripts/shard_tests.py:run_units`; a `sitecustomize.py` on `PYTHONPATH` to
propagate it into python children (nothing propagates into zsh/git children);
`scripts/docs_readers.json` (module → read paths + scanned directory prefixes +
`always_run` flag); a regeneration entrypoint; a drift check failing
code-touching runs closed; a CI selection step; the fail-closed
subprocess/git/glob classifier — **12–20 seat-hours**, plus another
refuter + delta-re-audit cycle (this PR has spent two), plus a standing tax: the
map changes on most code pushes, so a stale map costs a failed run (190
runner-min) and a regenerate-and-repush, and every new subprocess in a test is a
new silent hole.
...
2. **Attack the suite, not the gate.** Two modules are 4,026 s = 37% of 10,829
   suite-seconds; `WO-CRASHMATRIX-RELIABILITY` is already registered against
   the ~23-min indivisible test. Removing it is sound by construction.
3. **Push less often.** 37 docs-only pushes/week is an artifact of
   per-activation bookkeeping commits; batching bookkeeping to one push per
   activation cuts the count with zero CI change. `docs/process_traces/**`
   accounts for 107 of the 294 docs-only pushes.
...
- **Whether dropping T2 leaves required-check names dangling.** Removing
  `docs-readers` and the `changes` job changes the job set; branch-protection
  required checks referencing them would block merges until updated.
