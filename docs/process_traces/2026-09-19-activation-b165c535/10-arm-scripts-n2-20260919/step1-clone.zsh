...
print -r -- 'CHECK: test "$(shasum -a 256 "$LEDGER_SOURCE" | cut -d" " -f1)" = "$LEDGER_SOURCE_EXPECTED_SHA256"'
test "$(shasum -a 256 "$LEDGER_SOURCE" | cut -d" " -f1)" = "$LEDGER_SOURCE_EXPECTED_SHA256"
print -r -- 'CHECK: test "$(wc -l < "$CALIBRATION_LEDGER" | tr -d " ")" = 126'
test "$(wc -l < "$CALIBRATION_LEDGER" | tr -d " ")" = 126
"$PY" -B - <<'PY'
import json, os
