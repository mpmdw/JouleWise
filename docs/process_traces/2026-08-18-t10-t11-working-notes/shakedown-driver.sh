...
cd "$CLONE"
for i in 1 2 3; do
  echo "-- bundle $i (ledger reset to pristine canonical state)"
  ...
  git -C "$CLONE" add -A
  git -C "$CLONE" -c user.name="shakedown-driver" -c user.email="driver@local" commit -q -m "shakedown: pristine ledger reset before bundle $i" || true
