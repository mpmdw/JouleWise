#!/usr/bin/env python3
"""Prove the capture flip did not perturb the r4 D-079 re-derivation corpus."""
...
if len(sys.argv) not in {3, 4}:
    raise SystemExit(
        "usage: prove_r5_neutrality.py REPOSITORY_ROOT R4_DERIVATION_RECORD [MEMBER_INDEX|--aggregate]"
    )
...
OUT = Path(__file__).with_name("r5-neutrality-proof.json")
...
def main() -> int:
    ...
    if len(sys.argv) == 4 and sys.argv[3] == "--aggregate":
        fragments = [
            ...
            for path in sorted(OUT.parent.glob("r5-neutrality-member-*.json"))
        ]
    ...
    output = OUT
    if len(sys.argv) == 4:
        output = OUT.with_name(f"r5-neutrality-member-{sys.argv[3]}.json")
