# Delta re-audit brief — Paper-N fix round 2, FACT + METROLOGY + PIN lens

SESSION_MODE: delegated
WRITE_SCOPE: []

READ-ONLY refuter in a detached checkout of the fix-round-2 head (the runner names the commit; confirm
with `git log --oneline -1`). Base for THIS delta is the round-1 head `482a0cc4` (round 1 was audited in record 17); origin/main base is dbe6c675. Diff: `git diff 482a0cc4..HEAD --stat` and
`git diff 482a0cc4..HEAD -- docs/paper/draft-v2-skeleton.md`. Run Python only as `python3 -B`; never run
`docs/paper/build/build_paper.py`; never edit.

Inputs: the fix contract `docs/process_traces/2026-09-12-paper-n/18-brief-fix-round-2.md` (the lead's
dispositions), the seat's report `19-fix-round-2-astra-report.md` in the same directory (its DONE /
...
## Round-2 specifics
- Close DELTA-1/2/3 of record 17 (CURED / NOT CURED, quote). Check the F7 change: the §4 worked line's
  first sentence and the regex at `scripts/check_paper_replay_fence.py` — the three extracted literals
  must be unchanged (paste the extraction) and `tests.test_check_paper_replay_fence` must pass.
- Check the lexicon synchronization (`docs/paper/round7/built-terms-lexicon.md`, seven rows) against
  the prose: every row's stated home must be where the term is now first built.
- Check the new joint-interpolation allowance definition (§3) against `joulewise/reduce.py:554-555`
  and `floor_extraction.py:2100-2117`.
- Same-signature statement over BOTH rounds: does any defect repeat a class from records 02, 04, 16
  or 17? If yes, say so plainly — the lead treats a repeat as a consult trigger.
