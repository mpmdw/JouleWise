...
digests recomputed; all match their pins. Code read at `df86cee6` in `JouleWise-wt-coldgate-iw`
(`git rev-parse HEAD` confirmed). Read-only: no file in any worktree modified except this one; probes ran
`python3 -B` from `/tmp/opus-refuter-25/`.
...
harness (its zsh `launchctl-stub` and `_controlled_python` clock seam; no real `launchctl`, no
`~/Library/LaunchAgents`), advancing the fake clock at the DEAD-MAN bootstrap only — after the last
...
overruns it and a terminal check is still needed. The class closes by converting the
...
constant and no new refusal class. Existing point checks may stay as early exits; residual exposure is then
bounded by teardown duration and points in the SAFE direction (nothing loaded).
...
1. ONE idempotent `teardown_install()`, single entry, safe to call twice, the SOLE cleanup path.
...
3. `trap teardown_install EXIT` is INSUFFICIENT: `:2` sets `-e`, and zsh 5.9 skips the EXIT trap on
   `errexit` from inside a function (bench-proved). Each mutation and each `render` takes an explicit
   `… || { teardown_install; exit N; }`; INT/TERM/HUP traps call it before exiting 130/143/129.
...
fold 0) rests on unverified behaviour. Cost: one local hour a year, one literal, reversible. Clause (c)
becomes "any whole minute (seconds = 0) occurring exactly once in local time", refusals
...
moving two onto one day amends a registration made before data, which only Ed may do. Severity MATERIAL: §3
is byte-identical (`71337a83…072b`), so nothing was changed under the registration; the defect is silence.
...
`installed_agent_fence`, `run_night.py:68`, `launchd.plist.5:468` "The minute (0-59)", runbook `:1297` and
`:2085-2086` (§3 item 3). Verdicts converge on all six; two amendments, one material.
...
- **Q1 — AFFIRM.** Same postcondition I reached independently; its single commit gate before the success
  ...
  `print` calls), and its exactly-at-close regression additionally kills exhibit B F4's `>=`→`>` survivor.
...
- **Q3 — AMEND (BLOCKER).** Its teardown is `bootout … || true` then restore/remove files UNCONDITIONALLY.
  ...
  stderr `"rolled back com.joulewise.night"` — a loaded job, no plist, the fence blind, under a
  success-shaped rollback message. Exhibit B named this case ("A forced rollback-bootout failure likewise
