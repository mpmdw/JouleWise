...
Written **11:52 PDT 2026-09-15** (clock read). Opus 5, blind. Checkout
...
Predicate (the only liveness API):
...
not-found message, both ⇒ UNKNOWN. *Rejected:* a two-valued stub with an "error mode" flag — models the
...
1. **Single mutation channel.** One `LaunchdTransaction`, exactly three mutators (`write_plist`,
   `bootstrap`, `bootout`); nothing else may touch `launch_dir` or call launchctl.
...
errexit in a function skips the EXIT trap — measured, ruling 25). (A) makes I2 true *at three enumerated
sites* and I1/I3 nowhere.

**Still enumerable under (B):** the label set; the ABSENT signature; SIGKILL before the
journal write (ordering mitigates it — plists written *before* bootstrap, so the crash half-state is
files-present = fence-visible); `bootout` asynchrony (a dying job reads LOADED or UNKNOWN; both refuse).
...
- `PLANNED` — plan/pin/preflight/write-once checks and occupancy; **no mutation, no `mkdir`**
  ...
  UNKNOWN refuses (exit 3) rather than proceeding.
...
| PREPARED, FILES_WRITTEN | untouched | priors restored byte-identical (`copy2`, mtime) | orig |
...
Ruling 25 Q3's residual (a loaded dead-man restored as a file, not re-loaded) disappears; teardown still
never bootstraps.
...
`scripts/install_night_agent.sh` at `073a9763`** (grep → no match; ruling 25 Q3 step 2 deleted the
inline rollbacks). Its closing claim that every unsuccessful post-render exit removes this attempt's
plists and restores prior bytes is **false** for the RETAINED branch. Replace with the state table.

**Refusals surviving verbatim** (runbook `:1392-1402`; NIGHT_HANDBACK `:206-208`): install_span_closed,
...
(now_epoch_s, t0_epoch_s, install_close_epoch_s, deadman_epoch_s) with local-ISO rendering.
**New rows** (already in code, undocumented): the retained-state message + **exit 4** (`:276`, `:342`).
...
HUP, clock past selected_span_close, clock past install_close, liveness UNKNOWN, LOADED-after-bootout,
restore I/O error}; each cell asserts a 4-tuple (rc; per-label liveness from the stub log; per-plist
...
assertion*, the class-2 statement made machine-checkable and impossible to omit per-cell: the fence
never returns `None` while the stub log says a label is loaded. *Must-die set:* the signal mask;
`require_absent` at any deletion site; ABSENT widened to `rc != 0`; rc 113 without the stderr match;
...
uninstall; `:184` removes the plist in `cleanup_failed_install` with no liveness check; `:306` and
`:311-315` bootout unverified.

**Design now, land later.** Parameterise the transaction by label set from the first commit
...
magistrate that supervises the night; a regression there blinds the supervisor during the very window
the lane protects.
...
**Preserve** — behaviour byte-identical wherever documented.
...
4. Render-only exemption (ruling 28 Q6.3) — never invokes launchctl, must **not** refuse loaded labels.
   **I4 by construction**: a `NullLaunchctl` adapter raising on any call; never enters PREPARED.
5. Occupancy refusal on **both** labels, exit 3, naming it (`:302-306`).
6. One-pass template substitution (`:244-248`, re-audit 04 R1); MIN_PYTHON subprocess (`:64-84`);
   `cp -p` → `copy2` + mtime assertion; write-once refusal (`:283-291`); read-only refusals before any
   `mkdir` (`:309`); every Q4 refusal string and exit code.

**Discard.** The `$?`/EXIT-trap machinery (`:320-356`) incl. `trap - EXIT` (`:330`); the three
