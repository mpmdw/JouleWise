...
# DELTA RE-AUDIT — T26 item 2 gate-ledger checker, fix round 1 (b36d6c2d → 1529b09a)

Worktree `/Users/edr/code/JouleWise-wt-t26-c2` (detached @ 1529b09a). Read-only:
...
Fix rounds introduce defects (proven twice on this repo). Your job is the
DELTA: `git diff b36d6c2d..1529b09a`. Two refuters found fifteen items on the
first landing (luna F1–F10 contract lens; sol F1–F5 execution lens); the fix
brief dictated closures F-A…F-I. Read the fix brief at the end of this
message, then for EACH closure:
1. Confirm the landed edit matches the dictated shape (file:line), or say how
   it deviates.
2. Execute the counterfactual the closure names (revert that one edit
   mentally or via a scratch copy of the script under TMPDIR — never edit the
   tree) and confirm the named test fails for that reason.
3. Hunt for defects the fix INTRODUCED: the new `_split_table_row` scanner
   (backtick runs of unequal length, a backslash before a backtick, a row
   ending inside a code span, CRLF lines, leading/trailing pipes absent), the
   items-1–11 `commit OR path` acceptance (a 7-hex string that is BOTH a
   commit prefix and a filename; ambiguity direction), the `main()`
   try/except (does it swallow a defect list? does `--repo-root` pointing at
   a file give a stable line?), the `ref:` line (is `head.sha` defined on the
   `edited` event payload?).
4. Run `python3 -m unittest tests.test_check_gate_ledger tests.test_docs_freshness`
   and paste tails; run the checker on a synthetic body under TMPDIR with 12
   RUN rows (item 12 = `git rev-parse HEAD`) and paste the output line.

Same-signature statement (mandatory): does ANY class from the first-round
findings survive — merge-ref checkout, label fidelity, pointer parity,
malformed evidence untested, path-branch misclassification, traversal test
vacuity, traceback on bad input? Name each class and say KILLED/SURVIVES
with evidence.

Deliverable: `claude-codex-report/v1` review envelope with
`verdict.verdict` ("CLEAN" or "NOT CLEAN"), `verdict.closures` (F-A…F-I:
CONFIRMED/DEVIATES + counterfactual outcome), `verdict.findings`
(introduced defects: id, severity, file:line, input, observed), `tests`,
`same_signature`. No fixes.

## The fix brief (verbatim)
