...
The Mac measures the model and everything else drawing power through the same
electrical supply paths that feed its processor components. A busy background
...
stage above supplies the facts used by the stage below. JSON is a text format
that stores named fields and values. The last rectangle is the forcing result:
...
scheduled set of member runs that the runner executes. An individual run's
...
`idle_admission_whole_window_verdict` object decides whether the whole
measurement window had the required quiet-machine and reference evidence. A
...
Before measurement, `scripts/generate_arm_readiness.py` checks the prepared
files and pre-start evidence, while `scripts/launch_window.py` checks the
...
duration receipt, the retained calculation of whether the measured phases were
long enough for the prepared plan. These terminal messages are records to
...
- `format_unavailable` — the selected worker could not exchange the required
  data format.
...
- `unknown_error` — an unexpected failure reached the final safety fallback;
  use `failure_message` and `logs/controller.log`, the detailed event log in
  the member directory, to locate it.
...
- `cpu_busy_ratio_p95_exceeded` — the recorded upper-tail central-processor
  busy fraction, which summarizes unusually high rather than typical
  activity, exceeded the policy ceiling.
- `processor_combined_power_w_p95_exceeded` — the corresponding upper-tail
...
no `detail` field, so none is invented here.
...
- Preserve the record, correct the named cause, and rerun only under the
  governing operator procedure.
