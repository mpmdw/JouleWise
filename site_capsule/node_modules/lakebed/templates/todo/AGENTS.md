...
- Lakebed CLI should always be run with `{{LAKEBED_CLI_COMMAND}} [command]`. It is not a global. Launch with `npx` always.
...
```sh
{{LAKEBED_CLI_COMMAND}} deploy
```

Inspect local state while `{{LAKEBED_CLI_COMMAND}} dev` is running:

```sh
{{LAKEBED_CLI_COMMAND}} db list --port 3000
{{LAKEBED_CLI_COMMAND}} db dump --port 3000
{{LAKEBED_CLI_COMMAND}} db export --port 3000 --out backup.json
{{LAKEBED_CLI_COMMAND}} logs --port 3000
```
...
- User-uploaded files use built-in object storage (`client.storage`); `{{LAKEBED_CLI_COMMAND}} dev` keeps them in memory and resets on restart.
...
- Local state resets when `{{LAKEBED_CLI_COMMAND}} dev` restarts.
- Hosted deploys are served on `{{LAKEBED_APP_BASE_DOMAIN}}` subdomains.
